# TODO: Translation updated at 2021-09-01 18:42

# game/events/olivia.rpy:35
translate english olivia_lisa_tv_6729ce96:

    # Max_07 "{m}Девчонки смотрят какой-то сериал, а я бы лучше на них посмотрел... Вот только обещал не мешать...{/m}" nointeract
    Max_07 "{m}The girls are watching some soap opera. I'd rather watch them... But I promised not to interfere...{/m}" nointeract

# game/events/olivia.rpy:39
translate english olivia_lisa_tv_1ac759be:

    # Max_10 "[risky!t]{m}Нет, слишком опасно подглядывать за ними! Они в любой момент могут меня заметить... И хуже от этого будет только мне!{/m}" nointeract
    Max_10 "[risky!t]{m}No, it's too dangerous to spy on them! They might see me at any moment... And that would only make it worse for me!{/m}" nointeract

# game/events/olivia.rpy:49
translate english olivia_lisa_tv_f4d8507c:

    # Max_04 "{m}Красота! Сидеть вместе с ними на диване было бы куда интереснее... Можно было бы поглядывать одним глазком на них, а другим на экран. Эх, лучше здесь не задерживаться, они в любой момент могут меня заметить...{/m}"
    Max_04 "{m}Beautiful! Sitting on the couch with them would be much more interesting... I could look at them with one eye and at the screen with the other. Eh, I'd better not linger here, they might notice me at any moment...{/m}"

# game/events/olivia.rpy:51
translate english olivia_lisa_tv_661fc12b:

    # Max_04 "{m}Красота! Сидеть вместе с ними на диване было бы куда интереснее... Можно было бы поглядывать одним глазком на них, а другим на экран. Эх, лучше здесь не задерживаться, они в любой момент могут меня заметить...{/m}" nointeract
    Max_04 "{m}Beautiful! Sitting on the couch with them would be much more interesting... I could look at them with one eye and at the screen with the other. Eh, I'd better not linger here, they might notice me at any moment...{/m}" nointeract

# game/events/olivia.rpy:55
translate english olivia_lisa_tv_9e5614b7:

    # Max_09 "{m}Как же мне попасть вместе с ними на диван? Нужно, чтобы Лиза адекватнее относилась к моему возбуждённому виду, но как... Может, если она заметит, что я подглядываю за ней в душе, мне удастся с ней договориться и она не сдаст меня маме, а взамен она захочет увидеть голым меня?{/m}" nointeract
    Max_09 "{m}So how do I get on the couch with them? I need to get Lisa more acclimated to my appearance, but how? Maybe if I let her catch me spying on her in the shower, we can negotiate something?{/m}" nointeract

# game/events/olivia.rpy:58
translate english olivia_lisa_tv_ed5cbb38:

    # Max_09 "{m}Как же мне попасть вместе с ними на диван? Нужно, чтобы Лиза адекватнее относилась к моему возбуждённому виду, но как... Может, стоит чаще попадаться, когда я подглядываю за ней в душе?{/m}" nointeract
    Max_09 "{m}So how do I get on the couch with them? I need to get Lisa more acclimated to my appearance, but how? Maybe I should let her catch me spying on her in the shower again?{/m}" nointeract

# game/events/olivia.rpy:86
translate english olivia_first_meeting_7515fc87:

    # Lisa_01 "Вот и мой старший брат, знакомьтесь, это Оливия, моя одноклассница!"
    Lisa_01 "Here's my big brother, meet Olivia, my classmate!"

# game/events/olivia.rpy:87
translate english olivia_first_meeting_f436f9ae:

    # Max_04 "Привет, я Макс!"
    Max_04 "Hi, I'm Max!"

# game/events/olivia.rpy:88
translate english olivia_first_meeting_8a767ef2:

    # Olivia_01 "Приятно познакомиться, Макс!"
    Olivia_01 "Nice to meet you, Max!"

# game/events/olivia.rpy:89
translate english olivia_first_meeting_6fc69982:

    # Max_02 "Да, мне тоже... Рад, что у Лизы наконец-то появилась подруга."
    Max_02 "Yeah, me too... I'm glad Lisa made a friend."

# game/events/olivia.rpy:90
translate english olivia_first_meeting_55b077a5:

    # Olivia_04 "Я тоже рада. Вау... у вас такой большой, красивый и стильный дом... И такой классный бассейн!"
    Olivia_04 "I'm glad, too. Wow... You have such a big, beautiful and stylish house... And an amazing pool!"

# game/events/olivia.rpy:91
translate english olivia_first_meeting_5adfc42d:

    # Max_03 "Так отдыхайте, пока солнце ещё светит!"
    Max_03 "So rest while the sun is still shining!"

# game/events/olivia.rpy:92
translate english olivia_first_meeting_798cf6f1:

    # Lisa_02 "Ага, мы сейчас быстренько переоденемся и будем в бассейне плескаться, часок-другой. Может и позагорать успеем... если кое-кто нам мешать не будет..."
    Lisa_02 "Yeah, we're going to change quickly and realx in the pool for an hour or two. We might even get a tan... if someone doesn't get in our way..."

# game/events/olivia.rpy:93
translate english olivia_first_meeting_269ffcd7:

    # Max_01 "Если только немного..."
    Max_01 "I won't cause any trouble..."

# game/events/olivia.rpy:94
translate english olivia_first_meeting_0a346054:

    # Olivia_03 "Давай уже разденемся, а потом будем разговаривать."
    Olivia_03 "Let's get changed already, then we'll talk."

# game/events/olivia.rpy:95
translate english olivia_first_meeting_bdf9e8bb:

    # Lisa_01 "Да, мы скоро вернёмся..." nointeract
    Lisa_01 "Yes, we'll be right back..." nointeract

# game/events/olivia.rpy:101
translate english olivia_first_meeting_7fb7ef7c:

    # Max_07 "{m}А Оливия симпатичная девчонка! Надеюсь, она с этим Алексом только из-за его внешности и на деле, ничего особенного он из себя не представляет... Так или иначе, Лиза всё равно поделится со мной этой информацией, когда всё узнает...{/m}"
    Max_07 "{m}Olivia is a pretty girl! I hope this Alex guy isn't anything special... Anyway, Lisa will let me know when she finds out...{/m}"

# game/events/olivia.rpy:107
translate english olivia_first_meeting_4001b456:

    # Olivia_03 "Эй, Макс... О чём задумался?"
    Olivia_03 "Hey, Max... What are you thinking about?"

# game/events/olivia.rpy:108
translate english olivia_first_meeting_caf12e57:

    # Max_06 "Да так... Ого! Нет, не так... ого-го-го... Я заснул что ли?! Или ты просто что-то дома забыла?"
    Max_06 "It's just... Wow! No, not like that... wow, wow, wow... Did I fall asleep? Or did you just forget something at home?"

# game/events/olivia.rpy:109
translate english olivia_first_meeting_20e61eff:

    # Olivia_02 "Нет, не забыла. На самом деле, я взяла с собой больше, чем хотелось бы... У меня родители натуристы и мы дома все ходим голые, я к этому привыкла! Ну и я знаю, как это может шокировать, особенно мальчиков, поэтому и решила хотя бы низ купальника одеть..."
    Olivia_02 "No, I didn't. In fact, I took more with me than I would have liked... My parents are naturists and we like to live naked, I'm used to it! Well, I know it can be shocking, especially for boys, so I decided to wear at least a swimsuit bottom..."

# game/events/olivia.rpy:110
translate english olivia_first_meeting_95624c8d:

    # Lisa_02 "Ага, он ведь так много всего скрывает..."
    Lisa_02 "Yeah, like she's hiding so much..."

# game/events/olivia.rpy:111
translate english olivia_first_meeting_689fb74b:

    # Max_02 "Знаешь, Оливия, если тебе намного комфортнее вообще без купальника, то можешь снять! Никто против не будет..."
    Max_02 "You know, Olivia, if you're much more comfortable nude, you can take it off! No one will mind..."

# game/events/olivia.rpy:112
translate english olivia_first_meeting_7348c02f:

    # Lisa_03 "Ну ещё бы... Макс уже кажется отреагировал на это в своём стиле..."
    Lisa_03 "Of course... Max already seems to have responded to it in his own style..."

# game/events/olivia.rpy:117
translate english olivia_first_meeting_0bbd8816:

    # Olivia_05 "Да это нормально! У папы такая же реакция была, когда мы вместе с ним купались или загорали... Так что, я привыкла!"
    Olivia_05 "That's normal! My dad had the same reaction when we were swimming or sunbathing together... So, I'm used to it!"

# game/events/olivia.rpy:118
translate english olivia_first_meeting_d35082b4:

    # Max_07 "Слышала, Лиза? Это нормально! Оливия, а где твой папа сейчас?"
    Max_07 "Did you hear that, Lisa? It's okay! Olivia, where's your Dad now?"

# game/events/olivia.rpy:119
translate english olivia_first_meeting_a3a21fd8:

    # Olivia_06 "Ну... Они с мамой поругались некоторое время назад и сейчас он с нами не живёт. А ещё у нас сломался бассейн и его некому чинить... Можно я к вам буду приходить купаться?"
    Olivia_06 "Well... He and my Mom had a fight some time ago and now he doesn't live with us. Also, our pool is broken and there's no one to fix it... Can I come here and use your pool?"

# game/events/olivia.rpy:120
translate english olivia_first_meeting_7db6c66b:

    # Max_05 "Я всеми руками и не руками за!"
    Max_05 "I'm all for it!"

# game/events/olivia.rpy:121
translate english olivia_first_meeting_b88de960:

    # Olivia_04 "Здорово! Пойдём с нами купаться, Макс! Или загорать... Я вам так завидую... У вас есть чистый бассейн и пальмы почти ничего не закрывают... Тут как будто рай!"
    Olivia_04 "That's great! Come swim with us! Or sunbathe... I'm so jealous of you... You have a clean pool and the palm trees let you see the sky... It's like heaven!"

# game/events/olivia.rpy:122
translate english olivia_first_meeting_0100401a:

    # Max_07 "Я бы с радостью, да только вот кое-что будет вас смущать..."
    Max_07 "I'd love to, but there's just one thing that might make you uncomfortable..."

# game/events/olivia.rpy:128
translate english olivia_first_meeting_04039790:

    # Olivia_03 "Да ладно тебе прикрываться... Меня это нисколько не смущает! Честно говоря, я не видела таких больших ещё ни разу... У моего папы меньше раза в два, если не в три, да и у моего парня... Ой. Это я вслух сказала сейчас?!"
    Olivia_03 "Come on, you're covering up... I don't mind at all! Honestly, I've never seen one that big before... My dad's half, if not three times smaller, like my boyfriend's. Uhh... Did I just say that out loud?"

# game/events/olivia.rpy:129
translate english olivia_first_meeting_45794192:

    # Max_09 "Я уже понял, что тебя это не смущает, а вот Лизу ещё как..."
    Max_09 "I can see that you don't mind, but Lisa..."

# game/events/olivia.rpy:130
translate english olivia_first_meeting_a3c16fbb:

    # Olivia_05 "Правда?! Лиза, ты же говорила, вы с Максом в одной комнате живёте... Почему ты смущаешься?"
    Olivia_05 "Really?! Lisa, I thought you said you and Max slept in the same room... Why are you embarrassed?"

# game/events/olivia.rpy:131
translate english olivia_first_meeting_79999f7c:

    # Lisa_10 "Просто я долгое время думала, что у него стоит, потому что он что-то похабное со мной представляет."
    Lisa_10 "It's just that for a long time I thought he had a hard-on because he was thinking dirty things about me."

# game/events/olivia.rpy:132
translate english olivia_first_meeting_353cfe52:

    # Max_07 "А я ей кучу раз говорил, что это естественная реакция"
    Max_07 "And I've told her a bunch of times that it's a natural reaction."

# game/events/olivia.rpy:133
translate english olivia_first_meeting_ae3a419f:

    # Olivia_02 "Эх, ребята... Ну теперь-то вы всё прекрасно знаете и понимаете, так? Пора уже привыкать к особенностям друг друга, а то как дети малые... Ладно, я хочу позагорать, а потом купаться! Если захочешь к нам присоединиться, Макс, мы здесь..."
    Olivia_02 "Oh, guys... You understand each other now, right? It's time to get used to each other's bodies. All right, I want to get a tan, and then I want to swim. If you want to join us, Max, we're here..."

# game/events/olivia.rpy:134
translate english olivia_first_meeting_ced16fa4:

    # Max_01 "Ага, отдыхайте..."
    Max_01 "Sure, enjoy..."

# game/events/olivia.rpy:172
translate english olivia_first_night_visit_d18e20dc:

    # Olivia_01 "Приветик! А вот и я... Спасибо, что встретил, Макс."
    Olivia_01 "Hello! Here I am... Thanks for meeting me, Max."

# game/events/olivia.rpy:173
translate english olivia_first_night_visit_e2d51ab8:

    # Max_04 "Да без проблем. Проходи..."
    Max_04 "No problem. Come on in..."

# game/events/olivia.rpy:174
translate english olivia_first_night_visit_82837f34:

    # Olivia_02 "Значит, кроме нас и Лизы дома больше никого нет?"
    Olivia_02 "So no one else is home but us and Lisa?"

# game/events/olivia.rpy:175
translate english olivia_first_night_visit_8074ca27:

    # Max_03 "Да... Мама ушла к Эрику, а Алиса с тётей Кирой как всегда в это время тусят где-то. Вернутся поздно, так что у нас полно времени!"
    Max_03 "Yes... Mom went to Eric's, and Alice and Aunt Kira are hanging out as usual. They'll be back late, so we have plenty of time to ourselves!"

# game/events/olivia.rpy:181
translate english olivia_first_night_visit_9df66df1:

    # Olivia_03 "Отлично! Конечно, ночью особо не позагораешь, но в этом есть своя прелесть... Ну всё, Лиза, пойдём что-нибудь посмотрим на вашем большом экране! Макс, ты как, с нами?" nointeract
    Olivia_03 "Great! There's obviously no sunshine at night, but that doesn't mean we can't enjoy ourselves... Come on, Lisa, let's go watch something on your big screen. Max, are you in?" nointeract

# game/events/olivia.rpy:185
translate english olivia_first_night_visit_68265e05:

    # Olivia_05 "Только учтите, дресс-код на наше мероприятие - только нижнее бельё или пижамы! Кому в чём комфортнее..."
    Olivia_05 "Just remember, the dress code for our event is only underwear or pajamas! Whatever you're more comfortable in."

# game/events/olivia.rpy:186
translate english olivia_first_night_visit_c776259a:

    # Lisa_02 "А ты в чём будешь?"
    Lisa_02 "What will you be wearing?"

# game/events/olivia.rpy:187
translate english olivia_first_night_visit_f4cd0d5b:

    # Olivia_03 "Конечно голышом! Думаю, вы уже достаточно ко мне привыкли, чтобы не смущаться меня и моего образа жизни."
    Olivia_03 "Nothing, of course! I think you're used to me enough not to be embarrassed by my way of life."

# game/events/olivia.rpy:192
translate english olivia_first_night_visit_fddb5510:

    # Max_02 "Я особо и не смущался!"
    Max_02 "I wasn't embarrassed at all!"

# game/events/olivia.rpy:197
translate english olivia_first_night_visit_4e23085c:

    # Lisa_01 "Ну да, оно и видно! У Макса уже в шортах кое-что шевелится... И нам с ним {b}ТАКИМ{/b} придётся сидеть..."
    Lisa_01 "Yeah, I can see that! Max already has something moving in his shorts... I'm going to have to sit near {b}THAT{/b} "

# game/events/olivia.rpy:198
translate english olivia_first_night_visit_cdef23a7:

    # Olivia_02 "Для меня это не проблема, Лиза... А вот что на счёт тебя?"
    Olivia_02 "It's not a problem for me, Lisa... But what about you?"

# game/events/olivia.rpy:199
translate english olivia_first_night_visit_db0bd127:

    # Max_10 "Да ладно тебе, Лиза! Что тут такого?"
    Max_10 "Come on, Lisa! What's the big deal?"

# game/events/olivia.rpy:200
translate english olivia_first_night_visit_0d60f763:

    # Lisa_05 "Ничего... Я, в общем-то, привыкла уже. Почти..."
    Lisa_05 "It's okay... I'm pretty much used to it by now. Almost..."

# game/events/olivia.rpy:206
translate english olivia_first_night_visit_5272565d:

    # Olivia_04 "Ну и отлично! Пойдёмте уже..." nointeract
    Olivia_04 "Well, that's great! Let's go already..." nointeract

# game/events/olivia.rpy:214
translate english olivia_first_night_visit_failure_296a2ae7:

    # Lisa_10 "Ну нет, так не пойдёт! У Макса уже в шортах кое-что шевелится... Я не хочу, чтобы он с нами {b}ТАКОЙ{/b} сидел!"
    Lisa_10 "No, that's not good enough! Max already has something moving in his shorts... I don't want him sitting with us like {b}THAT{/b}!"

# game/events/olivia.rpy:215
translate english olivia_first_night_visit_failure_db0bd127:

    # Max_10 "Да ладно тебе, Лиза! Что тут такого?"
    Max_10 "Come on, Lisa! What's the big deal?"

# game/events/olivia.rpy:216
translate english olivia_first_night_visit_failure_83fd1208:

    # Olivia_06 "Похоже, Макс, у нас будет девичник... Уж извини! Лиза пока к такому не готова."
    Olivia_06 "Looks like we're having a girls' night out, Max... I'm sorry. Lisa's not ready for that yet."

# game/events/olivia.rpy:217
translate english olivia_first_night_visit_failure_5c164238:

    # Max_11 "Вот блин!"
    Max_11 "Oh, shit!"

# game/events/olivia.rpy:218
translate english olivia_first_night_visit_failure_f9b20282:

    # Lisa_13 "И подглядывать не вздумай!"
    Lisa_13 "And don't you dare peep!"

# game/events/olivia.rpy:224
translate english olivia_first_night_visit_failure_786c56dd:

    # Max_14 "Ладно..."
    Max_14 "Okay, uh..."

# game/events/olivia.rpy:238
translate english olivia_second_night_visit_8c66d3fd:

    # Olivia_01 "Привет, Макс. Вот я и снова к вам пришла... Как домашние? Никому не помешаю?"
    Olivia_01 "They, Max. Here I am again... Am I disturbing anyone?"

# game/events/olivia.rpy:239
translate english olivia_second_night_visit_389682e0:

    # Max_04 "Нет, всё в порядке... Проходи, чувствуй себя как дома."
    Max_04 "No, it's okay... Come on in, make yourself at home."

# game/events/olivia.rpy:245
translate english olivia_second_night_visit_ce53ea19:

    # Olivia_02 "С радостью бы искупалась, но так устала, что хочу только валяться и смотреть сериалы. Пойдём, Лиза... Макс, ты как, с нами?" nointeract
    Olivia_02 "I'd love to go swimming, but I'm so tired I just want to lie around and watch soap operas. Come on, Lisa... Max, are you coming?" nointeract

# game/events/olivia.rpy:249
translate english olivia_second_night_visit_10c0fcb2:

    # Olivia_03 "Лиза, а ты так и будешь в маечке и трусиках? Дома же никого нет."
    Olivia_03 "Lisa, are you going to stay in your shirt and panties? It's just us here."

# game/events/olivia.rpy:250
translate english olivia_second_night_visit_6d57f034:

    # Lisa_09 "Никого, ага... Ладно ты, а Макс как же?"
    Lisa_09 "No one, yeah... What about you and Max?"

# game/events/olivia.rpy:253
translate english olivia_second_night_visit_c1e3c67a:

    # Max_07 "Ты настолько меня стесняешься?"
    Max_07 "Are you that shy?"

# game/events/olivia.rpy:254
translate english olivia_second_night_visit_afa52dc9:

    # Lisa_13 "Ну, да. Как любая девочка любого мальчика... Разве это неправильно?"
    Lisa_13 "Well, yes. Like any girl around a boy... Is that wrong?"

# game/events/olivia.rpy:258
translate english olivia_second_night_visit_8a2d02ca:

    # Olivia_05 "Стесняшка ты наша. Макс сказал, чтобы я чувствовала себя как дома... Так что я раздеваюсь."
    Olivia_05 "You're shy. Max said to make me feel at home... So I'm taking my clothes off."

# game/events/olivia.rpy:262
translate english olivia_second_night_visit_b6105111:

    # Olivia_05 "Макс вот своего тела не стесняется. И как я поняла, он уже видел всё, что скрывается под твоей одеждой..."
    Olivia_05 "Max is not ashamed of his body. And from what I understand, he's already seen everything under your clothes..."

# game/events/olivia.rpy:263
translate english olivia_second_night_visit_c8a8ac31:

    # Lisa_10 "Если я разденусь, он же всё время будет глазеть на меня, а не на экран. И как тут не стесняться?!"
    Lisa_10 "If I take my clothes off, he'll be staring at me the whole time, not at the screen. How can you not be embarrassed?!"

# game/events/olivia.rpy:264
translate english olivia_second_night_visit_7b7cf7d4:

    # Max_07 "А как ты будешь со своим будущим парнем себя вести? Тоже будешь стесняться?"
    Max_07 "And how will you behave with your future boyfriend? Are you going to be shy with him, too?"

# game/events/olivia.rpy:265
translate english olivia_second_night_visit_13bc2e36:

    # Lisa_13 "Не знаю... Может быть. Мне кажется, не очень правильно возбуждать тебя своим обнажённым видом."
    Lisa_13 "I don't know... Maybe. I don't think it's right to get you excited with my body."

# game/events/olivia.rpy:269
translate english olivia_second_night_visit_93e86917:

    # Olivia_03 "Знаешь, Лиза, хоть вы брат и сестра, но вместе с тем - ты ещё и девочка. А природой заложено так, что девочки возбуждают мальчиков и наоборот. И это классно... чувствовать себя желанной!"
    Olivia_03 "You know, Lisa, even though you are brother and sister, you are also a girl. And it's nature's way of saying that girls excite boys and vice versa. And it's great... to feel wanted!"

# game/events/olivia.rpy:270
translate english olivia_second_night_visit_9f4ba9f8:

    # Lisa_09 "Ну... не знаю... Я не могу так сразу на это решиться."
    Lisa_09 "Well... I don't know... I can't make up my mind right away."

# game/events/olivia.rpy:271
translate english olivia_second_night_visit_1fb0f97c:

    # Max_00 "Ты подумай над этим. Потому что парням быстро наскучивают зажатые девушки."
    Max_00 "You think about it. Because guys get bored quickly with uptight girls."

# game/events/olivia.rpy:275
translate english olivia_second_night_visit_a6603bc7:

    # Olivia_04 "Такими темпами мы ничего не посмотрим. Пойдёмте уже..." nointeract
    Olivia_04 "At this rate, we won't watch anything. Let's go already..." nointeract

# game/events/olivia.rpy:287
translate english olivia_night_visit_r_c0291a99:

    # Olivia_01 "Привет, Макс. А вот и я! Дома никого?"
    Olivia_01 "Hey, Max. Here I am! Is anyone else home?"

# game/events/olivia.rpy:288
translate english olivia_night_visit_r_04f7e51a:

    # Max_04 "Привет! Рад тебя видеть. Дома только я и Лиза. Проходи..."
    Max_04 "Hi! Good to see you. It's just Lisa and I. Come on in..."

# game/events/olivia.rpy:293
translate english olivia_night_visit_r_b6a957f2:

    # Olivia_02 "Лиза, я полдня тебя сегодня не видела, а уже соскучилась... Жду не дождусь уже раздеться и заниматься ничегонеделанием. Только сериалы с любимыми друзьями!"
    Olivia_02 "Lisa, I haven't seen you for half a day, and I miss you already... I can't wait to get naked and relax. Just watching soap operas with my favorite friends!"

# game/events/olivia.rpy:294
translate english olivia_night_visit_r_9b51ac68:

    # Lisa_02 "Я тоже рада поваляться вместе с вами хоть один поздний вечерок." nointeract
    Lisa_02 "I love relaxing with you too!" nointeract

# game/events/olivia.rpy:306
translate english olivia_night_visit_r_8574b567:

    # Max_05 "Ого! Лиза у нас сегодня тоже почти голенькая будет! Рад, что не стесняешься..."
    Max_05 "Wow, Lisa's going to be almost naked today too! Glad you're not embarrassed...."

# game/events/olivia.rpy:307
translate english olivia_night_visit_r_a21cc315:

    # Lisa_02 "Ну... Почему бы немного не порадовать тебя, раз уж ты достойно помогал мне со всем, чем только можно на неделе."
    Lisa_02 "Well... I thought I'd make you a little happier, since you've been a decent help to me this week."

# game/events/olivia.rpy:308
translate english olivia_night_visit_r_43994455:

    # Max_03 "Хороший стимул... Мне нравится!"
    Max_03 "A reward... I like it!"

# game/events/olivia.rpy:309
translate english olivia_night_visit_r_d351d1a1:

    # Lisa_01 "Только не засматривайся на меня так сильно! Я ведь и одеться могу."
    Lisa_01 "Just don't look at me so much! I can still get dressed."

# game/events/olivia.rpy:316
translate english olivia_night_visit_r_252b0246:

    # Olivia_03 "Лиза, а ты почему маечку не снимаешь? Опять стесняешься..."
    Olivia_03 "Lisa, why don't you take your top off? You're shy again..."

# game/events/olivia.rpy:317
translate english olivia_night_visit_r_a6b1ccae:

    # Lisa_00 "Нет. У нас с Максом было... одно соглашение и он его нарушил. Поэтому радости у него сегодня будет поменьше."
    Lisa_00 "No. Max and I had... an agreement, and he broke it. So he's not going to get any satisfaction today."

# game/events/olivia.rpy:319
translate english olivia_night_visit_r_f773bcda:

    # Max_07 "Лиза, а ты почему маечку не снимаешь? Я думал, ты уже не особо нас стесняешься..."
    Max_07 "Lisa, why don't you take your top off? I thought you weren't shy around us anymore..."

# game/events/olivia.rpy:320
translate english olivia_night_visit_r_11ccf991:

    # Lisa_09 "Я бы сняла, да вот только не очень хочется тебя так радовать."
    Lisa_09 "I'd take it off, but I don't really want to make you that happy."

# game/events/olivia.rpy:321
translate english olivia_night_visit_r_24763cdf:

    # Max_10 "Да ладно тебе, Лиза! Почему?"
    Max_10 "Come on, Lisa! Why?"

# game/events/olivia.rpy:323
translate english olivia_night_visit_r_c53d4c7a:

    # Lisa_10 "Вот помогал бы мне на этой неделе хоть немного с домашним заданием, ручки бы мои пожалел, с посудой помог... Или ты думаешь, защитил меня от маминого наказания разок и всё? Голую грудь в студию?!"
    Lisa_10 "Let's see... You could have helped me with my homework, my hands, the dishes... Or do you think you could just protect me from Mom's punishment once and that would be enough? Please!"

# game/events/olivia.rpy:325
translate english olivia_night_visit_r_1c92ddb9:

    # Lisa_10 "Вот помогал бы мне на этой неделе хоть немного с домашним заданием, ручки бы мои пожалел, с посудой помог... С таким Максом я бы ещё рядом посидела без маечки, а так..."
    Lisa_10 "If only you had helped me with my homework, my hands, the dishes... I could still sit next to Max without his shirt on, but this way..."

# game/events/olivia.rpy:327
translate english olivia_night_visit_r_414a4238:

    # Lisa_10 "Вот помогал бы мне на этой неделе хоть немного с домашним заданием, ручки бы мои пожалел, с посудой помог... А ты даже от маминого наказания меня не защитил..."
    Lisa_10 "You could have helped me with my homework, my hands, the dishes... Or even saved me from Mom's punishment!"

# game/events/olivia.rpy:328
translate english olivia_night_visit_r_0d89d3d3:

    # Max_11 "Ну... Я мог бы..."
    Max_11 "Well... I could..."

# game/events/olivia.rpy:329
translate english olivia_night_visit_r_17b1c448:

    # Lisa_13 "Вот как сможешь, так и посмотрим. А пока что я ничего снимать с себя не хочу."
    Lisa_13 "When you can, we'll see. In the meantime, I don't want to take a single thing off."

# game/events/olivia.rpy:336
translate english olivia_night_visit_r_2fa10a26:

    # Olivia_04 "Эх, Макс! Прошляпил всё веселье. Ладно, пойдёмте уже..." nointeract
    Olivia_04 "Oh, Max! You missed all the fun. All right, let's just go..." nointeract

# game/events/olivia.rpy:338
translate english olivia_night_visit_r_a6603bc7:

    # Olivia_04 "Такими темпами мы ничего не посмотрим. Пойдёмте уже..." nointeract
    Olivia_04 "At this rate, we won't see anything. Let's go already..." nointeract

# game/events/olivia.rpy:378
translate english olivia_first_night_out_with_67a0d9d9:

    # Max_05 "Ух ты! Ничего себе! Я даже забыл, что хотел спросить..."
    Max_05 "Wow! Wow! I forgot what I was going to ask..."

# game/events/olivia.rpy:379
translate english olivia_first_night_out_with_1f749322:

    # Olivia_03 "Так садись, Макс... Потом вспомнишь!"
    Olivia_03 "So sit down, Max... You'll remember later!"

# game/events/olivia.rpy:380
translate english olivia_first_night_out_with_292d59ce:

    # Max_02 "Ах, вот... Я тоже буду сидеть голышом! Вы же не против?"
    Max_02 "Ah, here... I'll sit naked, too! You don't mind, do you?"

# game/events/olivia.rpy:381
translate english olivia_first_night_out_with_922025e6:

    # Olivia_05 "Нисколечко!"
    Olivia_05 "Not a bit!"

# game/events/olivia.rpy:382
translate english olivia_first_night_out_with_11409cf9:

    # Lisa_10 "Так, стоять! Это уже слишком! Я не уверена, что хочу сидеть рядом со своим голым братом. Хоть трусы оставь..." nointeract
    Lisa_10 "Okay, hold on! This is too much! I'm not sure I want to sit next to my naked brother. At least leave your underwear on..." nointeract

# game/events/olivia.rpy:389
translate english olivia_first_night_out_with_7542b55d:

    # Olivia_04 "Ого, Макс! Это смело! И внушительно..."
    Olivia_04 "Wow, Max! That's brave! And impressive..."

# game/events/olivia.rpy:390
translate english olivia_first_night_out_with_125c98a4:

    # Lisa_11 "Ну ты даёшь! Тебе как вообще, не стыдно перед нами в таком виде рассиживаться?"
    Lisa_11 "How aren't you... aren't you ashamed to sit like that in front of us?"

# game/events/olivia.rpy:391
translate english olivia_first_night_out_with_587b76a6:

    # Max_07 "Ты как с мальчиками-то собираешь встречаться, если даже со мной голым не можешь себя комфортно чувствовать? Привыкай..."
    Max_07 "How are you going to go out with boys if you can't even feel comfortable with me naked? Get used to it..."

# game/events/olivia.rpy:392
translate english olivia_first_night_out_with_771331f5:

    # Olivia_03 "Макс, кстати, прав... Тебе пора бы уже привыкать. Держаться с мальчиками за ручку и гулять - это здорово, но вы же не только этим будете заниматься."
    Olivia_03 "Max is right, by the way... You should get used to it by now. Holding hands with the boys and going for walks is great, but it's not all you'll be doing."

# game/events/olivia.rpy:393
translate english olivia_first_night_out_with_f5c5342c:

    # Lisa_10 "Поскромнее хоть сядьте оба, а то устроили тут нудистский диван."
    Lisa_10 "At least be more modest, both of you, because you're making a nudist couch here."

# game/events/olivia.rpy:394
translate english olivia_first_night_out_with_58c3781e:

    # Max_04 "Давайте смотреть, что где показывают интересного..."
    Max_04 "Let's see what's interesting..."

# game/events/olivia.rpy:399
translate english olivia_first_night_out_with_87c0e726:

    # Olivia_01 "Вот, можем этот сериал посмотреть... Вроде, что-то интересное происходит у них там. Надо вникать..."
    Olivia_01 "Here, we can watch this show... It sounds like something interesting is going on there. We should look into it..."

# game/events/olivia.rpy:400
translate english olivia_first_night_out_with_1183f165:

    # Max_02 "{m}Мне куда интереснее то, что происходит на диване... А здесь рядом со мной сидят две обалденные девчонки и одна из них - совершенно голая, как и я!{/m}"
    Max_02 "{m}I'm more interested in what's going on on the couch... And there are two gorgeous girls sitting next to me, and one of them is completely naked, just like me!{/m}"

# game/events/olivia.rpy:404
translate english olivia_first_night_out_with_ce4a4d53:

    # Max_03 "{m}Лиза пока ещё стесняется моего вида... Слегка отвернулась, но поглядывает в мою сторону иногда. А вот Оливия очень комфортно себя чувствует. Такая расслабленная и красивая...{/m}"
    Max_03 "{m}Lisa is still embarrassed to see me... She looks away a little, but she glances in my direction sometimes. Olivia, on the other hand, feels very comfortable. So relaxed and beautiful...{/m}"

# game/events/olivia.rpy:405
translate english olivia_first_night_out_with_f5a27884:

    # Olivia_02 "Ты как там, Макс? Всё ещё возбуждён моим видом? Ага, вижу, что да..."
    Olivia_02 "How are you doing there, Max? Still excited to see me? Yeah, I see you are..."

# game/events/olivia.rpy:406
translate english olivia_first_night_out_with_9ab76b64:

    # Lisa_09 "И долго он таким может быть?"
    Lisa_09 "How long can he stay like that?"

# game/events/olivia.rpy:407
translate english olivia_first_night_out_with_73fd36de:

    # Olivia_03 "Похоже, что долго... Но вы к этому привыкните, если будете со мной больше времени проводить."
    Olivia_03 "It seems like a long time... But you'll get used to it if you spend more time with me."

# game/events/olivia.rpy:411
translate english olivia_first_night_out_with_1b6f1ed5:

    # Max_05 "По мне, так отличная причина тебе приходить к нам чаще!"
    Max_05 "Sounds like a great reason for you to come see us more often!"

# game/events/olivia.rpy:412
translate english olivia_first_night_out_with_c9f25499:

    # Olivia_04 "Я бы с радостью, Макс. Было бы у меня больше свободного времени... Здесь и компания приятная, и вода в бассейне такая тёплая... Обожаю воду! Ну, вы уже в курсе."
    Olivia_04 "I would love to, Max. I wish I had more free time... The company is nice here, and the water in the pool is so warm... I love the water. Well, you already know that."

# game/events/olivia.rpy:413
translate english olivia_first_night_out_with_efc50a5f:

    # Lisa_01 "А Макс успокаивается наконец..."
    Lisa_01 "And he calms down at last..."

# game/events/olivia.rpy:418
translate english olivia_first_night_out_with_ab1aa461:

    # Max_04 "Самую малость."
    Max_04 "Just a little bit."

# game/events/olivia.rpy:419
translate english olivia_first_night_out_with_807cb8f9:

    # Olivia_03 "Это я просто немного поскромнее уселась, чтобы у Макса косоглазие не развилось. Мальчики - они такие, он даже если будет стараться смотреть на экран, глаза всё равно потянутся в сторону голых девичьих прелестей."
    Olivia_03 "I just sat a little more modestly so that Max wouldn't get squinty-eyed.Boys are like that, even if he tries to look at the screen, his eyes will still search for the naked girl's charms."

# game/events/olivia.rpy:420
translate english olivia_first_night_out_with_2ddf4686:

    # Lisa_02 "А у меня вот запросто получается смотреть сериал и никуда не отвлекаться."
    Lisa_02 "I, on the other hand, easily manage to watch the show without getting distracted."

# game/events/olivia.rpy:425
translate english olivia_first_night_out_with_5e1e7cd1:

    # Max_02 "{m}Вижу я, как у тебя глазки бегают, то на меня, то на Оливию. Наверняка, Лизе даже нравится то, что здесь происходит сегодня...{/m}"
    Max_02 "{m}I can see your eyes running back and forth between me and Olivia. I bet even Lisa likes what she sees...{/m}"

# game/events/olivia.rpy:426
translate english olivia_first_night_out_with_55adf1fc:

    # Olivia_06 "Ребята, у меня уже глаза слипаются. Давайте заканчивать, да я домой пойду..."
    Olivia_06 "Guys, my eyes are already falling asleep. Let's wrap it up, I'm going home..."

# game/events/olivia.rpy:427
translate english olivia_first_night_out_with_a32a19b0:

    # Lisa_00 "Может не стоит ночью по улице бродить. Оставайся у нас, а утром вернёшься домой."
    Lisa_00 "Maybe you shouldn't wander around at night. Stay with us and you can go home in the morning."

# game/events/olivia.rpy:428
translate english olivia_first_night_out_with_e353bc4d:

    # Max_03 "Да, оставайся на ночь здесь!"
    Max_03 "Yes, stay the night!"

# game/events/olivia.rpy:429
translate english olivia_first_night_out_with_b0e3e2ee:

    # Olivia_01 "Ой, я вообще-то не планировала, но вы правы, лучше остаться у вас до утра."
    Olivia_01 "Oh, I hadn't actually planned to, but you're right, it's better to stay with you until morning."

# game/events/olivia.rpy:430
translate english olivia_first_night_out_with_e7f08135:

    # Lisa_01 "Пойдёмте тогда в комнату..."
    Lisa_01 "Let's go into our room then..."

# game/events/olivia.rpy:436
translate english olivia_first_night_out_with_638cf225:

    # Max_04 "Кто куда ляжет?"
    Max_04 "Who will lie down where?"

# game/events/olivia.rpy:437
translate english olivia_first_night_out_with_a4ff6d3b:

    # Lisa_02 "Естественно, Оливия ляжет со мной, а ты как всегда... сам по себе."
    Lisa_02 "Naturally, Olivia will lie down with me, and you, as usual... on your own."

# game/events/olivia.rpy:438
translate english olivia_first_night_out_with_bf95efa9:

    # Max_02 "Оливия же в гостях... Может определим её на твою кровать, а ты со мной?"
    Max_02 "Olivia is a guest... Why don't we put her on your bed and you come with me?"

# game/events/olivia.rpy:439
translate english olivia_first_night_out_with_e113e5b9:

    # Lisa_01 "Чтобы ты всю ночь тыкался в меня своей штуковиной здоровенной?! Размечтался!"
    Lisa_01 "So you can poke me all night with that big thing of yours?! You wish!"

# game/events/olivia.rpy:440
translate english olivia_first_night_out_with_9d8d213d:

    # Olivia_02 "Хорошая попытка, Макс! Утром меня не теряйте, я уйду рано. Всем спокойной ночи..."
    Olivia_02 "Nice try, Max! Don't miss me in the morning, I'll leave early. Good night, everybody..."

# game/events/olivia.rpy:441
translate english olivia_first_night_out_with_e05418ee:

    # Max_01 "Сладких снов."
    Max_01 "Sweet dreams."

# game/events/olivia.rpy:457
translate english olivia_second_night_out_with_f1daf02f:

    # Lisa_11 "А вы ещё мне предлагаете раздеться! Это Макс только на тебя одну, Оливия, так реагирует... А представь, что будет, если разденусь и я? Его штука вообще, наверно, взорвётся!"
    Lisa_11 "And you suggest that I take my clothes off! Max is the only one who reacts this way, Olivia... Can you imagine what would happen if I got naked too? His thing would probably explode!"

# game/events/olivia.rpy:458
translate english olivia_second_night_out_with_e5631105:

    # Max_03 "Ничего не взорвётся, не переживай."
    Max_03 "Nothing will explode, don't worry."

# game/events/olivia.rpy:459
translate english olivia_second_night_out_with_ff43958c:

    # Olivia_03 "Лучше радуйся за брата, Лиза. Далеко не каждому мальчику настолько везёт с размерами... Не каждый день такое можно увидеть в живую. Вернее... ты-то как раз и можешь видеть!"
    Olivia_03 "Better be happy for your brother, Lisa. Not every boy is so lucky with his size... It's not every day you get to see it in person. Or rather... you get to see it!"

# game/events/olivia.rpy:460
translate english olivia_second_night_out_with_d1e724aa:

    # Lisa_03 "Вот только ради этого и живу... Чтобы хоть разок за день увидеть стояк Макса!"
    Lisa_03 "That's all I live for... Just to see Max's boner every day!"

# game/events/olivia.rpy:465
translate english olivia_second_night_out_with_dfc7aba6:

    # Olivia_05 "Хочешь сказать, ты никогда не думала о Максе, как о мальчике, с которым можно что-то попробовать... Ну... что-то такое..."
    Olivia_05 "You mean you've never thought of Max as a boy to try something with... Well... something like that..."

# game/events/olivia.rpy:466
translate english olivia_second_night_out_with_e6bc223b:

    # Lisa_13 "Нет, конечно! Скажешь тоже..."
    Lisa_13 "No, of course not! I can't believe you're saying that too..."

# game/events/olivia.rpy:467
translate english olivia_second_night_out_with_e4b6d8d8:

    # Max_02 "{m}Интересно, как бы Оливия отреагировала, если бы Лиза рассказала, как я учу её целоваться и где мои руки при этом оказываются?! Святую тут из себя строит. Ну да ладно...{/m}"
    Max_02 "{m}I wonder how Olivia would react if she knew about our kissing lessons! She's acting like a saint. Oh, well...{/m}"

# game/events/olivia.rpy:468
translate english olivia_second_night_out_with_bd177cb8:

    # Olivia_03 "Так я тебе и поверила, проказница... Ну а ты, Макс?"
    Olivia_03 "Sure, Sure, you rascal... What about you, Max?"

# game/events/olivia.rpy:472
translate english olivia_second_night_out_with_0ab5792e:

    # Max_04 "Конечно, да! Но я тут бессилен, это же непроизвольно."
    Max_04 "Of course, yes! But I'm powerless here, it's involuntary."

# game/events/olivia.rpy:473
translate english olivia_second_night_out_with_44544fc0:

    # Olivia_04 "Видишь, Лиза, какой у тебя брат честный! Не то, что Алекс."
    Olivia_04 "You see, Lisa, what an honest brother you have! Nothing like Alex."

# game/events/olivia.rpy:474
translate english olivia_second_night_out_with_d7a49712:

    # Max_07 "А что с ним?"
    Max_07 "What's wrong with him?"

# game/events/olivia.rpy:475
translate english olivia_second_night_out_with_be6fd8f1:

    # Olivia_00 "Ах, да... Ты же, наверно, не в курсе. Алекс в наглую, не порвав со мной, пытался клеить Лизу! Скотина такая!"
    Olivia_00 "Oh, yeah... You probably didn't know that. Alex brazenly tried to hit on Lisa! What a jerk!"

# game/events/olivia.rpy:476
translate english olivia_second_night_out_with_07e7c841:

    # Max_09 "Вот как! Погодите... Но раз вы обе здесь, значит у вас всё хорошо?"
    Max_09 "That's it! Wait a minute... But since you're both here, you must be doing well, right?"

# game/events/olivia.rpy:480
translate english olivia_second_night_out_with_bac5e609:

    # Lisa_02 "У нас - да! У Алекса не очень. Он двух потрясающих девчонок упустил! Хотя, теперь зная, какой он козёл, не исключаю, что он уже кого-то ещё клеит..."
    Lisa_02 "We do! Alex, not so much. He missed out on two amazing girls! Although, now that I know how much of a jerk he is, I'm sure he's already hitting on someone else..."

# game/events/olivia.rpy:481
translate english olivia_second_night_out_with_455d7806:

    # Olivia_06 "Если уже не склеил! Нет, даже думать об этом не хочу... Фу!"
    Olivia_06 "If you haven't already glued it together! No, I don't even want to think about it... Ugh!"

# game/events/olivia.rpy:482
translate english olivia_second_night_out_with_c9b2dc7c:

    # Max_00 "Получается, Оливия с парнем рассталась?"
    Max_00 "So Olivia broke up with her boyfriend?"

# game/events/olivia.rpy:483
translate english olivia_second_night_out_with_7482216d:

    # Lisa_01 "Именно так."
    Lisa_01 "That's right."

# game/events/olivia.rpy:488
translate english olivia_second_night_out_with_205fcde2:

    # Olivia_00 "А главное, Алекс не расстроился ни от того, что я его бросила, ни от того, что Лиза не стала с ним встречаться. Уверена, это потому что у него был запасной вариант."
    Olivia_00 "And most importantly, Alex wasn't upset that I broke up with him, or that Lisa didn't go out with him. I'm sure it was because he had a backup."

# game/events/olivia.rpy:489
translate english olivia_second_night_out_with_f2a6a804:

    # Max_07 "Вот почему я предпочитаю секс без отношений! Трахаешься, когда хочешь и с кем хочешь... И никакой \"Санта-Барбары\"!"
    Max_07 "That's why I prefer sex without a relationship! You can have sex whenever you want, with whomever you want... without problems!"

# game/events/olivia.rpy:490
translate english olivia_second_night_out_with_f7150c4a:

    # Lisa_11 "Что-о-о-о!"
    Lisa_11 "Wha-a-a-t?!"

# game/events/olivia.rpy:491
translate english olivia_second_night_out_with_3a4a3ad7:

    # Max_02 "Это я вас таким образом от темы с Алексом решил отвлечь."
    Max_02 "This is my way of distracting you from the subject of Alex."

# game/events/olivia.rpy:496
translate english olivia_second_night_out_with_fcee3664:

    # Lisa_03 "Фух... А то мне сразу в голову всякое невероятное полезло!"
    Lisa_03 "Phew... I had all sorts of unbelievable things going through my head!"

# game/events/olivia.rpy:497
translate english olivia_second_night_out_with_ecb528d2:

    # Olivia_05 "Ха! Это было смешно, Макс! Я рада, что оказалась в такой тёплой компании. Надеюсь, так будет и дальше..."
    Olivia_05 "Ha! That was funny, Max! I'm glad to be in such warm company. I hope it stays that way..."

# game/events/olivia.rpy:498
translate english olivia_second_night_out_with_767fd836:

    # Max_04 "Ага, хорошо сидим!"
    Max_04 "Yeah, we really get along!"

# game/events/olivia.rpy:499
translate english olivia_second_night_out_with_2ffdea61:

    # Lisa_02 "Знаете, я в таком хорошем настроении сейчас, от того, что для нас с Оливией всё так хорошо закончилось, что готова даже снять майку..."
    Lisa_02 "You know, I'm in such a good mood right now, because everything ended so well for me and Olivia, that I'm ready to take my shirt off..."

# game/events/olivia.rpy:500
translate english olivia_second_night_out_with_edcfe1f3:

    # Max_06 "Вот это ничего себе!"
    Max_06 "Wow, that's really something!"

# game/events/olivia.rpy:504
translate english olivia_second_night_out_with_41a2f02d:

    # Olivia_03 "Да ладно! Макс же тут... Ты вроде не хотела при нём..."
    Olivia_03 "Oh, come on! Max is here... I thought you didn't want to do that in front of him..."

# game/events/olivia.rpy:505
translate english olivia_second_night_out_with_a1d40e09:

    # Lisa_05 "Да пусть смотрит, если хочет. Будем считать, что ему сегодня очень повезло, потому что я хочу делиться своей радостью!"
    Lisa_05 "Let him watch if he wants to. Let's consider him very lucky today, because I want to share my joy!"

# game/events/olivia.rpy:506
translate english olivia_second_night_out_with_9833c5bf:

    # Max_05 "{m}Во сестрёнка даёт! Естественно, я хочу на это смотреть... Не то, чтобы в наглую, но всё-таки такого количества голых сисек на этом диване ещё не было!{/m}"
    Max_05 "{m}That's my sister! Naturally, I want to look at it... It's not that I'm brazen, but I've never seen so many naked boobs on this couch before!{/m}"

# game/events/olivia.rpy:507
translate english olivia_second_night_out_with_af8b6b2b:

    # Olivia_02 "А Макс притих. Наверно, весь в раздумьях, куда лучше смотреть! Хоть бы на экран поглядел для приличия."
    Olivia_02 "And Max is quiet. He's probably wondering where to look! He should at least look at the screen for decency."

# game/events/olivia.rpy:508
translate english olivia_second_night_out_with_9d4433da:

    # Max_03 "Голова никак не поворачивается."
    Max_03 "It seems my head can't turn."

# game/events/olivia.rpy:513
translate english olivia_second_night_out_with_050b3b4f:

    # Lisa_01 "Ничего, ты сама говорила - привыкнет... Я же немного привыкла, что вы тут голые сидите."
    Lisa_01 "It's all right, you said yourself - he'll get used to it... I'm a little used to you sitting here naked myself."

# game/events/olivia.rpy:514
translate english olivia_second_night_out_with_2664741f:

    # Olivia_01 "А я бы уже спать пошла. Уже поздно... Я вся иззевалась. Вы ещё не хотите?"
    Olivia_01 "And I would have gone to bed already. It's late... I'm all sore. How about you?"

# game/events/olivia.rpy:515
translate english olivia_second_night_out_with_e274b123:

    # Lisa_02 "Можно. Пойдёмте, пока все тут не заснули. Вот бы тётя Кира удивилась..."
    Lisa_02 "Let's go before everyone falls asleep here. Aunt Kira would be in for a surprise..."

# game/events/olivia.rpy:521
translate english olivia_second_night_out_with_1718f5fe:

    # Max_04 "Спокойной ночи, девчонки. Я в восторге от наших с вами ночных посиделок!"
    Max_04 "Good night, girls. I love our nightly get-togethers!"

# game/events/olivia.rpy:522
translate english olivia_second_night_out_with_be029ca0:

    # Lisa_03 "По тебе и так прекрасно видно, как сильно ты доволен. Уверена, надеешься, что я забуду маечку надеть и прямо так и лягу..."
    Lisa_03 "You can see how happy you are. I'm sure you're hoping I'll forget to put my top on and just lie down like this..."

# game/events/olivia.rpy:523
translate english olivia_second_night_out_with_0d262a66:

    # Max_02 "Ага. Зря, да?"
    Max_02 "Well surely you will, right?"

# game/events/olivia.rpy:524
translate english olivia_second_night_out_with_66fdb528:

    # Lisa_02 "Конечно, зря! Может сериалы среди ночи я и буду смотреть в одних трусиках, и то, это не факт... Но это не значит, что я теперь всё буду делать в таком виде!"
    Lisa_02 "Don't count on it! I might watch soap operas in the middle of the night in my panties, But that doesn't mean I'm going to sleep like that!"

# game/events/olivia.rpy:525
translate english olivia_second_night_out_with_6c069ffd:

    # Max_03 "Ладно. Уже и помечтать нельзя."
    Max_03 "Okay, geez... Can't even dream anymore."

# game/events/olivia.rpy:526
translate english olivia_second_night_out_with_2ee7a236:

    # Olivia_06 "В прошлый раз я же тихо и незаметно ушла? Никого не побеспокоила?"
    Olivia_06 "I left quietly last time, didn't I? I didn't bother anyone?"

# game/events/olivia.rpy:527
translate english olivia_second_night_out_with_e5665dce:

    # Lisa_01 "Нет, я даже не заметила, как ты встала..."
    Lisa_01 "No, I didn't even notice you getting up..."

# game/events/olivia.rpy:528
translate english olivia_second_night_out_with_9b2e7333:

    # Max_01 "Да, я тоже. Сладких снов, девчонки."
    Max_01 "You're fine, Olivia. Sweet dreams, girls."

# game/events/olivia.rpy:546
translate english olivia_repeatable_night_out_with_01d47c87:

    # Lisa_02 "Ты чего завис, Макс? Давай садись быстрее..."
    Lisa_02 "What are you hangin' around for, Max? Come on, get in quick..."

# game/events/olivia.rpy:547
translate english olivia_repeatable_night_out_with_35ff1b35:

    # Max_02 "Знаешь, бывают моменты, когда засмотришься на что-то и потом никак оторваться не можешь..."
    Max_02 "You know, there are times when you look at something and then you can't look away..."

# game/events/olivia.rpy:548
translate english olivia_repeatable_night_out_with_3e8c189f:

    # Lisa_01 "Может, мне тогда стоит одеться, чтобы ты над душой не стоял?"
    Lisa_01 "Maybe I should get dressed, then, so you don't have to stand over me."

# game/events/olivia.rpy:549
translate english olivia_repeatable_night_out_with_d5bd5179:

    # Max_04 "Нет, нет, нет... Я уже раздеваюсь!"
    Max_04 "No, no, no... I'm already undressing!"

# game/events/olivia.rpy:553
translate english olivia_repeatable_night_out_with_01d47c87_1:

    # Lisa_02 "Ты чего завис, Макс? Давай садись быстрее..."
    Lisa_02 "What are you hangin' around for, Max? Come on, get in quick..."

# game/events/olivia.rpy:554
translate english olivia_repeatable_night_out_with_f90d7bab:

    # Max_02 "Да я вот задумался, как мне тебя уговорить маечку снять... Только, по-моему, без шансов, да?"
    Max_02 "I was wondering how I could convince you to take your top off... I don't think there's any chance, is there?"

# game/events/olivia.rpy:555
translate english olivia_repeatable_night_out_with_fe51560e:

    # Lisa_01 "Именно! Ты или тогда не стой над душой, или раздевайся и присоединяйся... пока я добрая."
    Lisa_01 "Exactly! You either don't stand over me then, or get undressed and join in... while I'm being nice."

# game/events/olivia.rpy:556
translate english olivia_repeatable_night_out_with_34230b3b:

    # Max_04 "От такой симпатичной компании, как вы, я не откажусь!"
    Max_04 "I wouldn't refuse such good-looking company as you!"

# game/events/olivia.rpy:564
translate english olivia_repeatable_night_out_with_217e442e:

    # Olivia_05 "Ты бы поосторожнее запрыгивал на диван, Макс! А то своей огромной штуковиной по Лизе ещё попадёшь! Тебе же потом придётся объяснять всем своим, откуда у неё фингал."
    Olivia_05 "You'd better be more careful jumping on the couch, Max! Or you'll hit Lisa with that huge thing! You'll have to explain to your friends how she got the black eye."

# game/events/olivia.rpy:565
translate english olivia_repeatable_night_out_with_f33032d6:

    # Max_03 "Уж извините... Со мной лучше ртом не зевать!"
    Max_03 "I'm sorry... Just try not to leave your mouth open!"

# game/events/olivia.rpy:566
translate english olivia_repeatable_night_out_with_323f301e:

    # Lisa_02 "Что правда, то правда! Я видела Макса голым уже столько раз, а всё ещё удивляюсь."
    Lisa_02 "What's true is true! I've seen Max naked so many times, and I'm still surprised."

# game/events/olivia.rpy:567
translate english olivia_repeatable_night_out_with_07085c52:

    # Max_05 "И кто на кого, в итоге, больше всех засматривается?"
    Max_05 "I wonder who the bigger pervert really is?"

# game/events/olivia.rpy:570
translate english olivia_repeatable_night_out_with_797a3326:

    # Lisa_02 "Ты поосторожнее, Макс, запрыгивай на диван, а то этой своей штукой ещё по мне попадёшь! Я не хочу потом с фингалом ходить и придумывать историю, в которой не фигурирует твой член."
    Lisa_02 "Careful, Max, hop up on the couch, or you'll hit me with that thing! I don't want to make up a story to explain my black eye."

# game/events/olivia.rpy:571
translate english olivia_repeatable_night_out_with_79cd0f1c:

    # Max_03 "Со мной лучше быть начеку! Тогда и фингалов никаких не будет."
    Max_03 "You better watch out for me! Then there won't be any injuries."

# game/events/olivia.rpy:572
translate english olivia_repeatable_night_out_with_dfd0b5de:

    # Olivia_05 "Я таких беззастенчивых парней, как Макс, ещё не встречала. Хотя, оно и понятно, почему... Есть чем гордиться!"
    Olivia_05 "I've never met such shameless guys as Max. Although it's understandable why... He's got something to be proud of!"

# game/events/olivia.rpy:573
translate english olivia_repeatable_night_out_with_1c4fe2d2:

    # Max_05 "Спасибо! Я, конечно, не против этого внимания, прикованного к моему члену, но вы и на экран поглядывайте тоже."
    Max_05 "Thank you! I certainly don't mind all this attention being focused on my dick, but you should look at the screen, too."

# game/events/olivia.rpy:582
translate english olivia_repeatable_night_out_with_965e183f:

    # Olivia_04 "О! Давайте это посмотрим... Жалко, что не на самое начало серии попали, а то как-то совсем уж непонятно, что там происходит."
    Olivia_04 "Oh! Let's watch this... Too bad we didn't get to the very beginning of the series, because it's kind of hard to see what's going on there."

# game/events/olivia.rpy:583
translate english olivia_repeatable_night_out_with_72187a45:

    # Lisa_01 "Непонятно, но лично меня заинтриговало... Мне интересно, что дальше будет."
    Lisa_01 "It's not clear, but personally I'm intrigued... I wonder what will happen next."

# game/events/olivia.rpy:584
translate english olivia_repeatable_night_out_with_00b10636:

    # Max_04 "{m}А вот мне нет разницы, что происходит в сериале... Ведь на этом диване есть то, что в сериалах не каждый день увидишь!{/m}"
    Max_04 "{m}But I don't care what happens on the show... After all, there are things on this couch that you don't see on TV!{/m}"

# game/events/olivia.rpy:587
translate english olivia_repeatable_night_out_with_22d3a585:

    # Olivia_04 "Вот, можем этот сериал посмотреть... Вроде, что-то интересное происходит у них там. Надо вникать..."
    Olivia_04 "Here, we can watch this show... It sounds like something interesting is going on there. We have to look into it..."

# game/events/olivia.rpy:588
translate english olivia_repeatable_night_out_with_00c76c93:

    # Lisa_01 "Тогда смотрим... Может, по ходу просмотра во всём разберёмся."
    Lisa_01 "Then let's see... Maybe we'll figure it all out as we go along."

# game/events/olivia.rpy:589
translate english olivia_repeatable_night_out_with_f51c8651:

    # Max_04 "{m}А вот мне куда интереснее то, что происходит на диване... Здесь, рядом со мной, сидят две обалденные девчонки, которые не против моей компании!{/m}"
    Max_04 "{m}But I'm much more interested in what's going on on the couch... Here, next to me, there are two gorgeous girls who don't mind my company!{/m}"

# game/events/olivia.rpy:597
translate english olivia_repeatable_night_out_with_f6fc3e47:

    # Olivia_02 "Здорово, что можно просто вот так, побездельничать в пустом и шикарном доме... Да ещё и с такими классными ребятами, как вы!"
    Olivia_02 "It's great to be able to just hang out in an empty, fancy house like this... And with great friends like you!"

# game/events/olivia.rpy:598
translate english olivia_repeatable_night_out_with_c95bc97f:

    # Lisa_03 "Ага. Особенно здорово Максу. Мальчики, наверно, только о таком и мечтают?!"
    Lisa_03 "Yeah. Especially great for Max. That must be what all the boys want, right?!"

# game/events/olivia.rpy:601
translate english olivia_repeatable_night_out_with_a0ea375d:

    # Olivia_02 "А хорошо закрутили, совсем у них дела плохи... Я аж на месте не могу усидеть. Хочется раскинуться на диване, но для вас это будет слишком откровенно."
    Olivia_02 "They're doing a good job, they're not doing well at all... I can't even sit still. I want to stretch out on the couch, but that would show to much."

# game/events/olivia.rpy:602
translate english olivia_repeatable_night_out_with_45fbf759:

    # Lisa_03 "Мне и самой хочется... Особенно здорово будет Максу, он ведь наверняка о таком и мечтает сейчас!"
    Lisa_03 "I want to do it myself... It would be especially great for Max, he's probably dreaming about it right now!"

# game/events/olivia.rpy:603
translate english olivia_repeatable_night_out_with_0a4c9412:

    # Max_02 "Не только о таком. Но и то, что сейчас - тоже круто!"
    Max_02 "It's not just about that. But this is still pretty great!"

# game/events/olivia.rpy:609
translate english olivia_repeatable_night_out_with_05e08a2a:

    # Olivia_01 "Тебе повезло с братом, Лиза. Мальчишки думают только об одном, но такими уж их сделала природа. И Макс не исключение, но он хотя бы внимателен к нам."
    Olivia_01 "You're lucky to have a brother, Lisa. Boys only think about one thing, but that's how nature made them. Max is no exception, but at least he pays attention to us."

# game/events/olivia.rpy:610
translate english olivia_repeatable_night_out_with_0f6c9465:

    # Lisa_02 "Макс очень внимательный, когда хочет за кем-нибудь подсмотреть!"
    Lisa_02 "Max is very attentive when he wants to spy on someone!"

# game/events/olivia.rpy:611
translate english olivia_repeatable_night_out_with_d1813384:

    # Max_01 "Я внимательный всегда."
    Max_01 "I'm always attentive."

# game/events/olivia.rpy:617
translate english olivia_repeatable_night_out_with_59450400:

    # Olivia_03 "Ну и как думаешь, Макс... Правильно герои сериала делают или нет?"
    Olivia_03 "So what do you think, Max... Are the characters in the show doing the right thing or not?"

# game/events/olivia.rpy:618
translate english olivia_repeatable_night_out_with_268c1645:

    # Max_03 "Так или иначе - да. Ведь впереди ещё много сезонов..."
    Max_03 "One way or another, yes. After all, there are many more seasons to come..."

# game/events/olivia.rpy:619
translate english olivia_repeatable_night_out_with_925a0535:

    # Lisa_09 "Эй, ты испортил всю интригу! Как мы теперь дальше будем смотреть, зная, что у них всё получится?"
    Lisa_09 "Hey, you ruined all the suspense! How are we going to watch now, knowing that they're going to make it?"

# game/events/olivia.rpy:625
translate english olivia_repeatable_night_out_with_74291412:

    # Max_07 "Так не всем же может повезти! Кто-нибудь да огребёт... возможно..."
    Max_07 "Well, not everyone can get lucky! Somebody's going to get hurt... maybe..."

# game/events/olivia.rpy:626
translate english olivia_repeatable_night_out_with_72c801d9:

    # Olivia_04 "Твоя правда. Ладно, тихо... Мы смотреть будем или обсуждать?"
    Olivia_04 "You're right. Okay, quiet... Are we going to watch or discuss?"

# game/events/olivia.rpy:627
translate english olivia_repeatable_night_out_with_6cd95a15:

    # Lisa_01 "Конечно, смотреть! Обсудить и потом можно... Если Макс опять о чём-нибудь не проболтается."
    Lisa_01 "Of course you can watch! We can talk about it later... If Max doesn't blab about something again."

# game/events/olivia.rpy:633
translate english olivia_repeatable_night_out_with_dd6e369a:

    # Olivia_06 "Вот и серия заканчивается. Вы как, спать хотите? Я вот уже зеваю, но оно и понятно, ночь на дворе..."
    Olivia_06 "This is the end of the series. Do you guys want to go to bed? I'm already yawning, but that's understandable, it's the middle of the night."

# game/events/olivia.rpy:634
translate english olivia_repeatable_night_out_with_0b752bfe:

    # Lisa_05 "Ага, давайте пойдём по кроваткам, пока здесь не заснули." nointeract
    Lisa_05 "Yeah, let's go to bed before we fall asleep here." nointeract

# game/events/olivia.rpy:643
translate english olivia_repeatable_night_out_with_1e38110f:

    # Max_04 "Если вдруг вам надоест спать вместе, я всегда рад приютить любую из вас на своей кровати."
    Max_04 "If you happen to get tired of sleeping together, I'm always happy to snuggle either of you in my bed."

# game/events/olivia.rpy:644
translate english olivia_repeatable_night_out_with_e56be7a5:

    # Olivia_03 "Спасибо, Макс! Трусы хоть одень, чтобы было не так заметно, насколько ты рад этим фантазиям."
    Olivia_03 "Thank you, Max! At least put some underpants on so it's not so obvious how excited you are about these fantasies."

# game/events/olivia.rpy:645
translate english olivia_repeatable_night_out_with_6538cfc3:

    # Lisa_02 "И с чего ты взял, что нам надоест?"
    Lisa_02 "And what makes you think we're going to get bored?"

# game/events/olivia.rpy:646
translate english olivia_repeatable_night_out_with_893cf053:

    # Max_02 "Ну мало ли."
    Max_02 "Well, you never know."

# game/events/olivia.rpy:647
translate english olivia_repeatable_night_out_with_76458fcb:

    # Olivia_01 "Всем спокойной ночи..."
    Olivia_01 "Good night, everyone..."

# game/events/olivia.rpy:648
translate english olivia_repeatable_night_out_with_cb768b4d:

    # Max_01 "Приятных снов."
    Max_01 "Sweet dreams."

translate english strings:

    # game/events/olivia.rpy:35
    old "{i}подсмотреть{/i}"
    new "{i}View{/i}"

    # game/events/olivia.rpy:95
    old "{i}дождаться девчонок{/i}"
    new "{i}Wait for the girls{/i}"

    # game/events/olivia.rpy:181
    old "Конечно с вами!"
    new "Of course I'm with you!"

    # game/events/olivia.rpy:294
    old "Я с вами!"
    new "I'm with you!"

    # game/events/olivia.rpy:382
    old "{i}раздеться и сесть рядом{/i}"
    new "{i}Sit down next to each other{/i}"

    # game/events/olivia.rpy:634
    old "{i}идти в свою комнату{/i}"
    new "{i}Go to your room{/i}"

# TODO: Translation updated at 2021-11-11 13:38

# game/events/olivia.rpy:293
translate english olivia_night_visit_r_dcf86a41:

    # Lisa_13 "Но без Макса! Он сильно провинился, так что сегодня мы с тобой будем только вдвоём, Оливия. А ты, Макс, не мешай нам!"
    Lisa_13 "But without Max! He's been misbehaving, so it'll just be you and me tonight, Olivia. And you, Max, stay out of our way!"

# game/events/olivia.rpy:295
translate english olivia_night_visit_r_5e8eeb58:

    # Max_11 "Ну вот..." nointeract
    Max_11 "Here we go..." nointeract

# game/events/olivia.rpy:680
translate english olivia_dressed_stay_in_room_186ccac5:

    # Olivia_05 "Привет, Макс! Такая хорошая погода стоит, а ты в комнате сидишь... И зря!"
    Olivia_05 "Hello, Max! The weather is so nice, and you're just sitting in your room?"

# game/events/olivia.rpy:682
translate english olivia_dressed_stay_in_room_c0dccde6:

    # Olivia_05 "Привет, Макс! Зря в комнате сидишь, на улице чудесная погода... Мы вот с Лизой сейчас будем отдыхать во дворе, пока солнышко светит!"
    Olivia_05 "Hello, Max! You shouldn't be sitting in your room, the weather outside is wonderful... Lisa and I are going to relax in the yard while the sun is shining!"

# game/events/olivia.rpy:686
translate english olivia_dressed_stay_in_room_740cab84:

    # Lisa_09 "А ещё нам нужно переодеться. Будь добр, выйди из комнаты ненадолго..." nointeract
    Lisa_09 "Also, we need to change. Would you be so kind as to leave the room for a moment?" nointeract

# game/events/olivia.rpy:688
translate english olivia_dressed_stay_in_room_132c666f:

    # Lisa_00 "А ещё нам нужно переодеться. Будь добр, погуляй немного или отвернись..." nointeract
    Lisa_00 "Also, we need to change. Would you be so kind as to walk around for a while, or turn your back..." nointeract

# game/events/olivia.rpy:695
translate english olivia_dressed_stay_in_room_08276951:

    # Lisa "{b}Лиза:{/b} И не вздумай подглядывать! Это не займёт много времени..." nointeract
    Lisa "{b}Lisa:{/b} And don't even think about peeking! This won't take long..." nointeract

# game/events/olivia.rpy:697
translate english olivia_dressed_stay_in_room_cdba7705:

    # Lisa "{b}Лиза:{/b} И я бы хотела, чтобы ты постарался не подглядывать... Знаю, сложно, но пожалуйста!" nointeract
    Lisa "{b}Lisa:{/b} And I'd like you to try not to peek... I know it's hard, but please!" nointeract

# game/events/olivia.rpy:723
translate english olivia_dressed_moment0_8a87f589:

    # Lisa_00 "Ой, Макс! А ты можешь немного погулять? Мы собирались переодеться..." nointeract
    Lisa_00 "Oh, Max! Can't you take a little walk? We were going to change..." nointeract

# game/events/olivia.rpy:725
translate english olivia_dressed_moment0_b8c05569:

    # Lisa_09 "Откуда я знаю, что ты видел, а что ещё нет? Как бы там ни было, я стесняюсь и хочу переодеться без свидетелей."
    Lisa_09 "How do I know what you've seen and what you haven't? Anyway, I'm shy and want to change without witnesses."

# game/events/olivia.rpy:726
translate english olivia_dressed_moment0_af5d43f0:

    # Olivia_01 "Что сказать, Макс... Ты в пролёте!" nointeract
    Olivia_01 "What can I say, Max... You're out of luck!" nointeract

# game/events/olivia.rpy:729
translate english olivia_dressed_moment0_c92765a1:

    # Lisa_01 "Спасибо, Макс. Мы быстренько!"
    Lisa_01 "Thank you, Max. We'll be quick!"

# game/events/olivia.rpy:730
translate english olivia_dressed_moment0_87d5067e:

    # Olivia_01 "Лично мне Макс нисколько не мешает..." nointeract
    Olivia_01 "Personally, Max doesn't bother me at all..." nointeract

# game/events/olivia.rpy:745
translate english olivia_dressed_moment2_20fd5016:

    # Lisa_11 "Макс! Мы же переодеваемся!!! {p=3}{nw}"
    Lisa_11 "Max! We're changing!!! {p=3}{nw}"

# game/events/olivia.rpy:753
translate english olivia_dressed_moment2_c171310b:

    # Lisa_13 "Выйди и закрой за собой дверь!"
    Lisa_13 "Get out and close the door behind you!"

# game/events/olivia.rpy:754
translate english olivia_dressed_moment2_7c31b019:

    # Olivia_03 "Да, Макс! Тебе должно быть очень стыдно за такие выходки." nointeract
    Olivia_03 "Yes, Max! Not cool." nointeract

# game/events/olivia.rpy:758
translate english olivia_dressed_moment2_58aafaf5:

    # Lisa_10 "Пожалуйста, выйди и дверь за собой закрой!"
    Lisa_10 "Please get out and shut the door behind you!"

# game/events/olivia.rpy:759
translate english olivia_dressed_moment2_276f7949:

    # Olivia_03 "Да ладно тебе, Лиза. Мы же все всё у друг друга уже видели." nointeract
    Olivia_03 "Come on, Lisa. We've all seen everything by now." nointeract

# game/events/olivia.rpy:774
translate english olivia_dressed_moment1_43215628:

    # Lisa_11 "Макс! Мы же переодеваемся!!! Выйди и закрой за собой дверь!" nointeract
    Lisa_11 "Max! We're changing! Get out and close the door behind you!" nointeract

# game/events/olivia.rpy:777
translate english olivia_dressed_moment1_9c7c76de:

    # Lisa_11 "Макс! Мы же переодеваемся!!! Пожалуйста, выйди и дверь за собой закрой!" nointeract
    Lisa_11 "Max! We're changing! Please get out and close the door behind you!" nointeract

# game/events/olivia.rpy:783
translate english olivia_dressed_lvl_2_7968a2a6:

    # Olivia_02 "Ох, нам бы твои проблемы..."
    Olivia_02 "Oh, we wish we had your problems..."

# game/events/olivia.rpy:784
translate english olivia_dressed_lvl_2_91ad6784:

    # Lisa_10 "Ты не глазей, а жди за дверью! Или хочешь, чтобы я маме об этом рассказала?" nointeract
    Lisa_10 "You don't stare, you wait outside the door! Or do you want me to tell Mom?" nointeract

# game/events/olivia.rpy:787
translate english olivia_dressed_lvl_2_32b71976:

    # Olivia_02 "Оу, похоже Макс хочет сказать, что мы с тобой очень соблазнительно выглядим!"
    Olivia_02 "Oh, I guess what Max is trying to say is that you and I look super cute!"

# game/events/olivia.rpy:788
translate english olivia_dressed_lvl_2_81a439ed:

    # Lisa_10 "Ну, Макс! Я ведь и маме всё могу рассказать, если не уйдёшь." nointeract
    Lisa_10 "Come on, Max! I can tell Mom if you don't leave." nointeract

# game/events/olivia.rpy:791
translate english olivia_dressed_lvl_2_eba9367a:

    # Olivia_02 "Спасибо. Ты тоже ничего... Только из шорт от радости не выпрыгни."
    Olivia_02 "Thank you. You're all right, too... Just don't jump out of your shorts for joy."

# game/events/olivia.rpy:793
translate english olivia_dressed_lvl_2_c401595c:

    # Lisa_09 "Ну и долго ты ещё пялиться будешь?! Выйди и подожди. Мы недолго..." nointeract
    Lisa_09 "How much longer are you going to stare?! Get out and wait. We won't be long..." nointeract

# game/events/olivia.rpy:800
translate english olivia_dressed_lvl_3_97aa3ff0:

    # Lisa_01 "Ты, видимо, только об этом и мечтаешь... Но нечего глазеть, как мы одеваемся! Лучше займи себя чем-нибудь полезным..."
    Lisa_01 "I guess that's all you want to do... But don't stare at us! Better occupy yourself with something useful..."

# game/events/olivia.rpy:801
translate english olivia_dressed_lvl_3_f073ff95:

    # Max_02 "Как раз это и делаю..."
    Max_02 "That's just what I'm doing..."

# game/events/olivia.rpy:802
translate english olivia_dressed_lvl_3_a21098ed:

    # Olivia_02 "Осторожней, Макс. Косоглазие заработаешь и все тебя начнут дурачком считать..."
    Olivia_02 "Careful, Max. You might go cross-eyed..."

# game/events/olivia.rpy:803
translate english olivia_dressed_lvl_3_12bd6daa:

    # Lisa_02 "Да мы и так... Ладно, молчу." nointeract
    Lisa_02 "We're already... Okay, I'll shut up." nointeract

# game/events/olivia.rpy:806
translate english olivia_dressed_lvl_3_81729ace:

    # Lisa_01 "Вот иди и радуйся где-нибудь в другом месте. А мы хотим спокойно одеться..."
    Lisa_01 "So go and enjoy yourself somewhere else. And we want to get dressed in peace..."

# game/events/olivia.rpy:807
translate english olivia_dressed_lvl_3_34689a50:

    # Max_02 "Подожди! Оливия же не будет одевается. Что-то не сходится..."
    Max_02 "Wait! Olivia's not getting dressed. Something doesn't add up..."

# game/events/olivia.rpy:808
translate english olivia_dressed_lvl_3_01edfdc0:

    # Lisa_09 "Ну, Макс!" nointeract
    Lisa_09 "Well, Max." nointeract

# game/events/olivia.rpy:811
translate english olivia_dressed_lvl_3_61ed6671:

    # Lisa_13 "А ты и доволен! И хватит на меня глазеть... Выйди и подожди за дверью!"
    Lisa_13 "And you're happy! And stop staring at me... Get out and wait behind the door!"

# game/events/olivia.rpy:812
translate english olivia_dressed_lvl_3_a32fa991:

    # Olivia_02 "А может она их и вовсе не одевала?"
    Olivia_02 "Or maybe she wasn't wearing them at all?"

# game/events/olivia.rpy:813
translate english olivia_dressed_lvl_3_8d0ac507:

    # Max_08 "Да-а-а?!"
    Max_08 "Yeah?!"

# game/events/olivia.rpy:814
translate english olivia_dressed_lvl_3_3daab7f8:

    # Olivia_03 "Может быть. А может и нет. Но какая теперь разница? Ты же всё равно пофантазируешь на эту тему..."
    Olivia_03 "Maybe. Or maybe not. But what does it matter now? You're going to fantasize about it anyway..."

# game/events/olivia.rpy:815
translate english olivia_dressed_lvl_3_d7323289:

    # Lisa_10 "Оливия!" nointeract
    Lisa_10 "Olivia!" nointeract

# game/events/olivia.rpy:818
translate english olivia_dressed_lvl_3_319973cf:

    # Lisa_01 "Спасибо. Но давай ты не будешь пялиться и подождёшь за дверью, мы недолго..."
    Lisa_01 "Thank you. But why don't you stop staring and wait behind the door, we won't be long..."

# game/events/olivia.rpy:819
translate english olivia_dressed_lvl_3_5c400cb2:

    # Olivia_02 "Смотри, Макс... Так ведь и привыкнуть можно. И станет это простой обыденностью."
    Olivia_02 "Look, Max... You could get used to it. Then it'll be nothing but a simple routine."

# game/events/olivia.rpy:820
translate english olivia_dressed_lvl_3_453d6107:

    # Max_04 "Не станет! Ты не так часто приходишь, а Лиза меня до сих пор стесняется..."
    Max_04 "it won't! You don't come that often, and Lisa stil gets embarrassed..."

# game/events/olivia.rpy:822
translate english olivia_dressed_lvl_3_b98d244b:

    # Lisa_02 "Макс, займись чем-нибудь путным." nointeract
    Lisa_02 "Max, go do something useful." nointeract

# game/events/olivia.rpy:837
translate english olivia_dressed_try_to_peek_144e8df6:

    # Max_05 "[lucky!t]{m}Кажется, они не заметили, что я подглядываю! Эти две аппетитные попки могут свести с ума любого... Но именно сейчас сводят меня и это так приятно!{/m}"
    Max_05 "[lucky!t]{m}They didn't seem to notice that I was peeping! Those two luscious asses could drive anyone crazy... But right now they're driving me crazy, and it feels so good!{/m}"

# game/events/olivia.rpy:839
translate english olivia_dressed_try_to_peek_be20eb18:

    # Max_05 "[lucky!t]{m}Никто не видит, что я подглядываю... Мне даже всё равно, если они заметят. Рискнуть и увидеть, как эти девочки оголяют свои сладенькие прелести в паре метров от меня, стоит того!{/m}"
    Max_05 "[lucky!t]{m}No one sees me peeking... I don't even care if they notice. It's worth the risk to see these girls exposing their sweet little charms a few feet away from me!{/m}"

# game/events/olivia.rpy:850
translate english olivia_dressed_try_to_peek_24da8c0a:

    # Max_03 "[lucky!t]{m}Ой, Оливия увидела, что я подглядываю! Но ничего не говорит Лизе... Ну ещё бы. Она же натуристка и это её нисколько не смущает. Забавные сиськи... В смысле, забавная ситуация!{/m}"
    Max_03 "[lucky!t]{m}Oh, Olivia saw me peeking! But she didn't say anything to Lisa... Of course she doesn't. She's a naturist and she's not embarrassed by it.{/m}"

# game/events/olivia.rpy:852
translate english olivia_dressed_try_to_peek_895988b1:

    # Max_03 "[lucky!t]{m}Оливия заметила, что я подглядываю! Хорошо, что она натуристка, а то сейчас бы криков было... А так, можно смотреть и наслаждаться. Она так забавно пальчиком грозит...{/m}"
    Max_03 "[lucky!t]{m}Olivia noticed that I was peeping! It's a good thing she's a nudist, or there'd be screaming now... But this way, you can watch and enjoy it. She's got a funny way of wagging her finger...{/m}"

# game/events/olivia.rpy:864
translate english olivia_dressed_try_to_peek_f458687d:

    # Lisa_11 "[spotted!t]Ну, Макс! Прекращай подглядывать! Если не отвернёшься или ещё раз такое выкинешь, я на тебя маме нажалуюсь... Тебе ясно?!"
    Lisa_11 "[spotted!t]Come on, Max! Stop peeping! If you don't turn your back or do that again, I'll report you to Mom... Do you understand?!"

# game/events/olivia.rpy:866
translate english olivia_dressed_try_to_peek_585185bb:

    # Olivia_03 "Ай-я-яй, как нехорошо подглядывать, Макс. Ай-я-яй, какой негодник!" nointeract
    Olivia_03 "Tsk Tsk Tsk, it's no good peeking, Max. Tsk, Tsk, what a rascal!" nointeract

# game/events/olivia.rpy:868
translate english olivia_dressed_try_to_peek_f165e904:

    # Olivia_02 "Да я-то не стесняюсь. Мне наоборот, куда комфортнее без одежды..."
    Olivia_02 "I'm not embarrassed. On the contrary, I'm much more comfortable without clothes..."

# game/events/olivia.rpy:869
translate english olivia_dressed_try_to_peek_e5e895f1:

    # Lisa_10 "А я вот стесняюсь! И перестань на меня так глазеть, Макс! А то я Алису попрошу, чтобы она тебя отпинала..."
    Lisa_10 "I'm embarrassed! And stop staring at me like that, Max! Or I'll ask Alice to kick you..."

# game/events/olivia.rpy:871
translate english olivia_dressed_try_to_peek_eb8e36a6:

    # Max_08 "Всё, я отвернулся!" nointeract
    Max_08 "That's it, I'm turning away!" nointeract

# game/events/olivia.rpy:876
translate english olivia_dressed_try_to_peek_eba9367a:

    # Olivia_02 "Спасибо. Ты тоже ничего... Только из шорт от радости не выпрыгни."
    Olivia_02 "Thank you. You're all right, too... Just don't jump out of your shorts in joy."

# game/events/olivia.rpy:878
translate english olivia_dressed_try_to_peek_5e5bea76:

    # Lisa_09 "Ну и долго ты ещё пялиться на меня будешь?! Отвернись. Мы недолго..." nointeract
    Lisa_09 "How long are you going to keep staring at me? Turn around. We won't be..." nointeract

# game/events/olivia.rpy:882
translate english olivia_dressed_try_to_peek_943d13a8:

    # Lisa_11 "[spotted!t]Опять ты подглядываешь! И не стыдно тебе?! Делать тебе больше нечего, только глазеть, как мы переодеваемся... Прекращай!"
    Lisa_11 "[spotted!t]You're peeping again! Aren't you ashamed?! You've got nothing better to do than watch us change... Stop it!"

# game/events/olivia.rpy:884
translate english olivia_dressed_try_to_peek_d2b966ac:

    # Olivia_03 "Какой же ты Макс проказник! Ни стыда, ни совести..." nointeract
    Olivia_03 "What a prankster you are! No shame, no conscience..." nointeract

# game/events/olivia.rpy:886
translate english olivia_dressed_try_to_peek_c1321b9d:

    # Olivia_02 "Ммм... А где это Макс... Только ведь тут вроде был! А теперь нет..."
    Olivia_02 "Mmm... Where's Max... I thought he was here. And now he's not..."

# game/events/olivia.rpy:887
translate english olivia_dressed_try_to_peek_66f1c3a5:

    # Lisa_02 "Вот представь, ну никак не могу! Стесняюсь. Можно я уже спокойно переоденусь? Без этого твоего сверлящего и озабоченного взгляда..."
    Lisa_02 "Just imagine, I can't do it! I'm embarrassed. Can I change in peace now? Without that piercing, concerned look of yours..."

# game/events/olivia.rpy:888
translate english olivia_dressed_try_to_peek_a18c539b:

    # Max_02 "Можно. Только погодите пару секунд, я фотоаппарат достану..."
    Max_02 "Yes, we can. Just wait a few seconds, I'll get my camera..."

# game/events/olivia.rpy:889
translate english olivia_dressed_try_to_peek_ab614b1d:

    # Lisa_09 "Ну, Макс!"
    Lisa_09 "Come on, Max!"

# game/events/olivia.rpy:891
translate english olivia_dressed_try_to_peek_48e1fee0:

    # Max_03 "Да шучу я... Всё, отвернулся!" nointeract
    Max_03 "I'm just kidding... That's it, I turned away!" nointeract

# game/events/olivia.rpy:896
translate english olivia_dressed_try_to_peek_e11ee97e:

    # Lisa_01 "Спасибо. Но давай ты не будешь пялиться и отвернёшься... Хорошо?"
    Lisa_01 "Thank you. But how about you don't stare and look away... Okay?"

# game/events/olivia.rpy:897
translate english olivia_dressed_try_to_peek_e22e7d62:

    # Max_04 "Мне понравилось, как ты двигалась... Не повторишь?"
    Max_04 "I liked the way you moved... Would you do it again?"

# game/events/olivia.rpy:898
translate english olivia_dressed_try_to_peek_cd4c9bac:

    # Olivia_02 "Поддерживаю! А то я пропустила этот захватывающий момент..."
    Olivia_02 "I second that!"

# game/events/olivia.rpy:900
translate english olivia_dressed_try_to_peek_dbf3b763:

    # Lisa_02 "Ну вы меня совсем сейчас засмущаете!" nointeract
    Lisa_02 "You're embarrassing me right now!" nointeract

translate english strings:

    # game/events/olivia.rpy:690
    old "{i}подождать за дверью, пока девочки переоденутся{/i}"
    new "{i}Wait until the girls have changed{/i}"

    # game/events/olivia.rpy:781
    old "Аж глаза разбегаются, на кого смотреть..."
    new "It's hard to decide who to look at..."

    # game/events/olivia.rpy:781
    old "У меня встал!"
    new "I'm up!"

    # game/events/olivia.rpy:781
    old "Извините... Мне стыдно, но смотритесь вы красиво!"
    new "Sorry... I'm ashamed, but you look beautiful!"

    # game/events/olivia.rpy:798
    old "Сколько округлостей красивых! Глаза разбегаются..."
    new "So many beautiful rounded shapes! It's hard to decide where to look..."

    # game/events/olivia.rpy:798
    old "Как-то даже в шортах тесно стало..."
    new "Somehow even the shorts became cramped..."

    # game/events/olivia.rpy:798
    old "Кого-то угораздило, очень удачно для меня, снять трусики..."
    new "Someone had the misfortune, but very fortunate for me, to take their panties off..."

    # game/events/olivia.rpy:798
    old "Извините... Вид, конечно, очаровательнее некуда!"
    new "Excuse me... The view, of course, couldn't be more enchanting!"

    # game/events/olivia.rpy:865
    old "Да ладно вам, можете меня не стесняться!"
    new "Come on, don't be embarrassed by me!"

    # game/events/olivia.rpy:865
    old "Извините, не удержался... Хорошо выглядите, девчонки!"
    new "Sorry, I couldn't resist... You girls look good!"

    # game/events/olivia.rpy:883
    old "А вы представьте, что меня тут на самом деле нет!"
    new "Imagine I'm not really here!"

    # game/events/olivia.rpy:883
    old "Извините, не удержался... Красота ведь неописуемая!"
    new "Sorry, I couldn't resist... It's beautiful, isn't it?"

# TODO: Translation updated at 2022-01-27 20:19

# game/events/olivia.rpy:986
translate english olivia_ann_meeting_72a9dc2e:

    # Ann_00 "Что-то случилось, дорогой?"
    Ann_00 "Is something wrong, honey?"

# game/events/olivia.rpy:987
translate english olivia_ann_meeting_61ee095e:

    # Max_01 "Пока что нет, но к нам в гости сейчас должна прийти одноклассница Лизы."
    Max_01 "Not yet, but a classmate of Lisa's is coming to visit."

# game/events/olivia.rpy:988
translate english olivia_ann_meeting_12ec5499:

    # Ann_04 "Ой! У Лизы подружка появилась? Это очень хорошо."
    Ann_04 "Oh! Lisa has a girlfriend? That's very good."

# game/events/olivia.rpy:989
translate english olivia_ann_meeting_a910fad5:

    # Max_04 "Да и она уже была у нас в гостях. Днём после школы..."
    Max_04 "Yes, and she had already been to visit us. In the afternoon after school..."

# game/events/olivia.rpy:995
translate english olivia_ann_meeting_0eda89f8:

    # Lisa_01 "Мама, знакомься, это Оливия, моя одноклассница!"
    Lisa_01 "Mom, this is Olivia, my classmate!"

# game/events/olivia.rpy:996
translate english olivia_ann_meeting_fb8ee967:

    # Olivia_04 "Здравствуйте! Мне очень приятно с вами познакомиться. У вас потрясающая дочь!"
    Olivia_04 "Hello! It is a pleasure to meet you. You have an amazing daughter!"

# game/events/olivia.rpy:1004
translate english olivia_ann_meeting_3cbfb1ec:

    # Ann_08 "Здравствуй, Оливия. Мне тоже приятно. А ещё я очень рада, что Лиза наконец-то нашла себе подругу. А то новое место, сама понимаешь..."
    Ann_08 "Hello, Olivia. It's nice to meet you, too. I'm also very glad that Lisa finally found a friend. The new place, you know..."

# game/events/olivia.rpy:1005
translate english olivia_ann_meeting_e555d37c:

    # Olivia_05 "Я тоже рада. У вас такой большой, красивый и стильный дом... И такой классный бассейн!"
    Olivia_05 "I'm glad, too. You have such a big, beautiful and stylish house... And such a great pool!"

# game/events/olivia.rpy:1006
translate english olivia_ann_meeting_7ba98055:

    # Lisa_02 "Мам, мы с Оливией сейчас переоденемся и будем в бассейне плескаться, ты не против?"
    Lisa_02 "Mom, Olivia and I are going to change and splash around in the pool, do you mind?"

# game/events/olivia.rpy:1007
translate english olivia_ann_meeting_ed750a19:

    # Ann_02 "Конечно, девочки, развлекайтесь сколько угодно."
    Ann_02 "Sure, girls, have all the fun you want."

# game/events/olivia.rpy:1008
translate english olivia_ann_meeting_bf2b83f9:

    # Olivia_03 "Привет, Макс! Рада тебя видеть."
    Olivia_03 "Hi, Max! It's good to see you."

# game/events/olivia.rpy:1009
translate english olivia_ann_meeting_0b901396:

    # Max_02 "Привет, Оливия! Хорошо, что заглянула."
    Max_02 "Hey, Olivia! Good of you to stop by."

# game/events/olivia.rpy:1016
translate english olivia_ann_meeting_2bb1adf1:

    # Ann_05 "Девочка, вроде, хорошая. Ладно, пусть развлекаются, а я пойду отдохну к себе."
    Ann_05 "The girl seems nice. Well, let them have their fun, and I'm going to rest in my room."

# game/events/olivia.rpy:1017
translate english olivia_ann_meeting_8a119134:

    # Max_07 "Не торопись, мам. Подожди немного. Тебе нужно кое-что увидеть."
    Max_07 "Take your time, Mom. Wait a little while. There's something you need to see."

# game/events/olivia.rpy:1018
translate english olivia_ann_meeting_6de9e9c0:

    # Ann_07 "Хорошо. Ну, а тебе, Макс, как Оливия, нравится?"
    Ann_07 "Okay. Well, how do you like Olivia, Max?"

# game/events/olivia.rpy:1019
translate english olivia_ann_meeting_87a80392:

    # Max_01 "Да, она ничего такая. Правда, у неё есть... некоторая особенность... Она натуристка. Знаешь, кто это такие?"
    Max_01 "Yeah, she's all right. She does have a... a certain peculiarity... She's a naturist. You know what they are?"

# game/events/olivia.rpy:1020
translate english olivia_ann_meeting_e53b9053:

    # Ann_02 "Хм... Звучит, как уклон во что-то натуральное. Может, это связано с питанием?"
    Ann_02 "Hmm... Sounds like a bias toward something natural. Maybe it has something to do with nutrition?"

# game/events/olivia.rpy:1021
translate english olivia_ann_meeting_ecc99f9b:

    # Max_04 "Сейчас Оливия вернётся и объяснит, кто это такие. А вот как раз и они..."
    Max_04 "Olivia will be right back to explain who they are. And here they are..."

# game/events/olivia.rpy:1029
translate english olivia_ann_meeting_2245b664:

    # Ann_15 "Оливия, а где твой купальник? Лиза, ты почему не дала своей подруге купальник, хотя бы тот, чёрный?"
    Ann_15 "Olivia, where's your swimsuit? Lisa, why didn't you give your friend your swimsuit, at least the black one?"

# game/events/olivia.rpy:1030
translate english olivia_ann_meeting_7eac38fc:

    # Olivia_02 "Да вы не переживайте на этот счёт! Я совсем не стесняюсь. У меня родители натуристы и мы дома все ходим голые, я привыкла!"
    Olivia_02 "Don't worry about it! I'm not shy at all. My parents are naturists and we all go home naked, I'm used to it!"

# game/events/olivia.rpy:1031
translate english olivia_ann_meeting_8dfdf625:

    # Max_03 "У них не семья, а праздник какой-то!"
    Max_03 "It's not a family, it's a holiday!"

# game/events/olivia.rpy:1039
translate english olivia_ann_meeting_2bb03421:

    # Ann_14 "Ну, я рада, что ты не стесняешься, но вот я стесняюсь за Макса... Кажется, он уже отреагировал в своём стиле... У нас он единственный мужчина в доме, поэтому может временами... напрягаться."
    Ann_14 "Well, I'm glad you're not embarrassed, but I'm embarrassed for Max... I think he's already reacted in his own way... He's the only man in the house, so he can get a little... tense."

# game/events/olivia.rpy:1040
translate english olivia_ann_meeting_71cb94fe:

    # Olivia_03 "Да это нормально! У папы такая же реакция была, когда мы вместе с ним купались или загорали, пока он ещё был с нами... Так что, для меня это обычное дело!"
    Olivia_03 "That's normal! My dad had the same reaction when we went swimming with him or sunbathed while he was still with us... So, for me, it's normal!"

# game/events/olivia.rpy:1041
translate english olivia_ann_meeting_e64f98f3:

    # Ann_13 "Это так... странно, Оливия... А где твой папа сейчас?"
    Ann_13 "It's so... strange, Olivia... Where's your dad now?"

# game/events/olivia.rpy:1042
translate english olivia_ann_meeting_65d93586:

    # Olivia_01 "Ну... Они с мамой поругались уже давненько и сейчас он с нами не живёт. А ещё у нас сломался бассейн и его некому чинить... Можно я к вам буду приходить купаться?"
    Olivia_01 "Well... He and my mom had a fight a long time ago and now he doesn't live with us. Also, our pool is broken and there's no one to fix it... Can I come here and use your pool?"

# game/events/olivia.rpy:1043
translate english olivia_ann_meeting_0b55a823:

    # Lisa_03 "Мам, скажи да, скажи да!"
    Lisa_03 "Mom, say yes, say yes!"

# game/events/olivia.rpy:1044
translate english olivia_ann_meeting_902a71d5:

    # Ann_01 "Конечно, Оливия. Если тебя не смущает Макс, который только на тебя и пялится, то я не против. Можешь приходить в любое время! Ну а я пойду по делам, а то уже достаточно позагорала. Развлекайтесь!"
    Ann_01 "Sure, Olivia. If you don't mind Max staring at you, I don't mind. You can come over any time you want! Well, I'm gonna run some errands, because I've had enough sun. Have fun!"

# game/events/olivia.rpy:1052
translate english olivia_ann_meeting_3d9d7b14:

    # Max_02 "Как мне кажется, всё прошло довольно хорошо."
    Max_02 "The way I see it, it went pretty well."

# game/events/olivia.rpy:1053
translate english olivia_ann_meeting_11a6a1b3:

    # Olivia_06 "Думаешь, твоя мама разрешит нам собираться вместе по ночам?"
    Olivia_06 "Do you think your mom will let us get together at night?"

# game/events/olivia.rpy:1054
translate english olivia_ann_meeting_02812b71:

    # Max_07 "Даже если и нет, то я смогу её уговорить. Ну, или как минимум, очень постараюсь уговорить."
    Max_07 "Even if not, I can talk her into it. Well, or at least I'll try very hard to convince her."

# game/events/olivia.rpy:1055
translate english olivia_ann_meeting_512f166a:

    # Lisa_13 "Ты, главное, про меня не забудь в этих уговорах. А то мама запросто может сказать, что я ещё маленькая, хотя с Оливией мы ровесницы."
    Lisa_13 "Don't forget about me in this persuasion. I'm a little girl, but I'm the same age as Olivia, she said."

# game/events/olivia.rpy:1056
translate english olivia_ann_meeting_1a2cd1ba:

    # Max_09 "Не знаю, не знаю, Лиза... Это уже будет непросто. Вот если бы вы с Оливией намазали друг друга кремом для загара, то я бы мог и тебя отстоять перед мамой."
    Max_09 "I don't know, I don't know, Lisa... It's not going to be easy. If you and Olivia put sunscreen on each other, I could stand up to our mother."

# game/events/olivia.rpy:1057
translate english olivia_ann_meeting_d996de1a:

    # Olivia_03 "Ах вот как, да? Это мы запросто!"
    Olivia_03 "Oh, yeah? We can do that!"

# game/events/olivia.rpy:1058
translate english olivia_ann_meeting_41617c79:

    # Lisa_02 "У тебя-то этот самый крем хоть есть?"
    Lisa_02 "Do you even have that cream?"

# game/events/olivia.rpy:1064
translate english olivia_ann_meeting_76e86aa6:

    # Max_10 "Сейчас, к сожалению нет, но я обязательно его куплю."
    Max_10 "Not at the moment, unfortunately, but I will definitely buy it."

# game/events/olivia.rpy:1065
translate english olivia_ann_meeting_77380e2d:

    # Lisa_01 "Очень жаль, Макс. Вот будет крем - мы и намажемся специально для тебя."
    Lisa_01 "That's too bad, Max. When we get some cream, we'll put it on just for you."

# game/events/olivia.rpy:1072
translate english olivia_ann_meeting_4d4507bf:

    # Max_11 "{m}Эх... Такой момент упустил... Теперь только до следующего раза.{/m}"
    Max_11 "{m}Eh... Missed such a moment... Now just until the next time.{/m}"

# game/events/olivia.rpy:1076
translate english olivia_ann_meeting_32d70104:

    # Max_05 "Разумеется есть! Могу даже сам вас намазать, руки у меня почти не заняты."
    Max_05 "Of course there is! I can even smear it on you myself, my hands are hardly busy."

# game/events/olivia.rpy:1077
translate english olivia_ann_meeting_c7b26a60:

    # Lisa_01 "О нет... За крем, конечно, спасибо, но от твоей помощи мы как-нибудь воздержимся, правда Оливия? Не хватало, чтобы ты свои озабоченные ручонки распускал."
    Lisa_01 "Oh no... Thanks for the cream, of course, but we'll refrain from your help, won't we, Olivia? I don't want you to get your horny little hands all over the place."

# game/events/olivia.rpy:1079
translate english olivia_ann_meeting_2aec5f80:

    # Olivia_05 "Ага, мы и сами прекрасно справимся. Давай крем. Чур гостей мажут первыми!" nointeract
    Olivia_05 "Yeah, we're fine on our own. Give me the cream. Dibs on the guests first!" nointeract

# game/events/olivia.rpy:1098
translate english olivia_lisa_sunscreen_0_916ccd9a:

    # Lisa_05 "Я же ведь помажу только спину, Оливия? Остальное ты сама..."
    Lisa_05 "I'm only going to do the back, aren't I, Olivia? You do the rest..."

# game/events/olivia.rpy:1099
translate english olivia_lisa_sunscreen_0_4b7517eb:

    # Max_08 "Э-э-э! Какую только спину?! Тебе нужно натереть кремом всё её тело, Лиза."
    Max_08 "Uh-uh! What back?! You need to rub cream all over her body, Lisa."

# game/events/olivia.rpy:1100
translate english olivia_lisa_sunscreen_0_9a51e79c:

    # Olivia_03 "Ой, Макс, вот ты разбежался. Может ты тогда за фотоаппаратом или видеокамерой по такому случаю сразу сбегаешь?"
    Olivia_03 "Oh, Max, you're such a scatterbrain. Maybe you'll run right away for a camera or a video camera on such an occasion?"

# game/events/olivia.rpy:1101
translate english olivia_lisa_sunscreen_0_63f097d0:

    # Max_03 "Что-то мне подсказывает, что пока я буду бегать, вы уже закончите. Я лучше останусь здесь, чтобы ничего не пропустить."
    Max_03 "Something tells me that while I'm running, you'll be finished. I'd better stay here so I don't miss anything."

# game/events/olivia.rpy:1102
translate english olivia_lisa_sunscreen_0_62a4a929:

    # Lisa_02 "Эх, а мы ведь могли бы и подождать."
    Lisa_02 "Eh, and we could have waited."

# game/events/olivia.rpy:1103
translate english olivia_lisa_sunscreen_0_963a3477:

    # Max_09 "Нет, меня не проведёшь! Эй, ты ей попку совсем не натёрла, не говоря уже обо всём остальном... Это не дело!"
    Max_09 "No, I'm not fooled! Hey, you haven't rubbed her bottom at all, let alone everything else... That's not the point!"

# game/events/olivia.rpy:1104
translate english olivia_lisa_sunscreen_0_58ca6b5e:

    # Olivia_05 "Со всем остальным я прекрасно справлюсь сама. Поворачивайся ко мне, Лиза, я тебя тоже намажу."
    Olivia_05 "I can do the rest just fine on my own. Turn to me, Lisa, and I'll put some on you, too."

# game/events/olivia.rpy:1115
translate english olivia_lisa_sunscreen_0_a00e937b:

    # Max_02 "Оливия, ты бы ей купальник развязала. Так будет удобнее..."
    Max_02 "Olivia, you should untie her swimsuit. It would be more comfortable..."

# game/events/olivia.rpy:1116
translate english olivia_lisa_sunscreen_0_1b5600b7:

    # Lisa_01 "В твоих похотливых идеях, Макс, здесь никто не нуждается! Мы сами разберёмся."
    Lisa_01 "No one here needs your lustful ideas, Max! We'll figure it out."

# game/events/olivia.rpy:1117
translate english olivia_lisa_sunscreen_0_c5a55342:

    # Max_07 "Лиза, я же за тебя переживаю. Вдруг Оливия из-за твоего купальника намажет не всё."
    Max_07 "Lisa, I'm worried about you. What if Olivia won't put everything on because of your swimsuit?"

# game/events/olivia.rpy:1118
translate english olivia_lisa_sunscreen_0_5e735faf:

    # Olivia_02 "Не переживай, я ничего не упущу. Или, может, всё-таки развязать купальник?"
    Olivia_02 "Don't worry, I won't miss a thing. Or should I untie my swimsuit?"

# game/events/olivia.rpy:1137
translate english olivia_lisa_sunscreen_0_95df47e9:

    # Max_05 "Никаких \"может\". Снимай с неё всё!"
    Max_05 "No \"maybes.\" Take it all off!"

# game/events/olivia.rpy:1120
translate english olivia_lisa_sunscreen_0_7e394240:

    # Lisa_11 "Нет, ребята, вы чего?! Не надо меня раздевать, мне и так хорошо."
    Lisa_11 "No, guys, what are you doing?! You don't have to undress me, I'm fine as I am."

# game/events/olivia.rpy:1121
translate english olivia_lisa_sunscreen_0_2c278307:

    # Olivia_01 "Тогда всё, Лиза, я закончила. А тебе, Макс, спасибо за крем."
    Olivia_01 "That's it then, Lisa, I'm done. And to you, Max, thank you for the cream."

# game/events/olivia.rpy:1122
translate english olivia_lisa_sunscreen_0_ffc96ca2:

    # Max_01 "Не за что, девчонки. Мне всё понравилось, но можно было это делать и более страстно."
    Max_01 "You're welcome, girls. I liked it all, but it could have been done more passionately."

# game/events/olivia.rpy:1123
translate english olivia_lisa_sunscreen_0_c7bb4508:

    # Lisa_03 "Других пожеланий я и не ожидала."
    Lisa_03 "I didn't expect anything else."

# TODO: Translation updated at 2022-02-09 00:00

# game/events/olivia.rpy:184
translate english olivia_first_night_visit_af6afb32:

    # Olivia_02 "Значит, кроме нас и Лизы дома только ваша мама?"
    Olivia_02 "So, besides us and Lisa, it's just your mom at home?"

# game/events/olivia.rpy:185
translate english olivia_first_night_visit_40050d52:

    # Max_03 "Да... Алиса с тётей Кирой как всегда в это время тусят где-то допоздна. Мама у себя в комнате, но может прийти нас проверить. Главное сильно не шуметь."
    Max_03 "Yes... Alice and Aunt Kira are out late as usual. Mom's in her room, but she can come check on us. The main thing is not to make too much noise."

# game/events/olivia.rpy:265
translate english olivia_second_night_visit_4bfe6ec9:

    # Olivia_03 "Лиза, а ты так и будешь в маечке и трусиках? Мы ведь шуметь не собираемся, так что твоя мама ничего не увидит. А даже если и придёт, то мы тебя прикроем."
    Olivia_03 "Lisa, are you going to stay in your top and panties? We're not going to make any noise, so your mom won't see anything. And even if she does, we'll cover for you."

# game/events/olivia.rpy:266
translate english olivia_second_night_visit_96357a48:

    # Lisa_09 "Да я больше не из-за мамы, а из-за Макса. Только и будет делать, что пялиться."
    Lisa_09 "It's not because of my mom anymore, it's because of Max. All he'll do is stare."

# game/events/olivia.rpy:309
translate english olivia_night_visit_r_11acffe5:

    # Olivia_01 "Привет, Макс. А вот и я! Как дома обстановка?"
    Olivia_01 "Hey, Max. There I am! How's the house?"

# game/events/olivia.rpy:310
translate english olivia_night_visit_r_e85cfe71:

    # Max_04 "Привет! Рад тебя видеть. Дома только я, Лиза и мама. Проходи..."
    Max_04 "Hi! Good to see you. It's just me, Lisa, and Mom at home. Come on in..."

# TODO: Translation updated at 2022-02-16 00:40

# game/events/olivia.rpy:683
translate english olivia_repeatable_night_out_with_eab840c0:

    # Lisa_02 "И с чего ты взял, что нам надоест? Я даже маечку одевать не буду. Макс уже всё видел, а без неё удобнее..."
    Lisa_02 "And what makes you think we'll get bored? I'm not even going to wear a tank top. Max has already seen everything, and it's more comfortable without it..."

# game/events/olivia.rpy:684
translate english olivia_repeatable_night_out_with_ba36eeb5:

    # Max_02 "Ох, что за дивный вечерок!"
    Max_02 "Oh, what a lovely evening!"

# game/events/olivia.rpy:685
translate english olivia_repeatable_night_out_with_0a6cf6a4:

    # Olivia_01 "Ночь вообще-то, Макс. Укладываемся спать..."
    Olivia_01 "It's the middle of the night, Max. Let's go to bed..."
