...
label lisa_eric_sex_ed_practice:
    ...
    label .practice_1:  # первая практика
        if not dishes_washed:
            ...
            $ var_pose = '00'
            scene Lisa_practice with fade4
            if get_rel_eric()[0] == 3:
                # Настоящая дружба
                ...
                # Фальшивая дружба и Откровенная вражда
        ...
        if get_rel_eric()[0] == 3:
            ...
            $ ClothingNps('eric', eric.plan_name)
            return
        else:
            # Фальшивая дружба и Откровенная вражда
            ...
            if get_rel_eric()[0] < 0:
                ...
                $ poss['seduction'].open(28)    # Макс помешал Эрику и Лизе во время практических занятий (Откровенная вражда)
            else:
                # Фальшивая дружба
                ...
                menu:
                    Max_00 "Понял, постараюсь не облажаться."
                    "{i}уйти мыть посуду{/i}":
                        $ flags.lisa_sexed = 10         # Эрик уступил Лизу Максу (Д-)
                        $ lisa.dcv.seduce.stage = 5     # можно поговорить с Лизой о книжке
                        $ poss['seduction'].open(31)    # Макс помешал Эрику и Лизе во время практических занятий (Фальшивая дружба)
    ...
    label .practice_2:  # вторая практика
        # 2-ая практика, только для для Настоящей дружбы
        if not dishes_washed:
            # Макс не стал мыть посуду за Лизу и подглядывает с самого начала (полная версия 2-ой практики)
            # myroom-lessons-00 + myroom-lessons-00-lisa-(01a/01b)-eric-(01/01a)
            $ var_pose = '00'
            scene Lisa_practice with fade4
        ...
        if dishes_washed:
            with fade4
            # Макс решил сперва помыть посуду, а уже потом подглядывать (укороченная версия 2-ой практики)
        ...
        # myroom-lessons-04 + myroom-lessons-04-eric-(01/01a) + myroom-lessons-04-lisa-(01a/01b)
        $ var_pose = '04'
        ...
        # myroom-lessons-05 + myroom-lessons-05-lisa-(01a/01b) + myroom-lessons-05-eric-(01/01a)
        $ var_pose = '05'
...
label lisa_eric_zero_practice_war:
    ...
    menu:
        "{i}сперва помыть посуду, а затем проследить{/i}" if get_rel_eric()[0] != 2:    # !!!Для хитрой дружбы только вариант "проследить сразу"!!!
            # Макс моет посуду
            $ dishes_washed = True
