...
label cam0_ann_yoga:
    if int(tm[3:4])%3 == 0: # смена позы каждые 10 минут
        $ renpy.show('Ann cams yoga 0'+str(random_randint(1, 3))+ann.dress, at_list=[laptop_screen])
    elif int(tm[3:4])%3 == 1:
        $ renpy.show('Ann cams yoga 0'+str(random_randint(4, 6))+ann.dress, at_list=[laptop_screen])
    else:
        $ renpy.show('Ann cams yoga 0'+str(random_randint(7, 9))+ann.dress, at_list=[laptop_screen])
    show FG cam-shum-act at laptop_screen
