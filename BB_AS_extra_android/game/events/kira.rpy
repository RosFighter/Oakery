...
label kira_bath:
    ...
    menu:
        ...
        "{i}постучаться{/i}" if not kira.flags.m_foot:
            ...
            if flags.ladder < 1:
                Max_09 "{m}Нужно найти способ как-то подглядеть за девчонками в ванной. Может, попробовать вечером со двора?!{/m}"   #если Макс ни разу не пытался подглядеть вечером за девчонками в ванной
            elif flags.ladder < 2:
                Max_09 "{m}Нужно найти способ как-то подглядеть за девчонками в ванной. Может, стоит осмотреться изнутри?!{/m}"    # если Макс подглядывал со двора, но сам не мылся
            elif flags.ladder < 3:
                Max_09 "{m}Нужно уже наконец купить стремянку, чтобы подглядывать за тем, что происходит в ванной...{/m}"   #если всё ещё не куплена стремянка, но она уже есть в магазине
            else:
                Max_09 "{m}В следующий раз лучше заглянуть со стремянки...{/m}"     # стремянка уже установлена
            jump .end

        "{i}войти{/i}" if kira.flags.m_foot > 0:
            # уже состоялся массаж ног в ванной
            if flags.eric_banished and not kira.flags.showdown_e:
                # Эрика выгнали, но Макс ещё не поговорил об этом с Кирой
    ...
    label .ladder:
        ...
        if _in_replay or all([kira.flags.porno, lisa.flags.m_foot>1, poss['massage'].used(4)]):
            # совместный просмотр порно уже был, Макс делал девчонкам массаж ног, был полностью успешный массаж Алисе со снятым верхом купальника
            show Kira bath-window 02
            Max_05 "{m}Ого! Да она сперва решила поиграть со своей киской... Вот это классно! Только почему бы не делать это лёжа в ванной? Такое ощущение, что она это специально...{/m}"   #если был совместный просмотр порно с Кирой
            ...
            menu:
                Max_08 "{m}Что? Она зовёт меня к себе?! Да ладно! Как-то мне это не очень нравится...{/m}"
                "{i}идти к Кире{/i}":
                    $ kira.flags.m_foot = 1
                    if not _in_replay:
                        $ poss['massage'].open(5)
    ...
    label .kira_mass_bath_first:
        ...
        if not _in_replay:
            $ poss['aunt'].open(4)  # Макс попадает к Кире в ваннуюи делает ей массаж ног
            $ mgg.cleanness = 100
    ...
    label .mass_bath:
        ...
        if lisa.dcv.seduce.stage<2:
            # первый урок поцелуев у Киры ещё не пройден
            ...
            if not _in_replay:
                ...
                $ poss['aunt'].open(5)  # Макс получает fj от Киры
            jump .end_sleep
    ...
    label .cuni_bj:
        ...
        menu:
            ...
            "{i}работать языком быстрее{/i}" if kira.dcv.photo.stage < 2 or not _in_replay:
                pass
            "Садись на меня, тётя Кира!" if kira.dcv.photo.stage > 1:
                jump .horsewoman
            "Давай-ка сюда свою шикарную попку!" if kira.dcv.photo.stage > 1:
                jump .dogstyle
        ...
        menu:
            Max_22 "{m}Вот чёрт! Она так глубоко насаживает на него свой ротик... Ещё чуть-чуть и я взорвусь! А она сосёт всё быстрее...{/m}"
    ...
    label .caught:
        ...
        if get_rel_eric()[0] == 3 and kira.dcv.battle.stage == 1:
            ...
            $ kira.dcv.battle.stage = 2     # Эрик поймал после сдачи
            ...
            $ kira.dcv.battle.stage = 3     # Эрик поймал сам
        menu:
    ...
    if any([kira.flags.promise,                                 # за Максом должок
        ...
        menu:
            ...
            "Да, что-то не спится... К тебе можно?" if kira.stat.handjob < 1:
                ...
                $ renpy.show('Kira pool-night '+random_choice(['01', '02', '03']))
...
label kira_sleep_night:
    ...
    $ kira.hourly.sleep = 1
    menu:
        Max_04 "{m}Ага! Тётя Кира спит. Хорошо, что её ночнушка слегка просвечивает... Ухх, как она хороша...{/m}"
        "{i}подойти поближе{/i}":
...
label kira_night_tv:
    ...
    if 'serial' in film:
        # кира смотрит сериал
        menu:
            Max_01 "{m}Тётя Кира ещё не легла спать... смотрит сериалы. Может, стоит задержаться?{/m}"
            "{i}подойти ближе{/i}":
                pass
        scene BG lounge-tv-00
        $ renpy.show('Kira tv '+pose3_4+kira.dress)
        menu:
            Max_04 "{m}Да-а-а, кто-то залипает на сериалы, а я вот залипаю на свою тётю...{/m}"
            "Тётя Кира?":
                # lounge-tv-01 + tv-kira-(01/02/03) + tv-max-00b
                $ var_pose = '00'
                scene tv_talk kira
                ...
                menu:
                    ...
                    "Мне что-то не спится. Можно тоже телик с тобой посмотреть?" if 'kira' not in flags.how_to_kiss:
                        menu:
            ...
            "{i}посмотреть, что будет дальше{/i}":
                ...
                menu:
                    Max_05 "{m}Ухх... Она начала ласкать свою киску... Вот это уже горячо!{/m}"
                    "Тётя Кира?":
                        # lounge-tv-01 + tv-kira-m-(01/02/03) + tv-max-00b
                        $ var_pose = '00'
                        ...
                        menu:
                            ...
                            "Ни капли!" if poss['aunt'].used(2):
                                ...
                                jump .porn
                            "Да ты продолжай, я посижу, посмотрю..." if poss['aunt'].used(2):
                                # lounge-tv-01 + tv-kira-(01/02/03) + tv-max-00b
                                scene tv_talk kira
                                # $ renpy.show('Kira tv-closer '+pose3_4+kira.dress)
        ...
        if not _in_replay:
            $ poss['aunt'].open(2)  # Макс видид, как Кира смотрит порно
    if not _in_replay:
        $ SetCamsGrow(house[4], 150)

    menu:
        Max_01 "{m}Тётя Кира ещё не легла спать... смотрит порнушку. Может, стоит задержаться?{/m}"
        "{i}подойти ближе{/i}":
            pass
    scene BG lounge-tv-00
    $ renpy.show('Kira tv m-'+pose3_4+kira.dress)
    menu:
        Max_05 "{m}Ухх... Она начала ласкать свою киску... Вот это уже горячо! От такой компании я не откажусь...{/m}"
        "Тётя Кира?":
            # lounge-tv-01 + tv-kira-m-(01/02/03) + tv-max-00b
            $ var_pose = '00'
            scene tv_talk kira_m
        ...
        menu:
            ...
            "Вообще-то, с лучшей тётей на свете!" ('soc', mgg.social * 2, 90) if (online_cources[0].cources[1].less > 0 and kira.dcv.feature.stage>2) or lisa.dcv.seduce.stage>3:
                ...
                if rand_result:
                    ...
                    $ poss['aunt'].open(3)  # Макс смог уговорить Киру на совместный просмотр порно
                else:
                    jump .failed
            "Вообще-то, с лучшей тётей на свете!" if not ((online_cources[0].cources[1].less > 0 and kira.dcv.feature.stage>2) or lisa.dcv.seduce.stage>3):  # пока НЕ пройден 1й урок второго курса общения и первые три разговора с Кирой, убедить нельзя
                jump .failed
    ...
    label .porn_view:
        ...
        if kira.flags.porno==1:
            ...
            # tv-mass-01 + tv-wp-01-max-01-kira-01
            scene BG tv-mass-01
            ...
            # tv-mass-01 + tv-wp-01-max-02-kira-02
            ...
            menu:
                Max_20 "{m}Да, Кира... Твоя помощь бы мне не помешала... Хотя... похоже, я сейчас кончу!{/m}"
                "{i}кончить{/i}":
                    pass

            # tv-kiss-03 + tv-wp-03-max-01-kira-01
            scene BG tv-kiss-03
            ...
            if persistent.memories['kira_night_tv.porn_view'] < 1:
                $ persistent.memories['kira_night_tv.porn_view'] = 1
            #ракурс с экраном
            scene BG tv-watch-01
            $ renpy.show('tv kira-porn-00 '+random_choice(['01', '02', '03']), at_list=[tv_screen,])
            $ renpy.show('Kira tv-watch 01'+kira.dress)
            show Max tv-watch 01c
            ...
            if kira.flags.m_foot:
                ...
                menu:
                    ...
                    "{i}снять шорты{/i}" if not kira.stat.blowjob and not kira.flags.promise:
                        # tv-mass-05 + tv-wp-02-max-01a-kira-01a
                        $ var_stage = 'a'
                        scene kira_watch_play_02
                        # $ suf = 'a'
            ...
            menu:
                Max_20 "{m}Да, Кира... Твоя помощь бы мне не помешала... Хотя... похоже, я сейчас кончу и без этого!{/m}"
                "{i}кончить{/i}":
                    # tv-kiss-03 + tv-wp-03-max-01-kira-01 или + tv-wp-03-max-01a-kira-01a
                    scene BG tv-kiss-03
                    $ renpy.show('Kira tv-closer max-04'+var_stage)
    ...
    label .promise_cuni:
        ...
        menu:
            ...
            "{i}раздеть её и повернуть{/i}":
                $ r1 = random_choice([1, 2])
        if r1<2:
            # lounge-tv-01 + tv-cun-02-max-01a-kira-01a
        ...
        menu:
            ...
            "{i}продолжить языком{/i}":
                $ r1 = random_choice([1, 2, 3])
        if r1<2:
            # lounge-tv-01 + tv-cun-04-max-01a-kira-01a
            scene tv lounge-tv-01
    ...
    label .first_lesson:
        ...
        menu:
            ...
            "{i}поцеловать её{/i}":
                pass
        $ r1 = random_choice(['02', '03'])
        $ renpy.show('Kira tv-kiss 1-'+r1)
        menu:
            Max_03 "{m}Ого... Какой горячий ротик... И она так ловко орудует язычком... Так... И что мне, как... Эх, учиться мне ещё и учиться... А что, я очень даже не против такому учиться!{/m}"
    ...
    label .second_lesson:
        ...
        $ renpy.show('Kira tv-kiss 1-'+r1)
        menu:
            Max_03 "{m}О да! Кажется, мне никогда не надоест такая практика... Она так увлечённо целуется... Что она там говорила, про нужный момент? Может быть, это как раз он?{/m}"
            ...
            "{i}подтянуть её ближе к себе{/i}":
                ...
                menu:
                    ...
                    "{i}прикоснуться к её попке{/i}":
                        ...
                        $ Skill('kissing', 0.2, 3.0)
                        menu:
                            Max_03 "{m}Обалденно! Неужели это не сон и я действительно трогаю её суперскую попку?! Ну и что, что это попка моей тёти... На ощупь она просто восхитительна - нежная и упругая! Но пора двигаться выше. Охх... Кажется, я начинаю возбуждаться...{/m}"
                            "{i}прикоснуться к её груди{/i}":
                                pass
    ...
    label .repeat_lesson:
        ...
        menu:
            ...
            "{i}поцеловать её{/i}":
                $ renpy.show('Kira tv-kiss 1-'+random_choice(['02', '03']))
        menu:
            Max_04 "{m}Кажется, мне никогда не надоест это делать с такой женщиной... Ну и что, что она моя тётя... Но она так классно целуется!{/m}"
            "{i}подтянуть её ближе к себе{/i}":
                scene BG tv-mass-05
                show Kira tv-kiss 2-01
        ...
        show Kira tv-kiss 3-01
        menu:
            Max_02 "{m}О да! Как же это приятно, ощущать в руке упругость груди тёти Киры и наслаждаться её жаркими поцелуями... О таких уроках я и мечтать раньше не мог!{/m}"
            "{i}развязать её ночнушку{/i}":
                show Kira tv-kiss 3-02
    ...
    label .tv_cuni:
        if not _in_replay:
            $ SetCamsGrow(house[4], 200)
        # tv-mass-05 + tv-wp-02-max-02-kira-02
        scene BG tv-mass-05
        show Kira tv-closer max-03b
        ...
        menu:
            ...
            "{i}начать ласкать её киску языком{/i}":
                # tv-mass-03 + tv-cun-01-max-01a-kira-01
                scene BG tv-mass-03
                show Kira tv-game cun-01ab
        menu:
            Kira_02 "Макс... Если тебе мешают мои трусики, я их сниму... Или сними их сам."
            "{i}раздеть её{/i}":
                # tv-cun-01 + tv-cun-01-max-02a-kira-02a
                scene BG tv-cun-01
...
label kira_shower:
    ...
    if kira.daily.shower > 0:
        menu:
            Max_00 "{m}Кира сейчас принимает душ, не стоит ей мешать...{/m}"
            "{i}уйти{/i}":
                return
    $ kira.daily.shower = 1
    menu:
    ...
    label .ladder:
        ...
        $ kira.daily.shower = 2
        $ r1 = random_choice(['b', 'c', 'd'])
        $ renpy.show('Kira bath-window-morning '+random_choice(['01', '02', '03'])+r1)
    ...
    label .promise_cuni:
        ...
        menu:
            ...
            "{i}раздеться и присоединиться к Кире{/i}":
                $ renpy.show('Kira shower-Max '+random_choice(['01','02']))
...
label kira_lisa_shower:
    ...
    if lisa.daily.shower > 0:
        menu:
            Max_00 "{m}Лиза и Кира принимают душ, не стоит им мешать...{/m}"
            "{i}уйти{/i}":
                return
    $ lisa.daily.shower = 1
    menu:
    ...
    label .ladder:
        ...
        if r0 == 1:
            ...
            $ renpy.show('Lisa bath-window-morning '+random_choice(['01', '02', '03'])+r1, at_list=[ladder_left_shift,])
            $ renpy.show('Kira bath-window-morning '+random_choice(['01', '02', '03'])+r1, at_list=[ladder_right_shift,])
            show FG bathroom-morning-00
            if r1=='c':
...
label kira_alice_shower:
    ...
    if alice.daily.shower > 0:
        menu:
            Max_00 "{m}Кира и Алиса принимают душ, не стоит им мешать...{/m}"
            "{i}уйти{/i}":
                return
...
label return_from_club:
    ...
    label .striptease:
        ...
        $ renpy.show("Kira after-club s03-"+random_choice(['01', '02', '03']))
        ...
        $ renpy.show("Kira after-club s04-"+random_choice(['01', '02', '03']))
        ...
        $ renpy.show("Kira after-club s05-"+random_choice(['01', '02', '03']))
        ...
        if kira.dcv.feature.stage < 6:
            ...
            $ renpy.show("Kira after-club s05-"+random_choice(['04', '05']))
    ...
    label .boobs:
        ...
        menu:
            Max_01 "{m}Надеюсь, мама не будет сильно её осуждать за то, что она легла спать голая... С кем не бывает! А хороша тётя, так и гипнотизирует своей красотой. Но пора и мне спать...{/m}"
            "{i}отправиться спать{/i}":
                $ renpy.end_replay()
                $ current_room = house[0]
                jump Sleep
    ...
    label .ass:
        ...
        menu:
            Max_01 "{m}Надеюсь, мама не будет сильно её ругать за то, что она легла спать голая... Со всеми может случиться! А хороша тётя, так и гипнотизирует своей красотой. Но пора и мне спать...{/m}"
            "{i}отправиться спать{/i}":
                $ renpy.end_replay()
                $ current_room = house[0]
                jump Sleep
...
label kira_bath_with_eric:
    ...
    menu:
        ...
        "{i}воспользоваться стремянкой{/i}" if flags.ladder > 2:
            ...
            if kira.dcv.battle.stage > 2:
                Max_10 "{m}Хватит уже трахать её в рот, как какую-то порнозвезду! Она конечно порнозвезда, но не круглые же сутки...{/m}"   # Макс не сдавал Киру
            else:
                Max_07 "{m}Неплохо так Эрик её голову на свой член напяливает! Как будто так и надо...{/m}"   # Макс сдал Киру
            $ spent_time += 10
            ...
            if kira.dcv.battle.stage > 2:
                menu:
                    Max_09 "{m}Вот же сволочь! Спустил всё до последней капли Кире прямо в рот! Ну Эрик, ты у меня сполна получишь рано или поздно...{/m}"   # Макс не сдавал Киру
                    "{i}уйти{/i}":
                        pass
            else:
                menu:
                    Max_02 "{m}Кира не упустила ни единой капли мимо! И как будто никто ни у кого только что смачно не отсасывал... Чистая работа!{/m}"   # Макс сдал Киру
                    "{i}уйти{/i}":
                        pass
