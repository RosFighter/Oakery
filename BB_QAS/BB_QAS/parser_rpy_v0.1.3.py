...
# папку с нужным языком из старой игры копируем в папку old_tl
...
def insert_from_old():
    ...
    # считываем все файлы в папке old_tl\[lang]
    files = glob(root + '\\old_tl\\' + lang + '\\**\\*.rpy', recursive=True)
    for file in files:
        file_name = file.replace(root + "\\old_tl\\" + lang+"\\", '')
