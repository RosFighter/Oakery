# TODO: Translation updated at 2021-09-01 17:03

translate german strings:

    # game/screens.rpy:244
    old "массаж, "
    new "Massage, "

    # game/screens.rpy:244
    old "убеждение, "
    new "Überzeugung, "

    # game/screens.rpy:244
    old "сексуальный опыт, "
    new "sexuelle Erfahrung, "

    # game/screens.rpy:244
    old "поцелуи, "
    new "Küssen, "

    # game/screens.rpy:244
    old "удача, "
    new "Glück, "

    # game/screens.rpy:244
    old "скрытность, "
    new "Schleichen, "

    # game/screens.rpy:270
    old "[txt] \n{i}{size=[sz]}{color=[col]}([skill!t]шанс: [vis]%){/color}{/size}{/i}"
    new "[txt] \n{i}{size=[sz]}{color=[col]}([skill!t]chance: [vis]%){/color}{/size}{/i}"

    # game/screens.rpy:328
    old "История"
    new "History"

    # game/screens.rpy:329
    old "Пропуск"
    new "Bestätigen"

    # game/screens.rpy:330
    old "Авто"
    new "Auto"

    # game/screens.rpy:331
    old "Сохранить"
    new "Speichern"

    # game/screens.rpy:332
    old "Б.Сохр"
    new "S.Schpeichern"

    # game/screens.rpy:333
    old "Б.Загр"
    new "S.Laden"

    # game/screens.rpy:334
    old "Опции"
    new "Optionen"

    # game/screens.rpy:374
    old "Начать новую игру"
    new "Neues Spiel starten"

    # game/screens.rpy:380
    old "Загрузить игру"
    new "Spiel laden"

    # game/screens.rpy:383
    old "Сохранить игру"
    new "Spiel speichern"

    # game/screens.rpy:387
    old "Настройки"
    new "Einstellungen"

    # game/screens.rpy:392
    old "Завершить повтор"
    new "Wiederholung beenden"

    # game/screens.rpy:399
    old "Выйти из игры"
    new "Spiel beenden"

    # game/screens.rpy:405
    old "Назад"
    new "Zurück"

    # game/screens.rpy:482
    old "НОВАЯ ИГРА"
    new "NEUES SPIEL"

    # game/screens.rpy:488
    old "ПРОДОЛЖИТЬ"
    new "FORTSETZEN"

    # game/screens.rpy:491
    old "ЗАГРУЗИТЬ"
    new "LADEN"

    # game/screens.rpy:495
    old "НАСТРОЙКИ"
    new "EINSTELLUNGEN"

    # game/screens.rpy:496
    old "ВЫЙТИ"
    new "ENDE"

    # game/screens.rpy:527
    old "ГЛАВНОЕ МЕНЮ"
    new "HAUPTMENUE"

    # game/screens.rpy:661
    old "Введите описание файла сохранения:"
    new "Beschreibung für den Spielstand angeben:"

    # game/screens.rpy:682
    old "(Нет описания)"
    new "(keine Beschreibung)"

    # game/screens.rpy:701
    old "{} страница"
    new "Seite {}"

    # game/screens.rpy:701
    old "Автосохранения"
    new "Autom. Speicher"

    # game/screens.rpy:701
    old "Быстрые сохранения"
    new "Schnellspeichern"

    # game/screens.rpy:776
    old "{#file_time}%a, %d %b %Y, %H:%M"
    new "{#file_time}%a, %b %d %Y, %H:%M"

    # game/screens.rpy:776
    old "Пустой слот"
    new "Leerer Platz"

    # game/screens.rpy:784
    old "ДЕНЬ [load_day]"
    new "TAG [load_day]"

    # game/screens.rpy:834
    old "{#file_time}%d %b %Y, %H:%M"
    new "{#file_time}%b %d %Y, %H:%M"

    # game/screens.rpy:858
    old "<"
    new "<"

    # game/screens.rpy:861
    old "{#auto_page}Автосохр."
    new "{#auto_page}Autom. Speichern."

    # game/screens.rpy:864
    old "{#quick_page}Быстрые сохр."
    new "{#quick_page}Schnellspeichern."

    # game/screens.rpy:870
    old ">"
    new ">"

    # game/screens.rpy:939
    old "Режим экрана"
    new "Anzeige"

    # game/screens.rpy:940
    old "Оконный"
    new "Fenster"

    # game/screens.rpy:941
    old "Полный"
    new "Vollbild"

    # game/screens.rpy:944
    old "Сторона отката"
    new "Rollback-Seite"

    # game/screens.rpy:945
    old "Отключено"
    new "Deaktiviert"

    # game/screens.rpy:946
    old "Левая"
    new "Links"

    # game/screens.rpy:947
    old "Правая"
    new "Rechts"

    # game/screens.rpy:953
    old "Всего текста"
    new "Ungesehener Text"

    # game/screens.rpy:954
    old "После выборов"
    new "Nach Auswahl"

    # game/screens.rpy:955
    old "Переходов"
    new "Übergänge"

    # game/screens.rpy:958
    old "Язык"
    new "Sprache"

    # game/screens.rpy:959
    old "Русский"
    new "Русский"

    # game/screens.rpy:960
    old "English"
    new "English"

    # game/screens.rpy:961
    old "Deutsch"
    new "Deutsch"

    # game/screens.rpy:967
    old "Дополнительно"
    new "Zusätzlich"

    # game/screens.rpy:968
    old "Patreon-интро"
    new "Patreon-Intro"

    # game/screens.rpy:969
    old "Оригинальное интро"
    new "originales Intro"

    # game/screens.rpy:973
    old "Запрашивать название при сохранении"
    new "Name beim Speichern abfragen"

    # game/screens.rpy:974
    old "Прозрачное текстовое окно"
    new "transparente Textboxen"

    # game/screens.rpy:975
    old "Отображать все \"Возможности\""
    new "Alle \"Möglichkeiten\" anzeigen"

    # game/screens.rpy:988
    old "Скорость текста"
    new "Text-Geschwindigkeit"

    # game/screens.rpy:992
    old "Скорость авточтения"
    new "Auto-Vorlaufzeit"

    # game/screens.rpy:999
    old "Громкость музыки"
    new "Musik Lautstärke"

    # game/screens.rpy:1006
    old "Громкость звуков"
    new "Sound Lautstärke"

    # game/screens.rpy:1012
    old "Тест"
    new "Test"

    # game/screens.rpy:1016
    old "Громкость голоса"
    new "Sprach Lautstärke"

    # game/screens.rpy:1027
    old "Без звука"
    new "Stumm"

    # game/screens.rpy:1148
    old "История диалогов пуста."
    new "Die Dialoggeschichte ist leer."

    # game/screens.rpy:1211
    old "Помощь"
    new "Hilfe"

    # game/screens.rpy:1220
    old "Клавиатура"
    new "Tastatur"

    # game/screens.rpy:1221
    old "Мышь"
    new "Mause"

    # game/screens.rpy:1224
    old "Геймпад"
    new "Gamepad"

    # game/screens.rpy:1237
    old "Enter"
    new "Enter"

    # game/screens.rpy:1238
    old "Прохождение диалогов, активация интерфейса."
    new "Erweitert den Dialog und aktiviert die Schnittstelle."

    # game/screens.rpy:1241
    old "Пробел"
    new "Platz"

    # game/screens.rpy:1242
    old "Прохождение диалогов без возможности делать выбор."
    new "Fortschritt des Dialogs ohne Auswahlmöglichkeiten."

    # game/screens.rpy:1245
    old "Стрелки"
    new "Pfeiltasten"

    # game/screens.rpy:1246
    old "Навигация по интерфейсу."
    new "Navigation in der Oberfläche."

    # game/screens.rpy:1249
    old "Esc"
    new "Escape"

    # game/screens.rpy:1250
    old "Вход в игровое меню."
    new "Zugang zum Spielmenü."

    # game/screens.rpy:1253
    old "Ctrl"
    new "Strg"

    # game/screens.rpy:1254
    old "Пропускает диалоги, пока зажат."
    new "Überspringt den Dialog, bei Taste gedrückt halten."

    # game/screens.rpy:1257
    old "Tab"
    new "Tab"

    # game/screens.rpy:1258
    old "Включает режим пропуска."
    new "Umschalten des Dialogüberspringens."

    # game/screens.rpy:1261
    old "Page Up"
    new "Seite runter"

    # game/screens.rpy:1262
    old "Откат назад по сюжету игры."
    new "zurück zu früherem Dialog."

    # game/screens.rpy:1265
    old "Page Down"
    new "Seite hoch"

    # game/screens.rpy:1266
    old "Откатывает предыдущее действие вперёд."
    new "weiter zu späterem Dialog."

    # game/screens.rpy:1270
    old "Скрывает интерфейс пользователя."
    new "Benutzeroberfläche ausblenden."

    # game/screens.rpy:1274
    old "Делает снимок экрана."
    new "Bildschirmfoto erstellen."

    # game/screens.rpy:1278
    old "Включает поддерживаемый {a=https://www.renpy.org/l/voicing}синтезатор речи{/a}."
    new "Hilfsmittel umschalten {a=https://www.renpy.org/l/voicing}Sprachausgabe{/a}."

    # game/screens.rpy:1284
    old "Левый клик"
    new "Linksklick"

    # game/screens.rpy:1288
    old "Клик колёсиком"
    new "Mittelklick"

    # game/screens.rpy:1292
    old "Правый клик"
    new "Rechtsklick"

    # game/screens.rpy:1296
    old "Колёсико вверх\nКлик на сторону отката"
    new "Mausrad hoch\nRollback Seite anklicken"

    # game/screens.rpy:1300
    old "Колёсико вниз"
    new "Mausrad runter"

    # game/screens.rpy:1307
    old "Правый триггер\nA/Нижняя кнопка"
    new "rechter Trigger\nA/Taster unten"

    # game/screens.rpy:1311
    old "Левый Триггер\nЛевый Бампер"
    new "linker Trigger\nlinke Schulter"

    # game/screens.rpy:1315
    old "Правый бампер"
    new "rechte Schulter"

    # game/screens.rpy:1320
    old "Крестовина, Стики"
    new "D-Pad, Sticks"

    # game/screens.rpy:1324
    old "Start, Guide"
    new "Start, Guide"

    # game/screens.rpy:1328
    old "Y/Верхняя кнопка"
    new "Y/Top Button"

    # game/screens.rpy:1331
    old "Калибровка"
    new "Kalibrieren"

    # game/screens.rpy:1397
    old "Да"
    new "Ja"

    # game/screens.rpy:1398
    old "Нет"
    new "Nein"

    # game/screens.rpy:1444
    old "Пропускаю"
    new "Überspringen"

    # game/screens.rpy:1668
    old "Скрыть интерфейс"
    new "Interface ausblenden"

    # game/screens.rpy:1669
    old "Меню"
    new "Menü"

# TODO: Translation updated at 2021-10-04 18:39

translate german strings:

    # game/screens.rpy:987
    old "Переодевания Лизы пропускаются, если Макс в комнате"
    new "Lisas Umziehen wird übersprungen, wenn Max im Zimmer ist"

