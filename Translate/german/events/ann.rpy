# TODO: Translation updated at 2021-09-01 17:03

# game/events/ann.rpy:11
translate german ann_sleep_6173ff68:

    # Max_00 "{m}В это время мама обычно спит.\nМне кажется, не стоит её будить...{/m}" nointeract
    Max_00 "{m}Mom schläft um diese Zeit normalerweise.\nes ist wohl besser, wenn ich sie nicht aufwecken...{/m}" nointeract

# game/events/ann.rpy:18
translate german ann_sleep_2e3b88bc:

    # Max_01 "{m}Класс! Мама спит... Даже не верится, что у этой конфетки трое детей... В жизни бы в такое не поверил!{/m}" nointeract
    Max_01 "{m}Ja! Mom schläft... Ich kann nicht glauben, dass dieser heiße Arsch drei Kinder bekommen hat... Ich würde es im Leben nicht glauben!{/m}" nointeract

# game/events/ann.rpy:20
translate german ann_sleep_e9347a52:

    # Max_04 "{m}О, да! Какая у мамы попка! Всё-таки хорошо, что здесь так жарко и все спят не укрываясь... Просто супер!{/m}" nointeract
    Max_04 "{m}Oh ja! Was für ein Arsch! Es ist gut, dass es hier drin so heiß ist und jeder ohne Decke schläft.{/m}" nointeract

# game/events/ann.rpy:22
translate german ann_sleep_f158a828:

    # Max_07 "{m}Обалденно! Как же повезло, что у меня такая горячая мама... Выглядит потрясающе, аж глаза отрывать не хочется!{/m}" nointeract
    Max_07 "{m}Fantastisch! Ich bin so glücklich, so eine heiße Mutter zu haben... Ich will meine Augen nicht von ihr lassen!{/m}" nointeract

# game/events/ann.rpy:25
translate german ann_sleep_78b3f9d0:

    # Max_01 "{m}Класс! Мама спит в ночнушке... Даже не верится, что у этой конфетки трое детей... В жизни бы в такое не поверил!{/m}" nointeract
    Max_01 "{m}Schön! Mom schläft in ihrem Nachthemd... Ich kann nicht glauben, dass sie drei Kinder bekommen hat... Ihr Körper ist perfekt!{/m}" nointeract

# game/events/ann.rpy:27
translate german ann_sleep_19a34828:

    # Max_04 "{m}О, да! Какая у мамы попка! Всё-таки хорошо, что здесь так жарко и все спят не укрываясь... Её попку даже немного видно через ночнушку!{/m}" nointeract
    Max_04 "{m}Oh ja! Was für ein toller Arsch von meiner Mutter! Gut, dass es hier so heiß ist und alle ohne Decke schlafen... Man kann sogar ihren Arsch durch ihr Nachthemd sehen!{/m}" nointeract

# game/events/ann.rpy:29
translate german ann_sleep_1cc5b28f:

    # Max_07 "{m}Обалденно! Как же повезло, что у меня такая горячая мама... В этой ночнушке она выглядит потрясающе, аж глаза отрывать не хочется!{/m}" nointeract
    Max_07 "{m}Fantastisch! Was für ein Glück, dass ich so eine heiße Mutter habe... Sie sieht in diesem Nachthemd umwerfend aus, man kann die Augen nicht von ihr lassen!{/m}" nointeract

# game/events/ann.rpy:38
translate german ann_sleep_d64da859:

    # Max_03 "{m}Чёрт, у меня самая аппетитная мама на свете! Вот бы снять с неё всё белье и пристроиться сзади... Но лучше потихоньку уходить, пока она не проснулась.{/m}" nointeract
    Max_03 "{m}Verdammt, ich habe die schärfste Mutti der Welt! Ich wünschte, ich könnte ihr die gesamte Wäsche ausziehen und mich an sie kuscheln... Aber ich gehe lieber leise, bevor sie aufwacht.{/m}" nointeract

# game/events/ann.rpy:40
translate german ann_sleep_eedcea80:

    # Max_02 "{m}Ухх! Так и хочется прижаться к этой обворожительной попке и шалить всю ночь... Но пора уходить, а то она может проснуться.{/m}" nointeract
    Max_02 "{m}Wow! Ich würde mich am liebsten an diesen hübschen Arsch kuscheln und die ganze Nacht rummachen... Aber es ist Zeit zu gehen, sonst wacht sie vielleicht auf.{/m}" nointeract

# game/events/ann.rpy:42
translate german ann_sleep_abe8ecd4:

    # Max_05 "{m}Вот это да! От вида этих раздвинутых ножек становится всё равно, что она моя мама... Слишком соблазнительно! Только бы она сейчас не проснулась...{/m}" nointeract
    Max_05 "{m}Wow! Der Anblick dieser Beine macht mir Lust zu erkunden und zu sehen, wohin sie hinführen... Zu verlockend! Solange sie jetzt nicht aufwacht...{/m}" nointeract

# game/events/ann.rpy:45
translate german ann_sleep_249e8b49:

    # Max_03 "{m}Чёрт, у меня самая аппетитная мама на свете! Вот бы пристроиться сзади и запустить руки под эту сорочку... Но лучше потихоньку уходить, пока она не проснулась.{/m}" nointeract
    Max_03 "{m}Verdammt, ich habe die schärfste Mutti der Welt! Ich wünschte, ich könnte ihr von hinten unter das Oberteil fahren... Aber ich gehe besser leise, bevor sie aufwacht.{/m}" nointeract

# game/events/ann.rpy:47
translate german ann_sleep_3eaf0f95:

    # Max_02 "{m}Ухх! Так и хочется задрать её сорочку, прижаться к этой обворожительной попке и шалить всю ночь... Но пора уходить, а то она может проснуться.{/m}" nointeract
    Max_02 "{m}Ooh! Ich möchte ihr einfach nur das Oberteil hochschieben, mich an diesen hübschen Arsch kuscheln und die ganze Nacht daran rummachen... Aber es ist Zeit zu gehen, sonst wacht sie vielleicht auf.{/m}" nointeract

# game/events/ann.rpy:49
translate german ann_sleep_80d64f9d:

    # Max_05 "{m}Вот это да! От вида этих раздвинутых ножек становится всё равно, что она моя мама... Слишком уж соблазнительно она выглядит в этой сорочке! Только бы она сейчас не проснулась...{/m}" nointeract
    Max_05 "{m}Wow! Beim Anblick dieser gespreizten Schenkel ist es mir egal, ob sie meine Mutter ist... Sie sieht zu verführerisch aus in diesem Oberteil! Ich hoffe, sie wacht jetzt nicht auf...{/m}" nointeract

# game/events/ann.rpy:64
translate german ann_shower_71ff78e8:

    # Max_00 "{m}Я уже попался сегодня на подглядывании за мамой. Не стоит злить её ещё больше.{/m}"
    Max_00 "{m}Ich bin heute schon dabei erwischt worden, wie ich meine Mutter beobachtet habe. Es gibt keinen Grund, sie noch mehr zu verärgern.{/m}"

# game/events/ann.rpy:67
translate german ann_shower_f74760e1:

    # Max_00 "{m}Я уже подсматривал сегодня за мамой. Не стоит искушать судьбу слишком часто.{/m}"
    Max_00 "{m}Ich habe mir heute schon Mom angesehen. Man sollte das Schicksal nicht zu oft herausfordern.{/m}"

# game/events/ann.rpy:70
translate german ann_shower_8b54d4a6:

    # Max_00 "{m}Сегодня мама и так сегодня едва не поймала меня. Не стоит искушать судьбу слишком часто.{/m}"
    Max_00 "{m}Mom hat mich heute fast erwischt. Man sollte das Schicksal nicht zu oft herausfordern.{/m}"

# game/events/ann.rpy:74
translate german ann_shower_686b9c2d:

    # Max_00 "{m}Мама сейчас принимает душ...{/m}" nointeract
    Max_00 "{m}Mom duscht gerade...{/m}" nointeract

# game/events/ann.rpy:80
translate german ann_shower_ecc96d65:

    # Max_00 "{m}Похоже, мама принимает душ...{/m}" nointeract
    Max_00 "{m}Sieht aus, als würde Mom eine Dusche nehmen...{/m}" nointeract

# game/events/ann.rpy:91
translate german ann_shower_ladder_76e8b72b:

    # Max_04 "{m}Посмотрим, что у нас тут...{/m}"
    Max_04 "{m}Mal sehen, was wir hier haben...{/m}"

# game/events/ann.rpy:104
translate german ann_shower_ladder_22ab693f:

    # Max_07 "{m}Да-а... Распахнутый халатик на маме - это просто изумительное шоу! Такие соблазнительные сосочки... да ещё и так близко... Ммм...{/m}"
    Max_07 "{m}Ja, äh... Der offene Bademantel an Mom ist einfach eine tolle Show! So verführerische Nippel... und auch so nah... {/m}"

# game/events/ann.rpy:106
translate german ann_shower_ladder_31453ab1:

    # Max_05 "{m}О, да! Мама решила не надевать трусики и правильно сделала, потому что увидеть эту киску с утра пораньше - просто сказка!{/m}"
    Max_05 "{m}Oh, ja! Mom verzichtet auf ein Höschen und hat das Richtige getan, denn diese Muschi am frühen Morgen zu sehen, ist ein Märchen!{/m}"

# game/events/ann.rpy:108
translate german ann_shower_ladder_e12e58b5:

    # Max_03 "{m}Вот это повезло... Мама в одних лишь трусиках, а её упругая грудь предстаёт передо мной во всей своей красе! Так бы любовался и любовался ей...{/m}"
    Max_03 "{m}Nun, das ist ein Glück... Mom trägt nur ein Höschen, und ihre weichen Brüste erscheinen mir in ihrer ganzen Pracht! Deshalb bewundere ich sie...{/m}"

# game/events/ann.rpy:110
translate german ann_shower_ladder_1cf5ec43:

    # Max_06 "{m}Ничего себе! Такое зрелище не каждый раз увидишь - она же совершенно голая! Только бы со стремянки не упасть от такого вида... Как было бы круто потискать все её округлости!{/m}"
    Max_06 "{m}Wow! Man sieht es nicht oft, sie ist völlig nackt! Solange ich nicht so von der Leiter falle... Es wäre so cool, ihren prallen Körper zu streicheln!{/m}"

# game/events/ann.rpy:115
translate german ann_shower_ladder_1409147d:

    # Max_07 "{m}Мои зрители явно пропускают много всего интересного! Мне однозначно стоит установить сюда ещё одну камеру...{/m}"
    Max_07 "{m}Meine Zuschauer verpassen eindeutig eine Menge toller Sachen! Ich sollte hier auf jeden Fall eine weitere Kamera installieren...{/m}"

# game/events/ann.rpy:116
translate german ann_shower_ladder_46e55bf6:

    # Max_00 "{m}Лучше бы мне уже уйти, пока никто не увидел...{/m}"
    Max_00 "{m}Ich verschwinde lieber, bevor mich jemand sieht... {/m}"

# game/events/ann.rpy:135
translate german ann_shower_start_peeping_d307b6b4:

    # Max_07 "{m}Ух, аж завораживает! Повезло же, что у меня такая сексуальная мама... Надеюсь, она меня не заметит...{/m}" nointeract
    Max_07 "{m}Wow! Ich habe Glück, so eine heiße Mutter zu haben... Ich hoffe, sie bemerkt mich nicht...{/m}" nointeract

# game/events/ann.rpy:155
translate german ann_shower_alt_peepeng_9519b19b:

    # Max_03 "[undetect!t]{m}Обалдеть можно! Не каждый день выпадает такое счастье, любоваться этой красотой! Её большая упругая грудь и стройная фигурка просто загляденье...{/m}"
    Max_03 "[undetect!t]{m}Das ist der Wahnsinn! Nicht jeden Tag, dass man eine Chance bekommt, diese Schönheit zu bewundern! Ihre großen festen Brüste und schlanke Figur sind einfach ein Anblick zu sehen...{/m}"

# game/events/ann.rpy:157
translate german ann_shower_alt_peepeng_bb7b0422:

    # Max_05 "[undetect!t]{m}О, да! Зрелище просто потрясающее... Такой сочной попке может позавидовать любая женщина! Какая мокренькая...{/m}"
    Max_05 "[undetect!t]{m}Oh, ja! Der Anblick ist einfach unglaublich... So ein saftiger Arsch kann den Neid jeder Frau erregen! Wie feucht...{/m}"

# game/events/ann.rpy:170
translate german ann_shower_closer_peepeng_9519b19b:

    # Max_03 "[undetect!t]{m}Обалдеть можно! Не каждый день выпадает такое счастье, любоваться этой красотой! Её большая упругая грудь и стройная фигурка просто загляденье...{/m}"
    Max_03 "[undetect!t]{m}Erstaunlich! Es kommt nicht jeden Tag vor, dass man die Chance hat, diese Schönheit zu bewundern! Ihre großen, weichen Brüste und ihre schlanke Figur sind nur ein Blick...{/m}"

# game/events/ann.rpy:172
translate german ann_shower_closer_peepeng_bb7b0422:

    # Max_05 "[undetect!t]{m}О, да! Зрелище просто потрясающее... Такой сочной попке может позавидовать любая женщина! Какая мокренькая...{/m}"
    Max_05 "[undetect!t]{m}Was für eine Show! So ein praller Arsch, den jeder bewundern kann...{/m}"

# game/events/ann.rpy:185
translate german ann_shower_not_luck_6222d56c:

    # Max_12 "{color=[orange]}{i}Кажется, мама что-то заподозрила!{/i}{/color}\n{m}Упс... надо бежать, пока она меня не увидела!{/m}"
    Max_12 "{color=[orange]}{i}Ich glaube, Mom ist misstrauisch!{/i}{/color}\n{m}Ups ... ich sollte abhauen, bevor sie mich sieht!{/m}"

# game/events/ann.rpy:196
translate german ann_shower_not_luck_9b36810d:

    # Ann_15 "[spotted!t]Макс!!! Что ты здесь делаешь? А ну быстро отвернись!!!" nointeract
    Ann_15 "[spotted!t]Max! Was machst du denn hier? Dreh dich sofort um!!!!" nointeract

# game/events/ann.rpy:208
translate german ann_shower_serious_talk_9d79db42:

    # Ann_19 "Ты что, подглядываешь за мной? Тебе должно быть стыдно! Нас ждёт серьёзный разговор..." nointeract
    Ann_19 "Was, starrst du mich an? Du solltest dich schämen! Wir werden ein ernstes Gespräch führen..." nointeract

# game/events/ann.rpy:214
translate german ann_shower_serious_talk_5cc0688c:

    # Ann_12 "[succes!t]Случайность, говоришь? Ну ладно, поверю. А теперь бегом отсюда!"
    Ann_12 "[succes!t]Nur ein Zufall? Gut, ich glaube dir. Und jetzt verschwinde!"

# game/events/ann.rpy:215
translate german ann_shower_serious_talk_b81f7229:

    # Max_04 "Ага, хорошо, мам!"
    Max_04 "Okay, Mom!"

# game/events/ann.rpy:220
translate german ann_shower_serious_talk_65550d20:

    # Ann_16 "[failed!t]Случайно пробрался сюда, спрятался и глазеешь тут? Случайно?! А ну-ка марш отсюда! Перед завтраком поговорим!"
    Ann_16 "[failed!t]Du behauptest also, du hättest dich \"versehentlich\" angeschlichen, dich versteckt und mich angestarrt? Geh auf dein Zimmer! Wir reden vor dem Frühstück!"

# game/events/ann.rpy:221
translate german ann_shower_serious_talk_e071f814:

    # Max_10 "Хорошо..."
    Max_10 "Klar..."

# game/events/ann.rpy:223
translate german ann_shower_serious_talk_1f891e5d:

    # Ann_12 "Что, думаешь извинился и всё, можно снова подглядывать? Нет, Макс. В этот раз всё так просто не пройдёт. Сейчас иди отсюда, а перед завтраком поговорим!"
    Ann_12 "Du glaubst, wenn du dich entschuldigst, kannst du spionieren, so viel du willst? Nein, Max. Diesmal werde ich nicht nachsichtig mit dir sein. Geh jetzt. Wir reden vor dem Frühstück!"

# game/events/ann.rpy:224
translate german ann_shower_serious_talk_1c3cc022:

    # Max_11 "Хорошо..."
    Max_11 "Klar..."

# game/events/ann.rpy:226
translate german ann_shower_serious_talk_f1aa69a0:

    # Ann_13 "Что?! Ну всё, Макс, ты попал! Быстро вернулся в дом, а перед завтарком поговорим ещё на эту тему!"
    Ann_13 "Was?! Du bist in Schwierigkeiten, Max! Geh sofort auf dein Zimmer, wir unterhalten uns vor dem Frühstück!"

# game/events/ann.rpy:227
translate german ann_shower_serious_talk_471df3d0:

    # Max_14 "Хорошо..."
    Max_14 "Klar..."

# game/events/ann.rpy:262
translate german ann_dressed_work_269233ce:

    # Max_09 "{m}Сейчас 10 часов, а значит, мама собирается на работу...{/m}" nointeract
    Max_09 "{m}Es ist 10:00 Uhr, was bedeutet, dass Mom zur Arbeit geht...{/m}" nointeract

# game/events/ann.rpy:272
translate german ann_dressed_work_b21abe2e:

    # Ann "{b}Анна:{/b} Кто там?" nointeract
    Ann "{b}Ann:{/b} Wer ist da?" nointeract

# game/events/ann.rpy:275
translate german ann_dressed_work_2e55a9e3:

    # Ann "{b}Анна:{/b} Макс, я не одета. Собираюсь на работу. Подожди немного, дорогой."
    Ann "{b}Ann:{/b} Max, ich bin noch nicht angezogen. Warte nur einen Moment, Schatz."

# game/events/ann.rpy:276
translate german ann_dressed_work_f3117b6b:

    # Max_00 "Хорошо, мам."
    Max_00 "Okay, Mom"

# game/events/ann.rpy:279
translate german ann_dressed_work_2e55a9e3_1:

    # Ann "{b}Анна:{/b} Макс, я не одета. Собираюсь на работу. Подожди немного, дорогой."
    Ann "{b}Ann:{/b} Max, ich bin noch nicht angezogen. Warte nur einen Moment, Schatz."

# game/events/ann.rpy:280
translate german ann_dressed_work_f3117b6b_1:

    # Max_00 "Хорошо, мам."
    Max_00 "Okay, Mom"

# game/events/ann.rpy:294
translate german ann_dressed_work_79ab0f47:

    # Ann_13 "Макс! Я же учила тебя стучаться!" nointeract
    Ann_13 "Max! Klopf das nächste Mal an!" nointeract

# game/events/ann.rpy:298
translate german ann_dressed_work_66960e11:

    # Ann_12 "Спасибо, конечно. Но... Макс, не мог бы ты подождать за дверью, пока я оденусь?"
    Ann_12 "Danke, Max, aber... Würdest du bitte vor der Tür warten?"

# game/events/ann.rpy:299
translate german ann_dressed_work_c8682e5b:

    # Max_00 "Конечно, мам!"
    Max_00 "Sicher, Mom!"

# game/events/ann.rpy:301
translate german ann_dressed_work_3e8f5598:

    # Ann_12 "Очень здорово, Макс! Но сначала, ты закроешь дверь и я спокойно переоденусь, а уже после этого посмотрим, что у тебя там такое срочное..."
    Ann_12 "Okay, Max! Aber erst machst du die Tür zu, dann ziehe ich mich in Ruhe um, und dann sehen wir, was so dringend ist..."

# game/events/ann.rpy:302
translate german ann_dressed_work_c8682e5b_1:

    # Max_00 "Конечно, мам!"
    Max_00 "Alles klar, Mom."

# game/events/ann.rpy:296
translate german ann_dressed_work_a94cbdaa:

    # Max_00 "{m}Пожалуй, не стоило вот так врываться к маме... Надеюсь, подарок всё сгладит.{/m}"
    Max_00 "{m}Ich hätte wahrscheinlich nicht so bei Mom reinplatzen sollen... Ich hoffe, das Geschenk wird die Wogen glätten.{/m}"

# game/events/ann.rpy:308
translate german ann_dressed_work_6bbecd5d:

    # Ann_19 "Что?! Макс! А ну-ка быстро выйди и закрой дверь!"
    Ann_19 "Was? Max! Raus und die Tür zu, sofort!"

# game/events/ann.rpy:309
translate german ann_dressed_work_80f63239:

    # Max_00 "Как скажешь, мам..."
    Max_00 "Okay, Mom..."

# game/events/ann.rpy:312
translate german ann_dressed_work_a4ce807a:

    # Ann_07 "Ну, бывает. Я сама ещё не привыкла к тому, что замков нигде нет. Ладно, дорогой. Подожди за дверью, пока мама одевается. хорошо?"
    Ann_07 "Es ist okay, ich gewöhne mich auch noch an das Fehlen von Schlössern an den Türen. Jetzt warte bitte vor der Tür, bis ich mich angezogen habe, okay?"

# game/events/ann.rpy:313
translate german ann_dressed_work_d66e2468:

    # Max_00 "Хорошо, мам..."
    Max_00 "Okay, Mom..."

# game/events/ann.rpy:325
translate german ann_dressed_work_a3f1e45d:

    # Max_01 "{m}Ничего себе, вот это зрелище! Это я удачно выбрал момент... Но пора уходить, а то вдруг увидит меня в зеркало!{/m}"
    Max_01 "{m}Wow, was für ein Anblick! Ich habe einen günstigen Moment erwischt... Aber ich sollte verschwinden, bevor sie mich im Spiegel sieht{/m}"

# game/events/ann.rpy:341
translate german ann_dressed_work_gift_a14e771b:

    # Ann_01 "Ну вот, я одета. Ты сказал, что у тебя что-то есть для меня?! О чём это ты?"
    Ann_01 "Nun, hier bin ich angezogen. Du sagtest, du hättest etwas für mich? Wovon redest du eigentlich?"

# game/events/ann.rpy:342
translate german ann_dressed_work_gift_efc9083b:

    # Max_04 "У меня для тебя подарок! Ночнушка!"
    Max_04 "Ich habe ein Geschenk für dich! Es ist ein Negligé!"

# game/events/ann.rpy:343
translate german ann_dressed_work_gift_e131ac39:

    # Ann_06 "Ты это серьёзно? Но в честь чего?"
    Ann_06 "Ist es dir ernst damit? Aber wozu?"

# game/events/ann.rpy:344
translate german ann_dressed_work_gift_860498c1:

    # Max_05 "Просто ты - самая лучшая мама на свете!"
    Max_05 "Es ist nur so, dass du die beste Mom aller Zeiten bist!"

# game/events/ann.rpy:345
translate german ann_dressed_work_gift_56ae1652:

    # Ann_08 "Ох, Макс, ты мне льстишь! Это так... неожиданно! Спасибо тебе мой милый, я очень тронута!"
    Ann_08 "Oh, Max, du schmeichelst mir! Es ist so... aus heiterem Himmel! Danke, mein Liebling, ich bin sehr gerührt!"

# game/events/ann.rpy:346
translate german ann_dressed_work_gift_60f1c9d7:

    # Max_03 "Может, примеришь?"
    Max_03 "Würdest du es gerne anprobieren?"

# game/events/ann.rpy:347
translate german ann_dressed_work_gift_f5c9d21b:

    # Ann_06 "Примерить? Для тебя? Ну... ладно... Думаю, ты это заслужил. Подожди, пожалуйста, за дверью..."
    Ann_06 "Soll ich es anprobieren? Für dich? Nun... okay, ähm... Ich denke, du hast es verdient. Warte bitte draußen vor der Tür..."

# game/events/ann.rpy:348
translate german ann_dressed_work_gift_cc0857da:

    # Max_01 "Хорошо, мам."
    Max_01 "Okay, Mom."

# game/events/ann.rpy:350
translate german ann_dressed_work_gift_dec66268:

    # Ann "{b}Анна:{/b} Ничего себе, она полупрозрачная! Дорогой, ты же понимаешь, что твоя мама не может показаться в этом перед сыном..."
    Ann "{b}Ann:{/b} Wow, es ist durchsichtig! Schatz, dir ist doch klar, dass deine Mom das nicht vor ihrem Sohn tragen sollte..."

# game/events/ann.rpy:351
translate german ann_dressed_work_gift_461f061a:

    # Max_10 "Тебе не понравился подарок?!"
    Max_10 "Hat dir das Geschenk nicht gefallen?!"

# game/events/ann.rpy:352
translate german ann_dressed_work_gift_0f666c00:

    # Ann "{b}Анна:{/b} Нет, мне очень нравится! Это прекрасный подарок! Только вот, тебе не кажется, что ты ещё слишком мал, чтобы делать подобные подарки?"
    Ann "{b}Ann:{/b} Nein, es gefällt mir wirklich! Es ist ein wunderbares Geschenk! Aber bist du nicht zu jung, um solche Geschenke zu machen?"

# game/events/ann.rpy:353
translate german ann_dressed_work_gift_00ed580c:

    # Max_09 "Я уже большой, мам! Я же от души!"
    Max_09 "Ich bin reif, Mom! Das meine ich von ganzem Herzen."

# game/events/ann.rpy:354
translate german ann_dressed_work_gift_f9e66542:

    # Ann "{b}Анна:{/b} Ох, Макс, ты меня смущаешь, такой откровенный подарок, да ещё родной матери... Но всё равно, я очень это ценю... и ещё раз огромное спасибо!"
    Ann "{b}Ann:{/b} Oh, Max, du bringst mich in Verlegenheit, so ein freizügiges Geschenk, für deine eigene Mutter... Aber trotzdem, ich weiß das wirklich zu schätzen... und noch einmal, vielen Dank."

# game/events/ann.rpy:355
translate german ann_dressed_work_gift_75fcb6dd:

    # Max_02 "Думаю, смотрится она на тебе просто фантастически!"
    Max_02 "Ich finde, es steht dir fantastisch!"

# game/events/ann.rpy:358
translate german ann_dressed_work_gift_d954a734:

    # Ann_08 "Ох... Спасибо за комплимент, мой милый. Сразу видно, что мой сын настоящий мужчина! Иди ко мне, я тебя обниму..."
    Ann_08 "Oh... Vielen Dank für das Kompliment, mein Liebling. Man merkt, dass mein Sohn ein richtiger Mann ist! Komm her, ich umarme dich..."

# game/events/ann.rpy:361
translate german ann_dressed_work_gift_26390c24:

    # Max_05 "{m}О да... У меня действительно лучшая мама на свете! Какая же потрясающая у неё фигура... Так приятно прижиматься к ней... её упругой груди... Эту мечту не хочется отпускать!{/m}"
    Max_05 "{m}Oh, Natürlich... Ich habe wirklich die beste Mom der Welt! Was für eine tolle Figur sie hat... Es ist so schön, sich an ihr festzuhalten... ...ihre weichen, geschmeidigen Brüste... Das ist ein Traum, aus dem ich nicht aufwachen möchte!{/m}"

# game/events/ann.rpy:365
translate german ann_dressed_work_gift_b6991faf:

    # Ann_04 "Ну всё, мой дорогой, мне уже скоро на работу и нужно успеть сделать ещё кое-какие дела..." nointeract
    Ann_04 "In Ordnung, Schatz, ich muss bald zur Arbeit und habe noch einige Dinge zu erledigen..." nointeract

# game/events/ann.rpy:372
translate german ann_dressed_work_gift_e66adddb:

    # Ann_05 "[succes!t]Ты сегодня очень мил, Макс! За это я тебя даже в щёчку поцелую, чтобы ты почаще старался меня радовать..."
    Ann_05 "[succes!t]Du bist heute sehr nett, Max! Dafür gebe ich dir sogar einen Kuss auf die Wange als Anreiz, mir öfter zu gefallen..."

# game/events/ann.rpy:374
translate german ann_dressed_work_gift_05855820:

    # Max_06 "{m}Ого! Это даже больше того, на что я надеялся... И не менее приятно чувствовать прикосновение её губ на своём лице! Блаженно...{/m}"
    Max_06 "{m}Wow! Das ist noch mehr, als ich erhofft hatte... Und es ist genauso schön, die Berührung ihrer Lippen zu spüren.{/m}"

# game/events/ann.rpy:378
translate german ann_dressed_work_gift_c0df5b86:

    # Ann_04 "А теперь иди, сынок... Пора заниматься делами." nointeract
    Ann_04 "Jetzt... Zeit, zur Arbeit zu gehen." nointeract

# game/events/ann.rpy:398
translate german ann_dressed_work_fail_a9cf1e2d:

    # Ann_01 "[_text!t]Макс, я так на работу не успею собраться... Давай, сынок, иди... Пора заниматься делами." nointeract
    Ann_01 "[_text!t]Max, so schaffe ich es nicht zur Arbeit... Komm schon, Sohn, geh..." nointeract

# game/events/ann.rpy:406
translate german ann_dressed_work_loveyou_e41c68c6:

    # Ann_07 "И я тебя, Макс..."
    Ann_07 "Ich dich auch, Max..."

# game/events/ann.rpy:410
translate german ann_dressed_work_goodday_1a01074c:

    # Ann_02 "Спасибо, сынок! И тебе тоже..."
    Ann_02 "Vielen Dank, mein Sohn! Und dir auch..."

# game/events/ann.rpy:421
translate german ann_dressed_shop_734f1c10:

    # Max_09 "{m}Сегодня суббота, день шоппинга. Видимо, мама собирается...{/m}" nointeract
    Max_09 "{m}Sieht aus, als würde sich Mom für einen Einkaufstag fertig machen{/m}" nointeract

# game/events/ann.rpy:435
translate german ann_dressed_shop_b21abe2e:

    # Ann "{b}Анна:{/b} Кто там?" nointeract
    Ann "{b}Ann:{/b} Wer ist da?" nointeract

# game/events/ann.rpy:438
translate german ann_dressed_shop_bf2abaef:

    # Ann "{b}Анна:{/b} Нет, Макс. Я переодеваюсь. Подожди немного, дорогой."
    Ann "{b}Ann:{/b} Nein, Max. Ich ziehe mich gerade um. Warte ein wenig, Liebling."

# game/events/ann.rpy:439
translate german ann_dressed_shop_f3117b6b:

    # Max_00 "Хорошо, мам."
    Max_00 "Alles klar, Mom."

# game/events/ann.rpy:441
translate german ann_dressed_shop_2f691176:

    # Ann "{b}Анна:{/b} Макс, я не одета. Собираюсь на шопинг. Подожди немного, дорогой."
    Ann "{b}Ann:{/b} Max, ich bin nicht angezogen. Ich gehe einkaufen. Warte ein bisschen, Schatz."

# game/events/ann.rpy:442
translate german ann_dressed_shop_f3117b6b_1:

    # Max_00 "Хорошо, мам."
    Max_00 "Alles klar, Mom."

# game/events/ann.rpy:457
translate german ann_dressed_shop_79ab0f47:

    # Ann_13 "Макс! Я же учила тебя стучаться!" nointeract
    Ann_13 "Max! Klopfe das nächste mal vorher!" nointeract

# game/events/ann.rpy:461
translate german ann_dressed_shop_66960e11:

    # Ann_12 "Спасибо, конечно. Но... Макс, не мог бы ты подождать за дверью, пока я оденусь?"
    Ann_12 "Danke, Max, aber... Wartest du bitte vor der Tür?"

# game/events/ann.rpy:462
translate german ann_dressed_shop_c8682e5b:

    # Max_00 "Конечно, мам!"
    Max_00 "Sicher, Mom!"

# game/events/ann.rpy:464
translate german ann_dressed_shop_3e8f5598:

    # Ann_12 "Очень здорово, Макс! Но сначала, ты закроешь дверь и я спокойно переоденусь, а уже после этого посмотрим, что у тебя там такое срочное..."
    Ann_12 "Das ist schön, Max! Aber erst machst du die Tür zu, ich ziehe mich um, und dann sehen wir, was so dringend ist..."

# game/events/ann.rpy:465
translate german ann_dressed_shop_c8682e5b_1:

    # Max_00 "Конечно, мам!"
    Max_00 "Natürlich, Mom!"

# game/events/ann.rpy:455
translate german ann_dressed_shop_a94cbdaa:

    # Max_00 "{m}Пожалуй, не стоило вот так врываться к маме... Надеюсь, подарок всё сгладит.{/m}"
    Max_00 "{m}Ich hätte wahrscheinlich nicht so bei Mom reinplatzen sollen... Ich hoffe, das Geschenk wird die Wogen glätten.{/m}"

# game/events/ann.rpy:470
translate german ann_dressed_shop_9f399a69:

    # Ann_07 "И Макс... Постарайся больше не входить без стука, хорошо?"
    Ann_07 "Und, Max... Versuche nicht mehr reinzukommen, ohne vorher anzuklopfen, ok?"

# game/events/ann.rpy:471
translate german ann_dressed_shop_d66e2468:

    # Max_00 "Хорошо, мам..."
    Max_00 "Okay, Mom..."

# game/events/ann.rpy:479
translate german ann_dressed_shop_a3f1e45d:

    # Max_01 "{m}Ничего себе, вот это зрелище! Это я удачно выбрал момент... Но пора уходить, а то вдруг увидит меня в зеркало!{/m}"
    Max_01 "{m}Wow, was für ein Anblick! Ich habe meinen Moment gut gewählt... Aber es ist Zeit zu gehen, sonst sieht sie mich vielleicht im Spiegel.{/m}"

# game/events/ann.rpy:499
translate german ann_dressed_shop_gift_a14e771b:

    # Ann_01 "Ну вот, я одета. Ты сказал, что у тебя что-то есть для меня?! О чём это ты?"
    Ann_01 "Nun, ich bin angezogen. Du sagtest, du hättest etwas für mich? Wovon redest du denn da?"

# game/events/ann.rpy:500
translate german ann_dressed_shop_gift_efc9083b:

    # Max_04 "У меня для тебя подарок! Ночнушка!"
    Max_04 "Ich habe ein Geschenk für dich! Ein Nachthemd!"

# game/events/ann.rpy:501
translate german ann_dressed_shop_gift_e131ac39:

    # Ann_06 "Ты это серьёзно? Но в честь чего?"
    Ann_06 "Ist es dir ernst damit? Aber wozu?"

# game/events/ann.rpy:502
translate german ann_dressed_shop_gift_860498c1:

    # Max_05 "Просто ты - самая лучшая мама на свете!"
    Max_05 "Es ist nur so, dass du die beste Mom aller Zeiten bist!"

# game/events/ann.rpy:503
translate german ann_dressed_shop_gift_56ae1652:

    # Ann_08 "Ох, Макс, ты мне льстишь! Это так... неожиданно! Спасибо тебе мой милый, я очень тронута!"
    Ann_08 "Oh, Max, du schmeichelst mir! Es ist so... aus heiterem Himmel! Danke, mein Liebling, ich bin sehr gerührt!"

# game/events/ann.rpy:504
translate german ann_dressed_shop_gift_60f1c9d7:

    # Max_03 "Может, примеришь?"
    Max_03 "Möchtest du es anprobieren?"

# game/events/ann.rpy:505
translate german ann_dressed_shop_gift_f5c9d21b:

    # Ann_06 "Примерить? Для тебя? Ну... ладно... Думаю, ты это заслужил. Подожди, пожалуйста, за дверью..."
    Ann_06 "Soll ich es anprobieren? Für dich? Nun... okay, ähm... Ich denke, du hast es verdient. Warte bitte draußen vor der Tür..."

# game/events/ann.rpy:506
translate german ann_dressed_shop_gift_cc0857da:

    # Max_01 "Хорошо, мам."
    Max_01 "Okay, Mom"

# game/events/ann.rpy:508
translate german ann_dressed_shop_gift_dec66268:

    # Ann "{b}Анна:{/b} Ничего себе, она полупрозрачная! Дорогой, ты же понимаешь, что твоя мама не может показаться в этом перед сыном..."
    Ann "{b}Ann:{/b} Wow, es ist durchsichtig! Schatz, dir ist doch klar, dass deine Mutter das nicht vor ihrem Sohn tragen sollte..."

# game/events/ann.rpy:509
translate german ann_dressed_shop_gift_461f061a:

    # Max_10 "Тебе не понравился подарок?!"
    Max_10 "Hat dir das Geschenk nicht gefallen?!"

# game/events/ann.rpy:510
translate german ann_dressed_shop_gift_0f666c00:

    # Ann "{b}Анна:{/b} Нет, мне очень нравится! Это прекрасный подарок! Только вот, тебе не кажется, что ты ещё слишком мал, чтобы делать подобные подарки?"
    Ann "{b}Ann:{/b} Nein, es gefällt mir wirklich! Es ist ein wunderbares Geschenk! Aber bist du nicht zu jung, um solche Geschenke zu machen?"

# game/events/ann.rpy:511
translate german ann_dressed_shop_gift_00ed580c:

    # Max_09 "Я уже большой, мам! Я же от души!"
    Max_09 "Ich bin reif, Mom! Das meine ich von ganzem Herzen."

# game/events/ann.rpy:512
translate german ann_dressed_shop_gift_f9e66542:

    # Ann "{b}Анна:{/b} Ох, Макс, ты меня смущаешь, такой откровенный подарок, да ещё родной матери... Но всё равно, я очень это ценю... и ещё раз огромное спасибо!"
    Ann "{b}Ann:{/b} Oh, Max, du bringst mich in Verlegenheit, so ein aufschlussreiches Geschenk, an deine eigene Mutter... Aber trotzdem, ich weiß das wirklich zu schätzen... und noch einmal, vielen Dank."

# game/events/ann.rpy:513
translate german ann_dressed_shop_gift_75fcb6dd:

    # Max_02 "Думаю, смотрится она на тебе просто фантастически!"
    Max_02 "Ich finde, es steht dir fantastisch!"

# game/events/ann.rpy:516
translate german ann_dressed_shop_gift_d954a734:

    # Ann_08 "Ох... Спасибо за комплимент, мой милый. Сразу видно, что мой сын настоящий мужчина! Иди ко мне, я тебя обниму..."
    Ann_08 "Oh... Vielen Dank für das Kompliment, Schatz. Man merkt, dass mein Sohn ein richtiger Mann ist! Komm her, ich umarme dich..."

# game/events/ann.rpy:519
translate german ann_dressed_shop_gift_26390c24:

    # Max_05 "{m}О да... У меня действительно лучшая мама на свете! Какая же потрясающая у неё фигура... Так приятно прижиматься к ней... её упругой груди... Эту мечту не хочется отпускать!{/m}"
    Max_05 "{m}Oh, Natürlich... Ich habe wirklich die beste Mom der Welt! Was für eine tolle Figur sie hat... Es ist so schön, sich an ihr festzuhalten... ...ihre weichen, geschmeidigen Brüste... Das ist ein Traum, aus dem ich nicht aufwachen möchte!{/m}"

# game/events/ann.rpy:523
translate german ann_dressed_shop_gift_cda3b4f9:

    # Ann_04 "Ну всё, мой дорогой, нам с девочками ещё нужно успеть пробежаться по магазинам сегодня..." nointeract
    Ann_04 "Also gut, Schatz, die Mädchen und ich müssen heute noch einkaufen gehen..." nointeract

# game/events/ann.rpy:530
translate german ann_dressed_shop_gift_e66adddb:

    # Ann_05 "[succes!t]Ты сегодня очень мил, Макс! За это я тебя даже в щёчку поцелую, чтобы ты почаще старался меня радовать..."
    Ann_05 "[succes!t]Du bist heute sehr nett, Max! Dafür gebe ich Ihnen sogar einen Kuss auf die Wange als Anreiz, mir öfter zu gefallen..."

# game/events/ann.rpy:532
translate german ann_dressed_shop_gift_05855820:

    # Max_06 "{m}Ого! Это даже больше того, на что я надеялся... И не менее приятно чувствовать прикосновение её губ на своём лице! Блаженно...{/m}"
    Max_06 "{m}Wow! Das ist noch mehr, als ich erhofft hatte... Und es ist genauso schön, die Berührung ihrer Lippen zu spüren.{/m}"

# game/events/ann.rpy:536
translate german ann_dressed_shop_gift_c0df5b86:

    # Ann_04 "А теперь иди, сынок... Пора заниматься делами." nointeract
    Ann_04 "Jetzt... Zeit, zur Arbeit zu gehen." nointeract

# game/events/ann.rpy:556
translate german ann_dressed_shop_fail_f49b0b30:

    # Ann_01 "[_text!t]Макс, мне нужно ещё успеть сделать кое-какие дела... Давай, сынок, иди... Займись чем-нибудь." nointeract
    Ann_01 "[_text!t]Max, ich muss noch ein paar Dinge erledigen... Komm schon, Sohn, geh ... Mach etwas anderes." nointeract

# game/events/ann.rpy:564
translate german ann_dressed_shop_loveyou_e41c68c6:

    # Ann_07 "И я тебя, Макс..."
    Ann_07 "Und du, Max..."

# game/events/ann.rpy:568
translate german ann_dressed_shop_goodday_1a01074c:

    # Ann_02 "Спасибо, сынок! И тебе тоже..."
    Ann_02 "Vielen Dank, mein Sohn! Und dir auch..."

# game/events/ann.rpy:632
translate german ann_bath_fc2b25dc:

    # Max_00 "{m}Видимо, мама принимает ванну...{/m}" nointeract
    Max_00 "{m}Sieht aus, als würde die Mom ein Bad nehmen{/m}" nointeract

# game/events/ann.rpy:650
translate german ann_bath_cddc6bee:

    # Ann "{b}Анна:{/b} Кто там? Я принимаю ванну!" nointeract
    Ann "{b}Ann:{/b} Wer ist da? Ich nehme ein Bad!" nointeract

# game/events/ann.rpy:653
translate german ann_bath_29fc0d16:

    # Ann "{b}Анна:{/b} Дорогой, что ты хотел?" nointeract
    Ann "{b}Ann:{/b} Willst du etwas, Schatz?" nointeract

# game/events/ann.rpy:657
translate german ann_bath_13e7202f:

    # Ann "{b}Анна:{/b} Ну... хорошо, входи. Только не смотри!" nointeract
    Ann "{b}Ann:{/b} Na ja... okay, aber nicht gucken!" nointeract

# game/events/ann.rpy:681
translate german ann_bath_9b9b753a:

    # Ann "{b}Анна:{/b} Хорошо, я скоро закончу..."
    Ann "{b}Ann:{/b} Okay, ich werde nicht lange brauchen..."

# game/events/ann.rpy:682
translate german ann_bath_7852d704:

    # Max_00 "Ага..."
    Max_00 "Okay..."

# game/events/ann.rpy:673
translate german ann_bath_9d03c202:

    # Max_00 "{m}Эх... жаль, что стекло частично матовое. Так ничего не разглядеть! А если подобраться ближе, то мама может заметить...{/m}"
    Max_00 "{m}Schade, dass das Glas undurchsichtig ist, ich kann die besten Bereiche nicht sehen! Und noch näher heranzukommen ist zu riskant...{/m}"

# game/events/ann.rpy:675
translate german ann_bath_7fffd1ec:

    # Max_09 "{m}Нужно что-нибудь придумать...{/m}" nointeract
    Max_09 "{m}Ich muss mir etwas einfallen lassen...{/m}" nointeract

# game/events/ann.rpy:682
translate german ann_bath_4a929722:

    # Max_01 "{m}Надеюсь, что ни у кого не возникнет вопроса, а что же здесь делает стремянка... Как, что? Конечно стоит, мало ли что! А теперь начинается самое интересное...{/m}"
    Max_01 "{m}Ich hoffe, niemand stellt in Frage, warum hier eine Trittleiter steht... sie dient nur zu Ermittlungszwecken... mal sehen, womit wir es zu tun haben.{/m}"

# game/events/ann.rpy:696
translate german ann_bath_ladder_76e8b72b:

    # Max_04 "{m}Посмотрим, что у нас тут...{/m}"
    Max_04 "{m}Mal sehen, was wir hier haben...{/m}"

# game/events/ann.rpy:706
translate german ann_bath_ladder_3a5db899:

    # Max_03 "{m}Ох, как горячо! Разумеется, я не про воду, а про её внешний вид. Ухх... Мама потрясающе выглядит...{/m}" nointeract
    Max_03 "{m}Oh, wie heiß! Und ich meine nicht das Wasser. Uhh... Mom sieht unglaublich aus...{/m}" nointeract

# game/events/ann.rpy:712
translate german ann_bath_ladder_b37e9fe1:

    # Max_05 "{m}Ух ты, аж завораживает! Мамины водные процедуры могут посоперничать с самыми горячими эротическими роликами! Эта упругая грудь и эти длинные стройные ножки сведут с ума кого угодно...{/m}" nointeract
    Max_05 "{m}Wow, unglaublich! Ihre festen Brüste und diese langen, schlanken Beine werden jeden verrückt machen...{/m}" nointeract

# game/events/ann.rpy:721
translate german ann_bath_ladder_b37e9fe1_1:

    # Max_05 "{m}Ух ты, аж завораживает! Мамины водные процедуры могут посоперничать с самыми горячими эротическими роликами! Эта упругая грудь и эти длинные стройные ножки сведут с ума кого угодно...{/m}" nointeract
    Max_05 "{m}Wow, unglaublich! Ihre festen Brüste und diese langen, schlanken Beine werden jeden verrückt machen...{/m}" nointeract

# game/events/ann.rpy:727
translate german ann_bath_ladder_87fd27ff:

    # Max_07 "{m}Эх! Похоже, самое интересное закончилось... Хотя, смотреть как мама вытирает своё мокрое и соблазнительное тело не менее приятно! Ох, какая же у неё попка...{/m}" nointeract
    Max_07 "{m}Eh! Sieht aus, als wäre der spaßige Teil vorbei... Obwohl, Mom dabei zuzusehen, wie sie ihren nassen und verführerischen Körper abtrocknet, ist genau so unterhaltsam! Oh, was für einen Arsch sie hat...{/m}" nointeract

translate german strings:

    # game/events/ann.rpy:196
    old "{i}Отвернуться{/i}"
    new "{i}Wegdrehen{/i}"

    # game/events/ann.rpy:208
    old "Я не подглядывал. Это случайность!"
    new "Ich habe nicht spioniert, es war ein Versehen!"

    # game/events/ann.rpy:208
    old "Мам, извини..."
    new "Es tut mir leid, Mom..."

    # game/events/ann.rpy:208
    old "Попка у тебя - что надо!"
    new "Dein Arsch ist perfekt!"

    # game/events/ann.rpy:272
    old "Это я, Макс. Можно войти? У меня для тебя кое-что есть."
    new "Ich bin's, Max. Darf ich reinkommen? Ich habe etwas für dich."

    # game/events/ann.rpy:294
    old "Хорошо выглядишь, мам!"
    new "Du siehst toll aus, Mom!"

    # game/events/ann.rpy:294
    old "У меня для тебя кое-что есть."
    new "Ich habe etwas für dich."

    # game/events/ann.rpy:294
    old "Зачётные сиськи!"
    new "Schöne Brüste!"

    # game/events/ann.rpy:294
    old "Ой, извини. Я забыл..."
    new "Tut mir leid, ich vergaß..."

    # game/events/ann.rpy:365
    old "Ну мам! Этого было так мало, давай ещё..."
    new "Komm schon, Mom! Es war so kurz, lass es uns nochmal machen..."

    # game/events/ann.rpy:365
    old "Конечно, мам! Хорошего тебе дня..."
    new "Natürlich, Mom! Ich wünsche dir einen schönen Tag..."

    # game/events/ann.rpy:457
    old "Ой, извини..."
    new "Oh, Sorry..."

    # game/events/ann.rpy:650
    old "Это я, Макс."
    new "Ich bin's, Max."

    # game/events/ann.rpy:653
    old "Можно я войду?"
    new "Kann ich kurz rein kommen?"

    # game/events/ann.rpy:653
    old "Нет, ничего"
    new "Nein, nichts"

    # game/events/ann.rpy:653
    old "Я подожду..."
    new "Ich werde warten..."

    # game/events/ann.rpy:657
    old "Ой, нет, я передумал"
    new "Oh, sorry, ich habe meine Meinung geändert"

# TODO: Translation updated at 2021-11-11 14:52

# game/events/ann.rpy:126
translate german ann_shower_start_peeping_fc5db481:

    # Max_09 "{m}Лучше вообще свести подглядывания к минимуму, пока я не избавлюсь от Эрика. Чтобы никого ещё больше не расстраивать...{/m}"
    Max_09 "{m}Es ist besser, das Spionieren auf ein Minimum zu reduzieren, bis ich Eric los bin. Damit ich niemanden verärgere...{/m}"

# game/events/ann.rpy:128
translate german ann_shower_start_peeping_dda24c4d:

    # Max_01 "{m}Хорошо, что это не распространяется на Киру...{/m}" nointeract
    Max_01 "{m}Es ist gut, dass das nicht auf Kira zutrifft...{/m}" nointeract

# TODO: Translation updated at 2021-12-12 01:53

# game/events/ann.rpy:263
translate german ann_dressed_734f1c10:

    # Max_09 "{m}Сегодня суббота, день шоппинга. Видимо, мама собирается...{/m}" nointeract
    Max_09 "{m}Sieht aus, als würde sich Mom für einen Einkaufstag fertig machen{/m}" nointeract

# game/events/ann.rpy:265
translate german ann_dressed_269233ce:

    # Max_09 "{m}Сейчас 10 часов, а значит, мама собирается на работу...{/m}" nointeract
    Max_09 "{m}Es ist 10:00 Uhr, was bedeutet, dass Mom zur Arbeit geht...{/m}" nointeract

# game/events/ann.rpy:269
translate german ann_dressed_b21abe2e:

    # Ann "{b}Анна:{/b} Кто там?" nointeract
    Ann "{b}Ann:{/b} Wer ist da?" nointeract

# game/events/ann.rpy:272
translate german ann_dressed_bf2abaef:

    # Ann "{b}Анна:{/b} Нет, Макс. Я переодеваюсь. Подожди немного, дорогой."
    Ann "{b}Ann:{/b} Nein, Max. Ich ziehe mich gerade um. Warte ein wenig, Liebling."

# game/events/ann.rpy:274
translate german ann_dressed_2e55a9e3:

    # Ann "{b}Анна:{/b} Макс, я не одета. Собираюсь на работу. Подожди немного, дорогой."
    Ann "{b}Ann:{/b} Max, ich bin noch nicht angezogen. Warte nur einen Moment, Schatz."

# game/events/ann.rpy:275
translate german ann_dressed_f3117b6b:

    # Max_00 "Хорошо, мам."
    Max_00 "Alles klar, Mom."

# game/events/ann.rpy:278
translate german ann_dressed_2f691176:

    # Ann "{b}Анна:{/b} Макс, я не одета. Собираюсь на шопинг. Подожди немного, дорогой."
    Ann "{b}Ann:{/b} Max, ich bin nicht angezogen. Ich gehe einkaufen. Warte ein bisschen, Schatz."

# game/events/ann.rpy:279
translate german ann_dressed_f3117b6b_1:

    # Max_00 "Хорошо, мам."
    Max_00 "Alles klar, Mom."

# game/events/ann.rpy:312
translate german ann_dressed_a3f1e45d:

    # Max_01 "{m}Ничего себе, вот это зрелище! Это я удачно выбрал момент... Но пора уходить, а то вдруг увидит меня в зеркало!{/m}"
    Max_01 "{m}Wow, was für ein Anblick! Ich habe meinen Moment gut gewählt... Aber es ist Zeit zu gehen, sonst sieht sie mich vielleicht im Spiegel.{/m}"

# game/events/ann.rpy:331
translate german ann_dressed_stay_in_room_1c7b7b81:

    # Eric_09 "Макс, не мешай взрослым! В твоём положении это просто верх наглости! Найди себе занятие..." nointeract
    Eric_09 "Max, störe die Erwachsenen nicht! In deiner Situation ist das der Gipfel der Unverschämtheit! Such dir etwas zu tun..." nointeract

# game/events/ann.rpy:333
translate german ann_dressed_stay_in_room_ff7e7143:

    # Ann_00 "Сынок, мы с Эриком собирались переодеться. Не мешай взрослым, займись чем-нибудь..." nointeract
    Ann_00 "Sohn, Eric und ich wollten uns umziehen. Lass uns in Ruhe, geh und tu etwas ..." nointeract

# game/events/ann.rpy:336
translate german ann_dressed_stay_in_room_4e1690c6:

    # Ann_12 "Сынок, я собиралась переодеться. Не мешай маме. Займись чем-нибудь полезным..." nointeract
    Ann_12 "Sohn, ich wollte mich umziehen. Störe deine Mutter nicht. Tu etwas Nützliches..." nointeract

# game/events/ann.rpy:338
translate german ann_dressed_stay_in_room_0e69876a:

    # Ann_00 "Сынок, я собиралась переодеться. Иди пока, займись чем-нибудь..." nointeract
    Ann_00 "Sohn, ich wollte mich umziehen. Geh und tu etwas..." nointeract

# game/events/ann.rpy:358
translate german ann_dressed_moment0_1c7b7b81:

    # Eric_09 "Макс, не мешай взрослым! В твоём положении это просто верх наглости! Найди себе занятие..." nointeract
    Eric_09 "Max, störe die Erwachsenen nicht! In deiner Situation ist das der Gipfel der Unverschämtheit! Such dir etwas zu tun..." nointeract

# game/events/ann.rpy:360
translate german ann_dressed_moment0_ff7e7143:

    # Ann_00 "Сынок, мы с Эриком собирались переодеться. Не мешай взрослым, займись чем-нибудь..." nointeract
    Ann_00 "Sohn, Eric und ich wollten uns umziehen. Lass uns in Ruhe, geh und tu etwas ..." nointeract

# game/events/ann.rpy:366
translate german ann_dressed_moment0_104e65c7:

    # Ann_00 "Так, сынок... Ты можешь погулять? А то мне нужно переодеться..." nointeract
    Ann_00 "Also, Sohn... Kannst du einen Spaziergang machen? Weil ich muss mich umziehen..." nointeract

# game/events/ann.rpy:368
translate german ann_dressed_moment0_ad13a757:

    # Ann_13 "Конечно! Это как-то неправильно, если мать будет переодеваться при своём ребёнке. Так что, пожалуйста, выйди ненадолго." nointeract
    Ann_13 "Ja, natürlich! Es ist irgendwie falsch, wenn eine Mutter sich vor ihrem Kind umzieht. Geh also bitte eine Weile raus." nointeract

# game/events/ann.rpy:370
translate german ann_dressed_moment0_42ae37fd:

    # Ann_12 "Ишь ты, что удумал! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_12 "Sieh dich an! Hör mit diesem Unsinn auf und lassen deine Mutter sich in Ruhe umziehen." nointeract

# game/events/ann.rpy:372
translate german ann_dressed_moment0_19c5e329:

    # Ann_01 "Спасибо, Макс. Если ты что-то хотел, то я недолго..." nointeract
    Ann_01 "Vielen Dank, Max. Wenn du etwas willst, ich habe nicht lange gebraucht, um..." nointeract

# game/events/ann.rpy:389
translate german ann_dressed_moment2_a9e5c75c:

    # Ann_15 "Макс! Я же учила тебя стучаться! {p=3}{nw}"
    Ann_15 "Max! Ich habe dir beigebracht, wie man anklopft! {p=3}{nw}"

# game/events/ann.rpy:391
translate german ann_dressed_moment2_babcea3a:

    # Ann_15 "Макс! А стучаться кто будет?! {p=3}{nw}"
    Ann_15 "Max! Was soll das werden?! {p=3}{nw}"

# game/events/ann.rpy:399
translate german ann_dressed_moment2_aa9214ef:

    # Ann_14 "Нельзя вот так без предупреждения врываться в комнату! Что-то случилось?" nointeract
    Ann_14 "Du kannst nicht einfach so unangemeldet in ein Zimmer platzen! Ist etwas passiert?" nointeract

# game/events/ann.rpy:402
translate german ann_dressed_moment2_c391d1e4:

    # Ann_12 "Прекращай уже без предупреждения врываться в комнату! Или у тебя что-то срочное?" nointeract
    Ann_12 "Hör auf, unangemeldet ins Zimmer zu platzen! Liegt ein Notfall vor?" nointeract

# game/events/ann.rpy:417
translate german ann_dressed_moment1_1f19d7dd:

    # Ann_15 "Макс! Я же учила тебя стучаться! Нельзя вот так без предупреждения врываться в комнату! Что-то случилось?" nointeract
    Ann_15 "Max! Ich habe dir beigebracht zu klopfen! Du kannst nicht einfach so unangemeldet in ein Zimmer platzen! Stimmt etwas nicht?" nointeract

# game/events/ann.rpy:420
translate german ann_dressed_moment1_e1ca91d3:

    # Ann_15 "Макс! А стучаться кто будет?! Прекращай уже без предупреждения врываться в комнату! Или у тебя что-то срочное?" nointeract
    Ann_15 "Max! Wer klopft denn da?! Hör auf, unangemeldet ins Zimmer zu platzen! Handelt es sich etwa um einen Notfall?" nointeract

# game/events/ann.rpy:426
translate german ann_dressed_lvl_1_3e8f5598:

    # Ann_12 "Очень здорово, Макс! Но сначала, ты закроешь дверь и я спокойно переоденусь, а уже после этого посмотрим, что у тебя там такое срочное..."
    Ann_12 "Okay, Max! Aber erst machst du die Tür zu, dann ziehe ich mich in Ruhe um, und dann sehen wir, was so dringend ist..."

# game/events/ann.rpy:427
translate german ann_dressed_lvl_1_c8682e5b:

    # Max_00 "Конечно, мам!"
    Max_00 "Alles klar, Mom!"

# game/events/ann.rpy:429
translate german ann_dressed_lvl_1_a94cbdaa:

    # Max_00 "{m}Пожалуй, не стоило вот так врываться к маме... Надеюсь, подарок всё сгладит.{/m}"
    Max_00 "{m}Ich hätte wahrscheinlich nicht so bei Mom reinplatzen sollen... Ich hoffe, das Geschenk wird die Wogen glätten.{/m}"

# game/events/ann.rpy:435
translate german ann_dressed_lvl_1_0e39c2e7:

    # Ann_17 "Что?! Макс! Это что ещё за словечки такие? А ну-ка быстро выйди и закрой дверь!" nointeract
    Ann_17 "Was?! Max! Was ist das für eine Wortwahl? Raus und sofort die Tür zu!" nointeract

# game/events/ann.rpy:440
translate german ann_dressed_lvl_1_3f53b608:

    # Ann_17 "Что я слышу! Вряд ли я та женщина, которой стоит такое говорить, Макс. А теперь выйди и закрой дверь!" nointeract
    Ann_17 "Was höre ich denn da? Ich bin nicht die Art von Frau, die so etwas sagt, Max. Und jetzt raus mit dir und mach die Tür zu!" nointeract

# game/events/ann.rpy:445
translate german ann_dressed_lvl_1_d8e67a23:

    # Ann_13 "Как я переодеваюсь?! Сынок, ты что такое говоришь! Давай-ка выйди и закрой за собой дверь." nointeract
    Ann_13 "Wie soll ich mich umziehen?! Was redest du da, mein Sohn? Geh raus und schließ die Tür hinter dir." nointeract

# game/events/ann.rpy:448
translate german ann_dressed_lvl_1_bb28ab00:

    # Ann_12 "Спасибо, конечно. Но... Макс, не мог бы ты подождать за дверью, пока я оденусь?" nointeract
    Ann_12 "Danke, Max, aber... Würdest du bitte vor der Tür warten?" nointeract

# game/events/ann.rpy:456
translate german ann_dressed_lvl_2_3e8f5598:

    # Ann_12 "Очень здорово, Макс! Но сначала, ты закроешь дверь и я спокойно переоденусь, а уже после этого посмотрим, что у тебя там такое срочное..."
    Ann_12 "Okay, Max! Aber erst machst du die Tür zu, dann ziehe ich mich in Ruhe um, und dann sehen wir, was so dringend ist..."

# game/events/ann.rpy:457
translate german ann_dressed_lvl_2_c8682e5b:

    # Max_00 "Конечно, мам!"
    Max_00 "Alles klar, Mom."

# game/events/ann.rpy:459
translate german ann_dressed_lvl_2_a94cbdaa:

    # Max_00 "{m}Пожалуй, не стоило вот так врываться к маме... Надеюсь, подарок всё сгладит.{/m}"
    Max_00 "{m}Ich hätte wahrscheinlich nicht so bei Mom reinplatzen sollen... Ich hoffe, das Geschenk wird die Wogen glätten.{/m}"

# game/events/ann.rpy:465
translate german ann_dressed_lvl_2_d73d7740:

    # Ann_13 "Макс! И не стыдно тебе такое своей маме говорить, а? Ну-ка бегом за дверь, а то мешаешь." nointeract
    Ann_13 "Max! Schämst du dich nicht, so etwas zu deiner Mutter zu sagen, hm? Geh raus, du störst." nointeract

# game/events/ann.rpy:470
translate german ann_dressed_lvl_2_307c57f0:

    # Ann_13 "Даже не знаю, что на это ответить... Ну-ка бегом за дверь, а то засмущал меня." nointeract
    Ann_13 "Ich weiß nicht, was ich dazu sagen soll... Raus hier, du bringst mich in Verlegenheit." nointeract

# game/events/ann.rpy:474
translate german ann_dressed_lvl_2_659d5d18:

    # Ann_12 "Нет, конечно! Ишь ты, что удумал! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_12 "Nein, natürlich nicht! Schau dich an! Hör mit diesem Unsinn auf und lass deine Mutter sich in Ruhe umziehen." nointeract

# game/events/ann.rpy:478
translate german ann_dressed_lvl_2_a255adec:

    # Ann_02 "Приятно слышать. Но маме нужно переодеться... Так что подожди за дверью, хорошо?" nointeract
    Ann_02 "Das ist gut zu hören. Aber deine Mutter muss sich umziehen... Also warte einfach vor der Tür, okay?" nointeract

# game/events/ann.rpy:486
translate german ann_dressed_gift_a14e771b:

    # Ann_01 "Ну вот, я одета. Ты сказал, что у тебя что-то есть для меня?! О чём это ты?"
    Ann_01 "Nun, hier bin ich angezogen. Du sagtest, du hättest etwas für mich? Wovon redest du eigentlich?"

# game/events/ann.rpy:487
translate german ann_dressed_gift_efc9083b:

    # Max_04 "У меня для тебя подарок! Ночнушка!"
    Max_04 "Ich habe ein Geschenk für dich! Es ist ein Negligé!"

# game/events/ann.rpy:488
translate german ann_dressed_gift_e131ac39:

    # Ann_06 "Ты это серьёзно? Но в честь чего?"
    Ann_06 "Ist es dir ernst damit? Aber wozu?"

# game/events/ann.rpy:489
translate german ann_dressed_gift_860498c1:

    # Max_05 "Просто ты - самая лучшая мама на свете!"
    Max_05 "Es ist nur so, dass du die beste Mom aller Zeiten bist!"

# game/events/ann.rpy:490
translate german ann_dressed_gift_56ae1652:

    # Ann_08 "Ох, Макс, ты мне льстишь! Это так... неожиданно! Спасибо тебе мой милый, я очень тронута!"
    Ann_08 "Oh, Max, du schmeichelst mir! Es ist so... aus heiterem Himmel! Danke, mein Liebling, ich bin sehr gerührt!"

# game/events/ann.rpy:491
translate german ann_dressed_gift_60f1c9d7:

    # Max_03 "Может, примеришь?"
    Max_03 "Würdest du es gerne anprobieren?"

# game/events/ann.rpy:492
translate german ann_dressed_gift_f5c9d21b:

    # Ann_06 "Примерить? Для тебя? Ну... ладно... Думаю, ты это заслужил. Подожди, пожалуйста, за дверью..."
    Ann_06 "Soll ich es anprobieren? Für dich? Nun... okay, ähm... Ich denke, du hast es verdient. Warte bitte draußen vor der Tür..."

# game/events/ann.rpy:493
translate german ann_dressed_gift_cc0857da:

    # Max_01 "Хорошо, мам."
    Max_01 "Okay, Mom."

# game/events/ann.rpy:495
translate german ann_dressed_gift_dec66268:

    # Ann "{b}Анна:{/b} Ничего себе, она полупрозрачная! Дорогой, ты же понимаешь, что твоя мама не может показаться в этом перед сыном..."
    Ann "{b}Ann:{/b} Wow, es ist durchsichtig! Schatz, dir ist doch klar, dass deine Mom das nicht vor ihrem Sohn tragen sollte..."

# game/events/ann.rpy:496
translate german ann_dressed_gift_461f061a:

    # Max_10 "Тебе не понравился подарок?!"
    Max_10 "Hat dir das Geschenk nicht gefallen?!"

# game/events/ann.rpy:497
translate german ann_dressed_gift_0f666c00:

    # Ann "{b}Анна:{/b} Нет, мне очень нравится! Это прекрасный подарок! Только вот, тебе не кажется, что ты ещё слишком мал, чтобы делать подобные подарки?"
    Ann "{b}Ann:{/b} Nein, es gefällt mir wirklich! Es ist ein wunderbares Geschenk! Aber bist du nicht zu jung, um solche Geschenke zu machen?"

# game/events/ann.rpy:498
translate german ann_dressed_gift_00ed580c:

    # Max_09 "Я уже большой, мам! Я же от души!"
    Max_09 "Ich bin reif, Mom! Das meine ich von ganzem Herzen."

# game/events/ann.rpy:499
translate german ann_dressed_gift_f9e66542:

    # Ann "{b}Анна:{/b} Ох, Макс, ты меня смущаешь, такой откровенный подарок, да ещё родной матери... Но всё равно, я очень это ценю... и ещё раз огромное спасибо!"
    Ann "{b}Ann:{/b} Oh, Max, du bringst mich in Verlegenheit, so ein freizügiges Geschenk, für deine eigene Mutter... Aber trotzdem, ich weiß das wirklich zu schätzen... und noch einmal, vielen Dank."

# game/events/ann.rpy:500
translate german ann_dressed_gift_75fcb6dd:

    # Max_02 "Думаю, смотрится она на тебе просто фантастически!"
    Max_02 "Ich finde, es steht dir fantastisch!"

# game/events/ann.rpy:503
translate german ann_dressed_gift_d954a734:

    # Ann_08 "Ох... Спасибо за комплимент, мой милый. Сразу видно, что мой сын настоящий мужчина! Иди ко мне, я тебя обниму..."
    Ann_08 "Oh... Vielen Dank für das Kompliment, mein Liebling. Man merkt, dass mein Sohn ein richtiger Mann ist! Komm her, ich umarme dich..."

# game/events/ann.rpy:506
translate german ann_dressed_gift_26390c24:

    # Max_05 "{m}О да... У меня действительно лучшая мама на свете! Какая же потрясающая у неё фигура... Так приятно прижиматься к ней... её упругой груди... Эту мечту не хочется отпускать!{/m}"
    Max_05 "{m}Oh, Natürlich... Ich habe wirklich die beste Mom der Welt! Was für eine tolle Figur sie hat... Es ist so schön, sich an ihr festzuhalten... ...ihre weichen, geschmeidigen Brüste... Das ist ein Traum, aus dem ich nicht aufwachen möchte!{/m}"

# game/events/ann.rpy:510
translate german ann_dressed_gift_cda3b4f9:

    # Ann_04 "Ну всё, мой дорогой, нам с девочками ещё нужно успеть пробежаться по магазинам сегодня..." nointeract
    Ann_04 "Also gut, Schatz, die Mädchen und ich müssen heute noch einkaufen gehen..." nointeract

# game/events/ann.rpy:512
translate german ann_dressed_gift_b6991faf:

    # Ann_04 "Ну всё, мой дорогой, мне уже скоро на работу и нужно успеть сделать ещё кое-какие дела..." nointeract
    Ann_04 "In Ordnung, Schatz, ich muss bald zur Arbeit und habe noch einige Dinge zu erledigen..." nointeract

# game/events/ann.rpy:517
translate german ann_dressed_gift_e66adddb:

    # Ann_05 "[succes!t]Ты сегодня очень мил, Макс! За это я тебя даже в щёчку поцелую, чтобы ты почаще старался меня радовать..."
    Ann_05 "[succes!t]Du bist heute sehr nett, Max! Dafür gebe ich dir sogar einen Kuss auf die Wange als Anreiz, mir öfter zu gefallen..."

# game/events/ann.rpy:519
translate german ann_dressed_gift_05855820:

    # Max_06 "{m}Ого! Это даже больше того, на что я надеялся... И не менее приятно чувствовать прикосновение её губ на своём лице! Блаженно...{/m}"
    Max_06 "{m}Wow! Das ist noch mehr, als ich erhofft hatte... Und es ist genauso schön, die Berührung ihrer Lippen zu spüren.{/m}"

# game/events/ann.rpy:524
translate german ann_dressed_gift_c0df5b86:

    # Ann_04 "А теперь иди, сынок... Пора заниматься делами." nointeract
    Ann_04 "Jetzt... Zeit, zur Arbeit zu gehen." nointeract

# game/events/ann.rpy:544
translate german ann_dressed_fail_f49b0b30:

    # Ann_01 "[_text!t]Макс, мне нужно ещё успеть сделать кое-какие дела... Давай, сынок, иди... Займись чем-нибудь." nointeract
    Ann_01 "[_text!t]Max, ich muss noch ein paar Dinge erledigen... Komm schon, Sohn, geh ... Mach etwas anderes." nointeract

# game/events/ann.rpy:546
translate german ann_dressed_fail_a9cf1e2d:

    # Ann_01 "[_text!t]Макс, я так на работу не успею собраться... Давай, сынок, иди... Пора заниматься делами." nointeract
    Ann_01 "[_text!t]Max, so schaffe ich es nicht zur Arbeit... Komm schon, Sohn, geh..." nointeract

# game/events/ann.rpy:554
translate german ann_dressed_loveyou_e41c68c6:

    # Ann_07 "И я тебя, Макс..."
    Ann_07 "Und du, Max..."

# game/events/ann.rpy:558
translate german ann_dressed_goodday_1a01074c:

    # Ann_02 "Спасибо, сынок! И тебе тоже..."
    Ann_02 "Vielen Dank, mein Sohn! Und dir auch..."

translate german strings:

    # game/events/ann.rpy:365
    old "А я не помешаю. Начинай..."
    new "Ich werde nicht stören. Fang an..."

    # game/events/ann.rpy:424
    old "У тебя самые зачётные сиськи, которые я видел!"
    new "Du hast die geilsten Titten, die ich je gesehen habe!"

    # game/events/ann.rpy:424
    old "У тебя самый потрясный зад на свете, мам!"
    new "Du hast den tollsten Arsch der Welt, Mom!"

    # game/events/ann.rpy:424
    old "Я просто хотел посмотреть..."
    new "Ich wollte nur sehen, ob..."

    # game/events/ann.rpy:424
    old "Ой, извини. Я забыл... Хорошо выглядишь, мам!"
    new "Oh, das tut mir leid. Ich hab vergessen... Sieht gut aus, Mom!"

    # game/events/ann.rpy:454
    old "Да не прикрывай такую красоту, все свои же!"
    new "Diese Schönheit darf nicht verdeckt werden, die gehört ganz dir!"

    # game/events/ann.rpy:454
    old "Сразу видно, что попка у тебя тренированная!"
    new "Es ist offensichtlich, dass du einen trainierten Arsch hast!"

    # game/events/ann.rpy:454
    old "А посмотреть нельзя?"
    new "Kann man nichts sehen?"

    # game/events/ann.rpy:454
    old "Извини, всё позабыл, когда тебя увидел. Ты прекрасна!"
    new "Sorry, ich hab es vergessen, als ich dich sah. Du bist wunderschön!"

# TODO: Translation updated at 2022-02-16 00:40

# game/events/ann.rpy:587
translate german ann_dressed_balcony_919b6a05:

    # Max_07 "{m}Хм... Должно быть мама на балконе...{/m}"
    Max_07 "{m}Hm... Mom muss auf dem Balkon sein...{/m}"

# game/events/ann.rpy:595
translate german ann_dressed_balcony_34f4b4a9:

    # Max_09 "{m}То ли мама просто задумалась, то ли грустит...{/m}" nointeract
    Max_09 "{m}Entweder denkt Mom nur nach, oder sie ist traurig...{/m}" nointeract

# game/events/ann.rpy:601
translate german ann_dressed_balcony_2fd8332a:

    # Ann_12 "Ой, Макс! Ты слишком тихо ходишь. А я так, ничего, просто задумалась немного..."
    Ann_12 "Oh, Max! Du läufst zu leise. Ich tue nichts, ich denke nur ein bisschen nach..."

# game/events/ann.rpy:602
translate german ann_dressed_balcony_d6e397b3:

    # Max_07 "Расскажешь, о чём?"
    Max_07 "Sagst du mir, worüber du nachdenkst?"

# game/events/ann.rpy:612
translate german ann_dressed_balcony_2e72dc0c:

    # Max_05 "{m}Оу! Мама ещё не оделась... Это я вовремя зашёл. Она шикарно смотрится...{/m}" nointeract
    Max_05 "{m}Oh! Mom ist noch nicht angezogen... Es ist an der Zeit, dass ich mal vorbeischaue. Sie sieht toll aus...{/m}" nointeract

# game/events/ann.rpy:621
translate german ann_dressed_balcony_1f19d7dd:

    # Ann_15 "Макс! Я же учила тебя стучаться! Нельзя вот так без предупреждения врываться в комнату! Что-то случилось?" nointeract
    Ann_15 "Max! Ich habe dir beigebracht zu klopfen! Du kannst nicht einfach unangekündigt in ein Zimmer platzen! Stimmt etwas nicht?" nointeract

# game/events/ann.rpy:624
translate german ann_dressed_balcony_e1ca91d3:

    # Ann_15 "Макс! А стучаться кто будет?! Прекращай уже без предупреждения врываться в комнату! Или у тебя что-то срочное?" nointeract
    Ann_15 "Max! Wer klopft denn da?! Hör auf, unangemeldet ins Zimmer zu platzen! Oder handelt es sich um einen Notfall?" nointeract

# game/events/ann.rpy:633
translate german ann_dressed_balcony_a9e5c75c:

    # Ann_15 "Макс! Я же учила тебя стучаться! {p=3}{nw}"
    Ann_15 "Max! Ich habe dir beigebracht, dass man anklopft! {p=3}{nw}"

# game/events/ann.rpy:635
translate german ann_dressed_balcony_babcea3a:

    # Ann_15 "Макс! А стучаться кто будет?! {p=3}{nw}"
    Ann_15 "Max! Wer klopft denn da?! {p=3}{nw}"

# game/events/ann.rpy:638
translate german ann_dressed_balcony_aa9214ef:

    # Ann_14 "Нельзя вот так без предупреждения врываться в комнату! Что-то случилось?" nointeract
    Ann_14 "Du kannst nicht einfach so unerwartet in ein Zimmer platzen! Ist etwas passiert?" nointeract

# game/events/ann.rpy:641
translate german ann_dressed_balcony_c391d1e4:

    # Ann_12 "Прекращай уже без предупреждения врываться в комнату! Или у тебя что-то срочное?" nointeract
    Ann_12 "Hör auf, unangemeldet ins Zimmer zu platzen! Oder liegt ein Notfall vor?" nointeract

# game/events/ann.rpy:650
translate german ann_dressed_balkon_talk1_b906cb2e:

    # Ann_14 "У меня сейчас такое состояние, что я не знаю, как мне жить дальше... Я... Я ужасная мать..."
    Ann_14 "Ich bin gerade in einer Phase, in der ich nicht weiß, wie es weitergehen soll... Ich... Ich bin eine schreckliche Mutter..."

# game/events/ann.rpy:651
translate german ann_dressed_balkon_talk1_b034aec8:

    # Max_08 "Что?! Как ты можешь такое говорить?"
    Max_08 "Was? Wie kannst du das sagen?"

# game/events/ann.rpy:652
translate german ann_dressed_balkon_talk1_79bc09d7:

    # Ann_13 "А разве нет? Я... Я была так глупа, что привела в дом проходимца, который не только воспользовался мной, но и чуть не совратил моих девочек!"
    Ann_13 "Oder nicht? Ich... Ich war so dumm, uns ein Arschloch ins Haus zu holen, das mich nicht nur ausnutzte, sondern fast meine Mädchen missbraucht hat!"

# game/events/ann.rpy:653
translate german ann_dressed_balkon_talk1_de50c165:

    # Max_09 "Мам, прекрати, это уже всё в прошлом... Забудь..."
    Max_09 "Mom, hör auf, das ist alles Vergangenheit... Vergiss es..."

# game/events/ann.rpy:654
translate german ann_dressed_balkon_talk1_c7c7442f:

    # Ann_14 "Забыть? Забыть, как я чуть не отправила единственного сына в военный лагерь и... который один оказался настолько умным и благородным, что не только не разозлился на меня, а наоборот, спас всю семью от этого ублюдка Эрика."
    Ann_14 "Vergessen? Vergessen, dass ich meinen einzigen Sohn fast in ein Armeelager geschickt hätte und... der sich als einziger als so klug und mutig erwies, dass er nicht nur nicht böse auf mich war, sondern im Gegenteil die ganze Familie vor diesem Bastard Eric gerettet hat."

# game/events/ann.rpy:655
translate german ann_dressed_balkon_talk1_efb41ecc:

    # Max_07 "Ну хватит... Было и было. Прими и сделай выводы. Нельзя же всё время теперь ходить такой печальной и заниматься самобичеванием. Ты ни в чём не виновата!"
    Max_07 "Komm schon... Es war so und fertig. Akzeptiere es und ziehe deine eigenen Schlüsse. Man kann sich nicht die ganze Zeit selbst fertig machen. Es ist nicht deine Schuld!"

# game/events/ann.rpy:656
translate german ann_dressed_balkon_talk1_e8140f63:

    # Ann_17 "Возможно ты прав, но... присутствие Эрика на работе постоянно мне напоминает о тех событиях и мне это ужасно неприятно."
    Ann_17 "Du hast wahrscheinlich Recht, aber... Erics Anwesenheit bei der Arbeit erinnert mich ständig an diese Ereignisse, und ich fühle mich schrecklich dabei."

# game/events/ann.rpy:657
translate german ann_dressed_balkon_talk1_63d3ad78:

    # Max_15 "Да и чёрт с ним! Просто игнорируй его. К тому же, у тебя есть мы, и мы все тебя очень любим!"
    Max_15 "Zum Teufel damit! Ignoriere ihn einfach. Außerdem hast du uns, und wir alle lieben dich sehr!"

# game/events/ann.rpy:658
translate german ann_dressed_balkon_talk1_6335970c:

    # Ann_02 "Спасибо, мой хороший, за твои тёплые слова и поддержку, мне очень приятно это слышать."
    Ann_02 "Vielen Dank, für deine liebevollen Worte und deine Unterstützung, ich bin sehr froh, das zu hören."

# game/events/ann.rpy:659
translate german ann_dressed_balkon_talk1_59e5166a:

    # Max_04 "Не стоит благодарить, мам. Я очень тебя люблю! Ты самая замечательная мама на свете! Самая добрая и самая красивая!"
    Max_04 "Du brauchst mir nicht zu danken, Mom. Ich liebe dich so sehr! Du bist die wunderbarste Mutter der Welt! Die liebste und schönste!"

# game/events/ann.rpy:661
translate german ann_dressed_balkon_talk1_d77c6ad5:

    # Ann_04 "Ой! Вот же ты маленький льстец, Макс!"
    Ann_04 "Oh, du kleiner Schmeichler, Max!"

# game/events/ann.rpy:662
translate german ann_dressed_balkon_talk1_5e9b1161:

    # Max_07 "Это я-то маленький?"
    Max_07 "Bin ich der Kleine?"

# game/events/ann.rpy:663
translate german ann_dressed_balkon_talk1_5eb0124c:

    # Ann_05 "Ну, конечно, ты большой, Макс! Совсем взрослый, почти мужчина."
    Ann_05 "Aber natürlich bist du groß, Max! Du bist erwachsen geworden, fast ein Mann."

# game/events/ann.rpy:664
translate german ann_dressed_balkon_talk1_f86a1ee3:

    # Max_09 "Что значит почти?"
    Max_09 "Was meinst du mit \"fast\"?"

# game/events/ann.rpy:665
translate german ann_dressed_balkon_talk1_2e6b2825:

    # Ann_07 "Ну... То и значит, что полноценным мужчиной ты станешь только после того, как у тебя появится девушка."
    Ann_07 "Na ja... Das bedeutet, dass du erst dann ein Mann bist, wenn du eine Freundin hast."

# game/events/ann.rpy:666
translate german ann_dressed_balkon_talk1_a8851b6c:

    # Max_08 "Серьёзно? А поподробнее можно узнать, как и что делать с ней, чтобы стать мужчиной?"
    Max_08 "Ernsthaft? Nur so kann ich herausfinden, wie und was es bedeutet, ein Mann zu werden?"

# game/events/ann.rpy:667
translate german ann_dressed_balkon_talk1_f041c776:

    # Ann_02 "Макс, что за вопросы? Как будто ты до сих пор не знаешь."
    Ann_02 "Max, was für Fragen stellst du mir? Es klingt fast so, als wüsstest du es immer noch nicht."

# game/events/ann.rpy:668
translate german ann_dressed_balkon_talk1_d7d0349c:

    # Max_02 "Да это я просто так спросил... Теперь ты уже не такая печальная и задумчивая, как до моего прихода."
    Max_02 "Ich habe nur gefragt... Du bist jetzt nicht mehr so traurig und grüblerisch wie vor meiner Anwesenheit."

# game/events/ann.rpy:669
translate german ann_dressed_balkon_talk1_1c8db60a:

    # Ann_05 "Вот же ты хитрец! Нашёл чем меня отвлечь..."
    Ann_05 "Du bist ein Schlitzohr, nicht wahr? Du hast doch etwas, womit du mich ablenken willst..."

# game/events/ann.rpy:670
translate german ann_dressed_balkon_talk1_3696543d:

    # Max_03 "Могу отвлечь чем-нибудь приятным. Например, массажем спины. Думаю, это тебе бы сейчас помогло. Хочешь?"
    Max_03 "Ich kann dich mit etwas Schönem ablenken. Wie eine Rückenmassage. Ich glaube, das würde dir im Moment helfen. Willst du eine?"

# game/events/ann.rpy:671
translate german ann_dressed_balkon_talk1_eb82677e:

    # Ann_04 "Нет, Макс, сейчас мне некогда. Давай вечерком, сынок, у ТВ."
    Ann_04 "Nein, Max, jetzt gerade nicht. Lass es uns heute Abend vor dem Fernseher tun, mein Sohn."

# game/events/ann.rpy:672
translate german ann_dressed_balkon_talk1_a3767c9d:

    # Max_01 "Договорились, мам. И не забывай, если что, я всегда рядом. Обращайся!"
    Max_01 "Einverstanden, Mom. Und vergiss nicht: Ich bin immer da, wenn du mich brauchst. Jederzeit!"

# game/events/ann.rpy:673
translate german ann_dressed_balkon_talk1_59d69cc0:

    # Ann_01 "Обязательно!" nointeract
    Ann_01 "Absolut!" nointeract

translate german strings:

    # game/events/ann.rpy:596
    old "Мам, всё нормально?"
    new "Mom, ist alles in Ordnung?"

    # game/events/ann.rpy:613
    old "Мам?"
    new "Mom?"

# TODO: Translation updated at 2022-02-19 02:36

# game/events/ann.rpy:684
translate german ann_dressed_balkon_talk2_640d08ea:

    # Ann_14 "Это всё моя работа... Присутствие Эрика делает её просто невыносимой."
    Ann_14 "Das ist alles meine Schuld... Erics Anwesenheit macht es unerträglich."

# game/events/ann.rpy:685
translate german ann_dressed_balkon_talk2_799ed018:

    # Max_15 "Он пристаёт к тебе?"
    Max_15 "Baggert er dich an?"

# game/events/ann.rpy:686
translate german ann_dressed_balkon_talk2_624bc582:

    # Ann_18 "Нет, но его ехидная ухмылочка и постоянные намёки о возможных неприятностях, если я вдруг попытаюсь уволиться, сводят меня с ума."
    Ann_18 "Nein, aber sein abfälliges Grinsen und die ständigen Andeutungen, dass es Ärger geben könnte, wenn ich plötzlich versuche, zu kündigen, machen mich verrückt."

# game/events/ann.rpy:687
translate german ann_dressed_balkon_talk2_0ce3e59e:

    # Max_09 "Просто постарайся не обращать на него внимание."
    Max_09 "Versuche einfach, ihn zu ignorieren."

# game/events/ann.rpy:688
translate german ann_dressed_balkon_talk2_a8ecc8ef:

    # Ann_12 "Вряд ли это возможно, сынок, он же мой начальник." nointeract
    Ann_12 "Ich glaube nicht, dass das möglich ist, mein Sohn, er ist mein Chef." nointeract

# game/events/ann.rpy:693
translate german ann_dressed_balkon_talk2_9803998b:

    # Ann_13 "Макс, ты что это делаешь? За зад меня обнимаешь?!"
    Ann_13 "Max, was machst du da? Umarmst du meinen Arsch?!"

# game/events/ann.rpy:694
translate german ann_dressed_balkon_talk2_f912aa8d:

    # Max_07 "Ну да. Это я так приободрить тебя решил. Если бы он действительно хотел тебе что-то сделать, то он бы уже давно это сделал."
    Max_07 "Ja. Das ist meine Art, dich aufzumuntern. Wenn er dir wirklich etwas antun wollte, hätte er es schon längst getan."

# game/events/ann.rpy:695
translate german ann_dressed_balkon_talk2_769cdc0b:

    # Ann_17 "Сынок, такое вот твое приободрение не работает. Оно меня скорее смущает..."
    Ann_17 "Sohn, diese Art der Aufmunterung funktioniert nicht. Es ist mir ziemlich peinlich..."

# game/events/ann.rpy:697
translate german ann_dressed_balkon_talk2_0167e576:

    # Max_09 "Ладно, уберу... Просто Эрику нравится играть на нервах, вот и всё, что он может сделать."
    Max_09 "OK, ich lasse es... Eric spielt einfach gerne den Unantastbaren, das ist alles, was er kann."

# game/events/ann.rpy:698
translate german ann_dressed_balkon_talk2_273fc7d0:

    # Ann_12 "Возможно так и есть, но... мне бы очень не хотелось искушать судьбу... это ведь может отразиться и на вас, моих детях, а вы для меня самое дорогое."
    Ann_12 "Das mag sein, aber... Ich möchte das Schicksal nicht herausfordern... es könnte euch, meine Kinder, treffen, und ihr seid das Wertvollste, was ich habe."

# game/events/ann.rpy:699
translate german ann_dressed_balkon_talk2_682e32bc:

    # Max_10 "Ты тоже самое дорогое для меня... для нас... И нам бы не хотелось видеть тебя такой печальной, мам."
    Max_10 "Du bist auch das Liebste, was ich habe... für uns alle... Und wir würden es hassen, dich so traurig zu sehen, Mom."

# game/events/ann.rpy:700
translate german ann_dressed_balkon_talk2_261a6f2e:

    # Ann_02 "Спасибо, Макс. Мне очень приятно это слышать. Как хорошо, что у меня есть такой внимательный и заботливый сын."
    Ann_02 "Vielen Dank, Max. Ich bin sehr froh, das zu hören. Wie schön, einen so aufmerksamen und fürsorglichen Sohn zu haben."

# game/events/ann.rpy:701
translate german ann_dressed_balkon_talk2_23203ebd:

    # Max_07 "В любом случае, если ты захочешь сменить место работы и у нас, вдруг, появятся сложности с деньгами, мы все тебя поймём и поддержим."
    Max_07 "Wenn du den Job wechseln willst und wir plötzlich Geldprobleme haben, verstehen wir das alle und unterstützen dich."

# game/events/ann.rpy:702
translate german ann_dressed_balkon_talk2_bed6c909:

    # Ann_04 "Спасибо, мой хороший, я подумаю об этом..."
    Ann_04 "Danke, mein Schatz, ich werde darüber nachdenken..."

# game/events/ann.rpy:703
translate german ann_dressed_balkon_talk2_d9948e58:

    # Max_09 "Обязательно подумай. Нужно покончить с этим прошлым раз и навсегда!"
    Max_09 "Überleg es dir gut. Wir müssen mit dieser Vergangenheit ein für alle Mal Schluss machen!"

# game/events/ann.rpy:704
translate german ann_dressed_balkon_talk2_09e80fa0:

    # Ann_05 "Хм... Глядя на тебя в последнее время, я вижу, что ты очень повзрослел."
    Ann_05 "Hm... Wenn ich mir dich in letzter Zeit ansehe, kann ich sehen, dass du wirklich reifer geworden bist."

# game/events/ann.rpy:705
translate german ann_dressed_balkon_talk2_cdbf49b6:

    # Max_04 "Ну, тебе виднее."
    Max_04 "Das weißt du am besten."

# game/events/ann.rpy:706
translate german ann_dressed_balkon_talk2_650e344f:

    # Ann_08 "Так и есть. И мне нравится то, что я вижу..."
    Ann_08 "Das ist es. Und mir gefällt, was ich sehe..."

# game/events/ann.rpy:707
translate german ann_dressed_balkon_talk2_40bdc5b1:

    # Max_03 "Главное не унывай, мам. Всё образуется."
    Max_03 "Lass dich nicht entmutigen, Mom. Es wird schon alles klappen."

# game/events/ann.rpy:708
translate german ann_dressed_balkon_talk2_ed9bd79e:

    # Ann_07 "Хорошо! Спасибо, что поддержал меня и развеял тоску."
    Ann_07 "Also gut! Danke, dass du mich aufgemundert und meinen Kummer etwas vertrieben hast."

# game/events/ann.rpy:709
translate german ann_dressed_balkon_talk2_1c195d26:

    # Max_01 "Пожалуйста! Если что, я всегда рядом. Обращайся!"
    Max_01 "Gern geschehen! Ich bin immer da, wenn du mich brauchst. Keine Ursache!"

# game/events/ann.rpy:710
translate german ann_dressed_balkon_talk2_59d69cc0:

    # Ann_01 "Обязательно!" nointeract
    Ann_01 "Ganz genau!" nointeract

# game/events/ann.rpy:719
translate german ann_dressed_balkon_talk2_1f418bf5:

    # Max_09 "{m}Смущают её мои прикосновения... Ясно. Значит, нужно больше прикасаться тогда, когда это максимально естественно, чтобы она привыкла. Пожалуй, йога - то, что нужно! Стоит попробовать...{/m}"
    Max_09 "{m}Meine Berührung ist ihr peinlich... Verstehe. Ich muss sie also mehr berühren, wenn es so natürlich wie möglich ist, um sie daran zu gewöhnen. Ich denke, Yoga ist der richtige Weg! Es ist einen Versuch wert...{/m}"

translate german strings:

    # game/events/ann.rpy:689
    old "{i}приобнять маму за попку{/i}"
    new "{i}Mom von hinten umarmen{/i}"

# TODO: Translation updated at 2022-02-20 20:04

# game/events/ann.rpy:757
translate german ann_dressed_balkon_r_4897a021:

    # Ann_02 "Ой, Макс! Как ты так тихо подкрадываешься? Да, любуюсь... А ты почему без дела слоняешься?" nointeract
    Ann_02 "Oh, Max! Wieso schleichst du dich so an mich heran? Ja, ich bewundere nur... Was machst du hier draußen?" nointeract

# game/events/ann.rpy:760
translate german ann_dressed_balkon_r_0a883de4:

    # Ann_01 "Не то, чтобы мешаешь... Иди лучше чем-нибудь продуктивным займись. Маме пока некогда."
    Ann_01 "Nicht, dass du im Weg wärst... Du solltest lieber etwas Produktives tun. Für so etwas fehlt noch die Zeit."

# game/events/ann.rpy:761
translate german ann_dressed_balkon_r_58738e49:

    # Max_00 "Ладно, не буду мешать." nointeract
    Max_00 "Gut, ich lasse dich in Ruhe." nointeract

# game/events/ann.rpy:767
translate german ann_dressed_balkon_r_2280510e:

    # Ann_04 "Нет. Побудь, если хочется. Я не против." nointeract
    Ann_04 "Nein. Bleib, wenn du willst. Das stört mich nicht." nointeract

# game/events/ann.rpy:774
translate german ann_dressed_balkon_r_119f09c9:

    # Ann_02 "Как у тебя дела, Макс? Всё хорошо?"
    Ann_02 "Wie geht es dir, Max? Geht es dir gut?"

# game/events/ann.rpy:775
translate german ann_dressed_balkon_r_ca3ee871:

    # Max_04 "Да, порядок. А ты как? Не скучаешь?"
    Max_04 "Ja, es ist alles in Ordnung. Was ist mit dir? Langeweile?"

# game/events/ann.rpy:776
translate german ann_dressed_balkon_r_7fdac7ed:

    # Ann_05 "Ох, сынок, с вами разве заскучаешь. Да и на работе дел хватает." nointeract
    Ann_05 "Oh, mein Sohn, mit dir wird es nie langweilig werden. Auch bei der Arbeit gibt es viel zu tun." nointeract

# game/events/ann.rpy:781
translate german ann_dressed_balkon_r_55550510:

    # Max_07 "Ты, мам, побольше отдыхать не забывай."
    Max_07 "Vergiss nicht, dich mehr auszuruhen, Mom."

# game/events/ann.rpy:782
translate german ann_dressed_balkon_r_081cb55f:

    # Ann_07 "Это я и делаю сейчас. А ты давай беги, займись чем-нибудь полезным."
    Ann_07 "Das ist es, was ich jetzt tue. Du gehst und tust etwas Nützliches."

# game/events/ann.rpy:793
translate german ann_dressed_balkon_r_a9420db7:

    # Max_07 "Ты, мам, побольше отдыхать не забывай. У нас слишком прекрасный дом, чтобы грустить."
    Max_07 "Vergiss nicht, dich mehr auszuruhen, Mom. Wir haben ein zu schönes Zuhause, um traurig zu sein."

# game/events/ann.rpy:794
translate german ann_dressed_balkon_r_874166e7:

    # Ann_08 "Да, здесь здорово! Маленький рай. Ты зашёл меня просто вот так подбодрить?"
    Ann_08 "Ja, hier ist es großartig! Ein kleines Paradies. Bist du nur vorbeigekommen, um mich aufzuheitern?"

# game/events/ann.rpy:798
translate german ann_dressed_balkon_r_6b6b7f92:

    # Max_02 "Ну, да. Хотел напомнить, что я всегда рядом."
    Max_02 "Nun, ja. Ich wollte dich nur daran erinnern, dass ich immer da bin."

# game/events/ann.rpy:799
translate german ann_dressed_balkon_r_680063a0:

    # Ann_07 "Спасибо, но лучше беги, займись чем-нибудь полезным. У меня всё в порядке."
    Ann_07 "Danke, aber du solltest lieber gehen und etwas Sinnvolles tun. Es geht mir gut."

# game/events/ann.rpy:806
translate german ann_dressed_balkon_r_0cc8bfb1:

    # Max_03 "Ладно, не буду отвлекать. Я тебя люблю, мам!"
    Max_03 "Okay, ich will nicht stören. Ich liebe dich, Mom!"

# game/events/ann.rpy:807
translate german ann_dressed_balkon_r_dfe00014:

    # Ann_06 "Я тебя тоже, сынок." nointeract
    Ann_06 "Ich dich auch, mein Sohn." nointeract

translate german strings:

    # game/events/ann.rpy:604
    old "Видами любуешься, мам?"
    new "Genießt du die Aussicht, Mom?"

    # game/events/ann.rpy:758
    old "А что, я мешаю?"
    new "Störe ich?"

    # game/events/ann.rpy:758
    old "С тобой хотел побыть. Или я мешаю?"
    new "Ich bei dir sein. Oder störe ich?"

    # game/events/ann.rpy:768
    old "{i}приобнять маму{/i}"
    new "{i}Mom umarmen{/i}"

    # game/events/ann.rpy:777
    old "{i}продолжить обнимать{/i}"
    new "{i}Umarmung fortsetzen{/i}"

# TODO: Translation updated at 2022-02-23 00:12

# game/events/ann.rpy:665
translate german ann_dressed_balkon_talk1_37635b4b:

    # Ann_14 "Забыть? Забыть, как я чуть не отправила единственного сына в военный лагерь и... который один оказался настолько умным и благородным, что не только не разозлился на меня, а наоборот, спас всю семью от этого ублюдка - Эрика."
    Ann_14 "Vergessen? Vergessen, dass ich meinen einzigen Sohn fast in ein Armeelager geschickt hätte und... der sich als einziger als so klug und edel erwies, dass er nicht nur nicht böse auf mich war, sondern im Gegenteil die ganze Familie vor diesem Bastard Eric gerettet hat."

# game/events/ann.rpy:666
translate german ann_dressed_balkon_talk1_ac19285d:

    # Max_07 "Ну хватит... Было и было. Прими это и сделай выводы. Нельзя же всё время теперь ходить такой печальной и заниматься самобичеванием. Ты ни в чём не виновата!"
    Max_07 "Komm schon... Es war einmal so. Akzeptiere es und zieh deine eigenen Schlüsse. Man kann sich nicht die ganze Zeit selbst fertig machen. Es ist nicht deine Schuld!"

# TODO: Translation updated at 2022-02-28 02:39

# game/events/ann.rpy:960
translate german ann_bath_about_intime_0_cddc6bee:

    # Ann "{b}Анна:{/b} Кто там? Я принимаю ванну!" nointeract
    Ann "{b}Аnn:{/b} Wer ist da? Ich nehme ein Bad!" nointeract

# game/events/ann.rpy:961
translate german ann_bath_about_intime_0_006aa2f9:

    # Max_07 "Это я, Макс. Можно войти?"
    Max_07 "Ich bin's, Max. Darf ich reinkommen?"

# game/events/ann.rpy:962
translate german ann_bath_about_intime_0_765e20f4:

    # Ann "{b}Анна:{/b} Зачем, дорогой? Тебе что-то нужно?"
    Ann "{b}Аnn:{/b} Warum, mein Schatz? Brauchst du etwas?"

# game/events/ann.rpy:963
translate german ann_bath_about_intime_0_cc4bed10:

    # Max_09 "Да, мам. Хотел поговорить."
    Max_09 "Ja, Mom. Wir müssen reden."

# game/events/ann.rpy:964
translate german ann_bath_about_intime_0_62195f79:

    # Ann "{b}Анна{/b}: Если это может подождать, то я через полчасика освобожусь..." nointeract
    Ann "{b}Аnn:{/b} Kann das nicht warten? Ich bin in einer halben Stunde draußen..." nointeract

# game/events/ann.rpy:974
translate german ann_bath_about_intime_0_53cc7aca:

    # Ann_15 "Макс! Ты почему так нагло врываешься! Я же сказала, что скоро освобожусь! Не мог подождать?"
    Ann_15 "Max! Warum hast du die Dreistigkeit, hier hereinzuplatzen? Ich sagte doch, dass ich bald rauskomme! Kannst du nicht warten?"

# game/events/ann.rpy:975
translate german ann_bath_about_intime_0_1af3858f:

    # Max_08 "Нет, нужно кое-что обсудить, пока ты здесь."
    Max_08 "Nein, es gibt etwas zu besprechen, solange du hier bist."

# game/events/ann.rpy:978
translate german ann_bath_about_intime_0_01bf0619:

    # Ann_14 "И что там у тебя такого срочного?!"
    Ann_14 "Was ist denn so dringend?!"

# game/events/ann.rpy:979
translate german ann_bath_about_intime_0_a34fa2a4:

    # Max_07 "Хотел поговорить о девочках!"
    Max_07 "Ich wollte über Frauen reden!"

# game/events/ann.rpy:980
translate german ann_bath_about_intime_0_197357cf:

    # Ann_17 "И ты считаешь, что это настолько важно, что кроме как здесь об этом говорить нельзя, да?"
    Ann_17 "Und du hältst es für so wichtig, dass du nur hier darüber sprechen kannst, oder?"

# game/events/ann.rpy:981
translate german ann_bath_about_intime_0_7db297d2:

    # Max_01 "Ну... Не просто о девочках, а том, как им... делать приятно... там, внизу..."
    Max_01 "Na ja... Nicht nur über Frauen, sondern auch darüber, wie man sie dazu bringt... damit sie sich gut fühlen... Da unten..."

# game/events/ann.rpy:982
translate german ann_bath_about_intime_0_28faf8d1:

    # Ann_02 "Вот это вопросы у тебя, Макс! А что, у тебя девочка появилась?"
    Ann_02 "Was für Fragen du hast, Max! Hast du etwa ein Mädchen?"

# game/events/ann.rpy:983
translate german ann_bath_about_intime_0_4dd2948d:

    # Max_07 "Ещё нет, но появится же. А я хочу быть готовым! Вы мне с Эриком показали на тех уроках, как себя вести, если женщина... так сказать, ласкает мужчину, а вот как ласкать женщин - нет."
    Max_07 "Noch nicht, aber bald. Und ich will vorbereitet sein! Du und Eric habt mir in diesem Kurs gezeigt, wie man sich verhält, wenn eine Frau... sozusagen, einen Mann liebkost, aber wie man eine Frau liebkost, nicht."

# game/events/ann.rpy:984
translate german ann_bath_about_intime_0_435b556c:

    # Ann_14 "Ой, сынок, какие же это неудобные вопросы! Я всё ещё до конца не могу осознать, что ты уже почти взрослый."
    Ann_14 "Oh, mein Sohn, was sind das für unangenehme Fragen! Ich kann immer noch nicht ganz begreifen, dass du fast erwachsen bist."

# game/events/ann.rpy:985
translate german ann_bath_about_intime_0_b912cfb2:

    # Max_09 "Пора бы уже осознать, мам! И уроки ваши, какие-то односторонние. Как-то неправильно."
    Max_09 "Es wird Zeit, dass du das erkennst, Mom! Und dein Unterricht ist ziemlich einseitig. Es fühlt sich nicht richtig an."

# game/events/ann.rpy:986
translate german ann_bath_about_intime_0_24e945aa:

    # Ann_13 "Ну... Что-то я даже не знаю..."
    Ann_13 "Na ja... Etwas, das ich nicht verstehe..."

# game/events/ann.rpy:989
translate german ann_bath_about_intime_0_acd6b1b2:

    # Max_07 "Да ладно, я же уже такое видел с вами на тех уроках..."
    Max_07 "Komm schon, ich habe das schon mal bei dir in diesen Kursen gesehen..."

# game/events/ann.rpy:990
translate german ann_bath_about_intime_0_fdfa6acb:

    # Ann_12 "Ой, Макс, не напоминай. Это всё было исключительно для твоего... сексуального образования..."
    Ann_12 "Oh, Max, erinnere mich nicht daran. Das war alles nur für deine... Sexualerziehung..."

# game/events/ann.rpy:991
translate german ann_bath_about_intime_0_e2b46bea:

    # Max_08 "Так и это тоже для образования! Потому что я хочу не только получать ласку от девочек, но и давать её им. А это, как я понимаю, куда сложнее."
    Max_08 "Es geht also auch um Erziehung! Denn ich möchte nicht nur Zuneigung von Mädchen erhalten, sondern ihnen auch Zuneigung geben. Und das ist, soweit ich weiß, sehr viel schwieriger."

# game/events/ann.rpy:992
translate german ann_bath_about_intime_0_1f40fc0d:

    # Ann_13 "Ох, с учётом того, чему мы с Эриком уже тебя тогда научили, твоя просьба очень логична, сынок. Так что я поговорю об этом с Эриком..."
    Ann_13 "Wenn man bedenkt, was Eric und ich dir bereits beigebracht haben, ist deine Bitte sehr logisch, mein Sohn. Ich werde also mit Eric darüber sprechen..."

# game/events/ann.rpy:993
translate german ann_bath_about_intime_0_ee64e187:

    # Max_10 "Нет, мам, я хочу только с тобой этому учиться!"
    Max_10 "Nein, Mom, ich will es nur mit dir üben!"

# game/events/ann.rpy:994
translate german ann_bath_about_intime_0_bd74b95d:

    # Ann_17 "То есть, только со мной?! А Эрик тебе чем не угодил?"
    Ann_17 "Du meinst, nur ich?! Was ist mit Eric?"

# game/events/ann.rpy:995
translate german ann_bath_about_intime_0_0314b6cd:

    # Max_09 "Мне кажется, он это... направление не особо любит. Мне же лучше, чтобы такому учила именно женщина, ведь вам виднее."
    Max_09 "Ich glaube nicht, dass er ein... Ich glaube nicht, dass er sich für so etwas interessiert. Ich würde es lieber von einer Frau lernen lassen, denn Sie weiß es am besten."

# game/events/ann.rpy:996
translate german ann_bath_about_intime_0_ba4d6bb6:

    # Ann_12 "Виднее, конечно. Но без Эрика я даже не знаю... Не уверена... Ничего, если я подумаю об этом в более... комфортной обстановке?"
    Ann_12 "Das ist natürlich besser so. Aber ohne Eric weiß ich nicht... Ich bin mir nicht sicher... Ist es in Ordnung, wenn ich darüber in einer etwas... angenehmeren Umgebung nachdenke?"

# game/events/ann.rpy:997
translate german ann_bath_about_intime_0_3a30664f:

    # Max_01 "Да, мам! Я не буду мешать, расслабляйся." nointeract
    Max_01 "Ja, Mom! Ich lasse dich in Ruhe, entspann dich." nointeract

# TODO: Translation updated at 2022-06-17 00:19

# game/events/ann.rpy:18
translate german ann_sleep_e0ca90ce:

    # Max_09 "Хм... Мамы нет. Где же она?"
    Max_09 "Ähm ... Mom ist nicht da. Wo ist sie?"

# game/events/ann.rpy:974
translate german ann_bath_about_intime_0_3aa93926:

    # Ann "{b}Анна:{/b} Кто там? Я принимаю ванну!"
    Ann "{b}Ann:{/b} Wer ist da? Ich nehme ein Bad!"

# game/events/ann.rpy:978
translate german ann_bath_about_intime_0_a05b810e:

    # Ann "{b}Анна:{/b} Если это может подождать, то я через полчасика освобожусь..." nointeract
    Ann "{b}Ann:{/b} In einer halben Stunde bin ich fertig, falls du etwas warten kannst..." nointeract

# TODO: Translation updated at 2022-06-27 01:23

# game/events/ann.rpy:360
translate german ann_dressed_stay_in_room_659d5d18:

    # Ann_12 "Нет, конечно! Ишь ты, что удумал! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_12 "Nein, natürlich nicht! Sieh dich an! Hör mit diesem Unsinn auf und lasse mich in Ruhe umziehen." nointeract

# game/events/ann.rpy:362
translate german ann_dressed_stay_in_room_e4ef4b48:

    # Ann_05 "Приятно слышать, сынок. Спасибо, что подметил." nointeract
    Ann_05 "Das ist gut zu hören, mein Sohn. Vielen Dank für den Hinweis." nointeract

# game/events/ann.rpy:394
translate german ann_dressed_moment0_659d5d18:

    # Ann_12 "Нет, конечно! Ишь ты, что удумал! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_12 "Nein, natürlich nicht! Sieh dich an! Hör mit diesem Unsinn auf und lass mich in Ruhe umziehen." nointeract

# game/events/ann.rpy:396
translate german ann_dressed_moment0_e4ef4b48:

    # Ann_05 "Приятно слышать, сынок. Спасибо, что подметил." nointeract
    Ann_05 "Das ist gut zu hören, mein Sohn. Vielen Dank für den Hinweis." nointeract

# game/events/ann.rpy:398
translate german ann_dressed_moment0_84eb80a2:

    # Ann_05 "Макс! Тебе что, мало того, что ты уже увидел? Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_05 "Max! Hast du nicht schon genug gesehen? Hör mit diesem Unsinn auf und lass mich in Ruhe umziehen." nointeract

# game/events/ann.rpy:402
translate german ann_dressed_moment0_ad028372:

    # Ann_05 "Только под ноги смотри, сынок. А то мало ли куда лбом врежешься..." nointeract
    Ann_05 "Pass nur auf, wo du hintrittst, mein Sohn. Du willst dir doch nicht den Kopf stoßen..." nointeract

# game/events/ann.rpy:421
translate german ann_dressed_moment2_126ffe76:

    # Ann_13 "Ой, Макс! Почему не стучишься? {p=3}{nw}"
    Ann_13 "Oh, Max! Warum klopfst du nicht an? {p=3}{nw}"

# game/events/ann.rpy:433
translate german ann_dressed_moment2_2cd5729c:

    # Ann_02 "Ты что-то хотел или просто от безделья маешься?"
    Ann_02 "Wolltest du etwas oder läufst du nur herum?"

# game/events/ann.rpy:436
translate german ann_dressed_moment2_2cd5729c_1:

    # Ann_02 "Ты что-то хотел или просто от безделья маешься?"
    Ann_02 "Wolltest du etwas oder läufst du nur herum?"

# game/events/ann.rpy:458
translate german ann_dressed_moment1_dfc78518:

    # Ann_13 "Ой, Макс! Почему не стучишься? Ты что-то хотел или просто от безделья маешься?"
    Ann_13 "Oh, Max! Warum klopfst du nicht an? Wolltest du etwas, oder läufst du nur herum?"

# game/events/ann.rpy:461
translate german ann_dressed_moment1_dfc78518_1:

    # Ann_13 "Ой, Макс! Почему не стучишься? Ты что-то хотел или просто от безделья маешься?"
    Ann_13 "Oh, Max! Warum klopfst du nicht an? Wolltest du etwas, oder läufst du nur herum?"

# game/events/ann.rpy:525
translate german ann_dressed_lvl_3_48432cd3:

    # Max_01 "Нет, просто решил заглянуть. А ты что, всё ещё стесняешься? Это же я!"
    Max_01 "Nein, ich dachte nur, ich schaue mal rein. Bist du immer noch so verklemmt? Ich bin's doch nur!"

# game/events/ann.rpy:530
translate german ann_dressed_lvl_3_50567e5f:

    # Ann_04 "Да это я так, по привычке. Если у тебя ничего срочного, то выйди пожалуйста. Мне нужно переодеться..." nointeract
    Ann_04 "Es ist einfach zur Gewohnheit geworden. Wenn es nichts Dringendes gibt, geh bitte raus. Ich muss mich umziehen..." nointeract

# game/events/ann.rpy:532
translate german ann_dressed_lvl_3_fa70085d:

    # Ann_04 "Конечно стесняюсь! Ещё я тут всю себя на показ не выставляла, только из-за того, что ты уже всё видел. Если у тебя ничего срочного, то выйди пожалуйста. Мне нужно переодеться..." nointeract
    Ann_04 "Aber sicher doch! Außerdem habe ich mich hier nicht zur Schau gestellt, nur weil du schon alles gesehen hast. Wenn es nichts Dringendes gibt, geh bitte raus. Ich muss mich umziehen..." nointeract

# game/events/ann.rpy:535
translate german ann_dressed_lvl_3_659d5d18:

    # Ann_12 "Нет, конечно! Ишь ты, что удумал! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_12 "Nein, natürlich nicht! Sieh dich an! Hör auf mit diesem Unsinn und lass mich in Ruhe umziehen." nointeract

# game/events/ann.rpy:537
translate german ann_dressed_lvl_3_e4ef4b48:

    # Ann_05 "Приятно слышать, сынок. Спасибо, что подметил." nointeract
    Ann_05 "Das ist gut zu hören, mein Sohn. Vielen Dank für diesen Hinweis." nointeract

# game/events/ann.rpy:543
translate german ann_dressed_lvl_4_2af495cf:

    # Max_02 "Нет, просто решил заглянуть и полюбоваться на твою шикарную фигуру. Ты же не против?"
    Max_02 "Nein, ich dachte nur, ich schaue mal vorbei und bewundere deine tolle Figur. Es macht dir doch nichts aus, oder?"

# game/events/ann.rpy:544
translate german ann_dressed_lvl_4_b937a903:

    # Ann_04 "Ну... Если налюбовался, то выйди пожалуйста. Мне нужно переодеться..." nointeract
    Ann_04 "Na ja... Wenn du genug hast, geh bitte raus. Ich muss mich umziehen..." nointeract

# game/events/ann.rpy:547
translate german ann_dressed_lvl_4_84eb80a2:

    # Ann_05 "Макс! Тебе что, мало того, что ты уже увидел? Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_05 "Max! Hast du nicht schon genug gesehen? Hör mit diesem Unsinn auf und lass mich in Ruhe umziehen." nointeract

# game/events/ann.rpy:551
translate german ann_dressed_lvl_4_ad028372:

    # Ann_05 "Только под ноги смотри, сынок. А то мало ли куда лбом врежешься..." nointeract
    Ann_05 "Pass nur auf, wo du hingehst, mein Sohn. Du willst dir doch nicht den Kopf stoßen..." nointeract

translate german strings:

    # game/events/ann.rpy:358
    old "Да легко! Не буду мешать. Выглядишь, кстати, превосходно!"
    new "Alles klar! Ich gehe. Du siehst übrigens großartig aus!"

    # game/events/ann.rpy:385
    old "А можно остаться и посмотреть?"
    new "Kann ich bleiben und zusehen?"

    # game/events/ann.rpy:385
    old "Да легко! А повернуться можешь, пока я ухожу?"
    new "Alles klar! Kannst du dich umdrehen, während ich mich umziehe?"

