# TODO: Translation updated at 2021-09-01 18:42

# game/events/kira.rpy:23
translate english kira_bath_e51d277a:

    # Max_01 "Только один человек может в это время не спать и плескаться в ванне. И человек этот - Кира!" nointeract
    Max_01 "Only one person would be awake and in the bath at this time- Aunt Kira!" nointeract

# game/events/kira.rpy:26
translate english kira_bath_fef8596e:

    # Kira "{b}Кира:{/b} Кто там? Я принимаю ванну и абсолютно голая... Так что не входите, дайте отдохнуть!"
    Kira "{b}Kira:{/b} Who's there? I'm taking a bath and I'm completely naked... So don't come in!"

# game/events/kira.rpy:28
translate english kira_bath_5bdda7d4:

    # Max_09 "Нужно найти способ как-то подглядеть за девчонками в ванной. Может, попробовать вечером со двора?!"
    Max_09 "There needs to be a better vantage point to see into the bathroom. Maybe I should try the yard tonight?"

# game/events/kira.rpy:30
translate english kira_bath_b499c0e8:

    # Max_09 "Нужно найти способ как-то подглядеть за девчонками в ванной. Может, стоит осмотреться изнутри?!"
    Max_09 "There must be better way to check on the girls in the bathroom. Maybe I should take a look inside?"

# game/events/kira.rpy:32
translate english kira_bath_e602bda6:

    # Max_09 "Нужно уже наконец купить стремянку, чтобы подглядывать за тем, что происходит в ванной..."
    Max_09 "Looks like buying a stepladder will be my best chance to get a good view of the bathroom."

# game/events/kira.rpy:34
translate english kira_bath_67b54ecf:

    # Max_09 "В следующий раз лучше заглянуть со стремянки..."
    Max_09 "I should use the ladder next time..."

# game/events/kira.rpy:43
translate english kira_bath_226743d2:

    # Max_09 "Пусть лучше тётя Кира отдохнёт, а вернее отмоется от Эрика... А то как-то не очень хочется что-то делать в ванне сразу после него!"
    Max_09 "It's better for Aunt Kira to rest, or rather to get rid of Eric... I don't want his sloppy seconds!"

# game/events/kira.rpy:45
translate english kira_bath_b08ab956:

    # Max_09 "Пусть лучше тётя Кира отдохнёт и отмоется от Эрика... Можно было бы всё же зайти, но вдруг Эрик этого и ждёт. Лучше не рисковать!"
    Max_09 "It would be better if Aunt Kira rested and washed up from Eric's... I could still go in, but maybe that's what Eric was waiting for. I'd better not take any chances!"

# game/events/kira.rpy:54
translate english kira_bath_6a58c36d:

    # Max_01 "Надеюсь, что ни у кого не возникнет вопроса, а что же здесь делает стремянка... Как, что? Конечно стоит, мало ли что! А теперь начинается самое интересное..."
    Max_01 "I hope no one asks why a stepladder's here. Was it worth buying? Time to find out!"

# game/events/kira.rpy:70
translate english kira_bath_ladder_2b325faf:

    # Max_04 "Посмотрим, что у нас тут..."
    Max_04 "Let's see what we got here..."

# game/events/kira.rpy:77
translate english kira_bath_ladder_7cba9683:

    # Max_02 "Ох, чёрт! Какая аппетитная попка у неё... Это я удачно решил дождаться её возвращения! Давай, тётя Кира, этой попке пора стать мокренькой..."
    Max_02 "Oh! What a fat ass she's got... Lucky I waited for her to get back! Come on, Aunt Kira, it's time that ass got wet..."

# game/events/kira.rpy:81
translate english kira_bath_ladder_1e31a44d:

    # Max_05 "Ого! Да она сперва решила поиграть со своей киской... Вот это классно! Только почему бы не делать это лёжа в ванной? Такое ощущение, что она это специально..."
    Max_05 "Wow! She decided to play with her pussy... Oh, that's awesome. Just why not do it lying down in the bathroom? It feels like she was doing it on purpose..."

# game/events/kira.rpy:83
translate english kira_bath_ladder_a5472214:

    # Max_10 "Блин! Она меня видела! Вот это уже не круто... Хотя, это же тётя Кира... она же не станет меня сдавать маме? Или нет?!"
    Max_10 "Shit! She saw me! That's not good... It's Aunt Kira, though... she's not going to turn me in to Mom, is she? "

# game/events/kira.rpy:85
translate english kira_bath_ladder_25ce42f2:

    # Max_08 "Что? Она зовёт меня к себе?! Да ладно! Как-то мне это не очень нравится..." nointeract
    Max_08 "What? Is she calling me over?! I'm nervous, I don't like where this is going..." nointeract

# game/events/kira.rpy:95
translate english kira_bath_ladder_be5e0493:

    # Max_04 "Да-а-а, у неё просто обалденная грудь! Сосочки прямо там, где надо... Ох, я бы с ней ванну попринимал! Надеюсь, мне это приснится. Пора идти спать..."
    Max_04 "Yeah, she's got great tits! I wonder what her nipples taste like... Oh, I'd take a bath with her! I hope it at least happens in my dreams."

# game/events/kira.rpy:102
translate english kira_bath_kira_mass_bath_first_7398c496:

    # Kira_04 "А мне было интересно, осмелишься ли ты зайти... Значит, Макс, любишь подглядывать?"
    Kira_04 "I was wondering if you'd dare come in... So, Max, you like spying on your Aunt?"

# game/events/kira.rpy:103
translate english kira_bath_kira_mass_bath_first_c457930d:

    # Max_07 "Ну, немного..."
    Max_07 "Well, yeah, sometimes..."

# game/events/kira.rpy:104
translate english kira_bath_kira_mass_bath_first_93c94ec0:

    # Kira_05 "Немного? Это сколько? Раз в неделю? Или раз в день?"
    Kira_05 "Sometimes? How often is that? Once a week? Or once a day?"

# game/events/kira.rpy:105
translate english kira_bath_kira_mass_bath_first_7b934ba1:

    # Max_09 "Ты сдашь меня маме?"
    Max_09 "Are you gonna tell Mom?"

# game/events/kira.rpy:106
translate english kira_bath_kira_mass_bath_first_14b70a3b:

    # Kira_02 "Ох, конечно нет, Макс! Я и сама большая любительница за чем-нибудь... интересным понаблюдать."
    Kira_02 "Oh, of course not, Max! It's only normal to want to watch something you're interested in."

# game/events/kira.rpy:107
translate english kira_bath_kira_mass_bath_first_4507dc04:

    # Max_01 "Фух... хорошо."
    Max_01 "Phew... That's a relief."

# game/events/kira.rpy:108
translate english kira_bath_kira_mass_bath_first_cf835ccb:

    # Kira_07 "Хотя, всё будет зависеть от тебя!"
    Kira_07 "But it's going to depend on you, though!"

# game/events/kira.rpy:109
translate english kira_bath_kira_mass_bath_first_825c8fc0:

    # Max_08 "В каком это смысле?"
    Max_08 "Wait, what does that mean?"

# game/events/kira.rpy:114
translate english kira_bath_kira_mass_bath_first_af29342a:

    # Kira_01 "Я слышала, ты неплохо так массируешь ножки то одной сестре, то другой..."
    Kira_01 "I've heard some rumours that you're quite good with your hands.."

# game/events/kira.rpy:115
translate english kira_bath_kira_mass_bath_first_3b03420e:

    # Max_01 "Ну да, бывает такое."
    Max_01 "Well, yeah, but it's not how it sounds."

# game/events/kira.rpy:116
translate english kira_bath_kira_mass_bath_first_5ed6b6a0:

    # Kira_02 "Я сегодня очень устала... И если бы нашёлся тот, кто помог бы снять усталость с моих ног, то может быть, в качестве благодарности, я бы и не стала рассказывать своей сестре, какой у неё испорченный сын."
    Kira_02 "Well I'm very tired today... And if there was someone who could help relieve the stress from my legs, then maybe, as a thank you, I wouldn't tell my sister what a perverted son she has."

# game/events/kira.rpy:117
translate english kira_bath_kira_mass_bath_first_50c1f105:

    # Max_07 "Э-э-э... Что, прямо здесь?"
    Max_07 "Uh, uh... What, right here?"

# game/events/kira.rpy:118
translate english kira_bath_kira_mass_bath_first_d286de06:

    # Kira_05 "Ой, а я разве тебя смущаю, Макс?! Ты разве не за этим подглядывал? Чтобы как можно больше всего увидеть безнаказанно..."
    Kira_05 "Oh, are you embarrassed now? Isn't this what you were spying on? And here I am giving you a chance to gaze as much as you like..."

# game/events/kira.rpy:119
translate english kira_bath_kira_mass_bath_first_1f73cf5f:

    # Max_08 "Да, но... прямо здесь... в ванне?!"
    Max_08 "Yes, but... Right here... In the bath?!"

# game/events/kira.rpy:120
translate english kira_bath_kira_mass_bath_first_1994407b:

    # Kira_08 "Что ты, нет! Ты сядешь здесь, рядом с ванной, а я свешу тебе свои ноги... Ха! Конечно, в ванне! Я же вижу с каким трудом ты прикрываешь свой... энтузиазм... Расслабься уже и залезай в ванну. Или, может, хочешь уйти?"
    Kira_08 "Ha! Of course! I can see how hard it is for you to cover your own... Enthusiasm... Relax already and get in the tub. You're not going to leave now, are you?"

# game/events/kira.rpy:121
translate english kira_bath_kira_mass_bath_first_1f0f11e4:

    # Max_02 "Конечно, нет! Это я мигом..."
    Max_02 "Of course not! Here I come..."

# game/events/kira.rpy:125
translate english kira_bath_kira_mass_bath_first_a6050d8d:

    # Kira_07 "Ну и как, Макс, доволен видом? Удобно устроился?"
    Kira_07 "So, Max, are you happy with the view? Are you comfortable?"

# game/events/kira.rpy:126
translate english kira_bath_kira_mass_bath_first_4c5d4be5:

    # Max_05 "Ага, у меня просто невероятная тётя!"
    Max_05 "Yeah, I have an incredible Aunt!"

# game/events/kira.rpy:127
translate english kira_bath_kira_mass_bath_first_7c395da3:

    # Kira_02 "Да, я такая... Понимаю, ты был бы не прочь так всю ночь просидеть, но не пора ли сделать своей любимой тёте приятно?" nointeract
    Kira_02 "Hahaha yes you do... I know you wouldn't mind sitting there all night, but isn't it time to make your favorite Aunt happy?" nointeract

# game/events/kira.rpy:133
translate english kira_bath_kira_mass_bath_first_d2cc2bca:

    # Max_04 "{m}Я и мечтать о таком не мог! Что буду сидеть с совершенно голой тётей в одной ванне и массировать её ножки... А как изящно она прикрывает ими свою киску!{/m}"
    Max_04 "{m}I never would have thought I'd be sharing a bath with my Aunt like this... She's so classy with the way she's using her legs to conceal her entrance!{/m}"

# game/events/kira.rpy:134
translate english kira_bath_kira_mass_bath_first_9a97257d:

    # Kira_04 "Да, Макс, это очень приятно! Ты молодец... Кто тебя научил этому?"
    Kira_04 "Yes, Max, it's very nice! Well done... Who taught you that?"

# game/events/kira.rpy:135
translate english kira_bath_kira_mass_bath_first_b78e1d45:

    # Max_01 "Курсы на компьютере... Ну и на Алисе с Лизой тренировался."
    Max_01 "Online courses... Well, I also practiced on Alice and Lisa."

# game/events/kira.rpy:136
translate english kira_bath_kira_mass_bath_first_7e4304c9:

    # Kira_05 "Думаю, тебе нужно это развивать... Уверена, твои нежные, но сильные пальчики могут принести невероятное наслаждение!"
    Kira_05 "You should definitely keep at it... I know you're gentle, but strong fingers can also bring incredible pleasure!"

# game/events/kira.rpy:140
translate english kira_bath_kira_mass_bath_first_1095bd15:

    # Max_07 "И как мне это развивать?"
    Max_07 "And how do I develop this?"

# game/events/kira.rpy:141
translate english kira_bath_kira_mass_bath_first_00ca4913:

    # Kira_01 "У меня здесь есть одна бывшая одноклассница и, как я помню, она с мужем как раз занимается массажем и йогой. Я попробую с ней встретиться и договориться, чтобы они тебя поднатаскали в этом... Если, конечно, хочешь?"
    Kira_01 "I have an old friend here. She and her husband are specialists in massage and yoga. I'll try to meet with her and arrange for them to train you... If, of course, you want to?"

# game/events/kira.rpy:142
translate english kira_bath_kira_mass_bath_first_d2d2822b:

    # Max_03 "Конечно, да! Я сейчас понял, насколько это классно, делать массаж голой женщине!"
    Max_03 "Of course, Yes! I just realized how cool it is to massage a naked woman!"

# game/events/kira.rpy:143
translate english kira_bath_kira_mass_bath_first_b23d3900:

    # Kira_09 "Ты только не останавливайся, от твоих прикосновений мне становится так жарко..."
    Kira_09 "Just don't stop, your touch makes me feel so warm..."

# game/events/kira.rpy:146
translate english kira_bath_kira_mass_bath_first_0e8fa115:

    # Max_07 "Ухх, тётя Кира, ты..."
    Max_07 "Uhh, Aunt Kira, you..."

# game/events/kira.rpy:147
translate english kira_bath_kira_mass_bath_first_f06c6c95:

    # Kira_07 "Ой! А во что это там такое твёрдое и большое упёрлась моя нога?"
    Kira_07 "Oops! What's this long and hard object that my foot is resting on?"

# game/events/kira.rpy:148
translate english kira_bath_kira_mass_bath_first_5ac92d8a:

    # Max_02 "А то ты не знаешь?!"
    Max_02 "Can't you tell!?"

# game/events/kira.rpy:149
translate english kira_bath_kira_mass_bath_first_f7c1f2ae:

    # Kira_09 "Ну конечно знаю... Видимо, мне стало уже слишком хорошо, чтобы просто лежать без дела. Так что давай-ка закругляться, Макс. Уже так поздно..." nointeract
    Kira_09 "Ahahaha I'm just teasing. Let's call it a day for now, Max. I didn't realise it was so late..." nointeract

# game/events/kira.rpy:157
translate english kira_bath_kira_mass_bath_first_7302a902:

    # Max_04 "Ладно, тётя Кира. Я тогда пошёл. Спокойной ночи. Было очень классно побыть с тобой!"
    Max_04 "Okay, Aunt Kira. I'll go then. Good night. It was really nice to be with you!"

# game/events/kira.rpy:158
translate english kira_bath_kira_mass_bath_first_832dfdc4:

    # Kira_04 "Приятных снов, Макс. И да, пожалуй, в следующий раз, ты можешь просто заходить без стука. Если ещё не будешь спать..."
    Kira_04 "Sleep well, Max. And yes, I guess next time, you can just come in without knocking. If you're awake..."

# game/events/kira.rpy:159
translate english kira_bath_kira_mass_bath_first_422f0f66:

    # Max_05 "С радостью."
    Max_05 "I'd love to."

# game/events/kira.rpy:188
translate english kira_bath_mass_bath_097bbf0b:

    # Kira_01 "А, Макс, это ты! Всё полуночничаешь... А я вот хотела немного поваляться в ванне, перед тем, как идти спать. Присоединишься?"
    Kira_01 "Oh, Max, it's you! You're up late... I wanted to lie in the tub a little before I went to bed. Will you join?"

# game/events/kira.rpy:189
translate english kira_bath_mass_bath_702751e6:

    # Max_02 "Конечно, тётя Кира! Тебя я и дожидался." nointeract
    Max_02 "Of course, Aunt Kira! I was waiting for you." nointeract

# game/events/kira.rpy:196
translate english kira_bath_mass_bath_bde9bebf:

    # Kira_02 "О да, я вижу, как ты меня ждал! Залезай скорее..." nointeract
    Kira_02 "Oh, yes, I see how you've been waiting for me! Get in quickly..." nointeract

# game/events/kira.rpy:202
translate english kira_bath_mass_bath_a6050d8d:

    # Kira_07 "Ну и как, Макс, доволен видом? Удобно устроился?"
    Kira_07 "So, Max, are you happy with the view? Are you comfortable?"

# game/events/kira.rpy:203
translate english kira_bath_mass_bath_70ceb864:

    # Max_05 "Ага, у меня самая красивая тётя на свете!"
    Max_05 "Yeah, I have the most beautiful Aunt in the world!"

# game/events/kira.rpy:204
translate english kira_bath_mass_bath_7c395da3:

    # Kira_02 "Да, я такая... Понимаю, ты был бы не прочь так всю ночь просидеть, но не пора ли сделать своей любимой тёте приятно?" nointeract
    Kira_02 "Hahaha yes you do... I know you wouldn't mind sitting there all night, but isn't it time to make your favorite Aunt happy?" nointeract

# game/events/kira.rpy:211
translate english kira_bath_mass_bath_3289b3af:

    # Max_04 "{m}А ведь мечты сбываются, если очень захотеть! Сидим с тётей голые в ванне и я массирую её гладкие и стройные ножки. А как изящно она прикрывает ими свою киску!{/m}"
    Max_04 "{m}Dreams come true if you want them to! I get to sit with my Aunt naked in the bath. My sexy Aunt is letting me massage her smooth and slender legs. And how gracefully she covers her pussy with them!{/m}"

# game/events/kira.rpy:212
translate english kira_bath_mass_bath_6ab88328:

    # Kira_04 "Как хорошо... Это то, что мне так сейчас нужно! Хочется, чтобы это продолжалось как можно дольше..."
    Kira_04 "How nice... This is what I need right now! I want it to last as long as possible..."

# game/events/kira.rpy:215
translate english kira_bath_mass_bath_5c616ae9:

    # Max_03 "{m}Она так прекрасна... а мокренькая так вообще огонь! Её шикарная грудь так классно вздымается, когда она начинает глубоко дышать от моего массажа...{/m}"
    Max_03 "{m}She is so beautiful! Her tits are rising up and down as she takes a deep breaths from my massage...{/m}"

# game/events/kira.rpy:218
translate english kira_bath_mass_bath_65c7355c:

    # Kira_07 "Ты только не останавливайся, от твоих прикосновений мне становится так жарко! Напряжения, как не бывало..."
    Kira_07 "You just don't stop, your touch makes me feel so hot, But the tension is gone..."

# game/events/kira.rpy:219
translate english kira_bath_mass_bath_ee1cd498:

    # Max_01 "Это потому что я уже скоро закончу." nointeract
    Max_01 "That's because I'll be done soon." nointeract

# game/events/kira.rpy:225
translate english kira_bath_mass_bath_5a982a00:

    # Kira_08 "Ты молодец, Макс. Мне было очень приятно, а как легко моим ножкам стало! Спасибо тебе и давай-ка закругляться. Уже так поздно..."
    Kira_08 "You're good, Max. I was very pleased, and how easy it was for my legs! Let's wrap this up. It's so late..."

# game/events/kira.rpy:226
translate english kira_bath_mass_bath_7302a902:

    # Max_04 "Ладно, тётя Кира. Я тогда пошёл. Спокойной ночи. Было очень классно побыть с тобой!"
    Max_04 "Okay, Aunt Kira. I'll go then. Good night. It was really nice to be with you!"

# game/events/kira.rpy:227
translate english kira_bath_mass_bath_45071b84:

    # Kira_04 "Приятных снов, Макс."
    Kira_04 "Sleep well, Max."

# game/events/kira.rpy:232
translate english kira_bath_mass_bath_91d4ef49:

    # Kira_07 "Ты только не останавливайся, от твоих прикосновений мне становится так жарко! Напряжения, как не бывало..." nointeract
    Kira_07 "You just don't stop, your touch makes me feel so hot, But the tension is gone..." nointeract

# game/events/kira.rpy:237
translate english kira_bath_mass_bath_fea49aa2:

    # Kira_09 "Макс, уже так поздно... и меня настолько расслабила ванна и твой массаж, что я засыпаю. Так что давай закругляться."
    Kira_09 "Max, it's so late... and I'm so relaxed with the bath and your massage that I'm falling asleep. So let's wrap this up."

# game/events/kira.rpy:240
translate english kira_bath_mass_bath_7302a902_1:

    # Max_04 "Ладно, тётя Кира. Я тогда пошёл. Спокойной ночи. Было очень классно побыть с тобой!"
    Max_04 "Okay, Aunt Kira. I'll go then. Good night. It was really nice to be with you!"

# game/events/kira.rpy:241
translate english kira_bath_mass_bath_45071b84_1:

    # Kira_04 "Приятных снов, Макс."
    Kira_04 "Sleep well, Max."

# game/events/kira.rpy:248
translate english kira_bath_mass_bath_9aa686b4:

    # Kira_05 "О, ты прав, Макс! Думаю, очень непросто высидеть со мной в одной ванне и ограничиться лишь массажем..."
    Kira_05 "Oh, you're right, Max! I think it is very difficult to sit in the same bath with me and limit yourself to just a massage..."

# game/events/kira.rpy:249
translate english kira_bath_mass_bath_d80a6a38:

    # Max_05 "Ухх... от твоих прикосновений мне становится уже немного лучше!"
    Max_05 "Uhh... your touch makes me feel a little better!"

# game/events/kira.rpy:254
translate english kira_bath_mass_bath_66a1d6d3:

    # Kira_02 "Раз уж ты снял усталость с моих ножек, то может они, в качестве благодарности, смогут помочь и тебе..."
    Kira_02 "Since you removed the fatigue from my legs, then maybe as a thank you, I can help you..."

# game/events/kira.rpy:255
translate english kira_bath_mass_bath_1401d31c:

    # Max_03 "{m}Ох... Определённо смогут! Не зря я так хорошо размял её пальчики, они начинают такое вытворять... Ого! Тётя так возбудилась, что раздвинула ножки и начала ласкать свою киску...{/m}"
    Max_03 "{m}Oh... she definitely can! It's not for nothing that I rubbed her toes, now see look what they're doing... Wow! My Aunt is so excited that she's spreading her legs to rub her pussy...{/m}"

# game/events/kira.rpy:256
translate english kira_bath_mass_bath_0bcadd58:

    # Kira_07 "Ну как, Макс? Тебе нравится то, что твоя тётя умеет вытворять ногами?"
    Kira_07 "How's it going, Max? Do you like what your Aunt can do with her feet?"

# game/events/kira.rpy:257
translate english kira_bath_mass_bath_21d93cdd:

    # Max_20 "Да-а-а... Это очень приятно! Только не прекращай, тётя Кира!"
    Max_20 "Yeah... This is very nice! Just don't stop, Aunt Kira!"

# game/events/kira.rpy:265
translate english kira_bath_mass_bath_2f045ba2:

    # Kira_09 "Ох, Макс, как же меня заводит твой огромный член! Ты так жадно смотришь, как я ласкаю себя, свою грудь... Ах, моя киска вся горит, не отводи от неё глаз..." nointeract
    Kira_09 "Your young cock is turning me on! Do you like seeing me play with myself, my big tits? Oh, can you see how wet my pussy is, don't take your eyes off it like a good boy..." nointeract

# game/events/kira.rpy:275
translate english kira_bath_mass_bath_a7080c78:

    # Kira_06 "[norestrain!t]Ухх, Макс! Ты кончил раньше, чем мне хотелось..."
    Kira_06 "[norestrain!t]Uhh, Max! You came sooner than I wanted you to..."

# game/events/kira.rpy:280
translate english kira_bath_mass_bath_b30dfbdb:

    # Kira_10 "[restrain!t]Тебе нравится, как я ввожу пальчики в свою киску? Это так приятно и сладко, что я уже вот-вот кончу! Ох... давай, не сдерживайся..." nointeract
    Kira_10 "[restrain!t]You like the way I put my fingers in my pussy? They're sliding in so easily, I'm going to cum! Oh... Come on, don't hold back..." nointeract

# game/events/kira.rpy:286
translate english kira_bath_mass_bath_7193f533:

    # Kira_08 "Фух, Макс, ну мы и выдали с тобой..."
    Kira_08 "Phew, Max, well, we gave it away with you..."

# game/events/kira.rpy:287
translate english kira_bath_mass_bath_7e4401f1:

    # Max_02 "Да, тётя Кира, это было что-то нереальное!"
    Max_02 "Yes, Aunt Kira, it was something unreal!"

# game/events/kira.rpy:288
translate english kira_bath_mass_bath_94418467:

    # Kira_01 "Давай уже быстренько разбегаться, а то вдруг наши с тобой стоны кто-то услышал."
    Kira_01 "Let's go to bed quickly, in case somoene heard our noise."

# game/events/kira.rpy:289
translate english kira_bath_mass_bath_23817ff1:

    # Max_01 "Ага. Спокойной ночи, тётя Кира. С тобой очень здорово!"
    Max_01 "Good night, Aunt Kira. It's great having you here!"

# game/events/kira.rpy:290
translate english kira_bath_mass_bath_45071b84_2:

    # Kira_04 "Приятных снов, Макс."
    Kira_04 "Sleep well, Max."

# game/events/kira.rpy:300
translate english kira_bath_promise_cuni_a2ff7924:

    # Kira_02 "Раз уж ты снял усталость с моих ножек, то может порадуешь свою тётю, как обещал? Или ты уже забыл!?"
    Kira_02 "Since you've taken the tension off my legs, can you please your Aunt like you promised? Or have you already forgotten!?"

# game/events/kira.rpy:301
translate english kira_bath_promise_cuni_c2754243:

    # Max_03 "Конечно помню! И хочу порадовать тебя ещё..." nointeract
    Max_03 "Of course I remember! I'd love to make you happy again..." nointeract

# game/events/kira.rpy:306
translate english kira_bath_promise_cuni_7f37310e:

    # Kira_05 "Оу... Ты тот ещё шалунишка! У тебя такие нежные пальчики... Да... Поиграй с ней!" nointeract
    Kira_05 "Aww... You're a naughty boy! You have such delicate fingers... Yes... Play with it!" nointeract

# game/events/kira.rpy:310
translate english kira_bath_promise_cuni_2b19c412:

    # Kira_09 "Ухх... Как же ловко твой горячий язычок скользит там внизу... Это такие сладкие ощущения! Ещё быстрее... Ммм..." nointeract
    Kira_09 "Uhh... I love how your your hot tongue slides down there... taste my pussy! That's it ... Mmm..." nointeract

# game/events/kira.rpy:313
translate english kira_bath_promise_cuni_d3732a7c:

    # Kira_09 "Ухх... Да, ещё... Входи своими пальчиками поглубже... Это такие сладкие ощущения! Ещё быстрее... Ммм..." nointeract
    Kira_09 "Uhh... Yes, more... Get your fingers in deeper... That's it! Even faster ... Mmm..." nointeract

# game/events/kira.rpy:325
translate english kira_bath_promise_cuni_a6041d47:

    # Kira_11 "Макс... О да... продолжай вот так, я уже не могу сдерживаться... Сожми мою попку покрепче! Уже так близко... Да! Я кончаю... Ахх..."
    Kira_11 "Max... Yeah... go on like this, I can't hold back anymore... Squeeze my ass! So close ... Yes! I'm coming ... ahh..."

# game/events/kira.rpy:326
translate english kira_bath_promise_cuni_4d360262:

    # Max_03 "Похоже, и здесь я всё помассировал как надо!"
    Max_03 "It looks like I've massaged everything right here, too!"

# game/events/kira.rpy:331
translate english kira_bath_promise_cuni_3a13b724:

    # Kira_06 "Ещё как... Мне очень понравилось... Ты очень способный... племянник..."
    Kira_06 "Absolutely... I really liked it... You're very capable... nephew..."

# game/events/kira.rpy:332
translate english kira_bath_promise_cuni_9823a121:

    # Max_05 "И ты очень классная, тётя Кира! С тобой здорово!"
    Max_05 "And you're beautiful, Aunt Kira! You're great!"

# game/events/kira.rpy:333
translate english kira_bath_promise_cuni_0649d7bf:

    # Kira_01 "Спасибо, Макс, приятно такое слышать... Давай уже быстренько разбегаться, а то вдруг наши с тобой стоны кто-то услышал."
    Kira_01 "Thank you, Max, that's nice to hear... Let's get going, I think I might have been too loud."

# game/events/kira.rpy:334
translate english kira_bath_promise_cuni_08545885:

    # Max_01 "Ага. Спокойной ночи, тётя Кира."
    Max_01 "Good night, Aunt Kira."

# game/events/kira.rpy:335
translate english kira_bath_promise_cuni_45071b84:

    # Kira_04 "Приятных снов, Макс."
    Kira_04 "Sleep well, Max."

# game/events/kira.rpy:349
translate english kira_bath_cuni_bj_b0f7c22d:

    # Kira_05 "Оу... Макс, не выдержал и сам захотел поласкать мою киску? У тебя такие нежные пальчики... Да... Поиграй с ней!" nointeract
    Kira_05 "Aww... Max, couldn't stand it and wanted to touch my pussy? You have such delicate fingers... Yes... make me feel good!" nointeract

# game/events/kira.rpy:355
translate english kira_bath_cuni_bj_c7076e54:

    # Kira_09 "Ухх... Как же ловко твой горячий язычок скользит там внизу... Это такие сладкие ощущения! Ммм... Но я бы не отказалась от кое-чего большого и у тебя это есть... Что скажешь?" nointeract
    Kira_09 "Ugh... I love how your hot tongue slides down there... It's so good! Mmm... But I wouldn't mind something big and you've got it... What do you think?" nointeract

# game/events/kira.rpy:357
translate english kira_bath_cuni_bj_2b19c412:

    # Kira_09 "Ухх... Как же ловко твой горячий язычок скользит там внизу... Это такие сладкие ощущения! Ещё быстрее... Ммм..." nointeract
    Kira_09 "Uhh... your hot tongue feels good sliding all over me! Go faster... Mmm..." nointeract

# game/events/kira.rpy:361
translate english kira_bath_cuni_bj_e7c17474:

    # Kira_09 "Ухх... Да, ещё... Входи своими пальчиками поглубже... Это такие сладкие ощущения! Ммм... Не хочешь проникнуть в свою тётю чем-нибудь ещё?" nointeract
    Kira_09 "Ugh... Yes, also... Come in deeper with your fingers... It's so good! Mmm... Would you like to penetrate your aunt with something else?" nointeract

# game/events/kira.rpy:363
translate english kira_bath_cuni_bj_d3732a7c:

    # Kira_09 "Ухх... Да, ещё... Входи своими пальчиками поглубже... Это такие сладкие ощущения! Ещё быстрее... Ммм..." nointeract
    Kira_09 "Uhh... Yes, more... Get your fingers in deeper... That's it! Even faster ... Mmm..." nointeract

# game/events/kira.rpy:381
translate english kira_bath_cuni_bj_a6041d47:

    # Kira_11 "Макс... О да... продолжай вот так, я уже не могу сдерживаться... Сожми мою попку покрепче! Уже так близко... Да! Я кончаю... Ахх..."
    Kira_11 "Max... Yeah... go on like this, I can't hold back anymore... Squeeze my ass! Already so close... Yes! I'm cumming ... Ahh..."

# game/events/kira.rpy:382
translate english kira_bath_cuni_bj_4d360262:

    # Max_03 "Похоже, и здесь я всё помассировал как надо!"
    Max_03 "It looks like I massaged everything right here, too!"

# game/events/kira.rpy:383
translate english kira_bath_cuni_bj_b2b39a0b:

    # Kira_06 "Да... Ты у меня способный племянник... и я хочу твой член! Хочу почувствовать его у себя во рту..." nointeract
    Kira_06 "Yes... You are my capable nephew... Now I want your cock! I want to taste you..." nointeract

# game/events/kira.rpy:393
translate english kira_bath_cuni_bj_7a7528d8:

    # Kira_02 "О, Макс... Он такой твёрдый и горячий... Я чувствую, как сильно ты хочешь проникнуть им поглубже мне в рот!"
    Kira_02 "Oh, Max... It's so hard and hot... I'm going to drain every drop of cum out of you!"

# game/events/kira.rpy:394
translate english kira_bath_cuni_bj_f7a0a397:

    # Max_04 "Да... Я очень этого хочу!"
    Max_04 "Yes... I really want it!"

# game/events/kira.rpy:402
translate english kira_bath_cuni_bj_5603e12f:

    # Kira_05 "Сначала, я хочу приласкать его своим язычком... Вот так... Я знаю, что тебе это нравится..." nointeract
    Kira_05 "First, I'll let my tongue glide over him... Like this... You have such a perfect cock..." nointeract

# game/events/kira.rpy:420
translate english kira_bath_cuni_bj_bb70212d:

    # Max_21 "{m}Ухх... Как нежно она посасывает своими губами мой член. И язычком орудует с такой страстью... Просто бесподобно!{/m}" nointeract
    Max_21 "{m}uhh... She's teasing my cock so skillfully, it's sending shivers up my spine!{/m}" nointeract

# game/events/kira.rpy:431
translate english kira_bath_cuni_bj_f78fddf3:

    # Max_22 "{m}Вот чёрт! Она так глубоко насаживает на него свой ротик... Ещё чуть-чуть и я взорвусь! А она сосёт всё быстрее...{/m}" nointeract
    Max_22 "{m}Fuck! She's plunged my cock into her mouth... Just a little more and I'll explode! She such a good slut...{/m}" nointeract

# game/events/kira.rpy:434
translate english kira_bath_cuni_bj_0da4483d:

    # Max_06 "{m}Да... Она принимает всю мою сперму не вынимая член изо рта! О боже... Кажется, я её люблю! Хотя, что я болтаю... Но как же это приятно!{/m}"
    Max_06 "{m}Yes... She's taking all my cum while leaving my cock in her mouth! I think I'm in love - I mean, she's so talented!{/m}"

# game/events/kira.rpy:437
translate english kira_bath_cuni_bj_bab49e4f:

    # Kira_08 "Вот и твоё напряжение мы сняли! Какие мы сегодня молодцы... Мне очень это всё понравилось!"
    Kira_08 "Here's all the tension we removed! What good people we are today... I really liked it all!"

# game/events/kira.rpy:438
translate english kira_bath_cuni_bj_6f308174:

    # Max_05 "И мне тоже! Было очень классно!"
    Max_05 "Me too! It was very nice!"

# game/events/kira.rpy:439
translate english kira_bath_cuni_bj_94418467:

    # Kira_01 "Давай уже быстренько разбегаться, а то вдруг наши с тобой стоны кто-то услышал."
    Kira_01 "Let's leave quietly, just in case anyone heard us cum."

# game/events/kira.rpy:440
translate english kira_bath_cuni_bj_23817ff1:

    # Max_01 "Ага. Спокойной ночи, тётя Кира. С тобой очень здорово!"
    Max_01 "Good night, Aunt Kira. It's great to be with you!"

# game/events/kira.rpy:441
translate english kira_bath_cuni_bj_45071b84:

    # Kira_04 "Приятных снов, Макс."
    Kira_04 "Sleep well, Max."

# game/events/kira.rpy:451
translate english kira_bath_not_restrain_26d27f45:

    # Kira_08 "[norestrain!t]Ого! Кто-то у нас тут слишком перевозбудился... А я ведь только начала..."
    Kira_08 "[norestrain!t]Wow! Someone's getting overexcited here... I'm just getting started..."

# game/events/kira.rpy:452
translate english kira_bath_not_restrain_ff569f5a:

    # Max_20 "Фух... Я уже просто не мог... Принимать с тобой ванну - очень горячо, тётя Кира."
    Max_20 "Phew... I just couldn't... Take a bath with you - too hot, Aunt Kira."

# game/events/kira.rpy:455
translate english kira_bath_not_restrain_9fe7d7f1:

    # Kira_05 "Да, Макс, такая вот у тебя тётя! Но мне всё равно очень понравилось!"
    Kira_05 "Yes, Max, that's your Aunt! But I still really liked it!"

# game/events/kira.rpy:456
translate english kira_bath_not_restrain_6f308174:

    # Max_05 "И мне тоже! Было очень классно!"
    Max_05 "Me too! It was very cool!"

# game/events/kira.rpy:457
translate english kira_bath_not_restrain_94418467:

    # Kira_01 "Давай уже быстренько разбегаться, а то вдруг наши с тобой стоны кто-то услышал."
    Kira_01 "Let's go to bed quickly, in case somoene heard our noise."

# game/events/kira.rpy:458
translate english kira_bath_not_restrain_23817ff1:

    # Max_01 "Ага. Спокойной ночи, тётя Кира. С тобой очень здорово!"
    Max_01 "Good night, Aunt Kira. It's great having you here!"

# game/events/kira.rpy:459
translate english kira_bath_not_restrain_45071b84:

    # Kira_04 "Приятных снов, Макс."
    Kira_04 "Sleep well, Max."

# game/events/kira.rpy:468
translate english kira_bath_caught_3ca0ecaf:

    # Eric_12 "Ох нифига себе, Макс!!! Это я удачно зашёл..."
    Eric_12 "Oh, damn! What have I just walked in on?"

# game/events/kira.rpy:469
translate english kira_bath_caught_0569054a:

    # Max_13 "Это не то, чем кажется! Мы просто..."
    Max_13 "It's not what it looks like! We just..."

# game/events/kira.rpy:470
translate english kira_bath_caught_5e78d419:

    # Eric_06 "Неужели вы тут на самом деле на лыжах катаетесь?! А так сразу и не скажешь!"
    Eric_06 "Is this how you treat your guests!? There's no way you can explain this!"

# game/events/kira.rpy:471
translate english kira_bath_caught_a3113548:

    # Max_14 "Нет, просто так получилось..."
    Max_14 "No, it just happened..."

# game/events/kira.rpy:477
translate english kira_bath_caught_d574cb19:

    # Eric_04 "Да не переживай, Макс. Вам повезло, что это я, а не кто-нибудь из твоих сестёр или твоя мама. Вы бы могли спокойно вылететь из этого дома со скандалом!"
    Eric_04 "Don't worry about it, Max. You're lucky it was me who found you and not one of your sisters, or worse, your Mom. That would've caused a real scandal!"

# game/events/kira.rpy:478
translate english kira_bath_caught_7b917c05:

    # Kira_15 "Мне думается, что нам не так уж и повезло, видя то, как ты злорадствуешь!"
    Kira_15 "The way you said that doesn't make me feel so lucky!"

# game/events/kira.rpy:479
translate english kira_bath_caught_7b416a24:

    # Eric_05 "Это как посмотреть. Думаю, всем здесь понятно, что держать этот секрет в тайне от всех остальных я за просто так не стану..."
    Eric_05 "You should be. I'm the only one who would be willing to keep this a secret. So long as it's worth it for me."

# game/events/kira.rpy:480
translate english kira_bath_caught_11636359:

    # Kira_13 "И что ты хочешь?"
    Kira_13 "So what do you want?"

# game/events/kira.rpy:485
translate english kira_bath_caught_44eb4c63:

    # Eric_03 "Сомневаюсь, что это Макс тебя совратил, а значит отдуваться тебе, Кира! Мне должно быть {b}приятно{/b} хранить этот секрет! Если понимаешь, о чём я..."
    Eric_03 "I highly doubt this was all Max's idea, so I'm going to leave it to you, Kira! I won't say a thing about what you've done if you give me the right motivation..."

# game/events/kira.rpy:486
translate english kira_bath_caught_b905692f:

    # Kira_16 "По-видимому, секс за молчание?!"
    Kira_16 "Cut the crap, you want sex in exchange for silence, right?!"

# game/events/kira.rpy:487
translate english kira_bath_caught_4bc31a57:

    # Eric_08 "Приятно вести дела со взрослыми людьми. Правда, если я оттрахаю тебя, то криков ты точно не сдержишь, так что пока будет достаточно просто обалденного минета от тебя, Кира. Как ты на это смотришь?"
    Eric_08 "Glad to see you've still got the mind of an adult. But if I fuck you right now, there's no way you'll be quiet enough to not bother the others, so lets just have you suck my cock. How do you feel about that?"

# game/events/kira.rpy:488
translate english kira_bath_caught_6589a7bc:

    # Kira_14 "Мы тут слишком застигнуты врасплох. Нужно время, чтобы подумать."
    Kira_14 "I can't think straight right now. We need time to comprehend this."

# game/events/kira.rpy:489
translate english kira_bath_caught_3d38ed27:

    # Eric_02 "Только не тяните с этим, а то я заскучаю... Ещё увидимся, если вы примете верное решение."
    Eric_02 "I understand. Don't take too long... I'll see you later, if you make the right choice."

# game/events/kira.rpy:496
translate english kira_bath_caught_a45c03b9:

    # Max_08 "Ты правда собираешься по ночам отсасывать Эрику?!"
    Max_08 "Are you seriously going to blow him?!"

# game/events/kira.rpy:497
translate english kira_bath_caught_2e0c93cb:

    # Kira_13 "Похоже, придётся... Иначе, ты отправишься прямиком в летний лагерь, а я... Меня, скорее всего, тоже выгонят и боюсь отношения с твоей мамой у меня будут испорчены навсегда... А я этого очень не хочу..."
    Kira_13 "I don't think I have a choice... If I don't, it means military camp for you, I'll get kicked out, and my relationship with Ann will be ruined forever. And I couldn't stand that..."

# game/events/kira.rpy:498
translate english kira_bath_caught_c65783f7:

    # Max_09 "Может быть, у меня получится как-то \"смягчить\" условия Эрика..."
    Max_09 "Maybe I can convince Eric to make a deal with softer conditions..."

# game/events/kira.rpy:499
translate english kira_bath_caught_7ede8304:

    # Kira_14 "Да они и так довольно мягкие, Макс. Может быть даже так, что от твоих попыток смягчить эти условия, ты сделаешь только обратное. Я буду делать то, что могу... и умею... Хотя бы ради тебя, потому что и правда, это ведь я тебя соблазнила. За всё приходится платить..."
    Kira_14 "They're pretty lax already. But don't talk to Eric or you might risk making it worse. Just let me handle it, for your sake. This was all my fault to begin with... So I'll pay for it."

# game/events/kira.rpy:500
translate english kira_bath_caught_4bd20739:

    # Max_07 "Я всё равно постараюсь тебе как-то помочь!"
    Max_07 "I'll find a way to help you!"

# game/events/kira.rpy:501
translate english kira_bath_caught_65eedbf8:

    # Kira_10 "Только осторожно, Макс! Потому что пока, всё не так уж и плохо..."
    Kira_10 "Ok, but you have to be careful! He's got all the power here, and so far, his demands aren't that bad."

# game/events/kira.rpy:505
translate english kira_bath_caught_90341b4e:

    # Max_08 "Тётя Кира, ты же не собираешься по ночам отсасывать Эрику, правда?!"
    Max_08 "You're not actually going to blow him, are you?!"

# game/events/kira.rpy:506
translate english kira_bath_caught_2e0c93cb_1:

    # Kira_13 "Похоже, придётся... Иначе, ты отправишься прямиком в летний лагерь, а я... Меня, скорее всего, тоже выгонят и боюсь отношения с твоей мамой у меня будут испорчены навсегда... А я этого очень не хочу..."
    Kira_13 "I don't think I have a choice... If I don't, it means military camp for you, I'll get kicked out, and my relationship with Ann will be ruined forever. And I couldn't stand that..."

# game/events/kira.rpy:507
translate english kira_bath_caught_fd33e06d:

    # Max_09 "Надо что-нибудь придумать, потому что для меня это не вариант!"
    Max_09 "I have to think of something, because that's not an option for me!"

# game/events/kira.rpy:508
translate english kira_bath_caught_4a60b600:

    # Kira_14 "Ну, я буду делать то, что могу... и умею... Хотя бы ради тебя, потому что и правда, это ведь я тебя соблазнила. За всё приходится платить..."
    Kira_14 "Just let me handle it, for your sake. This was all my fault to begin with... So I'll pay for it."

# game/events/kira.rpy:509
translate english kira_bath_caught_65944a3a:

    # Max_11 "Мне это всё не нравится..."
    Max_11 "I don't like any of this..."

# game/events/kira.rpy:510
translate english kira_bath_caught_b1929586:

    # Kira_13 "Тогда, только ты сможешь с ним договориться о чём-то другом. Ну а мне придётся согласиться на это... Так что, вся надежда на тебя, Макс..."
    Kira_13 "I'm not in a position to do anything, but... Maybe you can negotiate some kind of agreement with him? I'll just do what I can for now..."

# game/events/kira.rpy:512
translate english kira_bath_caught_c0bc19eb:

    # Max_10 "Ладно, я понял..." nointeract
    Max_10 "I'll see what I can do..." nointeract

# game/events/kira.rpy:527
translate english kira_bath_horsewoman_2e8e9ba1:

    # Kira_09 "Ах, Макс... Д-а-а... Вот так... Хватит уже дразнить мою киску... Дай мне уже сесть на него! Какой он твёрдый... Ммм..." nointeract
    Kira_09 "Ah, Max... Y-a-a... That's it... Stop teasing my pussy. Let me sit on it. It's so hard... Mmm..." nointeract

# game/events/kira.rpy:542
translate english kira_bath_horsewoman_06f39603:

    # Kira_11 "[restrain!t]Охх... Да... Обожаю, когда можно попрыгать на чём-то весьма ощутимом! Д-а-а... Только не кончи раньше меня... Ммм... Я уже скоро! Как хорошо..."
    Kira_11 "[restrain!t]Ohhh... Yes... I love it when you can jump on something that fills me right up! Yeah. Just don't come before me. Mmm... I'll be right there. That feels good."

# game/events/kira.rpy:543
translate english kira_bath_horsewoman_7d6ba17c:

    # Max_20 "Ты обалденная, тётя Кира! Давай ещё быстрее..."
    Max_20 "You're amazing, Aunt Kira! Come on, hurry up..."

# game/events/kira.rpy:544
translate english kira_bath_horsewoman_bd9f2b57:

    # Kira_12 "О да! Я больше не могу, Макс... Ммм... Ох, как хорошо! Да, я кончаю... Ахх..." nointeract
    Kira_12 "Oh yes! I can't take it anymore, Max... Mmm... Oh, that feels good! Yes, I'm coming... Ahhh..." nointeract

# game/events/kira.rpy:551
translate english kira_bath_horsewoman_88fc2651:

    # Kira_07 "Вот так... Нравится кончать в свою тётю, а Макс? Мне вот понравилось!"
    Kira_07 "There you go... You like cumming in your aunt, huh, Max? I love it!"

# game/events/kira.rpy:552
translate english kira_bath_horsewoman_6f308174:

    # Max_05 "И мне тоже! Было очень классно!"
    Max_05 "Me too! It was very cool!"

# game/events/kira.rpy:559
translate english kira_bath_horsewoman_7193f533:

    # Kira_08 "Фух, Макс, ну мы и выдали с тобой..."
    Kira_08 "Phew, Max, you and I gave it away..."

# game/events/kira.rpy:560
translate english kira_bath_horsewoman_7e4401f1:

    # Max_02 "Да, тётя Кира, это было что-то нереальное!"
    Max_02 "Yes, Aunt Kira, it was something unreal!"

# game/events/kira.rpy:572
translate english kira_bath_horsewoman_26d27f45:

    # Kira_08 "[norestrain!t]Ого! Кто-то у нас тут слишком перевозбудился... А я ведь только начала..."
    Kira_08 "[norestrain!t]Wow! Someone's getting overexcited here... I'm just getting started..."

# game/events/kira.rpy:573
translate english kira_bath_horsewoman_ff569f5a:

    # Max_20 "Фух... Я уже просто не мог... Принимать с тобой ванну - очень горячо, тётя Кира."
    Max_20 "Phew... I just couldn't... Taking a bath with you is so much fun, Aunt Kira."

# game/events/kira.rpy:575
translate english kira_bath_horsewoman_b732174a:

    # Kira_05 "Да, Макс, такая вот у тебя тётя! Давай уже быстренько разбегаться, а то вдруг наши с тобой стоны кто-то услышал."
    Kira_05 "Yes, Max, that's your aunt! Let's run away quickly, before someone gets suspicious."

# game/events/kira.rpy:576
translate english kira_bath_horsewoman_4144ec85:

    # Max_01 "Ага. Спокойной ночи, тётя Кира. С тобой очень здорово!" nointeract
    Max_01 "Yeah. Good night, Aunt Kira. It's so great to be with you!" nointeract

# game/events/kira.rpy:589
translate english kira_bath_dogstyle_c700c47b:

    # Kira_09 "Вот, держи... Д-а-а... Вот так... Вводи его не спеша... Ох, Макс... Он с таким трудом входит в меня, даже когда я такая мокренькая... Ммм..." nointeract
    Kira_09 "Here you go... Y-a-a... That's it... Put it in slowly... Oh, Max... It's so hard to get it inside me, even when I'm so wet. Mmm..." nointeract

# game/events/kira.rpy:602
translate english kira_bath_dogstyle_4f355bc8:

    # Kira_11 "[restrain!t]Охх... Да... Вот так, Макс! Трахай меня... Ещё... Ещё сильнее! Как же чертовски приятно чувствовать твой член! Д-а-а... Ещё... Я уже близко!"
    Kira_11 "[restrain!t]Ohhh... Yes... That's it, Max! Fuck me... More... Harder! It feels so fucking good to feel your cock! Yeah... More... I'm close!"

# game/events/kira.rpy:605
translate english kira_bath_dogstyle_847cda3f:

    # Max_20 "Нравится, как я тебя трахаю, тётя Кира? Получай ещё..."
    Max_20 "You like the way I fuck you, Aunt Kira? Have some more..."

# game/events/kira.rpy:606
translate english kira_bath_dogstyle_e8270cba:

    # Kira_12 "О да! Трахай меня ещё быстрее! Я еле сдерживаюсь... Ох, как хорошо! Ещё! Да, вот так... Ммм... Да, я кончаю... Ахх..." nointeract
    Kira_12 "Oh yes! Fuck me even faster! I can hardly contain myself... Oh, that feels good! More! Yeah, like that... Mmm... Yeah, I'm coming... Ahhh..." nointeract

# game/events/kira.rpy:613
translate english kira_bath_dogstyle_88fc2651:

    # Kira_07 "Вот так... Нравится кончать в свою тётю, а Макс? Мне вот понравилось!"
    Kira_07 "There you go... You like cumming in your aunt, huh, Max? I love it!"

# game/events/kira.rpy:614
translate english kira_bath_dogstyle_6f308174:

    # Max_05 "И мне тоже! Было очень классно!"
    Max_05 "Me too! It was amazing!"

# game/events/kira.rpy:625
translate english kira_bath_dogstyle_9183245f:

    # Kira_05 "Ого, всю попку со спиной мне забрызгал, Макс! Приятно развлеклись перед сном, да?"
    Kira_05 "Wow, you sprayed all over my back, Max! You had a good time before bed, didn't you?"

# game/events/kira.rpy:626
translate english kira_bath_dogstyle_7e4401f1:

    # Max_02 "Да, тётя Кира, это было что-то нереальное!"
    Max_02 "Yes, Aunt Kira, it was something unreal!"

# game/events/kira.rpy:637
translate english kira_bath_dogstyle_fd38c112:

    # Kira_08 "[norestrain!t]Ого! Ты уже всё! Кто-то у нас тут слишком перевозбудился... Тебе ещё привыкать и привыкать к нашим шалостям..."
    Kira_08 "[norestrain!t]Wow! You're done! Somebody's got a little too much excitement here... You're still getting used to it..."

# game/events/kira.rpy:638
translate english kira_bath_dogstyle_ff569f5a:

    # Max_20 "Фух... Я уже просто не мог... Принимать с тобой ванну - очень горячо, тётя Кира."
    Max_20 "Phew... I just couldn't... Taking a bath with you is very hot, Aunt Kira."

# game/events/kira.rpy:640
translate english kira_bath_dogstyle_b732174a:

    # Kira_05 "Да, Макс, такая вот у тебя тётя! Давай уже быстренько разбегаться, а то вдруг наши с тобой стоны кто-то услышал."
    Kira_05 "Yes, Max, that's your aunt! Let's run away quickly, before someone gets suspicious."

# game/events/kira.rpy:641
translate english kira_bath_dogstyle_4144ec85:

    # Max_01 "Ага. Спокойной ночи, тётя Кира. С тобой очень здорово!" nointeract
    Max_01 "Good night, Aunt Kira. It's great having you here!" nointeract

# game/events/kira.rpy:671
translate english kira_bath_cum_in_her_mouth_8f9ac8f3:

    # Max_21 "[restrain!t]Ухх... Какие же горячие и ненасытные у тебя губки... Ты просто бесподобно сосёшь, тётя Кира! Да..." nointeract
    Max_21 "[restrain!t]Ugh... What hot and insatiable lips you have... You're an unbelievable dick-sucker, Aunt Kira. Yes..." nointeract

# game/events/kira.rpy:678
translate english kira_bath_cum_in_her_mouth_0da4483d:

    # Max_06 "{m}Да... Она принимает всю мою сперму не вынимая член изо рта! О боже... Кажется, я её люблю! Хотя, что я болтаю... Но как же это приятно!{/m}"
    Max_06 "{m}Yes... She's taking all my cum while leaving my cock in her mouth! I think I'm in love - I mean, she's so talented!{/m}"

# game/events/kira.rpy:682
translate english kira_bath_cum_in_her_mouth_bab49e4f:

    # Kira_08 "Вот и твоё напряжение мы сняли! Какие мы сегодня молодцы... Мне очень это всё понравилось!"
    Kira_08 "Here's all the tension we removed! What good people we are today... I really liked it all!"

# game/events/kira.rpy:683
translate english kira_bath_cum_in_her_mouth_6f308174:

    # Max_05 "И мне тоже! Было очень классно!"
    Max_05 "Me too! It was very nice!"

# game/events/kira.rpy:693
translate english kira_bath_cum_in_her_mouth_ec302532:

    # Kira_08 "[norestrain!t]Ого! Кто-то у нас тут слишком перевозбудился... Я даже не успела ничего сделать..."
    Kira_08 "[norestrain!t]Whoa! Somebody got a little overexcited here... I didn't even have time to do anything..."

# game/events/kira.rpy:694
translate english kira_bath_cum_in_her_mouth_ff569f5a:

    # Max_20 "Фух... Я уже просто не мог... Принимать с тобой ванну - очень горячо, тётя Кира."
    Max_20 "Phew... I just couldn't... Taking a bath with you is very hot, Aunt Kira."

# game/events/kira.rpy:698
translate english kira_bath_cum_in_her_mouth_9fe7d7f1:

    # Kira_05 "Да, Макс, такая вот у тебя тётя! Но мне всё равно очень понравилось!"
    Kira_05 "Yes, Max, that's your Aunt! But I still really liked it!"

# game/events/kira.rpy:699
translate english kira_bath_cum_in_her_mouth_6f308174_1:

    # Max_05 "И мне тоже! Было очень классно!"
    Max_05 "Me too! It was very nice!"

# game/events/kira.rpy:704
translate english kira_bath_end_sex_94418467:

    # Kira_01 "Давай уже быстренько разбегаться, а то вдруг наши с тобой стоны кто-то услышал."
    Kira_01 "Let's go to bed quickly, in case somoene heard our noise."

# game/events/kira.rpy:705
translate english kira_bath_end_sex_4144ec85:

    # Max_01 "Ага. Спокойной ночи, тётя Кира. С тобой очень здорово!" nointeract
    Max_01 "Good night, Aunt Kira. It's great having you here!" nointeract

# game/events/kira.rpy:728
translate english kira_night_swim_57679d55:

    # Max_03 "Ого... Купаешься голой, тётя Кира?! Классно смотришься!"
    Max_03 "Wow... Aunt Kira?! You look great!"

# game/events/kira.rpy:729
translate english kira_night_swim_3888423d:

    # Kira_01 "А, Макс... Я думала, что все уже спят. Хотела немного поплавать... А ты чего не спишь?"
    Kira_01 "Ah, Max... I thought everyone was asleep. I wanted to go for a swim... Why aren't you in bed?"

# game/events/kira.rpy:730
translate english kira_night_swim_6266263a:

    # Max_01 "Да, что-то не спится... К тебе можно?"
    Max_01 "Well, I can't sleep. Mind if I join?"

# game/events/kira.rpy:731
translate english kira_night_swim_19d1df0d:

    # Kira_02 "Ну, бассейн большой. К тому же, это ваш дом. Кому как не тебе решать? Но, давай не сегодня. Я очень устала и собиралась уже идти спать..."
    Kira_02 "Well, it's a big pool. Besides, it's your home. But, let's not do it today. I'm very tired and I was going to go to bed already..."

# game/events/kira.rpy:732
translate english kira_night_swim_f52acee3:

    # Max_04 "Тогда и я пойду спать... Спокойной ночи, тётя Кира."
    Max_04 "Then I'll go to bed, too... Good night, Aunt Kira."

# game/events/kira.rpy:733
translate english kira_night_swim_0341a199:

    # Kira_05 "Ага. Приятных снов, Макс."
    Kira_05 "Yeah. Sleep tight, Max."

# game/events/kira.rpy:737
translate english kira_night_swim_57679d55_1:

    # Max_03 "Ого... Купаешься голой, тётя Кира?! Классно смотришься!"
    Max_03 "Wow... Skinny dipping, Aunt Kira?! You look great!"

# game/events/kira.rpy:738
translate english kira_night_swim_41436e7f:

    # Kira_01 "А, Макс... Я думала, что все уже спят. Хотела немного поплавать... А ты чего не спишь?" nointeract
    Kira_01 "Ah, Max... I thought everyone was asleep. I wanted to go for a swim... Why aren't you in bed?" nointeract

# game/events/kira.rpy:747
translate english kira_night_swim_d6e151bf:

    # Kira_02 "Ну, бассейн большой. К тому же, это ваш дом. Кому как не тебе решать, с таким-то большим членом?"
    Kira_02 "Well it's a big pool, and it is your home. Who better to make that choice than the one with the biggest dick?"

# game/events/kira.rpy:748
translate english kira_night_swim_3989efe8:

    # Max_03 "А, ну да. Это просто ты голая, вот у меня и стоит."
    Max_03 "Oh, right. It's just that you're naked."

# game/events/kira.rpy:749
translate english kira_night_swim_5418a907:

    # Kira_05 "Абсолютно голая! Ладно, спускайся, а то я не могу спокойно на это смотреть..." nointeract
    Kira_05 "Completely naked! All right, come on down, I can't stand to watch this..." nointeract

# game/events/kira.rpy:754
translate english kira_night_swim_809f12e2:

    # Kira_04 "Да, теперь мне очень хорошо видно, насколько тебе нравится то, что скрывается под этой прозрачной водой..."
    Kira_04 "Now I can see how much you like what's in the water..."

# game/events/kira.rpy:755
translate english kira_night_swim_5fa643d7:

    # Max_04 "Да и над водой тут нисколько картина не хуже!"
    Max_04 "Well being in it with you doesn't compare!"

# game/events/kira.rpy:759
translate english kira_night_swim_afe0a4ee:

    # Kira_07 "А тебя, похоже, заводит, что кто-то может увидеть, как твоя обнажённая тётя крепко сжимает и дрочит твой огромный член, да Макс?"
    Kira_07 "Looks like you're getting turned on... Is it by the risk that someone might see your Aunt tightly jerking off your huge cock, Max?"

# game/events/kira.rpy:760
translate english kira_night_swim_d50e2e42:

    # Max_02 "О да!"
    Max_02 "Oh Yes!"

# game/events/kira.rpy:764
translate english kira_night_swim_f425302b:

    # Kira_06 "Значит, любишь, когда за тобой наблюдают?"
    Kira_06 "So the chance of being watched gets you hard?"

# game/events/kira.rpy:765
translate english kira_night_swim_a3a87085:

    # Max_07 "Ну, смотря кто..."
    Max_07 "Well, it depends on who's watching..."

# game/events/kira.rpy:769
translate english kira_night_swim_6b17ead2:

    # Kira_04 "Представь, что за тем, что я вытворяю своим языком с твоим членом, кто-то подглядывает... Может это Алиса... или Лиза... а может, даже твоя мама!"
    Kira_04 "Imagine someone spying on us, watching what I'm doing with my tongue to your cock... Maybe it's Alice... Or Lisa... Maybe even your Mom!"

# game/events/kira.rpy:770
translate english kira_night_swim_bfbddab0:

    # Max_20 "Ох, от таких мыслей я сейчас кончу, тётя Кира!"
    Max_20 "Oh, those thoughts will make me come, Aunt Kira!"

# game/events/kira.rpy:771
translate english kira_night_swim_1f0f4d2e:

    # Kira_02 "Какой же ты испорченный мальчик! Но не переживай, бассейн мы не испачкаем. Я чувствую, ты уже совсем близко... Давай..." nointeract
    Kira_02 "What a spoiled boy you are! But don't worry, we won't get the pool dirty. I can feel you getting very close..." nointeract

# game/events/kira.rpy:777
translate english kira_night_swim_296f109d:

    # Max_05 "Ух, тётя Кира, как хорошо... Тебя я всё же немного испачкал."
    Max_05 "Wow, Aunt Kira, amazing... But I did get you a little messy."

# game/events/kira.rpy:778
translate english kira_night_swim_f8441ad7:

    # Kira_05 "Это разве испачкал, Макс? Конечно нет... Главное, что в воду ничего не попало."
    Kira_05 "Well, the most important thing is none got in the water. That would be hard to explain."

# game/events/kira.rpy:779
translate english kira_night_swim_109b02fc:

    # Max_03 "Ага, как будто ничего и не было."
    Max_03 "Yeah, lucky you left no trace."

# game/events/kira.rpy:780
translate english kira_night_swim_42636203:

    # Kira_03 "Ладно, Макс, беги спать. Уже поздно. А то если нас заметят тут, будет много вопросов..."
    Kira_03 "All right, Max, better get to bed before we get noticed. There's going to be far too many questions if we're caught..."

# game/events/kira.rpy:785
translate english kira_night_swim_2d756490:

    # Max_04 "Только после тебя..."
    Max_04 "Only after you..."

# game/events/kira.rpy:786
translate english kira_night_swim_a9d98df3:

    # Kira_05 "О, спасибо, Макс! Приятно, что ты решил немного поухаживать за мной. Спокойной ночи! И шорты не забудь..."
    Kira_05 "Oh, thank you, Max! It's nice that you decided to be a gentlemen. Goodnight! And don't forget your shorts..."

# game/events/kira.rpy:787
translate english kira_night_swim_171022d2:

    # Max_01 "Не забуду... Спокойной ночи!"
    Max_01 "I'll try not to... Goodnight!"

# game/events/kira.rpy:799
translate english kira_night_swim_06dbdfc5:

    # Kira_06 "Ухх, впечатлил! Спускайся, тебе нужно немного охладиться. И как знать, может даже ты встретишь приключение на свой член!" nointeract
    Kira_06 "Well, impressive! Come on down, you need to cool off a bit. And who knows, you're dick might even go on an adventure!" nointeract

# game/events/kira.rpy:804
translate english kira_night_swim_62d66d9a:

    # Max_04 "Приключения - это я люблю..."
    Max_04 "I do love adventures..."

# game/events/kira.rpy:805
translate english kira_night_swim_2a55a9db:

    # Kira_05 "А я, как раз кстати, могу тебе их обеспечить..."
    Kira_05 "And I can provide them for you, just in time..."

# game/events/kira.rpy:814
translate english kira_night_swim_e6960ed7:

    # Kira_04 "Я уже собиралась отправляться спать, устала. Но так уж и быть, немного помогу тебе, а заканчивать будешь сам..."
    Kira_04 "I was about to go to bed, but so be it. I'll help you a little, but you just have to finish yourself..."

# game/events/kira.rpy:815
translate english kira_night_swim_17b0495e:

    # Max_07 "Ну, так не интересно..."
    Max_07 "Well, that's no fun."

# game/events/kira.rpy:819
translate english kira_night_swim_c4001dfb:

    # Kira_02 "Не интересно? Ну да, согласна... Тогда давай сделаем так, утром я буду принимать душ одна, но не откажусь от твоей компании!"
    Kira_02 "Not fun? Well, yes, I agree... So let's do this, I'll be in the shower alone in the morning. Maybe you should come keep me company?"

# game/events/kira.rpy:820
translate english kira_night_swim_2c22d5ff:

    # Max_03 "Так это я с радостью, тётя Кира!"
    Max_03 "So it's my pleasure, Aunt Kira!"

# game/events/kira.rpy:821
translate english kira_night_swim_25c9e40d:

    # Kira_07 "И если в душе ты меня порадуешь, то я порадую тебя сейчас, договорились?"
    Kira_07 "And if you promise to return the favour, I'll make you happy now, okay?"

# game/events/kira.rpy:822
translate english kira_night_swim_14117b6f:

    # Max_05 "Конечно!"
    Max_05 "Of course!"

# game/events/kira.rpy:826
translate english kira_night_swim_929307c1:

    # Kira_04 "Тогда, я пожалуй поиграю с тобой своим язычком. Тебе же нравится, когда я ласкаю им твой член?"
    Kira_04 "Then I think I'll use my tongue. You love it when it meets your cock, don't you?"

# game/events/kira.rpy:827
translate english kira_night_swim_a2bb097a:

    # Max_20 "О да, ты делаешь это великолепно! Продолжай тётя Кира..."
    Max_20 "Oh, Yes, you're doing great! Go on, Aunt Kira..."

# game/events/kira.rpy:838
translate english kira_night_swim_5a6dc4f5:

    # Kira_02 "Какой же ты испорченный мальчик! Всё время называешь меня тётей... Это так пикантно! Давай, кончи мне в рот, я этого очень хочу..." nointeract
    Kira_02 "What a spoiled boy you are! You keep calling me Aunt... It's so deviant! Come on, cum in my mouth, I want to taste you..." nointeract

# game/events/kira.rpy:844
translate english kira_night_swim_296f109d_1:

    # Max_05 "Ух, тётя Кира, как хорошо... Тебя я всё же немного испачкал."
    Max_05 "Wow, Aunt Kira, that was great... I did get you a little messy, though."

# game/events/kira.rpy:845
translate english kira_night_swim_f8441ad7_1:

    # Kira_05 "Это разве испачкал, Макс? Конечно нет... Главное, что в воду ничего не попало."
    Kira_05 "Well, the most important thing is none got in the water. That would be hard to explain."

# game/events/kira.rpy:846
translate english kira_night_swim_109b02fc_1:

    # Max_03 "Ага, как будто ничего и не было."
    Max_03 "Yeah, lucky you left no trace."

# game/events/kira.rpy:853
translate english kira_night_swim_1d59945b:

    # Kira_07 "[restrain!t]Похоже, против таких приёмов, ты просто так сдаваться не собираешься... Как думаешь, долго ли ты продержишься, если твой дружок окажется у меня во рту?"
    Kira_07 "[restrain!t]It looks like you're not going to give up just like that... How long do you think you'd last if I had your friend in my mouth?"

# game/events/kira.rpy:854
translate english kira_night_swim_0c3aba50:

    # Max_02 "Давай проверим..."
    Max_02 "Let's see..."

# game/events/kira.rpy:857
translate english kira_night_swim_fd5223cc:

    # Max_20 "{m}Д-а-а... Она сосёт просто потрясающе! Так смачно и жадно... Как же приятно, когда мой член проскальзывает сквозь её нежные губки прямо в рот!{/m}"
    Max_20 "{m}Yaah... She sucks so good. It's so hot and so wet... My cock feels so good gliding in and out of her mouth!{/m}"

# game/events/kira.rpy:858
translate english kira_night_swim_9c2b6ab8:

    # Max_19 "Охх... тётя Кира... Обожаю, когда ты так делаешь..." nointeract
    Max_19 "Ohhh... Aunt Kira... I love it when you do that..." nointeract

# game/events/kira.rpy:867
translate english kira_night_swim_793ef665:

    # Max_21 "[restrain!t]{m}Вот чёрт! Я двигаюсь ей навстречу, а она всё глубже берёт его в рот! И всё больше и больше ускоряется... Такое я долго не выдержу! Я уже на грани...{/m}"
    Max_21 "[restrain!t]{m}Oh, shit! I'm moving towards her, and she's taking it deeper and deeper into her mouth! And she's speeding up more and more... I can't take much more of this. I'm on the verge of...{/m}"

# game/events/kira.rpy:868
translate english kira_night_swim_820774f8:

    # Max_22 "Тётя Кира, да... Я сейчас кончу... Давай ещё глубже! А-а-а..."
    Max_22 "Aunt Kira, yes... I'm going to come... Go deeper! Ahhhh..."

# game/events/kira.rpy:874
translate english kira_night_swim_cbc33816:

    # Max_05 "{m}О да... Прямо ей в рот! Хотя, нет, и мимо немного попало... О боже... Как же это приятно!{/m}"
    Max_05 "{m}Oh yeah... Right in her mouth! Although, no, it did get a little bit in her mouth... Oh, dear... That feels so good!{/m}"

# game/events/kira.rpy:875
translate english kira_night_swim_61e5f4b9:

    # Kira_05 "Понравилось, Макс? Как же я увлеклась... Надеюсь, мы никого не разбудили!"
    Kira_05 "Did you like it, Max? I may have gotten carried away... I hope we didn't wake anyone up!"

# game/events/kira.rpy:876
translate english kira_night_swim_4521e956:

    # Max_03 "Вроде, нет... Всё тихо. Ты лучшая, тётя Кира!"
    Max_03 "I don't think so... It's all quiet. You're the best, Aunt Kira!"

# game/events/kira.rpy:889
translate english kira_night_swim_1a8dc447:

    # Kira_05 "Я тоже уже собираюсь идти спать... Приятных тебе снов, Макс."
    Kira_05 "I'm about to go to bed, too... Sleep well, Max."

# game/events/kira.rpy:890
translate english kira_night_swim_88e067a5:

    # Max_04 "Спокойной ночи, тётя Кира."
    Max_04 "Good night, Aunt Kira."

# game/events/kira.rpy:898
translate english kira_night_swim_no_restrain_6151b910:

    # Max_05 "[norestrain!t]Ух, тётя Кира, как хорошо... Тебя я всё же немного испачкал."
    Max_05 "[norestrain!t]Wow, Aunt Kira, that feels good... I did get you a little dirty, though."

# game/events/kira.rpy:899
translate english kira_night_swim_no_restrain_f8441ad7:

    # Kira_05 "Это разве испачкал, Макс? Конечно нет... Главное, что в воду ничего не попало."
    Kira_05 "Did you get that dirty, Max? Of course not... As long as nothing got into the water."

# game/events/kira.rpy:900
translate english kira_night_swim_no_restrain_109b02fc:

    # Max_03 "Ага, как будто ничего и не было."
    Max_03 "Yeah, lucky you left no trace."

# game/events/kira.rpy:904
translate english kira_night_swim_end_cum_42636203:

    # Kira_03 "Ладно, Макс, беги спать. Уже поздно. А то если нас заметят тут, будет много вопросов..."
    Kira_03 "All right, Max, better get to bed before we get noticed. There's going to be far too many questions if we're caught..."

# game/events/kira.rpy:909
translate english kira_night_swim_end_cum_2d756490:

    # Max_04 "Только после тебя..."
    Max_04 "Only after you..."

# game/events/kira.rpy:910
translate english kira_night_swim_end_cum_3ce678d0:

    # Kira_05 "О, спасибо, Макс! Приятно, что ты решил немного поухаживать за мной. Утром буду тебя ждать... Спокойной ночи! И шорты не забудь..."
    Kira_05 "Oh, thank you, Max! It's nice that you decided to court me for a while. I'll wait for you in the morning... Good night. And don't forget your shorts."

# game/events/kira.rpy:911
translate english kira_night_swim_end_cum_171022d2:

    # Max_01 "Не забуду... Спокойной ночи!"
    Max_01 "I'll try not to... Goodnight!"

# game/events/kira.rpy:928
translate english kira_sleep_night_96d0aaaf:

    # Max_04 "Ага! Тётя Кира спит. Хорошо, что её ночнушка слегка просвечивает... Ухх, как она хороша..." nointeract
    Max_04 "Aha! Aunt Kira is asleep. It's good that her nightgown is slightly translucent... Uhh, she's amazing..." nointeract

# game/events/kira.rpy:934
translate english kira_sleep_night_37f4a3ba:

    # Max_02 "Класс! Какая сладкая попка у моей тёти... Хочется любоваться бесконечно. И не только любоваться..."
    Max_02 "Wow! My Aunt has a sweet ass... I want to watch it forever. And not just that..."

# game/events/kira.rpy:936
translate english kira_sleep_night_c1353df2:

    # Max_03 "О, да! К этой шикарной попке я бы с удовольствием прижался... Как и к этим соблазнительным сисечкам. Ммм, очаровательна, как ни крути..."
    Max_03 "Oh, Yes! I'd love to have that ass to myself... and those juicy tits, too. Amazing..."

# game/events/kira.rpy:938
translate english kira_sleep_night_e88b00c2:

    # Max_05 "Чёрт, от вида этих раздвинутых и стройных ножек в шортах становится слишком тесно... Ещё бы, такая горячая красотка!"
    Max_05 "Damn, the sight of those spread and slender legs in shorts makes it too tight... Still, such a hot beauty!"

# game/events/kira.rpy:958
translate english kira_sleep_morning_aa901b38:

    # Max_04 "[txt!t]" nointeract
    Max_04 "[txt!t]" nointeract

# game/events/kira.rpy:964
translate english kira_sleep_morning_37f4a3ba:

    # Max_02 "Класс! Какая сладкая попка у моей тёти... Хочется любоваться бесконечно. И не только любоваться..."
    Max_02 "Cool! What a sweet ass my Aunt has... I want to admire endlessly. And not only that..."

# game/events/kira.rpy:966
translate english kira_sleep_morning_c1353df2:

    # Max_03 "О, да! К этой шикарной попке я бы с удовольствием прижался... Как и к этим соблазнительным сисечкам. Ммм, очаровательна, как ни крути..."
    Max_03 "Oh, yeah! I'd love to cling to that ass... And her seductive breasts..."

# game/events/kira.rpy:968
translate english kira_sleep_morning_e88b00c2:

    # Max_05 "Чёрт, от вида этих раздвинутых и стройных ножек в шортах становится слишком тесно... Ещё бы, такая горячая красотка!"
    Max_05 "Hell, the sight of her slender legs in those tight shorts... she's so hot!"

# game/events/kira.rpy:996
translate english kira_night_tv_234d0568:

    # Max_01 "Тётя Кира ещё не легла спать... смотрит сериалы. Может, стоит задержаться?" nointeract
    Max_01 "Aunt Kira hasn't gone to bed yet... watching soap operas. Maybe we should stay up." nointeract

# game/events/kira.rpy:1002
translate english kira_night_tv_e247cac6:

    # Max_04 "Да-а-а, кто-то залипает на сериалы, а я вот залипаю на свою тётю..." nointeract
    Max_04 "Yes, someone sticks to TV shows, and I stick to my Aunt..." nointeract

# game/events/kira.rpy:1009
translate english kira_night_tv_a95566ce:

    # Kira_01 "А? Макс? Как ты подкрался незаметно... Я думала, уже все спят давно... А меня тут один сериальчик зацепил, решила досмотреть..." nointeract
    Kira_01 "Huh? Max? You snuck up on me... I thought everyone was asleep, I got so stuck into watching this show..." nointeract

# game/events/kira.rpy:1012
translate english kira_night_tv_31c05ed9:

    # Kira_04 "Да без проблем, садись рядом. Будем досматривать сериал или найдём какой-нибудь боевик, фантастику?" nointeract
    Kira_04 "No problem, sit next to me. We can finish watching my show, or see what else is on?" nointeract

# game/events/kira.rpy:1015
translate english kira_night_tv_5c9ee096:

    # Kira_08 "Макс, а ты не слишком юн для этого? Я не думаю, что это хорошая идея."
    Kira_08 "Max, aren't you too young for this? I don't think that's a good idea."

# game/events/kira.rpy:1016
translate english kira_night_tv_fdf04382:

    # Max_09 "Ты прямо как моя мама! Я уже взрослый, а в этом доме не то что посмотреть эротику не выйдет, так даже просто произнести это нельзя."
    Max_09 "You're just like Mom! I'm an adult, and in this house, not only can't I watch mature films, I can't even talk about it!"

# game/events/kira.rpy:1017
translate english kira_night_tv_bc2faa2a:

    # Kira_14 "Вообще, я думаю, что тебе уже пора спать. Три часа ночи... Давай в другой раз поговорим об этом..."
    Kira_14 "Actually, I think it's time for you to go to bed. Three o'clock in the morning... Let's talk about it another time..."

# game/events/kira.rpy:1018
translate english kira_night_tv_27592ad9:

    # Max_08 "Вот так всегда..."
    Max_08 "It's always like this..."

# game/events/kira.rpy:1025
translate english kira_night_tv_97885daa:

    # Kira_00 "Хорошо, Макс. Спокойной ночи! Я тоже уже скоро ложусь спать, только серию досмотрю..."
    Kira_00 "All right, Max. Goodnight! I'm going to bed soon, too..."

# game/events/kira.rpy:1026
translate english kira_night_tv_e72c8e0b:

    # Max_00 "Ага, приятных снов..."
    Max_00 "Yeah, sweet dreams..."

# game/events/kira.rpy:1031
translate english kira_night_tv_ab0640d8:

    # Max_05 "Ухх... Она начала ласкать свою киску... Вот это уже горячо!" nointeract
    Max_05 "Uhh... She started rubbing her pussy... This is hot!" nointeract

# game/events/kira.rpy:1038
translate english kira_night_tv_a0ee876d:

    # Kira_07 "А? Макс? Как ты подкрался незаметно... Я думала, уже все спят давно... А я тут... отдыхаю, как видишь. Я тебя не смущаю таким своим видом?" nointeract
    Kira_07 "Huh? Max? I can't believe you snuck up on me, I thought everyone was asleep... I'm here... resting, as you can see. Am I embarrassing you with my appearance?" nointeract

# game/events/kira.rpy:1042
translate english kira_night_tv_9353de64:

    # Kira_04 "Если твоя мама узнает, то нам обоим влетит. Так что... Пусть этот инцидент останется тайной..."
    Kira_04 "If your Mom finds out, we'll both be in trouble. So, lets keep this a secret."

# game/events/kira.rpy:1043
translate english kira_night_tv_e03b5ea1:

    # Max_03 "Конечно, тётя Кира. Мне что-то не спится. Можно тоже телек с тобой посмотреть?"
    Max_03 "Of course, Aunt Kira. I can't sleep. Can I watch TV with you, too?"

# game/events/kira.rpy:1047
translate english kira_night_tv_413b9a2c:

    # Kira_04 "Ну что ты, Макс. Я просто не ожидала, что кто-то увидит. Так неловко... Мы ведь сохраним этот инцидент в тайне?"
    Kira_04 "Come on, Max. I just didn't expect anyone to see me. So awkward... We'll keep this incident a secret, won't we?"

# game/events/kira.rpy:1048
translate english kira_night_tv_e03b5ea1_1:

    # Max_03 "Конечно, тётя Кира. Мне что-то не спится. Можно тоже телек с тобой посмотреть?"
    Max_03 "Of course, Aunt Kira. I can't sleep. Can I watch TV with you, too?"

# game/events/kira.rpy:1067
translate english kira_night_tv_10418689:

    # Max_01 "Тётя Кира ещё не легла спать... смотрит порнушку. Может, стоит задержаться?" nointeract
    Max_01 "Aunt Kira hasn't gone to bed yet... watching porn. Should I stay longer?" nointeract

# game/events/kira.rpy:1073
translate english kira_night_tv_48edd355:

    # Max_05 "Ухх... Она начала ласкать свою киску... Вот это уже горячо! От такой компании я не откажусь..." nointeract
    Max_05 "Uhh... She started caressing her pussy... This is hot!" nointeract

# game/events/kira.rpy:1081
translate english kira_night_tv_8a132a75:

    # Kira_07 "А? Макс? Как ты подкрался незаметно... Я думала, уже все спят давно... А я тут... отдыхаю, как видишь. Присоединишься?" nointeract
    Kira_07 "Huh? Max? You managed to sneak up on me... I thought everyone was already asleep... I'm here... resting, as you can see. Want to join?" nointeract

# game/events/kira.rpy:1090
translate english kira_night_tv_a0ee876d_1:

    # Kira_07 "А? Макс? Как ты подкрался незаметно... Я думала, уже все спят давно... А я тут... отдыхаю, как видишь. Я тебя не смущаю таким своим видом?" nointeract
    Kira_07 "Huh? Max? You snuck up on me... I thought everyone was already asleep... I'm here... resting, as you can see. Am I embarrassing you with my appearance?" nointeract

# game/events/kira.rpy:1094
translate english kira_night_tv_9353de64_1:

    # Kira_04 "Если твоя мама узнает, то нам обоим влетит. Так что... Пусть этот инцидент останется тайной..."
    Kira_04 "If your Mom finds out, we'll both be in trouble. So, lets keep this a secret."

# game/events/kira.rpy:1095
translate english kira_night_tv_3d3fd53e:

    # Max_03 "Конечно, тётя Кира. А мне можно тоже... посмотреть этот фильм?"
    Max_03 "Of course, Aunt Kira. Can I... Watch this movie, too?"

# game/events/kira.rpy:1099
translate english kira_night_tv_413b9a2c_1:

    # Kira_04 "Ну что ты, Макс. Я просто не ожидала, что кто-то увидит. Так неловко... Мы ведь сохраним этот инцидент в тайне?"
    Kira_04 "Come on, Max. I just didn't expect anyone to see me. So awkward... We'll keep this incident a secret, won't we?"

# game/events/kira.rpy:1100
translate english kira_night_tv_3d3fd53e_1:

    # Max_03 "Конечно, тётя Кира. А мне можно тоже... посмотреть этот фильм?"
    Max_03 "Of course, Aunt Kira. Can I... Watch this movie, too?"

# game/events/kira.rpy:1114
translate english kira_night_tv_porn_4fa7f768:

    # Kira_08 "Макс, а не рано тебе такое смотреть? Если эротику, то ещё куда ни шло... но это самое настоящее порно! Хотя, и эротику тебе не стоит смотреть. Во всяком случае, со мной..."
    Kira_08 "Max, isn't it too soon for your to watch this? It might be different if it was just erotica, but its real porn! Although, you shouldn't watch erotica either. Not with me, anyway..."

# game/events/kira.rpy:1115
translate english kira_night_tv_porn_89c35f72:

    # Max_07 "Как будто я порно не видел... Мама с Эриком всё время тут порно смотрят, прежде чем уходят к себе наверх."
    Max_07 "It's not like I haven't seen porn... Mom and Eric watch porn here all the time before they go upstairs."

# game/events/kira.rpy:1118
translate english kira_night_tv_porn_437212c5:

    # Kira_07 "Ага. И ты не придумал ничего лучше, чем попробовать посмотреть со своей тётей порно среди ночи?" nointeract
    Kira_07 "Aha. And you can't think of anything better to do rather than watching porn with your aunt in the middle of the night?" nointeract

# game/events/kira.rpy:1126
translate english kira_night_tv_porn_97f63c86:

    # Kira_02 "[succes!t]Очень приятно это слышать, Макс. Порно, значит... Ну давай, присаживайся. Только руки не распускать. И не смущайся - я иногда... к себе прикасаюсь, так ощущения от фильма более... интересные, если ты меня понимаешь..."
    Kira_02 "[succes!t]Very nice to hear that, Max. Porn, then... Come on, sit down. Just keep your hands together. And don't be shy - I do sometimes... get pretty invested in the movie, so it's more of a feel-good experience, if you know what I mean..."

# game/events/kira.rpy:1127
translate english kira_night_tv_porn_b0a8e1c5:

    # Max_04 "Да без проблем! Давай уже смотреть..."
    Max_04 "No problem! Let's watch it..."

# game/events/kira.rpy:1149
translate english kira_night_tv_porn_view_e0773c75:

    # Kira_04 "Давай. Хорошо, что я привезла с собой такие фильмы, а то с запретами вашей мамы здесь легко с ума сойти можно..."
    Kira_04 "Let's. It's a good thing I brought such films with me, because it's easy to go crazy with all your Moms rules..."

# game/events/kira.rpy:1150
translate english kira_night_tv_porn_view_6c0fa600:

    # Max_10 "И не говори, тётя Кира. Сам изо дня в день мучаюсь... Столько девчонок красивых в доме, а если заметят стояк, так сразу извращенцем и ненормальным называют."
    Max_10 "You don't need to tell me. I suffer every day myself... So many beautiful girls in the house, and if they notice me even get a slight boner, they shame me as being a pervert and a freak."

# game/events/kira.rpy:1152
translate english kira_night_tv_porn_view_78cda72b:

    # Kira_05 "Так нужно донести до каждой из них, что это нормальная реакция! Хоть это и твоя мама с сёстрами, но они так же ещё и девочки."
    Kira_05 "So you need to convey to each of them that this is a normal reaction! They might be family, but they're still girls."

# game/events/kira.rpy:1153
translate english kira_night_tv_porn_view_1cd738e7:

    # Max_09 "Да я пытаюсь, нужно время..."
    Max_09 "Yes, I'm trying, it takes time..."

# game/events/kira.rpy:1155
translate english kira_night_tv_porn_view_acfeb964:

    # Kira_03 "Так, мы порно собирались смотреть или что? Всё, тихо... всё внимание на экран."
    Kira_03 "So, are we going to watch porn or what? All quiet... all eyes on the screen."

# game/events/kira.rpy:1156
translate english kira_night_tv_porn_view_b33af988:

    # Max_02 "{m}О да, так классно смотреть такое со своей тётей... Она ведь та ещё проказница! А в шортах-то как тесно...{/m}"
    Max_02 "{m}Oh Yes, it's so cool to watch this with my Aunt... she's wearing some really tight clothes...{/m}"

# game/events/kira.rpy:1161
translate english kira_night_tv_porn_view_7e12f89a:

    # Max_03 "{m}Ого, она трогает свою грудь! Блин, мне повезло с тётей. Она такая классная!{/m}"
    Max_03 "{m}Wow, she's touching her Breasts! Damn, she's so frisky!{/m}"

# game/events/kira.rpy:1162
translate english kira_night_tv_porn_view_9aab7bca:

    # Kira_07 "Макс, тебе тесно в твоих шортах? Я же ведь уже всё видела. Стесняешься меня? Мы с тобой сидим, смотрим порно, а ты прикрываешься?"
    Kira_07 "Max, are you hard? I've already seen everything. Embarrassed? We're sitting here watching porn, and you're covering up?"

# game/events/kira.rpy:1166
translate english kira_night_tv_porn_view_20687d60:

    # Kira_05 "В общем, если хочешь, можешь себя трогать. Как я уже говорила, фильм от этого становится интереснее..."
    Kira_05 "If you want, you can touch yourself. As I said, this makes the movie more interesting..."

# game/events/kira.rpy:1167
translate english kira_night_tv_porn_view_9c22a090:

    # Max_04 "Да для меня ты уже интереснее фильма..."
    Max_04 "You're way more interesting to me than the movie..."

# game/events/kira.rpy:1168
translate english kira_night_tv_porn_view_37bfea4c:

    # Kira_02 "Спасибо, Макс... Это приятно слышать... Но ты так всё пропустишь самое интересное на экране..." nointeract
    Kira_02 "Thanks, Max... That's good to hear... But you'll miss what's happening on screen..." nointeract

# game/events/kira.rpy:1176
translate english kira_night_tv_porn_view_d0733d64:

    # Max_05 "{m}С ума сойти... Я дрочу на глазах у своей тёти перед огромным экраном, на котором показывают порно... А она продолжает трогать свою грудь... и даже раздвинула ножки... И не стесняясь ласкает свою киску через трусики!{/m}"
    Max_05 "{m}Crazy... I can jerk off in front of my Aunt while she's touching her breasts... she's even spread her legs... now she's rubbing her pussy through her panties!{/m}"

# game/events/kira.rpy:1177
translate english kira_night_tv_porn_view_331b9e6e:

    # Kira_10 "Ох, он у тебя такой большой, Макс... Это всё так возбуждает... Ммм..."
    Kira_10 "Oh, it's so big, Max... It's all so exciting... Mmm..."

# game/events/kira.rpy:1178
translate english kira_night_tv_porn_view_9c499ca8:

    # Max_20 "{m}Да, Кира... Твоя помощь бы мне не помешала... Хотя... похоже, я сейчас кончу!{/m}" nointeract
    Max_20 "{m}Yes, Kira... I could use your help... Although... I think I'm going to cum!{/m}" nointeract

# game/events/kira.rpy:1185
translate english kira_night_tv_porn_view_f87d5fbf:

    # Kira_09 "Ох, ты уже? Так быстро... Ну, иди приведи себя в порядок и ложись спать. Тебе уже пора. Я тоже скоро... заканчиваю..."
    Kira_09 "Oh, already? So quickly... Well, go clean yourself up and go to bed. You should go. I just need to finish up on my part..."

# game/events/kira.rpy:1186
translate english kira_night_tv_porn_view_06e0a5a8:

    # Max_04 "Хорошо. Спокойной ночи, тётя Кира."
    Max_04 "Well. Good night, Aunt Kira."

# game/events/kira.rpy:1197
translate english kira_night_tv_porn_view_d678d53f:

    # Kira_04 "Давай. Благо, у меня большая коллекция таких фильмов..."
    Kira_04 "Let's. Fortunately, I have a large collection of such films..."

# game/events/kira.rpy:1198
translate english kira_night_tv_porn_view_927b8327:

    # Max_03 "Супер! Хотя я больше рад тому, с кем я это смотрю."
    Max_03 "Great! Although I'm more happy about who I'm watching it with."

# game/events/kira.rpy:1203
translate english kira_night_tv_porn_view_f6966f34:

    # Kira_07 "Не стесняйся, Макс... Вижу, тебе уже очень тесно в шортах, доставай свой член! На экране уже давно это сделали... Ты отстаёшь..."
    Kira_07 "Don't be shy, Max ... I can tell you're already hard in your shorts, just get your dick out!"

# game/events/kira.rpy:1204
translate english kira_night_tv_porn_view_fdc99e1b:

    # Max_04 "Меня уговаривать не надо..."
    Max_04 "You don't have to persuade me..."

# game/events/kira.rpy:1210
translate english kira_night_tv_porn_view_17733f47:

    # Kira_05 "Если хочешь, можешь даже снять шорты, чтобы резинка ничего не передавливала..."
    Kira_05 "If you want, you can even take off your shorts so that you're more comfortable..."

# game/events/kira.rpy:1211
translate english kira_night_tv_porn_view_fa813222:

    # Max_02 "Только если ты тоже разденешься..."
    Max_02 "Only if you undress, too..."

# game/events/kira.rpy:1212
translate english kira_night_tv_porn_view_7ed5b3d1:

    # Kira_02 "Как же без этого, Макс. Но только немножко... чтобы ты не кончил уж очень быстро..." nointeract
    Kira_02 "How about it, Max? But only a little... so you don't finish too quickly." nointeract

# game/events/kira.rpy:1221
translate english kira_night_tv_porn_view_1165d2ce:

    # Kira_05 "Да, Макс, так уже лучше... С тобой просмотр порно становится куда интереснее. Похоже, тебе совершенно не стыдно дрочить прямо перед своей тётей?"
    Kira_05 "Yes, Max, that's better... watching porn is a lot more interesting with you. Looks like you're not at all ashamed to jerk off in front of your Aunt?"

# game/events/kira.rpy:1222
translate english kira_night_tv_porn_view_d09919e5:

    # Max_01 "Ни капли не стыдно! Особенно, если ей самой это нравится..."
    Max_01 "Why would I be? I can tell she loves it!"

# game/events/kira.rpy:1223
translate english kira_night_tv_porn_view_08ac4b2e:

    # Kira_09 "О да, мне это очень нравится! От этого я становлюсь такой мокренькой... Ой, какая же я грязная девчонка..."
    Kira_09 "Oh, you've got me all figured out! You're right, it does a really good job of making me wet..."

# game/events/kira.rpy:1224
translate english kira_night_tv_porn_view_9e370c88:

    # Max_05 "{m}Да, тётя Кира, ты очень плохая девочка... Мы смотрим порно, а ты раздвинула ножки и с таким жаром ласкаешь свою киску, пока я дрочу...{/m}"
    Max_05 "{m}Yeah, Aunt Kira, you're just as perverted as I am... We watch porn together, and you let me jerk off to you spreading your legs and playing with your pussy...{/m}"

# game/events/kira.rpy:1225
translate english kira_night_tv_porn_view_176e65f5:

    # Kira_10 "Ох, он у тебя такой большой, Макс... Это всё так возбуждает... Как хорошо... Делай это быстрее..."
    Kira_10 "Oh, it's so big, Max... It's all so exciting... How nice... Do it faster..."

# game/events/kira.rpy:1226
translate english kira_night_tv_porn_view_1413c02b:

    # Max_20 "{m}Да, Кира... Твоя помощь бы мне не помешала... Хотя... похоже, я сейчас кончу и без этого!{/m}" nointeract
    Max_20 "{m}Yes, Kira... I could use your help... Although... it looks like I'm going to cum without it!{/m}" nointeract

# game/events/kira.rpy:1232
translate english kira_night_tv_porn_view_24d4d0dd:

    # Kira_09 "Ох, ты всё? Ну, тогда иди приведи себя в порядок и ложись спать. Тебе уже пора. Я тоже уже близка к тому... чтобы закончить..."
    Kira_09 "Oh, is that it? Well, then go clean yourself up and go to bed. You should go. I'll finish up here..."

# game/events/kira.rpy:1233
translate english kira_night_tv_porn_view_06e0a5a8_1:

    # Max_04 "Хорошо. Спокойной ночи, тётя Кира."
    Max_04 "Well. Good night, Aunt Kira."

# game/events/kira.rpy:1242
translate english kira_night_tv_promise_cuni_0ee59050:

    # Kira_05 "Да, Макс, так уже лучше... С тобой просмотр порно становится куда интереснее. Но, мы же не будем просто сидеть, смотреть на экран и ласкать себя! Может порадуешь свою тётю, как обещал? Или ты уже забыл!?"
    Kira_05 "Yes, Max, that's better... watching porn is so much hotter when it's with you. But I don't want to just sit here while we touch ourselves and look at the screen! Can you please your aunt like you promised? Or have you already forgotten!?"

# game/events/kira.rpy:1243
translate english kira_night_tv_promise_cuni_48f0e333:

    # Max_03 "Конечно помню! Сейчас развлечёмся..." nointeract
    Max_03 "Of course I remember! Let's have some fun..." nointeract

# game/events/kira.rpy:1248
translate english kira_night_tv_promise_cuni_328ceb80:

    # Kira_02 "Макс... Если тебе мешают мои трусики, я их сниму... Или сними их сам." nointeract
    Kira_02 "Max... If my panties are bothering you, I'll take them off... Or you can take them off yourself." nointeract

# game/events/kira.rpy:1259
translate english kira_night_tv_promise_cuni_0b64a46b:

    # Kira_07 "Оу... Решил поиграть со мной так, Макс... Моя киска так скучала по твоим ласкам... Не останавливайся, я это обожаю! А ты способный... племянник..." nointeract
    Kira_07 "Aww... You decided to play with me like this, Max... My pussy missed your fingers so much... Don't stop, I love it! You're so capable... nephew..." nointeract

# game/events/kira.rpy:1268
translate english kira_night_tv_promise_cuni_b6af5762:

    # Kira_10 "Ухх... Чёрт! Как хорошо... Ещё... глубже и быстрее... Да, вот так... Ммм..." nointeract
    Kira_10 "Uhh... fuck! That's good... More ... deeper and faster ... Yes, that's it... Mmm..." nointeract

# game/events/kira.rpy:1282
translate english kira_night_tv_promise_cuni_ee5fc1a0:

    # Kira_12 "Ох, Макс, не останавливайся! Сожми мою попку покрепче! Я уже так близко... Да! Я кончаю... Ахх..."
    Kira_12 "Oh, Max, don't stop! Squeeze my ass! I'm so close ... Yes! I'm coming ... ahh..."

# game/events/kira.rpy:1283
translate english kira_night_tv_promise_cuni_99d10a9a:

    # Max_02 "Какая у тебя сочная попка, тётя Кира..."
    Max_02 "I love the shape of your ass, Aunt Kira..."

# game/events/kira.rpy:1288
translate english kira_night_tv_promise_cuni_4a1dbaab:

    # Kira_06 "Да, Макс, она у меня такая... Я в восторге! Но лучше нам закругляться, а то я вполне могла кого-нибудь разбудить своими стонами."
    Kira_06 "Yes, Max, I loved it! But we'd better finish up here, I'm worried someone might have heard my moans."

# game/events/kira.rpy:1289
translate english kira_night_tv_promise_cuni_174945ac:

    # Max_01 "Конечно, тётя Кира. Ты лучшая! Спокойной ночи!"
    Max_01 "Of course, Aunt Kira. You are the best! Goodnight!"

# game/events/kira.rpy:1294
translate english kira_night_tv_teachkiss_7f3d8d36:

    # Kira_00 "Да, да... Момент вполне подходящий... Ну что же, давай приступим." nointeract
    Kira_00 "Yes, Yes... This is a good time... Well, let's get started." nointeract

# game/events/kira.rpy:1311
translate english kira_night_tv_first_lesson_dac19e17:

    # Kira_01 "Очень важная часть поцелуя - это прелюдия. Если ты накинешься на девушку в первый раз и начнёшь целовать, пусть даже и хорошо, то скорее всего, получишь отказ..."
    Kira_01 "A very important part leading to a kiss is foreplay. If you throw yourself at a girl for the first time and start kissing her, even if it's good, she probably won't like it..."

# game/events/kira.rpy:1312
translate english kira_night_tv_first_lesson_2eb0a36f:

    # Max_00 "И что делать?"
    Max_00 "What should I do?"

# game/events/kira.rpy:1313
translate english kira_night_tv_first_lesson_88d7c625:

    # Kira_02 "Ну, с прелюдиями я тебе не помогу, тут всё индивидуально. Просто, поймай нужный момент и попробуй меня поцеловать как умеешь, для начала..."
    Kira_02 "Well, I can't help you with foreplay, it's unique to the individual. Just catch the right moment and try to kiss me the way you want, for starters..."

# game/events/kira.rpy:1314
translate english kira_night_tv_first_lesson_e39e10f6:

    # Max_02 "Да это легко!"
    Max_02 "Yes, it's easy!"

# game/events/kira.rpy:1317
translate english kira_night_tv_first_lesson_a408cd9b:

    # Kira_04 "А вот и нет. Излишняя самоуверенность на многих действует, но скромных девушек может отпугнуть. Ты должен чувствовать ту, с которой планируешь... что ты там планируешь..."
    Kira_04 "Not quite. Overconfidence affects many people, and it can put off modest girls. You should get a feel for the one you're planning it with... "

# game/events/kira.rpy:1318
translate english kira_night_tv_first_lesson_863e12cb:

    # Max_04 "Тётя Кира, хватит меня дразнить, я всё это и так знаю!"
    Max_04 "Aunt Kira, stop teasing me, I know all this already!"

# game/events/kira.rpy:1319
translate english kira_night_tv_first_lesson_4ac5ca0b:

    # Kira_05 "Ну хорошо. Давай приступим..." nointeract
    Kira_05 "Oh well. Let's get started..." nointeract

# game/events/kira.rpy:1325
translate english kira_night_tv_first_lesson_ec5a1de2:

    # Max_03 "{m}Ого... Какой горячий ротик... И она так ловко орудует язычком... Так... И что мне, как... Эх, учиться мне ещё и учиться... А что, я очень даже не против такому учиться!{/m}" nointeract
    Max_03 "{m}Wow... her mouth is so hot... And she really knows what she's doing with her tongue... Oh, I still have learn... I should pay attention!{/m}" nointeract

# game/events/kira.rpy:1331
translate english kira_night_tv_first_lesson_659d8563:

    # Max_05 "{m}Ухх, она такая классная, что даже не хочется это прекращать! Так бы и целовал её вечно...{/m}"
    Max_05 "{m}Uhh, she's got me completely under her control! I don't want to stop...{/m}"

# game/events/kira.rpy:1333
translate english kira_night_tv_first_lesson_c592fe34:

    # Kira_01 "Ну всё, на сегодня хватит. Конечно, получается у тебя пока не очень, но это нормально. И ты знаешь, где меня найти, если захочешь потренироваться ещё... А теперь марш спать!"
    Kira_01 "That's enough for today. Of course, you're not doing very well yet, but that's okay. And you know where to find me if you want to practice more... Now go to bed!"

# game/events/kira.rpy:1338
translate english kira_night_tv_first_lesson_49b86e1c:

    # Kira_03 "Так, Макс, а вот это уже лишнее! Разве я позволяла тебе распускать руки?"
    Kira_03 "Now, Max, that's a little too much! Did I let you get away with it?"

# game/events/kira.rpy:1339
translate english kira_night_tv_first_lesson_76b11598:

    # Max_07 "Нет... Рука просто сама потянулась."
    Max_07 "No... my hand just moved on it's own."

# game/events/kira.rpy:1341
translate english kira_night_tv_first_lesson_3e6cfa25:

    # Kira_04 "Что я тебе говорила про излишнюю самоуверенность? Спешка хороша для другого дела... и то, только по настроению. Понял меня?"
    Kira_04 "What did I tell you about being overconfident? Haste is good for other things... but you have to read the mood. Understand me?"

# game/events/kira.rpy:1342
translate english kira_night_tv_first_lesson_7d29322a:

    # Max_03 "Ага... Я учту это."
    Max_03 "Aha... I'll keep that in mind."

# game/events/kira.rpy:1343
translate english kira_night_tv_first_lesson_291bb22f:

    # Kira_01 "Тогда, на сегодня мы закончили. Конечно, получается у тебя пока не очень, но это нормально. И ты знаешь, где меня найти, если захочешь потренироваться ещё... А теперь марш спать!"
    Kira_01 "Then we're done for today. Of course, you're not doing very well yet, but that's okay. And you know where to find me if you want to practice more... Now go to bed!"

# game/events/kira.rpy:1345
translate english kira_night_tv_first_lesson_62f4a650:

    # Max_01 "Конечно! Спасибо, тётя Кира!"
    Max_01 "Of course! Thank you, Aunt Kira!"

# game/events/kira.rpy:1365
translate english kira_night_tv_second_lesson_27509f98:

    # Kira_01 "В прошлый раз я тебе рассказала про прелюдии, но с ними мы разобрались. Так что, давай просто будем повышать твою технику опытным путём..."
    Kira_01 "I told you about foreplay last time, but we've dealt with it. So let's just improve your technique experimentally..."

# game/events/kira.rpy:1366
translate english kira_night_tv_second_lesson_c056e0e2:

    # Max_02 "Да я только за!" nointeract
    Max_02 "I'm all for it!" nointeract

# game/events/kira.rpy:1373
translate english kira_night_tv_second_lesson_b6c08242:

    # Max_04 "{m}Как же это приятно... Она такая... и такое делает язычком... Ух... Это просто невероятно...{/m}"
    Max_04 "{m}I'm starting to move in sync with her tounge... Ugh... This is just unbelievably good...{/m}"

# game/events/kira.rpy:1375
translate english kira_night_tv_second_lesson_dd31d423:

    # Kira_02 "Ну, уже лучше... Возвращаясь к прелюдии, кроме самих поцелуев важную роль играют так же и обоюдные ласки, которыми обмениваются партнёры во время поцелуев."
    Kira_02 "Well, that's better... Returning to the purpose of these lessons, in addition to the kisses themselves, the way you physically interact with your partner during the act also play an important role."

# game/events/kira.rpy:1376
translate english kira_night_tv_second_lesson_38dc4583:

    # Max_07 "Ласки, это в смысле..."
    Max_07 "That sounds very...technical. "

# game/events/kira.rpy:1377
translate english kira_night_tv_second_lesson_d54a4e9e:

    # Kira_07 "Прикосновения, Макс. Как и во время поцелуя, важно поймать нужный момент, чтобы не отпугнуть свою девушку твоим натиском, а наоборот - разжечь в ней страсть! Нужно чувствовать её и чего она хочет, понимаешь?"
    Kira_07 "Touching, Max. Much like before you kiss her, it's important to catch the right moment so that you don't scare your girlfriend, but ignite her passion! You need to feel her and what she wants, you know?"

# game/events/kira.rpy:1378
translate english kira_night_tv_second_lesson_7ddd1f8d:

    # Max_01 "Не особо. Надо пробовать, тётя Кира."
    Max_01 "Okay. We must try, Aunt Kira."

# game/events/kira.rpy:1379
translate english kira_night_tv_second_lesson_20a886ee:

    # Kira_05 "Ты прав. Когда вы оба уже во всю будете увлечены поцелуем, начни с лёгких прикосновений к талии, ножкам... Поверь мне, ты почувствуешь, нравится ли ей это... Ну что, попробуем?" nointeract
    Kira_05 "You're right. When you are both already fully engaged in a kiss, start with light touches to the waist, legs... Trust me, you'll feel if she likes it... Should we try that?" nointeract

# game/events/kira.rpy:1387
translate english kira_night_tv_second_lesson_58738ee4:

    # Max_03 "{m}О да! Кажется, мне никогда не надоест такая практика... Она так увлечённо целуется... Что она там говорила, про нужный момент? Может быть, это как раз он?{/m}" nointeract
    Max_03 "{m}Oh Yes! I don't think I'll ever get tired of this practice... She's such a passionate kisser... What did she say about the right moment? Could it be now?{/m}" nointeract

# game/events/kira.rpy:1392
translate english kira_night_tv_second_lesson_a1f7e689:

    # Kira_03 "Макс, это уже слишком! Зачем же так сразу? Как мне кажется, ты так ничего и не понял..."
    Kira_03 "Max, this is too much! Why so fast? I don't think you understood..."

# game/events/kira.rpy:1393
translate english kira_night_tv_second_lesson_6584b66b:

    # Max_07 "Просто решил начать с этого."
    Max_07 "I just decided to start with this."

# game/events/kira.rpy:1395
translate english kira_night_tv_second_lesson_3e6cfa25:

    # Kira_04 "Что я тебе говорила про излишнюю самоуверенность? Спешка хороша для другого дела... и то, только по настроению. Понял меня?"
    Kira_04 "What did I tell you about being overconfident? Haste is good for other things... but only if you read the mood. Understand me?"

# game/events/kira.rpy:1396
translate english kira_night_tv_second_lesson_7d29322a:

    # Max_03 "Ага... Я учту это."
    Max_03 "Aha... I'll keep that in mind."

# game/events/kira.rpy:1397
translate english kira_night_tv_second_lesson_291bb22f:

    # Kira_01 "Тогда, на сегодня мы закончили. Конечно, получается у тебя пока не очень, но это нормально. И ты знаешь, где меня найти, если захочешь потренироваться ещё... А теперь марш спать!"
    Kira_01 "Then we're done for today. Of course, you're not doing very well yet, but that's okay. And you know where to find me if you want to practice more... Now go to bed!"

# game/events/kira.rpy:1398
translate english kira_night_tv_second_lesson_62f4a650:

    # Max_01 "Конечно! Спасибо, тётя Кира!"
    Max_01 "Of course! Thank you, Aunt Kira!"

# game/events/kira.rpy:1405
translate english kira_night_tv_second_lesson_ce19a55f:

    # Kira_03 "Макс, ты немного спешишь! Зачем же так сразу? Как мне кажется, ты так ничего и не понял..."
    Kira_03 "Max, you're in a bit of a hurry! Why so fast? I don't think you understand..."

# game/events/kira.rpy:1406
translate english kira_night_tv_second_lesson_6584b66b_1:

    # Max_07 "Просто решил начать с этого."
    Max_07 "I just decided to start with this."

# game/events/kira.rpy:1407
translate english kira_night_tv_second_lesson_3e6cfa25_1:

    # Kira_04 "Что я тебе говорила про излишнюю самоуверенность? Спешка хороша для другого дела... и то, только по настроению. Понял меня?"
    Kira_04 "What did I tell you about being overconfident? Haste is good for other things... but you have to read the mood. Understand me?"

# game/events/kira.rpy:1408
translate english kira_night_tv_second_lesson_7d29322a_1:

    # Max_03 "Ага... Я учту это."
    Max_03 "Aha... I'll keep that in mind."

# game/events/kira.rpy:1409
translate english kira_night_tv_second_lesson_291bb22f_1:

    # Kira_01 "Тогда, на сегодня мы закончили. Конечно, получается у тебя пока не очень, но это нормально. И ты знаешь, где меня найти, если захочешь потренироваться ещё... А теперь марш спать!"
    Kira_01 "Then we're done for today. Of course, you're not doing very well yet, but that's okay. And you know where to find me if you want to practice more... Now go to bed!"

# game/events/kira.rpy:1410
translate english kira_night_tv_second_lesson_62f4a650_1:

    # Max_01 "Конечно! Спасибо, тётя Кира!"
    Max_01 "Of course! Thank you, Aunt Kira!"

# game/events/kira.rpy:1419
translate english kira_night_tv_second_lesson_2a8fdf78:

    # Max_02 "{m}Ничего себе! Кира меня не оттолкнула! Похоже, я всё делаю верно... и, наверно, можно двигаться дальше? Боже мой, какая же у неё гладкая кожа, просто шёлк!{/m}" nointeract
    Max_02 "{m}Wow! Kira didn't push me away! It seems that I'm doing everything right... and I guess we can move on? I can feel the heat radiating from her smooth skin, she's into this!{/m}" nointeract

# game/events/kira.rpy:1425
translate english kira_night_tv_second_lesson_ce19a55f_1:

    # Kira_03 "Макс, ты немного спешишь! Зачем же так сразу? Как мне кажется, ты так ничего и не понял..."
    Kira_03 "Max, you're in a bit of a hurry! Why so fast? I don't think you understand..."

# game/events/kira.rpy:1426
translate english kira_night_tv_second_lesson_4ae6adc4:

    # Max_07 "Ну поспешил немного, подумаешь!"
    Max_07 "Ah so I rushed, big deal!"

# game/events/kira.rpy:1428
translate english kira_night_tv_second_lesson_3e6cfa25_2:

    # Kira_04 "Что я тебе говорила про излишнюю самоуверенность? Спешка хороша для другого дела... и то, только по настроению. Понял меня?"
    Kira_04 "What did I tell you about being overconfident? Haste is good for other things... but you have to read the mood. Understand me?"

# game/events/kira.rpy:1429
translate english kira_night_tv_second_lesson_7d29322a_2:

    # Max_03 "Ага... Я учту это."
    Max_03 "Aha... I'll keep that in mind."

# game/events/kira.rpy:1430
translate english kira_night_tv_second_lesson_291bb22f_2:

    # Kira_01 "Тогда, на сегодня мы закончили. Конечно, получается у тебя пока не очень, но это нормально. И ты знаешь, где меня найти, если захочешь потренироваться ещё... А теперь марш спать!"
    Kira_01 "Then we're done for today. Of course, you're not doing very well yet, but that's okay. And you know where to find me if you want to practice more... Now go to bed!"

# game/events/kira.rpy:1431
translate english kira_night_tv_second_lesson_62f4a650_2:

    # Max_01 "Конечно! Спасибо, тётя Кира!"
    Max_01 "Of course! Thank you, Aunt Kira!"

# game/events/kira.rpy:1440
translate english kira_night_tv_second_lesson_4cd46d4a:

    # Max_03 "{m}Обалденно! Неужели это не сон и я действительно трогаю её суперскую попку?! Ну и что, что это попка моей тёти... На ощупь она просто восхитительна - нежная и упругая! Но пора двигаться выше. Охх... Кажется, я начинаю возбуждаться...{/m}" nointeract
    Max_03 "{m}Alright! I've finally got free reign to touch her ass! So what if it's my Aunts? Her curves feels soft and welcoming! But it's time to move higher... I think I'm getting excited...{/m}" nointeract

# game/events/kira.rpy:1447
translate english kira_night_tv_second_lesson_d088b9d5:

    # Max_04 "{m}Отлично! Кажется, ей действительно нравится, что я делаю... Она целует меня с такой жадностью, пока я мну её обалденную грудь... Она прикасается всё ниже и ниже, класс!{/m}"
    Max_04 "{m}Great! I think she's really liking it... her kisses are getting sloppier while I'm kneading her amazing breasts...{/m}"

# game/events/kira.rpy:1454
translate english kira_night_tv_second_lesson_d77c9445:

    # Kira_07 "Ммм... Макс... думаю... нам надо заканчивать. Уже ведь так поздно."
    Kira_07 "Mmm... Max ... I think ... we should finish. It's so late."

# game/events/kira.rpy:1457
translate english kira_night_tv_second_lesson_e9adc097:

    # Max_07 "Я думал, что у меня неплохо получается..."
    Max_07 "I thought I was pretty good at it..."

# game/events/kira.rpy:1458
translate english kira_night_tv_second_lesson_50f2907c:

    # Kira_04 "Уже значительно лучше. С сегодняшним уроком ты справился очень хорошо, но хорошего помаленьку. А то у тебя будет мозоль на языке... А теперь бегом спать! Я тут планирую ещё немного телек посмотреть..."
    Kira_04 "Better. You did very well in today's lesson, but only by a little. Now go to bed! I'm planning on watching some more TV..."

# game/events/kira.rpy:1464
translate english kira_night_tv_second_lesson_cb14d79c:

    # Kira_07 "Ммм... Макс... думаю... нам надо заканчивать. Уже ведь так поздно." nointeract
    Kira_07 "Mmm... Max ... I think ... we should finish. It's so late." nointeract

# game/events/kira.rpy:1472
translate english kira_night_tv_second_lesson_d78e1c5c:

    # Kira_04 "[failed!t]Уже значительно лучше. С сегодняшним уроком ты справился очень хорошо, но хорошего помаленьку. А то у тебя будет мозоль на языке... А теперь бегом спать! Я тут планирую ещё немного телек посмотреть..."
    Kira_04 "[failed!t]Better. You did very well in today's lesson, but only by a little. Now go to bed! I'm planning on watching some more TV..."

# game/events/kira.rpy:1477
translate english kira_night_tv_second_lesson_4466434c:

    # Kira_09 "[succes!t]О да... уже значительно лучше... Ты быстро схватываешь... Ммм..." nointeract
    Kira_09 "[succes!t]Oh Yes... already much better... You've become a fast learner... Mmm..." nointeract

# game/events/kira.rpy:1483
translate english kira_night_tv_second_lesson_dc5ca5df:

    # Max_05 "{m}Да! Я сделал это! Наконец-то я могу насладиться нежностью её обнажённой груди... Обалдеть можно, какая же она классная! Похоже, Кире тоже это нравится, я уже чувствую, как набухли её сосочки. Ухх, чёрт... она начала поглаживать мой член...{/m}"
    Max_05 "{m}Yes! I did it! I can finally enjoy her bare tits... It's awesome! Kira seems to like it too, and I can already feel her nipples stiffen. Uhh, shit... she started stroking my cock...{/m}"

# game/events/kira.rpy:1484
translate english kira_night_tv_second_lesson_2f02296e:

    # Kira_05 "О боже, Макс... Какой же ты развратник, забрался мне под ночнушку... Охх... Это очень мило, что я вызываю такое влечение у своего племянника, но нам надо остановиться..."
    Kira_05 "Oh, my God, Max... aren't you a playboy? Crawling under my nightgown ... Ohh... This is great and all, but we need to stop..."

# game/events/kira.rpy:1485
translate english kira_night_tv_second_lesson_e1b478f3:

    # Max_07 "Почему?"
    Max_07 "Why?"

# game/events/kira.rpy:1488
translate english kira_night_tv_second_lesson_702c9f0b:

    # Kira_04 "Потому что с сегодняшним уроком ты справился очень хорошо, но хорошего помаленьку. А то у тебя будет мозоль на языке... А теперь бегом спать! А мне ещё нужно придти немного в себя после этого..."
    Kira_04 "Because you did well enough in today's lesson, but only a little. Now go to bed! I still need to recover a little after that..."

# game/events/kira.rpy:1489
translate english kira_night_tv_second_lesson_40cf2e56:

    # Max_01 "Конечно! Спасибо, тётя Кира! Спокойной ночи!"
    Max_01 "Of course! Thank you, Aunt Kira!"

# game/events/kira.rpy:1505
translate english kira_night_tv_repeat_lesson_aa588da8:

    # Kira_01 "Столько энтузиазма... Меня начинают посещать мысли, что делаешь ты это уже даже не ради обучения поцелуям..."
    Kira_01 "So much enthusiasm... I'm starting to think that you're not even doing this for the sake of learning how to kiss..."

# game/events/kira.rpy:1506
translate english kira_night_tv_repeat_lesson_3412b7f7:

    # Max_02 "Не отвлекайся!" nointeract
    Max_02 "Don't get distracted!" nointeract

# game/events/kira.rpy:1510
translate english kira_night_tv_repeat_lesson_4d9fd55a:

    # Max_04 "{m}Кажется, мне никогда не надоест это делать с такой женщиной... Ну и что, что она моя тётя... Но она так классно целуется!{/m}" nointeract
    Max_04 "{m}I don't Think I'll ever get tired of doing this with a woman like that... So what if she's my aunt?.. But she's such a great kisser!{/m}" nointeract

# game/events/kira.rpy:1515
translate english kira_night_tv_repeat_lesson_eda0797b:

    # Max_01 "{m}Класс! Она и без моей помощи норовит страстно прижаться ко мне. Её нежная грудь слегка трётся об меня от чего мы начинаем целоваться ещё более страстно...{/m}"
    Max_01 "{m}Yes! Even without my help, I can feel her melting into me. Her soft tits are just rubbing against me, which just makes us kiss even more passionately...{/m}"

# game/events/kira.rpy:1518
translate english kira_night_tv_repeat_lesson_e763b08c:

    # Max_03 "{m}А от жара её мягкой попки под моей рукой можно потерять весь контроль... Но пора двигаться выше. Охх... Кажется, я начинаю возбуждаться...{/m}"
    Max_03 "{m}And the heat of her soft ass in my hand can make you lose all control... But it's time to move higher... I think I'm getting excited...{/m}"

# game/events/kira.rpy:1521
translate english kira_night_tv_repeat_lesson_8b5ea5ae:

    # Max_02 "{m}О да! Как же это приятно, ощущать в руке упругость груди тёти Киры и наслаждаться её жаркими поцелуями... О таких уроках я и мечтать раньше не мог!{/m}" nointeract
    Max_02 "{m}Oh Yes! I'm savouring the feeling of Aunt Kira's tits in my hands, and enjoying every second our tongues are swirling together... I could not have dreamed our lessons would turn out like this!{/m}" nointeract

# game/events/kira.rpy:1525
translate english kira_night_tv_repeat_lesson_c479e48c:

    # Max_05 "{m}У неё такая большая и шелковистая грудь, а сосочки твёрдые и набухшие... И ей это всё очень нравится, она дышит так жарко... Её рука так приятно и легко поглаживает головку моего члена!{/m}"
    Max_05 "{m}Her tits are so big and silky, and her nipples are hard and swollen...her breathing is starting to quicken while she's teasing the head of my cock!{/m}"

# game/events/kira.rpy:1529
translate english kira_night_tv_repeat_lesson_34c56996:

    # Kira_05 "О боже, Макс... Какой же ты развратник, забрался мне под ночнушку... Охх... Остановись, Макс..."
    Kira_05 "Oh, my god, Max... what a playboy, crawling under my nightgown... Ohh... Stop, Max..."

# game/events/kira.rpy:1530
translate english kira_night_tv_repeat_lesson_e1b478f3:

    # Max_07 "Почему?"
    Max_07 "Why?"

# game/events/kira.rpy:1533
translate english kira_night_tv_repeat_lesson_070d38d9:

    # Kira_04 "Хорошего помаленьку. У тебя уже получается гораздо лучше, чем раньше. И если захочешь попрактиковаться ещё, то ты знаешь куда и когда приходить... Но сейчас пора спать."
    Kira_04 "One step at a time. You're already doing a lot better than you used to. And if you want to practice more, you know where and when to come... But now it's time to go to bed."

# game/events/kira.rpy:1534
translate english kira_night_tv_repeat_lesson_40cf2e56:

    # Max_01 "Конечно! Спасибо, тётя Кира! Спокойной ночи!"
    Max_01 "Of course! Thank you, Aunt Kira! Goodnight!"

# game/events/kira.rpy:1543
translate english kira_night_tv_repeat_lesson_4104bc53:

    # Kira_05 "Ох... Макс, мои сосочки уже изнывают от желания, чтобы ты прикоснулся к ним своими губами и языком... Давай, не стесняйся!" nointeract
    Kira_05 "Oh... Max, my nipples are begging you to relieve them with your mouth... Come on, don't be shy!" nointeract

# game/events/kira.rpy:1552
translate english kira_night_tv_repeat_lesson_2df27d30:

    # Kira_06 "Д-а-а... Вот так, Макс... Поиграй со мной! Как же я уже хочу, чтобы ты опускался своим шустрым язычком всё ниже и ниже... прямо туда... Оххх..." nointeract
    Kira_06 "That's it, Max... Come play with me! I already want you to sink your nimble tongue lower and lower... right there... Ohhh..." nointeract

# game/events/kira.rpy:1557
translate english kira_night_tv_repeat_lesson_c494b70e:

    # Kira_02 "Макс... Если тебе мешают мои трусики, я их сниму... Или сними их сам."
    Kira_02 "Max... If my panties are bothering you, I'll take them off... Or just take them off yourself."

# game/events/kira.rpy:1558
translate english kira_night_tv_repeat_lesson_7d5bc607:

    # Max_01 "В другой раз..."
    Max_01 "Some other time..."

# game/events/kira.rpy:1561
translate english kira_night_tv_repeat_lesson_e594df12:

    # Kira_07 "Оу... Макс, ты где такому научился? В порнушке подсмотрел? Не останавливайся, я это обожаю! А ты способный... племянник..." nointeract
    Kira_07 "Aww... Max, where did you learn that? Did you see it in porn? Don't stop, I love it!" nointeract

# game/events/kira.rpy:1565
translate english kira_night_tv_repeat_lesson_aaa0a3e1:

    # Kira_09 "Да... Ещё чуть-чуть и я кончу... Проникни в мою киску своими ловкими пальчиками... Она уже такая мокренькая..." nointeract
    Kira_09 "Yes... Just a little more and I'll come... Get into my pussy with your fingers... I'm already so wet..." nointeract

# game/events/kira.rpy:1570
translate english kira_night_tv_repeat_lesson_ecb67c1f:

    # Kira_10 "Ухх... Чёрт! Как хорошо... Ещё... глубже и быстрее... Да, вот так... Ох, я кончаю... Д-а-а..."
    Kira_10 "Uhh... fuck! It feels good... Even... deeper and faster... Yes, that's it... Ohhh, I'm cumming..."

# game/events/kira.rpy:1571
translate english kira_night_tv_repeat_lesson_4dc1e4e3:

    # Max_02 "Вау, тётя Кира! Это было классно! Какая ты плохая девчонка..."
    Max_02 "Wow, Aunt Kira! That was great! You're such a bad girl..."

# game/events/kira.rpy:1572
translate english kira_night_tv_repeat_lesson_7a1bb7c8:

    # Kira_05 "Ты даже не представляеешь, насколько я испорченная девчонка! Долой ночнушку... Стало уж слишком жарко..."
    Kira_05 "You have no idea how spoiled I am! Off with the nightgown... It was getting too hot..."

# game/events/kira.rpy:1573
translate english kira_night_tv_repeat_lesson_2aa73922:

    # Max_04 "Это точно!"
    Max_04 "That's right!"

# game/events/kira.rpy:1576
translate english kira_night_tv_repeat_lesson_a7a084af:

    # Kira_07 "А ты, Макс, намного способнее, чем мне казалось... и заслужил кое-что особенное... Давай снимай шорты!" nointeract
    Kira_07 "And you, Max, are much more capable than I thought... and you deserve something special... Take off your shorts!" nointeract

# game/events/kira.rpy:1581
translate english kira_night_tv_repeat_lesson_9b0315d0:

    # Kira_02 "Тебя ожидают незабываемые ощущения, Макс! Тебе будет очень непросто это сделать, но постарайся продержаться как можно дольше... потому что я очень хороша в том, что сейчас будет!"
    Kira_02 "You will have an unforgettable experience, Max! It will be very difficult for you to do this, but try to hold out as long as possible... because I'm very good at what's coming!"

# game/events/kira.rpy:1582
translate english kira_night_tv_repeat_lesson_3ca762d4:

    # Max_03 "Думаю, я выдержу что угодно..."
    Max_03 "I think I can handle it..."

# game/events/kira.rpy:1588
translate english kira_night_tv_repeat_lesson_b3200739:

    # Max_07 "Охх... тётя Кира, как приятно... чёрт, это и правда будет непросто выдержать..."
    Max_07 "Ohh... Aunt Kira, how nice... hell, this is really going to be hard to handle..."

# game/events/kira.rpy:1590
translate english kira_night_tv_repeat_lesson_0b605960:

    # Kira_09 "Просто наслаждайся..." nointeract
    Kira_09 "Just enjoy..." nointeract

# game/events/kira.rpy:1611
translate english kira_night_tv_repeat_lesson_11330b66:

    # Max_20 "[restrain!t]{i}Ухх, она так нежно посасывает головку моего члена... Видно, что это не так-то просто... ей еле хватает рта, чтобы это сделать.{/i}" nointeract
    Max_20 "[restrain!t]{i}Uhh, she's gently exploring the head of my cock...It looks like she's deciding if she can take my full length.{/i}" nointeract

# game/events/kira.rpy:1626
translate english kira_night_tv_repeat_lesson_011a1437:

    # Max_21 "[restrain!t]{i}Обалдеть! Как глубоко она берёт его в рот! Блин, я уже на грани... Вот-вот и кончу... А она ускоряет темп!{/i}" nointeract
    Max_21 "[restrain!t]{i}Amazing! I'm practically buried in her mouth! Shit, I'm already on the edge... I'm about to finish... Now she's moving!{/i}" nointeract

# game/events/kira.rpy:1629
translate english kira_night_tv_repeat_lesson_609a6e0c:

    # Max_05 "{m}Фух... О да... Прямо туда... Неужели, она всё проглотила? Во даёт!!!{/m}"
    Max_05 "{m}Ugh... Yeah... Right there... Has she swallowed it all? Oh, she did!!!{/m}"

# game/events/kira.rpy:1632
translate english kira_night_tv_repeat_lesson_bf0e8227:

    # Kira_08 "Макс, ты же понимаешь, что только что кончил прямо в рот своей родной тёте? Не стыдно?"
    Kira_08 "Max, you do realize that you just came in your own Aunt's mouth? Do you feel ashamed?"

# game/events/kira.rpy:1633
translate english kira_night_tv_repeat_lesson_c9f858e0:

    # Max_03 "Ни капли!"
    Max_03 "Not in the slightest!"

# game/events/kira.rpy:1634
translate english kira_night_tv_repeat_lesson_d4beb3ab:

    # Kira_07 "Вот и правильно! Мне очень понравилось... А тебе?"
    Kira_07 "Ha! That's right! I really liked it... And you?"

# game/events/kira.rpy:1635
translate english kira_night_tv_repeat_lesson_b6765c65:

    # Max_05 "Да как такое может не понравиться! Это вообще самое офигенное, что было в моей жизни!"
    Max_05 "How can I not like it? This is the most awesome thing that has ever happened in my life!"

# game/events/kira.rpy:1638
translate english kira_night_tv_repeat_lesson_31ae6846:

    # Kira_05 "О, Макс... У тебя ещё не было почти ничего..."
    Kira_05 "Oh, Max... You haven't had almost anything yet..."

# game/events/kira.rpy:1639
translate english kira_night_tv_repeat_lesson_c5d2f6d4:

    # Max_02 "Но, надеюсь, ведь ещё будет, тётя Кира?"
    Max_02 "But I hope there will be more, Aunt Kira."

# game/events/kira.rpy:1640
translate english kira_night_tv_repeat_lesson_9bb8c535:

    # Kira_04 "Со мной?! Очень может быть... А теперь бегом спать! А мне ещё нужно придти немного в себя после всего этого..."
    Kira_04 "With me? There may very well be... Now go to bed! And I still need to recover a little after all this..."

# game/events/kira.rpy:1641
translate english kira_night_tv_repeat_lesson_174945ac:

    # Max_01 "Конечно, тётя Кира. Ты лучшая! Спокойной ночи!"
    Max_01 "Of course, Aunt Kira. You are the best! Goodnight!"

# game/events/kira.rpy:1652
translate english kira_night_tv_not_restrain_e3061273:

    # Kira_08 "[norestrain!t]Ого! Ты уже всё! И не стыдно тебе, забрызгать всё лицо своей тёте спермой?"
    Kira_08 "[norestrain!t]Wow! You're done! Aren't you ashamed of splattering your Aunt's face with cum?"

# game/events/kira.rpy:1653
translate english kira_night_tv_not_restrain_286a5398:

    # Max_05 "Ни капли!"
    Max_05 "Not in the slightest!"

# game/events/kira.rpy:1654
translate english kira_night_tv_not_restrain_c826c37c:

    # Kira_05 "Вот и правильно! Со временем, когда привыкнешь к этому, тебе будет проще сдерживаться."
    Kira_05 "Ha, That's right! In time, when you get used to it, it will be easier for you to hold back."

# game/events/kira.rpy:1655
translate english kira_night_tv_not_restrain_76e8042d:

    # Max_02 "Значит, будет следующий раз?!"
    Max_02 "So there will be a next time?!"

# game/events/kira.rpy:1658
translate english kira_night_tv_not_restrain_f5b16231:

    # Kira_04 "Очень может быть... А теперь бегом спать! А мне ещё нужно придти немного в себя после всего этого..."
    Kira_04 "There may very well be... Now go to bed! And I still need to recover a little after all this..."

# game/events/kira.rpy:1659
translate english kira_night_tv_not_restrain_174945ac:

    # Max_01 "Конечно, тётя Кира. Ты лучшая! Спокойной ночи!"
    Max_01 "Of course, Aunt Kira. You are the best! Goodnight!"

# game/events/kira.rpy:1667
translate english kira_night_tv_not_restrain2_da52627c:

    # Kira_08 "[norestrain!t]Ого! Ты уже всё! Похоже, тебе ещё привыкать и привыкать к тому, что может вытворять мой ротик, потому что это было только начало..."
    Kira_08 "[norestrain!t]Wow! You're done! It looks like you're still getting used to what my mouth can do, because that was just the beginning..."

# game/events/kira.rpy:1668
translate english kira_night_tv_not_restrain2_f7abf3aa:

    # Max_02 "Значит, до следующего раза?!"
    Max_02 "So, until next time?!"

# game/events/kira.rpy:1683
translate english kira_night_tv_teach_cuni_ef1ea769:

    # Kira_05 "Ох... Макс, мои сосочки уже изнывают от желания, чтобы ты прикоснулся к ним своими губами и языком..." nointeract
    Kira_05 "Oh... Max, my nipples are begging you to relieve them with your mouth... Come on, don't be shy!" nointeract

# game/events/kira.rpy:1692
translate english kira_night_tv_teach_cuni_2df27d30:

    # Kira_06 "Д-а-а... Вот так, Макс... Поиграй со мной! Как же я уже хочу, чтобы ты опускался своим шустрым язычком всё ниже и ниже... прямо туда... Оххх..." nointeract
    Kira_06 "That's it, Max... Come play with me! I already want you to sink your nimble tongue lower and lower... right there... Ohhh..." nointeract

# game/events/kira.rpy:1697
translate english kira_night_tv_teach_cuni_328ceb80:

    # Kira_02 "Макс... Если тебе мешают мои трусики, я их сниму... Или сними их сам." nointeract
    Kira_02 "Max... If my panties are bothering you, I'll take them off... Or just take them off yourself." nointeract

# game/events/kira.rpy:1702
translate english kira_night_tv_teach_cuni_fe50801f:

    # Kira_07 "Оу... Макс, моя киска так скучала по твоим ласкам... Не останавливайся, я это обожаю! А ты способный... племянник..." nointeract
    Kira_07 "Aww... Max, my pussy missed you so much... Don't stop, I love it! You're quite the capable one." nointeract

# game/events/kira.rpy:1706
translate english kira_night_tv_teach_cuni_aaa0a3e1:

    # Kira_09 "Да... Ещё чуть-чуть и я кончу... Проникни в мою киску своими ловкими пальчиками... Она уже такая мокренькая..." nointeract
    Kira_09 "Yes... Just a little more and I'll come... Get into my pussy with your fingers... I'm already so wet..." nointeract

# game/events/kira.rpy:1717
translate english kira_night_tv_teach_cuni_ecb67c1f:

    # Kira_10 "Ухх... Чёрт! Как хорошо... Ещё... глубже и быстрее... Да, вот так... Ох, я кончаю... Д-а-а..."
    Kira_10 "Uhh... fuck! It feels good... Even... deeper and faster... Yes, that's it... Ohhh, I'm cumming..."

# game/events/kira.rpy:1718
translate english kira_night_tv_teach_cuni_f090a75a:

    # Max_02 "Какая ты испорченная девчонка, тётя Кира..."
    Max_02 "What a spoiled girl you are, Aunt Kira..."

# game/events/kira.rpy:1721
translate english kira_night_tv_teach_cuni_b22c5b23:

    # Kira_07 "Да, я очень испорченная... а ты, Макс, заслужил кое-что особенное... Давай снимай шорты! Твой член так и напрашивается на то, чтобы его хорошенько приласкали..." nointeract
    Kira_07 "Yes, I am very spoiled... and you, Max, deserve something special... Take off your shorts! Your cock is begging for my attention..." nointeract

# game/events/kira.rpy:1742
translate english kira_night_tv_cuni_var2_0b64a46b:

    # Kira_07 "Оу... Решил поиграть со мной так, Макс... Моя киска так скучала по твоим ласкам... Не останавливайся, я это обожаю! А ты способный... племянник..." nointeract
    Kira_07 "Aww... You decided to play with me like this, Max... My pussy missed your fingers so much... Don't stop, I love it! You're so capable... nephew..." nointeract

# game/events/kira.rpy:1751
translate english kira_night_tv_cuni_var2_b6af5762:

    # Kira_10 "Ухх... Чёрт! Как хорошо... Ещё... глубже и быстрее... Да, вот так... Ммм..." nointeract
    Kira_10 "Uhh... fuck! That's good... More ... deeper and faster ... Yes, that's it... Mmm..." nointeract

# game/events/kira.rpy:1767
translate english kira_night_tv_cuni_var2_ea18dd2c:

    # Kira_12 "Ох, Макс, не останавливайся! Сожми мою попку покрепче! Я уже так близко... Моя киска так хочет кое-чего большого и твёрдого!"
    Kira_12 "Oh, Max, don't stop! Squeeze my ass harder! I'm so close... My pussy wants something big and hard!"

# game/events/kira.rpy:1771
translate english kira_night_tv_cuni_var2_ee5fc1a0:

    # Kira_12 "Ох, Макс, не останавливайся! Сожми мою попку покрепче! Я уже так близко... Да! Я кончаю... Ахх..."
    Kira_12 "Oh, Max, don't stop! Squeeze my ass! I'm so close ... Yes! I'm coming ... ahh..."

# game/events/kira.rpy:1772
translate english kira_night_tv_cuni_var2_99d10a9a:

    # Max_02 "Какая у тебя сочная попка, тётя Кира..."
    Max_02 "I love the shape of your ass, Aunt Kira..."

# game/events/kira.rpy:1777
translate english kira_night_tv_cuni_var2_57da463b:

    # Kira_07 "Да... а ты, Макс, заслужил кое-что особенное... Твой член так и напрашивается на то, чтобы его хорошенько приласкали..."
    Kira_07 "Yes... and you, Max, deserve something special... Your cock is begging for some attention..."

# game/events/kira.rpy:1778
translate english kira_night_tv_cuni_var2_f7a0a397:

    # Max_04 "Да... Я очень этого хочу!"
    Max_04 "Yes... I really want it!"

# game/events/kira.rpy:1784
translate english kira_night_tv_bj_f38ad1c4:

    # Kira_02 "Тебя ожидают незабываемые ощущения, Макс! Посмотрим, как долго ты сможешь продержаться на этот раз..." nointeract
    Kira_02 "You will have an unforgettable experience, Max! Let's see how long you can hold out this time..." nointeract

# game/events/kira.rpy:1792
translate english kira_night_tv_bj_87575487:

    # Max_07 "Охх... тётя Кира, как приятно..."
    Max_07 "Ohh... Aunt Kira, how nice..."

# game/events/kira.rpy:1794
translate english kira_night_tv_bj_0b605960:

    # Kira_09 "Просто наслаждайся..." nointeract
    Kira_09 "Just enjoy..." nointeract

# game/events/kira.rpy:1816
translate english kira_night_tv_bj_4e70d008:

    # Max_20 "[restrain!t]{i}Ухх, она так нежно посасывает головку моего члена... Не представляю, как ей удаётся поместить её в рот!{/i}" nointeract
    Max_20 "[restrain!t]{i}Uhh, she's sucking the head of my cock so gently... I don't know how she manages to get it in her mouth!{/i}" nointeract

# game/events/kira.rpy:1837
translate english kira_night_tv_bj_011a1437:

    # Max_21 "[restrain!t]{i}Обалдеть! Как глубоко она берёт его в рот! Блин, я уже на грани... Вот-вот и кончу... А она ускоряет темп!{/i}" nointeract
    Max_21 "[restrain!t]{i}Amazing! I'm practically buried in her mouth! Shit, I'm already on the edge... I'm about to finish... Now she's moving!{/i}" nointeract

# game/events/kira.rpy:1840
translate english kira_night_tv_bj_2d0d8f41:

    # Max_05 "{m}Фух... О да... Прямо ей в рот! Она всё глотает! О боже... Как же это приятно!{/m}"
    Max_05 "{m}Ugh... Yeah... Right in her mouth! She swallows everything! Oh God... she's talented, isn't she?{/m}"

# game/events/kira.rpy:1844
translate english kira_night_tv_bj_2f0c8e1d:

    # Kira_07 "Ну вот. Понравилось? И ни капли мимо... ну, почти."
    Kira_07 "Well. You like it? And not a drop wasted... well, almost."

# game/events/kira.rpy:1845
translate english kira_night_tv_bj_a2bb381b:

    # Max_05 "Ты потрясающая, тётя Кира! До следующего раза?!"
    Max_05 "You're amazing, Aunt Kira! Until next time?!"

# game/events/kira.rpy:1858
translate english kira_night_tv_tv_cuni_6764d776:

    # Kira_05 "Да, Макс, так уже лучше... С тобой просмотр порно становится куда интереснее. Но, мы же не будем просто сидеть, смотреть на экран и ласкать себя! Верно?"
    Kira_05 "That's better... You make watching porn a lot more fun. But, we're not just gonna sit around and look at each other, right?"

# game/events/kira.rpy:1859
translate english kira_night_tv_tv_cuni_2c995eaa:

    # Max_01 "Мне нравится, к чему ты клонишь, тётя Кира..." nointeract
    Max_01 "I like where you're going with this, Aunt Kira..." nointeract

# game/events/kira.rpy:1864
translate english kira_night_tv_tv_cuni_328ceb80:

    # Kira_02 "Макс... Если тебе мешают мои трусики, я их сниму... Или сними их сам." nointeract
    Kira_02 "Max... If my panties are bothering you, I'll take them off... Or just take them off yourself." nointeract

# game/events/kira.rpy:1869
translate english kira_night_tv_tv_cuni_fe50801f:

    # Kira_07 "Оу... Макс, моя киска так скучала по твоим ласкам... Не останавливайся, я это обожаю! А ты способный... племянник..." nointeract
    Kira_07 "Aww... Max, my pussy missed you so much... Don't stop, I love it! You're quite the capable one." nointeract

# game/events/kira.rpy:1873
translate english kira_night_tv_tv_cuni_aaa0a3e1:

    # Kira_09 "Да... Ещё чуть-чуть и я кончу... Проникни в мою киску своими ловкими пальчиками... Она уже такая мокренькая..." nointeract
    Kira_09 "Yes... Just a little more and I'll come... Get into my pussy with your fingers... I'm already so wet..." nointeract

# game/events/kira.rpy:1884
translate english kira_night_tv_tv_cuni_ecb67c1f:

    # Kira_10 "Ухх... Чёрт! Как хорошо... Ещё... глубже и быстрее... Да, вот так... Ох, я кончаю... Д-а-а..."
    Kira_10 "Uhh... fuck! It feels good... Even... deeper and faster... Yes, that's it... Ohhh, I'm cumming..."

# game/events/kira.rpy:1885
translate english kira_night_tv_tv_cuni_f090a75a:

    # Max_02 "Какая ты испорченная девчонка, тётя Кира..."
    Max_02 "What a spoiled girl you are, Aunt Kira..."

# game/events/kira.rpy:1888
translate english kira_night_tv_tv_cuni_4931b5c7:

    # Kira_07 "Да, я очень испорченная... а ты, Макс, заслужил кое-что особенное... Твой член так и напрашивается на то, чтобы его хорошенько приласкали..."
    Kira_07 "Yes, I'm very spoiled... now I think you deserve something special, I can see your cock is begging for attention..."

# game/events/kira.rpy:1889
translate english kira_night_tv_tv_cuni_f7a0a397:

    # Max_04 "Да... Я очень этого хочу!"
    Max_04 "Yes... I really want it!"

# game/events/kira.rpy:1907
translate english kira_night_tv_tv_sex1_6a99226d:

    # Kira_10 "Ухх... Чёрт! Как хорошо... Ещё... глубже и быстрее... Ещё чуть-чуть и я кончу... Может быть, ты воспользуешься чем-то потолще пальчиков и язычка?" nointeract
    Kira_10 "Ugh... Damn! That feels good... More... Deeper and faster... Just a little bit more and I'll cum... Maybe you could use something thicker?" nointeract

# game/events/kira.rpy:1922
translate english kira_night_tv_tv_sex2_dfb388ae:

    # Kira_12 "Ох, Макс, не останавливайся! Сожми мою попку покрепче! Я уже так близко... Моя киска так хочет кое-чего большого и твёрдого!" nointeract
    Kira_12 "Oh, Max, don't stop! Squeeze my ass harder! I'm so close... My pussy wants something big and hard!" nointeract

# game/events/kira.rpy:1929
translate english kira_night_tv_tv_sex2_69d27069:

    # Kira_09 "Ох, Макс... Я обожаю этот момент! Д-а-а... Вот так... Вводи его не спеша... хочу немного привыкнуть к его размерам... Ммм..." nointeract
    Kira_09 "Oh, Max... I love this moment! Y-a-a... That's it... Take it slow... I want to get used to his size... Mmmm..." nointeract

# game/events/kira.rpy:1949
translate english kira_night_tv_tv_sex2_371a8e90:

    # Kira_11 "[restrain!t]Как приятно чувствовать твой член, Макс! Охх... Да... Вгоняй его в мою киску сильнее... Ещё! Оттрахай меня пожёстче... Д-а-а... Ещё..."
    Kira_11 "[restrain!t]It feels so good to feel your cock, Max! Ohhh... Yes... Push it into my pussy harder... More, fuck me harder. Yeah... More..."

# game/events/kira.rpy:1952
translate english kira_night_tv_tv_sex2_118f7848:

    # Max_20 "Какая у тебя сочная попка, тётя Кира..."
    Max_20 "I love the shape of your ass, Aunt Kira..."

# game/events/kira.rpy:1953
translate english kira_night_tv_tv_sex2_c67f39e0:

    # Kira_12 "О да! Ещё... ещё, Макс... Да, вот так... Ммм... Трахай меня ещё быстрее! Я еле сдерживаюсь... Ох, как хорошо! Ещё! Да, я кончаю... Ахх..." nointeract
    Kira_12 "Oh yes! More... more, Max... Yeah, that's it... Mmm... Fuck me even faster! I can hardly contain myself. Oh, that feels good. More! Yes, I'm coming... Ahhh..." nointeract

# game/events/kira.rpy:1960
translate english kira_night_tv_tv_sex2_fa08c779:

    # Kira_07 "Должно быть блаженно, кончать в свою тётю, да Макс? Я к этому готова всегда, но за других женщин говорить не могу, так что лучше быть осторожнее..."
    Kira_07 "It must be blissful to cum inside your aunt, huh, Max? I'm always ready for that, but I can't speak for other women, so you better be careful..."

# game/events/kira.rpy:1961
translate english kira_night_tv_tv_sex2_0a14aaaa:

    # Max_07 "Да, тётя Кира! Значит, до следующего раза?!"
    Max_07 "Yes, Aunt Kira! Until next time, then?"

# game/events/kira.rpy:1969
translate english kira_night_tv_tv_sex2_5d671a08:

    # Kira_05 "Ого, сколько её в тебе, Макс! Всю попку мне залил, безобразник... Это я любя! Славно развлеклись, правда?"
    Kira_05 "Wow, that was a lot, Max! You poured it all over my ass, you naughty boy... I love it! That was fun, wasn't it?"

# game/events/kira.rpy:1970
translate english kira_night_tv_tv_sex2_ef5f22f0:

    # Max_02 "Да, тётя Кира! Значит, до следующего раза?!"
    Max_02 "Yes, Aunt Kira! Until next time, then?"

# game/events/kira.rpy:1991
translate english kira_night_tv_tv_sex2_ed53830e:

    # Kira_05 "[norestrain!t]Ого, сколько её в тебе, Макс! Всю попку мне залил, безобразник... Это я любя! Славно развлеклись, правда?"
    Kira_05 "[norestrain!t]Wow, that was a lot, Max! You poured it all over my ass, you naughty boy... I love it! That was fun, wasn't it?"

# game/events/kira.rpy:1993
translate english kira_night_tv_tv_sex2_ef5f22f0_1:

    # Max_02 "Да, тётя Кира! Значит, до следующего раза?!"
    Max_02 "Yes, Aunt Kira! Until next time, then?"

# game/events/kira.rpy:2001
translate english kira_night_tv_tv_sex2_46cfabbd:

    # Kira_08 "[norestrain!t]Ого! Ты уже всё! Похоже, тебе ещё привыкать и привыкать к тому наслаждению, которое нас ждало дальше..."
    Kira_08 "[norestrain!t]wow! You're done already! Looks like you're still getting used to and getting used to it..."

# game/events/kira.rpy:2003
translate english kira_night_tv_tv_sex2_38f54446:

    # Max_07 "Да, тётя Кира, извини! Не сдержался... Значит, до следующего раза?!"
    Max_07 "Yes, Aunt Kira, I'm sorry! I couldn't help it... Until next time, then?"

# game/events/kira.rpy:2012
translate english kira_night_tv_missionary_87407bd3:

    # Kira_09 "Ох, Макс... Какой же это классный момент! Д-а-а... Вот так... Вводи его не спеша... чтобы я привыкла... Ммм..." nointeract
    Kira_09 "Oh, Max... I'm looking forward to this! Yeah... That's it... Put it in slowly... So I can get used to it... Mmm..." nointeract

# game/events/kira.rpy:2026
translate english kira_night_tv_missionary_20627b65:

    # Kira_11 "[restrain!t]Как приятно чувствовать твой член, Макс! Охх... Да... Вгоняй его в меня сильнее... Ещё! Оттрахай меня как следует... Д-а-а... Ещё..."
    Kira_11 "[restrain!t]It feels so good to feel your cock, Max! Ohhh... Yes... Push it into me harder... More! Fuck me hard... Yeah... More..."

# game/events/kira.rpy:2029
translate english kira_night_tv_missionary_ccabbb5f:

    # Max_20 "Какая ты испорченная девчонка, тётя Кира..."
    Max_20 "You're so spoiled, Aunt Kira..."

# game/events/kira.rpy:2030
translate english kira_night_tv_missionary_a5c44566:

    # Kira_12 "О да! Ещё... ещё, Макс... Да, вот так... Ммм... Трахай меня ещё сильнее! Я еле сдерживаюсь... Ох, как хорошо! Ещё! Да, я кончаю... Ахх..." nointeract
    Kira_12 "Oh yes! More... more, Max... Yes, that's it... Mmm... Fuck me harder! I can hardly contain myself. Oh, that feels good. More! Yes, I'm coming... Ahhh..." nointeract

# game/events/kira.rpy:2037
translate english kira_night_tv_missionary_fa08c779:

    # Kira_07 "Должно быть блаженно, кончать в свою тётю, да Макс? Я к этому готова всегда, но за других женщин говорить не могу, так что лучше быть осторожнее..."
    Kira_07 "It must be blissful to cum inside your aunt, huh, Max? I'm always ready for that, but I can't speak for other women, so you better be careful..."

# game/events/kira.rpy:2038
translate english kira_night_tv_missionary_0a14aaaa:

    # Max_07 "Да, тётя Кира! Значит, до следующего раза?!"
    Max_07 "Yes, Aunt Kira! Until next time, then?"

# game/events/kira.rpy:2046
translate english kira_night_tv_missionary_cc7dbac3:

    # Kira_05 "Ого, сколько её в тебе, Макс! Весь живот мне залил, безобразник... Это я любя! Славно развлеклись, правда?"
    Kira_05 "Wow, that's a lot, Max! You got it all over my stomach, you nuaghty boy... I love it! That was fun, wasn't it?"

# game/events/kira.rpy:2047
translate english kira_night_tv_missionary_ef5f22f0:

    # Max_02 "Да, тётя Кира! Значит, до следующего раза?!"
    Max_02 "Yes, Aunt Kira! Until next time, then?"

# game/events/kira.rpy:2068
translate english kira_night_tv_missionary_1abefedc:

    # Kira_05 "[norestrain!t]Ого, сколько её в тебе, Макс! Весь живот мне залил, безобразник... Это я любя! Славно развлеклись, правда?"
    Kira_05 "[norestrain!t]That's a lot, Max! You've poured it all over my belly, you naughty... I love it! That was fun, wasn't it?"

# game/events/kira.rpy:2070
translate english kira_night_tv_missionary_ef5f22f0_1:

    # Max_02 "Да, тётя Кира! Значит, до следующего раза?!"
    Max_02 "Yes, Aunt Kira! Until next time, then?"

# game/events/kira.rpy:2078
translate english kira_night_tv_missionary_46cfabbd:

    # Kira_08 "[norestrain!t]Ого! Ты уже всё! Похоже, тебе ещё привыкать и привыкать к тому наслаждению, которое нас ждало дальше..."
    Kira_08 "[norestrain!t]wow! You're done already! Looks like you're still getting used to and getting used to it..."

# game/events/kira.rpy:2080
translate english kira_night_tv_missionary_38f54446:

    # Max_07 "Да, тётя Кира, извини! Не сдержался... Значит, до следующего раза?!"
    Max_07 "Yes, Aunt Kira, I'm sorry! I couldn't help it... Until next time, then?"

# game/events/kira.rpy:2099
translate english kira_night_tv_cum_in_mouth_4be7addd:

    # Max_20 "[restrain!t]{i}Ухх, как же смачно она посасывает головку моего члена... Не представляю, как ей удаётся поместить её в рот, но она справляется! А я кончаю... Д-а-а...{/i}"
    Max_20 "[restrain!t]{i}Uh, she's sucking my cock so hard... I can't imagine how she manages to fit it in her mouth, but she does! I'm going to cum... Y-a-a...{/i}"

# game/events/kira.rpy:2105
translate english kira_night_tv_cum_in_mouth_5d1bbc4c:

    # Max_05 "{i}Фух... О да... Прямо ей в рот! Она всё глотает! О боже... Как же это приятно!{/i}"
    Max_05 "{i}Phew... Oh yes... Right in her mouth! She swallows everything! Oh, God... That feels so good!{/i}"

# game/events/kira.rpy:2106
translate english kira_night_tv_cum_in_mouth_2f0c8e1d:

    # Kira_07 "Ну вот. Понравилось? И ни капли мимо... ну, почти."
    Kira_07 "There you go. Did you like it? And not a drop lost... Well, almost."

# game/events/kira.rpy:2107
translate english kira_night_tv_cum_in_mouth_a2bb381b:

    # Max_05 "Ты потрясающая, тётя Кира! До следующего раза?!"
    Max_05 "You're amazing, Aunt Kira! Until next time?"

# game/events/kira.rpy:2115
translate english kira_night_tv_minet_after_sex_4713ca96:

    # Kira_07 "[restrain!t]Фух, удивляюсь, как ты ещё не кончил, Макс! Твой член так и напрашивается на то, чтобы его хорошенько приласкали..."
    Kira_07 "[restrain!t]Whew, I'm surprised you haven't cum yet, Max! Your dick is just begging for it..."

# game/events/kira.rpy:2117
translate english kira_night_tv_minet_after_sex_f7a0a397:

    # Max_04 "Да... Я очень этого хочу!"
    Max_04 "Yes... I want it very much!"

# game/events/kira.rpy:2121
translate english kira_night_tv_minet_after_sex_f0b5050a:

    # Kira_02 "Устраивайся поудобнее... А я позабочусь о том, чтобы ты сладко-сладко кончил мне в рот! Если конечно выдержишь то, что я буду делать языком..."
    Kira_02 "Make yourself comfortable... And I'll make sure you cum in my mouth! If you can last long enough with my tongue..."

# game/events/kira.rpy:2122
translate english kira_night_tv_minet_after_sex_f3feae2f:

    # Max_03 "О да, ты им такое вытворяешь!"
    Max_03 "Oh yes, you do that!"

# game/events/kira.rpy:2133
translate english kira_night_tv_minet_after_sex_87575487:

    # Max_07 "Охх... тётя Кира, как приятно..."
    Max_07 "Ohhh... Aunt Kira, how nice..."

# game/events/kira.rpy:2134
translate english kira_night_tv_minet_after_sex_0b605960:

    # Kira_09 "Просто наслаждайся..." nointeract
    Kira_09 "Just enjoy it..." nointeract

# game/events/kira.rpy:2148
translate english kira_night_tv_minet_after_sex_ac8a5ca0:

    # Max_20 "[restrain!t]{i}Ухх, она так нежно посасывает головку моего члена... Не представляю, как ей удаётся поместить её в рот!{/i}"
    Max_20 "[restrain!t]{i}Uhh, she's sucking the head of my cock so gently... I don't know how she manages to get it in her mouth!{/i}"

# game/events/kira.rpy:2150
translate english kira_night_tv_minet_after_sex_2cc14887:

    # Max_19 "Да, вот так тётя Кира, твои губки творят чудеса! Охх..." nointeract
    Max_19 "Yes, that's it Aunt Kira, your lips do wonders! Ohhh..." nointeract

# game/events/kira.rpy:2163
translate english kira_night_tv_minet_after_sex_b9d9402f:

    # Max_21 "[restrain!t]{i}Обалдеть! Как глубоко она берёт его в рот! Блин, я уже на грани... Вот-вот и кончу... А она ускоряет темп!{/i}"
    Max_21 "[restrain!t]{i}Amazing! I'm practically buried in her mouth! Shit, I'm already on the edge... I'm about to finish... Now she's moving!{/i}"

# game/events/kira.rpy:2165
translate english kira_night_tv_minet_after_sex_6aa1b7d8:

    # Max_22 "Тётя Кира, ещё быстрее... Да... Я сейчас кончу... А-а-а..."
    Max_22 "Aunt Kira, even faster... Yes... I'm going to cum... Ah..."

# game/events/kira.rpy:2166
translate english kira_night_tv_minet_after_sex_5d1bbc4c:

    # Max_05 "{i}Фух... О да... Прямо ей в рот! Она всё глотает! О боже... Как же это приятно!{/i}"
    Max_05 "{i}Phew... Oh yes... Right in her mouth! She swallows everything! Oh, God... That feels so good!{/i}"

# game/events/kira.rpy:2171
translate english kira_night_tv_minet_after_sex_2f0c8e1d:

    # Kira_07 "Ну вот. Понравилось? И ни капли мимо... ну, почти."
    Kira_07 "Well. You like it? And not a drop wasted... well, almost."

# game/events/kira.rpy:2172
translate english kira_night_tv_minet_after_sex_a2bb381b:

    # Max_05 "Ты потрясающая, тётя Кира! До следующего раза?!"
    Max_05 "You're amazing, Aunt Kira! Until next time?"

# game/events/kira.rpy:2185
translate english kira_night_tv_minet_after_sex_da52627c:

    # Kira_08 "[norestrain!t]Ого! Ты уже всё! Похоже, тебе ещё привыкать и привыкать к тому, что может вытворять мой ротик, потому что это было только начало..."
    Kira_08 "[norestrain!t]Wow! You're done! It looks like you're still getting used to what my mouth can do, because that was just the beginning..."

# game/events/kira.rpy:2187
translate english kira_night_tv_minet_after_sex_f7abf3aa:

    # Max_02 "Значит, до следующего раза?!"
    Max_02 "So, until next time?!"

# game/events/kira.rpy:2194
translate english kira_night_tv_end_sex_3bfad864:

    # Kira_04 "Да, я буду ждать... А теперь бегом спать! А мне ещё нужно придти немного в себя после всего этого..."
    Kira_04 "Yes, I will wait... Now go to bed! And I still need to recover a little after all this..."

# game/events/kira.rpy:2195
translate english kira_night_tv_end_sex_469c1d2c:

    # Max_01 "Конечно, тётя Кира. Ты лучшая! Спокойной ночи!" nointeract
    Max_01 "Of course, Aunt Kira. You are the best! Goodnight!" nointeract

# game/events/kira.rpy:2201
translate english kira_night_tv_good_night_13e6e29b:

    # Kira_02 "Хорошо, Макс. Спокойной ночи! Я тоже уже ложусь спать... скоро."
    Kira_02 "All right, Max. Goodnight! I'm going to bed, too... soon."

# game/events/kira.rpy:2202
translate english kira_night_tv_good_night_2dcc842e:

    # Max_01 "Ага, приятных снов..."
    Max_01 "Yeah, sweet dreams..."

# game/events/kira.rpy:2208
translate english kira_night_tv_failed_45397e89:

    # Kira_14 "[failed!t]Приятно слышать, Макс, но тебе пора спать! Три часа ночи... Мне уже и самой пора ложиться спать."
    Kira_14 "[failed!t]Good to hear, Max, but it's time for you to go to bed! Three o'clock in the morning... It's time for me to go to bed myself."

# game/events/kira.rpy:2209
translate english kira_night_tv_failed_27592ad9:

    # Max_08 "Вот так всегда..."
    Max_08 "It's always like this..."

# game/events/kira.rpy:2212
translate english kira_night_tv_failed_120d2a41:

    # Max_09 "{m}Тётю Киру довольно трудно в чём-то убедить... Возможно, онлайн-курсы мне с этим помогут.{/m}"
    Max_09 "{m}Aunt Kira isn't easily convinced. I should take some online courses to make me more persuasive.{/m}"

# game/events/kira.rpy:2226
translate english kira_shower_90e1e372:

    # Max_00 "Кира сейчас принимает душ, не стоит ей мешать..." nointeract
    Max_00 "Kira is taking a shower now, so don't bother her..." nointeract

# game/events/kira.rpy:2231
translate english kira_shower_c78a15a2:

    # Max_01 "Интересно, кто сейчас в душе?" nointeract
    Max_01 "I wonder who's in the shower now?" nointeract

# game/events/kira.rpy:2244
translate english kira_shower_ladder_2b325faf:

    # Max_04 "Посмотрим, что у нас тут..."
    Max_04 "Let's see what we have here..."

# game/events/kira.rpy:2252
translate english kira_shower_ladder_547cf525:

    # Max_07 "Странно, что тётя Кира в полотенце... Она же любит везде посверкать своими почти голыми прелестями! Наверно, не проснулась ещё толком..."
    Max_07 "It's strange to see that she's wearing a towel. She probably hasn't fully woken up yet..."

# game/events/kira.rpy:2254
translate english kira_shower_ladder_ee7e4217:

    # Max_03 "Здорово, тётя Кира в одних трусиках любуется собой! Отпадные у неё сиськи, люблю эти аппетитные сосочки..."
    Max_03 "Aunt Kira's admiring herself in her panties! Her tits are so round and full, and I love her perky nipples..."

# game/events/kira.rpy:2256
translate english kira_shower_ladder_90b187c3:

    # Max_06 "Голая тётя Кира - это шикарно! Ей нужно в кино играть роковых красоток, с такой-то внешностью и фигурой..."
    Max_06 "Naked Aunt Kira is stunning! How has no one ever offered her a movie role? She should take advantage of those killer curves..."

# game/events/kira.rpy:2260
translate english kira_shower_ladder_f790b709:

    # Max_07 "Мои зрители явно пропускают много всего интересного! Мне однозначно стоит установить сюда ещё одну камеру..."
    Max_07 "My viewers are obviously missing out on a lot of great stuff! I should definitely install another camera here..."

# game/events/kira.rpy:2261
translate english kira_shower_ladder_90ff417e:

    # Max_00 "Хоть и не хочется, но пока меня не заметили, лучше уходить..."
    Max_00 "Even though I don't want to, I'd better leave before I'm noticed..."

# game/events/kira.rpy:2275
translate english kira_shower_start_peeping_ceaa567f:

    # Max_05 "Ага! Тётя Кира сегодня одна... и похоже, решила немножко себя развлечь принимая водные процедуры... Это я удачно зашёл!" nointeract
    Max_05 "Aha! Aunt Kira is alone today... and it seems that she decided to entertain herself a little... Aren't I lucky!" nointeract

# game/events/kira.rpy:2287
translate english kira_shower_start_peeping_adae6fbb:

    # Max_07 "Супер! Тётя Кира в душе... и совсем одна... такая голая и мокренькая... Вот это зрелище!" nointeract
    Max_07 "Great! Aunt Kira is in the shower... and all alone... so naked and wet... What a sight!" nointeract

# game/events/kira.rpy:2305
translate english kira_shower_start_peeping_1a39c5f6:

    # Max_03 "Ухх... Наблюдать за моей тётей просто загляденье! Её округлости и изящность движений очень возбуждают..." nointeract
    Max_03 "Uhh... Watching my Aunt is a sight to behold! Her curves and grace of movement are so alluring..." nointeract

# game/events/kira.rpy:2307
translate english kira_shower_start_peeping_a32344ac:

    # Max_05 "За этим можно долго смотреть, а лучше присоединиться... Не зря же меня тётя Кира пригласила!" nointeract
    Max_05 "I could admire the view, but Aunt Kira didn't invite me here just to watch!" nointeract

# game/events/kira.rpy:2315
translate english kira_shower_start_peeping_f934835c:

    # Max_03 "Ухх... Наблюдать за моей тётей просто загляденье! Её округлости и изящность движений очень возбуждают..."
    Max_03 "Uhh... Watching my Aunt is a sight to behold! Her roundness and grace of movement are very exciting..."

# game/events/kira.rpy:2317
translate english kira_shower_start_peeping_eab86a2d:

    # Max_05 "Вот так, тётя Кира... Хорошенько поласкай свою киску для меня! Ох, как же она этим наслаждается... Вот будет неловко, если она меня увидит! Хотя, ей уж точно не будет..."
    Max_05 "That's it, aunt Kira... Give your pussy a good rub for me! Oh, how she enjoys it... It would be awkward if she saw me! Although, she certainly won't..."

# game/events/kira.rpy:2328
translate english kira_shower_alt_peepeng_1a39c5f6:

    # Max_03 "Ухх... Наблюдать за моей тётей просто загляденье! Её округлости и изящность движений очень возбуждают..." nointeract
    Max_03 "Uhh... Watching my Aunt is a sight to behold! Her curves and grace of movement are so alluring..." nointeract

# game/events/kira.rpy:2330
translate english kira_shower_alt_peepeng_a32344ac:

    # Max_05 "За этим можно долго смотреть, а лучше присоединиться... Не зря же меня тётя Кира пригласила!" nointeract
    Max_05 "I could admire the view, but Aunt Kira didn't invite me here just to watch!" nointeract

# game/events/kira.rpy:2338
translate english kira_shower_alt_peepeng_f934835c:

    # Max_03 "Ухх... Наблюдать за моей тётей просто загляденье! Её округлости и изящность движений очень возбуждают..."
    Max_03 "Uhh... Watching my Aunt is a sight to behold! Her curves and grace of movement are so alluring..."

# game/events/kira.rpy:2340
translate english kira_shower_alt_peepeng_eab86a2d:

    # Max_05 "Вот так, тётя Кира... Хорошенько поласкай свою киску для меня! Ох, как же она этим наслаждается... Вот будет неловко, если она меня увидит! Хотя, ей уж точно не будет..."
    Max_05 "That's it, Aunt Kira... Play with your pussy for me! She's really focused... It would be awkward if she saw me! Although, something tells me she won't..."

# game/events/kira.rpy:2351
translate english kira_shower_promise_cuni_c2262ba3:

    # Kira_02 "А, Макс, конечно можно! Тебя и дожидаюсь. Присоединяйся..." nointeract
    Kira_02 "Oh, Max, of course you can! I'm waiting for you. Come..." nointeract

# game/events/kira.rpy:2356
translate english kira_shower_promise_cuni_6662fc98:

    # Kira_05 "А я тут стою, вся такая мокренькая и думаю - забыл обо мне мой племянничек или нет."
    Kira_05 "And I'm standing here, all wet and wondering if my nephew forgot about me or not."

# game/events/kira.rpy:2357
translate english kira_shower_promise_cuni_7e9dc3d1:

    # Max_03 "Как о тебе можно забыть, тётя Кира? Тебя нужно как следует порадовать за вчерашнее..." nointeract
    Max_03 "How can I forget about you, Aunt Kira? I just had to return the favour..." nointeract

# game/events/kira.rpy:2364
translate english kira_shower_promise_cuni_d3c0b1c3:

    # Kira_04 "Ох... Ты тот ещё шалунишка! Я обожаю, когда ласкают мою грудь и особенно мои сосочки... Ммм..." nointeract
    Kira_04 "Oh... You're a naughty boy! I love it when you look after my breasts and nipples... Mmm..." nointeract

# game/events/kira.rpy:2368
translate english kira_shower_promise_cuni_56e27392:

    # Kira_10 "Да, Макс... Продолжай вот так и я уже не смогу сдержаться... У тебя такие ловкие пальчики... Да..." nointeract
    Kira_10 "Yes, Max... Keep going like this and I won't be able to hold back... You have such nice fingers ... Yes..." nointeract

# game/events/kira.rpy:2372
translate english kira_shower_promise_cuni_7ca5775c:

    # Kira_11 "О да... Ещё, Макс... Я уже близко... Так приятно..." nointeract
    Kira_11 "Oh, Yes ... also, Max... I'm close... So good..." nointeract

# game/events/kira.rpy:2378
translate english kira_shower_promise_cuni_37a341b6:

    # Kira_12 "Ухх... Да, ещё... быстрее и глубже... Ммм, ещё чуть-чуть и я кончу... Ах!"
    Kira_12 "Uhh... Yes, more... faster and deeper... Mmm, just a little more and I'll come... Ah!"

# game/events/kira.rpy:2379
translate english kira_shower_promise_cuni_59052a0a:

    # Max_04 "Ну как, тётя Кира? Хорошо я тебя порадовал?"
    Max_04 "Well, Aunt Kira? Did I please you?"

# game/events/kira.rpy:2380
translate english kira_shower_promise_cuni_3a13b724:

    # Kira_06 "Ещё как... Мне очень понравилось... Ты очень способный... племянник..."
    Kira_06 "Absolutely... I really liked it... You're very capable... nephew..."

# game/events/kira.rpy:2384
translate english kira_shower_promise_cuni_fc122d84:

    # Max_07 "Скоро уже, должно быть, завтрак будет готов, так что надо тихонько расходиться, а то нас кто-нибудь увидит."
    Max_07 "Breakfast will probably be ready soon, so we should leave quietly before we get seen."

# game/events/kira.rpy:2385
translate english kira_shower_promise_cuni_6a25c28c:

    # Kira_07 "Да, Макс, я как раз должна немного... успеть отойти..."
    Kira_07 "Yes, Max, I just need a second..."

# game/events/kira.rpy:2403
translate english kira_lisa_shower_e1e82272:

    # Max_00 "Лиза и Кира принимают душ, не стоит им мешать..." nointeract
    Max_00 "Kira and Lisa are taking a shower, do not disturb them..." nointeract

# game/events/kira.rpy:2408
translate english kira_lisa_shower_c78a15a2:

    # Max_01 "Интересно, кто сейчас в душе?" nointeract
    Max_01 "I wonder who's in the shower now?" nointeract

# game/events/kira.rpy:2421
translate english kira_lisa_shower_ladder_2b325faf:

    # Max_04 "Посмотрим, что у нас тут..."
    Max_04 "Let's see what we have here..."

# game/events/kira.rpy:2460
translate english kira_lisa_shower_ladder_aa3e76cc:

    # Max_07 "Странно, что тётя Кира в полотенце... Она же любит везде посверкать своими почти голыми прелестями! Наверно, не проснулась ещё толком, а за кадром ещё и Лиза принимает душ!"
    Max_07 "It's strange to see that she's wearing a towel. She probably hasn't fully woken up yet. And Lisa is in the shower!"

# game/events/kira.rpy:2462
translate english kira_lisa_shower_ladder_22c3d9c6:

    # Max_03 "Здорово, тётя Кира в одних трусиках любуется собой, а за кадром ещё и Лиза принимает душ! Отпадные у неё сиськи, люблю эти аппетитные сосочки..."
    Max_03 "Aunt Kira's admiring herself in her panties, and Lisa is in the shower. Her tits are so round and full, and I love her perky nipples..."

# game/events/kira.rpy:2464
translate english kira_lisa_shower_ladder_fbd5fc9b:

    # Max_06 "Голая тётя Кира - это шикарно, а за кадром ещё и Лиза принимает душ! Ей нужно в кино играть роковых красоток, с такой-то внешностью и фигурой..."
    Max_06 "Naked Aunt Kira is stunning! And it looks like Lisa is in the shower. How has no one ever offered her a movie role? She should take advantage of those killer curves..."

# game/events/kira.rpy:2482
translate english kira_lisa_shower_ladder_df5b66fd:

    # Max_03 "Лиза смотрится в подаренном мною халатике очень соблазнительно... Особенно когда так хорошо видно её упругие сисечки, а за кадром ещё и тётя Кира принимает душ!"
    Max_03 "Lisa looks very inviting in that robe... Especially when you can see her tits so clearly! Aunt Kira seems to be in the shower."

# game/events/kira.rpy:2484
translate english kira_lisa_shower_ladder_ba9256f1:

    # Max_07 "Моя обворожительная сестрёнка в одних трусиках... Так и хочется зайти и стянуть их с её прекрасной попки, а за кадром ещё и тётя Кира принимает душ!"
    Max_07 "My adorable sister is in her panties... I just want to come in and pull them off her beautiful ass! And Aunt Kira is in the shower!"

# game/events/kira.rpy:2486
translate english kira_lisa_shower_ladder_14a55f78:

    # Max_06 "Утро может быть действительно очень добрым, если удаётся полюбоваться совершенно голенькой Лизой, а за кадром ещё и тётя Кира принимает душ!"
    Max_06 "It's a great morning when I can see Lisa completely naked. And Aunt Kira is in the shower."

# game/events/kira.rpy:2493
translate english kira_lisa_shower_ladder_79519da7:

    # Max_05 "Отлично, сегодня Лиза с тётей Кирой вместе! Красуются перед зеркалом в одних трусиках... Это просто заглядение!"
    Max_05 "Great, today Lisa and Aunt Kira are together! They're both in the mirror wearing underwear... This is just a sight to behold!"

# game/events/kira.rpy:2495
translate english kira_lisa_shower_ladder_30333acb:

    # Max_06 "Ухх, невероятно горячие и голенькие тётя с моей младшей сестрёнкой! Любовался бы бесконечно, как они красуются перед зеркалом..."
    Max_06 "Nothing like seeing my sexy Aunt naked with my little sister! I'd love to get between them..."

# game/events/kira.rpy:2500
translate english kira_lisa_shower_ladder_f790b709:

    # Max_07 "Мои зрители явно пропускают много всего интересного! Мне однозначно стоит установить сюда ещё одну камеру..."
    Max_07 "My viewers are obviously missing out on a lot of great stuff! I should definitely install another camera here..."

# game/events/kira.rpy:2501
translate english kira_lisa_shower_ladder_87af87e8:

    # Max_00 "Ладно, хорошего понемногу, а то ещё заметит меня здесь кто-нибудь..."
    Max_00 "Alright, that'll do, before I get noticed."

# game/events/kira.rpy:2510
translate english kira_lisa_shower_start_peeping_c64a3986:

    # Max_07 "Отлично! Лиза вместе с тётей Кирой сегодня оказались в одно и то же время в душе... Очень соблазнительно!" nointeract
    Max_07 "Great! Lisa and Aunt Kira were in the shower at the same time today... Very tempting!" nointeract

# game/events/kira.rpy:2526
translate english kira_lisa_shower_start_peeping_ddbc59ae:

    # Max_04 "Эти две киски такие мокрые... Глаз не оторвать! Ну и как в этом доме не быть извращенцем?!"
    Max_04 "These two pussies are so wet... I can't take my eyes off you! How can I not be a pervert in this house?"

# game/events/kira.rpy:2540
translate english kira_lisa_shower_alt_peepeng_ddbc59ae:

    # Max_04 "Эти две киски такие мокрые... Глаз не оторвать! Ну и как в этом доме не быть извращенцем?!"
    Max_04 "These two pussies are so wet... I can't take my eyes off! How can I not be a pervert in this house?"

# game/events/kira.rpy:2553
translate english kira_alice_shower_d719e4e2:

    # Max_00 "Кира и Алиса принимают душ, не стоит им мешать..." nointeract
    Max_00 "Kira and Alice are taking a shower, don't bother them..." nointeract

# game/events/kira.rpy:2558
translate english kira_alice_shower_c78a15a2:

    # Max_01 "Интересно, кто сейчас в душе?" nointeract
    Max_01 "I wonder who's in the shower now?" nointeract

# game/events/kira.rpy:2571
translate english kira_alice_shower_ladder_2b325faf:

    # Max_04 "Посмотрим, что у нас тут..."
    Max_04 "Let's see what we got here..."

# game/events/kira.rpy:2609
translate english kira_alice_shower_ladder_d9e41a49:

    # Max_07 "Странно, что тётя Кира в полотенце... Она же любит везде посверкать своими почти голыми прелестями! Наверно, не проснулась ещё толком, а за кадром ещё и Алиса принимает душ!"
    Max_07 "It's strange that aunt Kira is wearing a towel... She probably hasn't woken up yet, and Alice is also taking a shower!"

# game/events/kira.rpy:2611
translate english kira_alice_shower_ladder_716d82cb:

    # Max_03 "Здорово, тётя Кира в одних трусиках любуется собой, а за кадром ещё и Алиса принимает душ! Отпадные у неё сиськи, люблю эти аппетитные сосочки..."
    Max_03 "Great, Aunt Kira is admiring herself in just her panties, and Alice is also taking a shower!"

# game/events/kira.rpy:2613
translate english kira_alice_shower_ladder_78119736:

    # Max_06 "Голая тётя Кира - это шикарно, а за кадром ещё и Алиса принимает душ! Ей нужно в кино играть роковых красоток, с такой-то внешностью и фигурой..."
    Max_06 "Naked Aunt Kira is stunning! And it looks like Alice is in the shower. How has no one ever offered her a movie role? She should take advantage of those killer curves..."

# game/events/kira.rpy:2629
translate english kira_alice_shower_ladder_0159a8ee:

    # Max_02 "Может Алиса и в халатике, но сиськи её видны просто замечательно, а за кадром ещё и тётя Кира принимает душ!"
    Max_02 "Alice may be in her gown, but her tits are visible just fine, and Aunt Kira is taking a shower!"

# game/events/kira.rpy:2631
translate english kira_alice_shower_ladder_1d3dd6d3:

    # Max_04 "Алиса сегодня без халатика... в одних трусиках... Гдядя на эту красоту, можно мечтать лишь об одном, а за кадром ещё и тётя Кира принимает душ!"
    Max_04 "Alice isn't wearing a robe today... there's no way I can help but get excited looking at her. And Aunt Kira is taking a shower."

# game/events/kira.rpy:2633
translate english kira_alice_shower_ladder_36f201a2:

    # Max_06 "Алиса сегодня совершенно голая! И даже не представляет, что тем самым дарит мне возможность любоваться всеми её прелестями, а за кадром ещё и тётя Кира принимает душ!"
    Max_06 "Alice is completely naked today! And she doesn't even know that this gives me the opportunity to admire her. Even Aunt Kira is taking a shower."

# game/events/kira.rpy:2641
translate english kira_alice_shower_ladder_e400863b:

    # Max_05 "Класс, две самые непослушные девчонки этого дома вместе! Так изящно позируют перед зеркалом... Глаза аж разбегаются!"
    Max_05 "Yes! The two naughtiest girls in this house at the same time! I can't take my eyes off them!"

# game/events/kira.rpy:2643
translate english kira_alice_shower_ladder_40f09602:

    # Max_06 "О да, сегодня тётя Кира и Алиса зажигают своими голыми попками перед зеркалом! Это можно прировнять к новому чуду света..."
    Max_06 "Aunt Kira and Alice are finally both naked together! This is a moment I'll have etched into my memory..."

# game/events/kira.rpy:2648
translate english kira_alice_shower_ladder_f790b709:

    # Max_07 "Мои зрители явно пропускают много всего интересного! Мне однозначно стоит установить сюда ещё одну камеру..."
    Max_07 "My viewers are obviously missing out on a lot of great stuff! I should definitely install another camera here..."

# game/events/kira.rpy:2649
translate english kira_alice_shower_ladder_87af87e8:

    # Max_00 "Ладно, хорошего понемногу, а то ещё заметит меня здесь кто-нибудь..."
    Max_00 "Alright, that'll do, before I get noticed..."

# game/events/kira.rpy:2658
translate english kira_alice_shower_start_peeping_15451dbe:

    # Max_07 "Ого... Две очень плохие девочки сегодня моются вместе... тётя Кира и Алиса! Как же они хороши..." nointeract
    Max_07 "Wow... Two very bad girls are washing together today... Aunt Kira and Alice! Very nice..." nointeract

# game/events/kira.rpy:2674
translate english kira_alice_shower_start_peeping_2ddf2244:

    # Max_02 "О, да... Поскользить чем-нибудь между их сисечками было бы невероятно круто!"
    Max_02 "Oh, Yes... Slipping something between their tits would be a dream!"

# game/events/kira.rpy:2688
translate english kira_alice_shower_alt_peepeng_72bf06b5:

    # Max_03 "Да-а-а... Вот бы оказаться между двумя этими мокрыми попками... Я бы уж их помылил!"
    Max_03 "I'd love to be in between those wet asses... I'd clean them properly!"

# game/events/kira.rpy:2704
translate english return_from_club_ad7639d7:

    # Kira_03 "Алиса, тише... Все уже спят! Нужно тихо, тихо, тихо... О... Макс? А ты почему не спишь?!"
    Kira_03 "Alice, hush... Everyone's asleep! We need to be quiet... Oh... Max? Why aren't you in bed?!"

# game/events/kira.rpy:2705
translate english return_from_club_6f56009f:

    # Max_02 "Да что-то не спится..."
    Max_02 "I can't sleep..."

# game/events/kira.rpy:2709
translate english return_from_club_b2446dcf:

    # Kira_05 "Это зря... Надо спать... Мы вот с Алисой тоже собираемся спать... Конечно, не вместе... Хотя... Она такая классная!"
    Kira_05 "Well you'd better go to bed. Alice and I are going too... Not together, of course, even though I don't think she'd be entirely against the idea!"

# game/events/kira.rpy:2710
translate english return_from_club_4f524a35:

    # Max_07 "Кажется, кто-то пьян..."
    Max_07 "Someone seems a little tipsy..."

# game/events/kira.rpy:2711
translate english return_from_club_b5fd31fe:

    # Alice_02 "Ага, еле дотащила её. А ведь она сильная. Ещё и сопротивляется! Хотела в какой-то другой ночной клуб пойти, продолжить тусить... Еле отговорила!"
    Alice_02 "I could hardly carry her! She's tougher than she looks. She thought it'd be a great idea to keep partying and wanted to go to another club... I barely talked her out of it!"

# game/events/kira.rpy:2712
translate english return_from_club_8fb778db:

    # Max_08 "А ты трезва?"
    Max_08 "And you're sober?"

# game/events/kira.rpy:2713
translate english return_from_club_3539a73a:

    # Alice_05 "Конечно, Макс! Ты же знаешь, что я не пью. Да и нужно было кому-то приглядывать за тётей Кирой. Она очень любит развлекаться, да так, что стыдно рассказать..."
    Alice_05 "Of course, Max! You know I don't drink. And someone has to keep an eye on Aunt Kira. She likes to have a lot of fun, as you can see..."

# game/events/kira.rpy:2714
translate english return_from_club_8ed21348:

    # Max_07 "И что было?"
    Max_07 "So what happened?"

# game/events/kira.rpy:2715
translate english return_from_club_ecb03d5b:

    # Kira_07 "Ой, Макс... Чего только не было... И стриптизёры, и стриптиз... И я даже..."
    Kira_07 "Oh, Max... There's been all kinds of things... Like strippers, and stripping... And I've even..."

# game/events/kira.rpy:2716
translate english return_from_club_7524e1f4:

    # Max_08 "Что?"
    Max_08 "What?"

# game/events/kira.rpy:2717
translate english return_from_club_02edae7e:

    # Alice_06 "А вот это уже не обязательно рассказывать. Я как вспомню, краснеть от стыда начинаю. Скажу лишь, что стриптиз танцевала тётя Кира, а остальное совсем не для твоих ушей..."
    Alice_06 "You don't have to go into it! It's embarrassing to hear. Let's just say Aunt Kira got a little frisky and leave it at that."

# game/events/kira.rpy:2718
translate english return_from_club_9bbffc91:

    # Max_03 "Ого... Я всё пропустил!"
    Max_03 "Wow... I missed everything!"

# game/events/kira.rpy:2719
translate english return_from_club_5fc4d28e:

    # Kira_06 "Точно! Нужно будет взять Макса с собой в следующий раз!"
    Kira_06 "That's right! We'll have to take Max with us next time!"

# game/events/kira.rpy:2720
translate english return_from_club_9e8526f9:

    # Max_04 "Я только за!"
    Max_04 "I'm all for that!"

# game/events/kira.rpy:2721
translate english return_from_club_c0085d94:

    # Alice_03 "Не сомневаюсь, Макс. Но всё, хватит разговоров. Я пойду переодену и уложу тётю Киру в гостиной, а ты иди в свою комнату и ложись спать. Пока!" nointeract
    Alice_03 "I'm sure you are, but that's enough for tonight. I'll get changed and put Aunt Kira to bed in the living room. You should call it a night too!" nointeract

# game/events/kira.rpy:2724
translate english return_from_club_fb094ae4:

    # Alice_05 "Наверняка из-за момента, что её нужно переодеть, да? Какой же ты испорченный мальчишка, Макс!"
    Alice_05 "Probably because she needs to get changed? You're such a spoiled brat!"

# game/events/kira.rpy:2725
translate english return_from_club_65dce436:

    # Max_09 "Мне теперь и тёте любимой нельзя помочь без упрёков?! Может, это вы тут все испорченные, а я один адекватный. Тётя Кира, ты ведь не против, если я тебе помогу?"
    Max_09 "What, I can't even help my Aunt without you judging me?! Maybe you're the one who's spoiled! Aunt Kira, you don't mind if I help, do you?"

# game/events/kira.rpy:2726
translate english return_from_club_7471c8c0:

    # Kira_05 "Ой, Макс... Я просто не могу отказать такому галантному вниманию. Алиса, можешь гулять... Макс обо мне позаботится."
    Kira_05 "Oh, Max... I just can't refuse such a generous offer... Alice, it's okay. Max will take care of me."

# game/events/kira.rpy:2727
translate english return_from_club_051ee7df:

    # Alice_12 "Ну как знаете! Мне и без этого есть чем заняться. Доброй ночи."
    Alice_12 "Suit yourself! I've dealt with enough this evening as is. Good night!"

# game/events/kira.rpy:2728
translate english return_from_club_7b99fe1e:

    # Max_01 "Ага, доброй. Пойдём, тётя Кира, я тебя провожу..." nointeract
    Max_01 "Yeah, whatever. Come on, Aunt Kira, I'll walk you out..." nointeract

# game/events/kira.rpy:2738
translate english return_from_club_ece3a784:

    # Kira_02 "Должна сказать, Макс, я довольно неплохо иду... если учесть, сколько я выпила..."
    Kira_02 "I have to say, I'm doing pretty well considering how much I've drank..."

# game/events/kira.rpy:2739
translate english return_from_club_165edfa1:

    # Max_02 "Да, но я тебя всё равно немного придержу, чтобы никаких казусов не вышло. Приятное платьице... на ощупь."
    Max_02 "Well, let's just take it easy now so there's no accidents... That's a nice dress, by the way."

# game/events/kira.rpy:2740
translate english return_from_club_ca48013f:

    # Kira_05 "Да, я его обожаю... Оно так классно облегает мои формы! Никто не остаётся равнодушным... к моей попке... за которую ты меня так мило придерживаешь."
    Kira_05 "Yes, I love it... It fits me perfectly! It really suits my ass, which I can see you're holding on to..."

# game/events/kira.rpy:2741
translate english return_from_club_d85c0008:

    # Max_04 "Так удобнее всего тебя направлять... А может расскажешь сейчас, что там ты за стриптиз устроила?"
    Max_04 "It's the best way to keep you from losing balance... Why don't you tell me what kind of striptease you did?"

# game/events/kira.rpy:2747
translate english return_from_club_c199a7b0:

    # Kira_07 "Зачем рассказывать, Макс, когда можно показать?! Присаживайся, если хочешь посмотреть это шоу..."
    Kira_07 "Hmmm, you know the rule - Show don't tell? Why don't you sit down?"

# game/events/kira.rpy:2748
translate english return_from_club_3b9356a1:

    # Max_05 "Ого! Такого поворота событий я не ожидал!"
    Max_05 "Wow! I didn't expect it to come to this!"

# game/events/kira.rpy:2755
translate english return_from_club_8cd28747:

    # Kira_05 "И правильно! Такая ночь, как тут спать... Мы вот с Алисой отлично провели время. Оказывается, твоя старшая сестрёнка умеет зажигать!"
    Kira_05 "And rightly so! How could you even sleep on a night like this... Alice and I had a great time, turns out your sister is a party animal!"

# game/events/kira.rpy:2756
translate english return_from_club_e5a34f9f:

    # Max_03 "Ого. Рассказывайте, что было?"
    Max_03 "Wow. Tell me, what happened?"

# game/events/kira.rpy:2762
translate english return_from_club_4e6931d0:

    # Kira_07 "Скажу по секрету... Это касается трусиков Алисы! Но это секрет, так ведь?"
    Kira_07 "I'll tell you a secret... It's about your panties! But you'll keep it between us, right?"

# game/events/kira.rpy:2763
translate english return_from_club_1d4756e8:

    # Alice_05 "Именно! И Макса мы в это посвящать не будем... К тому же даже сама тётя Кира сейчас без трусиков!"
    Alice_05 "Exactly! And we won't tell Max about it... Besides, you're not even wearing panties right now!"

# game/events/kira.rpy:2764
translate english return_from_club_29fbae93:

    # Max_04 "Какие пикантные секреты!"
    Max_04 "Spicy!"

# game/events/kira.rpy:2765
translate english return_from_club_5145f6a5:

    # Kira_02 "Так, Макс, я отправляюсь спать. Проводишь меня до дивана?"
    Kira_02 "OK, Max, I'm off to bed. Can you walk me to the couch?"

# game/events/kira.rpy:2766
translate english return_from_club_1215df56:

    # Alice_06 "Эй, а как же я?! Я хочу принять ванну перед сном, а у меня полотенце наверху! Вдруг я упаду, пока буду подниматься..."
    Alice_06 "Hey, what about me?! I want to take a bath before bed and I have a towel upstairs! I might fall over on my way up..."

# game/events/kira.rpy:2767
translate english return_from_club_ba66703f:

    # Max_07 "Я принесу тебе полотенце, как только уложу тётю Киру спать. Идёт?"
    Max_07 "I'll get you a towel as soon as I've put Aunt Kira to bed. All right?"

# game/events/kira.rpy:2768
translate english return_from_club_fde1b591:

    # Alice_12 "Только давай быстрее!" nointeract
    Alice_12 "Just make it quick!" nointeract

# game/events/kira.rpy:2773
translate english return_from_club_f9df4462:

    # Max_01 "Вот, держи конфетку, чтобы не скучать, пока я с тётей..."
    Max_01 "Here, have some candy so you don't get bored while I'm with her..."

# game/events/kira.rpy:2774
translate english return_from_club_babe3fd1:

    # Alice_02 "Ой, спасибо, это я люблю, Макс. На ночь лучше, конечно, сладости не есть, но разок можно. Ничего плохого не случится. Всё, жду полотенце..."
    Alice_02 "Oh, thank you, I love that, Max. You probably shouldn't have sweets at night, but once in a while won't hurt. I'll be waiting for the towel..."

# game/events/kira.rpy:2781
translate english return_from_club_ece3a784_1:

    # Kira_02 "Должна сказать, Макс, я довольно неплохо иду... если учесть, сколько я выпила..."
    Kira_02 "I have to say, I'm going pretty well... considering how much I've been drinking..."

# game/events/kira.rpy:2782
translate english return_from_club_165edfa1_1:

    # Max_02 "Да, но я тебя всё равно немного придержу, чтобы никаких казусов не вышло. Приятное платьице... на ощупь."
    Max_02 "Well, let's just take it easy now so there's no accidents... That's a nice dress, by the way."

# game/events/kira.rpy:2783
translate english return_from_club_ca48013f_1:

    # Kira_05 "Да, я его обожаю... Оно так классно облегает мои формы! Никто не остаётся равнодушным... к моей попке... за которую ты меня так мило придерживаешь."
    Kira_05 "Yes, I love it... It fits me perfectly! It really suits my ass, which I can see you're holding on to..."

# game/events/kira.rpy:2784
translate english return_from_club_d85c0008_1:

    # Max_04 "Так удобнее всего тебя направлять... А может расскажешь сейчас, что там ты за стриптиз устроила?"
    Max_04 "It's the best way to keep you from losing balance... Why don't you tell me what kind of striptease you did?"

# game/events/kira.rpy:2790
translate english return_from_club_72f9af45:

    # Kira_07 "Зачем рассказывать, Макс, когда можно показать?! Присаживайся, если хочешь посмотреть это шоу..." nointeract
    Kira_07 "Hmmm, you know the rule- Show don't tell? Why don't you sit down?" nointeract

# game/events/kira.rpy:2804
translate english return_from_club_ece3a784_2:

    # Kira_02 "Должна сказать, Макс, я довольно неплохо иду... если учесть, сколько я выпила..."
    Kira_02 "I have to say, I'm doing pretty well considering how much I've drank..."

# game/events/kira.rpy:2805
translate english return_from_club_165edfa1_2:

    # Max_02 "Да, но я тебя всё равно немного придержу, чтобы никаких казусов не вышло. Приятное платьице... на ощупь."
    Max_02 "Well, let's just take it easy now so there's no accidents... That's a nice dress, by the way."

# game/events/kira.rpy:2806
translate english return_from_club_ca48013f_2:

    # Kira_05 "Да, я его обожаю... Оно так классно облегает мои формы! Никто не остаётся равнодушным... к моей попке... за которую ты меня так мило придерживаешь."
    Kira_05 "Yes, I love it... It fits me perfectly! It really suits my ass, which I can see you're holding on to..."

# game/events/kira.rpy:2807
translate english return_from_club_d85c0008_2:

    # Max_04 "Так удобнее всего тебя направлять... А может расскажешь сейчас, что там ты за стриптиз устроила?"
    Max_04 "It's the best way to keep you from losing balance... Why don't you tell me what kind of striptease you did?"

# game/events/kira.rpy:2813
translate english return_from_club_72f9af45_1:

    # Kira_07 "Зачем рассказывать, Макс, когда можно показать?! Присаживайся, если хочешь посмотреть это шоу..." nointeract
    Kira_07 "Hmmm, you know the rule- Show don't tell? Why don't you sit down?" nointeract

# game/events/kira.rpy:2828
translate english return_from_club_striptease_f36bfafe:

    # Kira_02 "А вот твой дружок уже отреагировал на то, как я приспустила платье... Я постараюсь, чтобы ты наслаждался каждой секундой этого стриптиза."
    Kira_02 "But your friend already reacted to the way I lowered my dress... I'll make sure you enjoy every second of this striptease."

# game/events/kira.rpy:2829
translate english return_from_club_striptease_36ca198c:

    # Max_06 "Я просто нереально рад, что у меня такая потрясающая тётя!"
    Max_06 "I'm just incredibly happy that I have such an amazing aunt!"

# game/events/kira.rpy:2836
translate english return_from_club_striptease_c1d6cb9b:

    # Kira_04 "Ох! Кажется, я забыла надеть трусики, когда отправилась в клуб... И как же такое могло со мной произойти?!"
    Kira_04 "Oh! I think I forgot to put on my panties when I went to the club... How could this happen to me?!"

# game/events/kira.rpy:2837
translate english return_from_club_striptease_348c88bb:

    # Max_02 "Всё, что не делается - делается к лучшему! Забыла и забыла, я совсем не против..."
    Max_02 "Sometimes things happen for a reason! I definitely don't mind..."

# game/events/kira.rpy:2844
translate english return_from_club_striptease_49cfce3b:

    # Kira_07 "Тебе нравится, как я двигаюсь? Уверена, ты уже представил, как эта попка с жаром прыгает на твоём огромном члене! Ухх, я и сама начинаю намокать от этих мыслей..."
    Kira_07 "Do you like the way I move? I'm sure you've already imagined my ass bouncing on your huge cock! Ugh, I'm starting to get wet just thinking about it..."

# game/events/kira.rpy:2845
translate english return_from_club_striptease_c050609d:

    # Max_03 "Так может стоит в самом деле немного попрыгать перед сном?"
    Max_03 "A little bouncing around before bed might be a good idea."

# game/events/kira.rpy:2852
translate english return_from_club_striptease_1c4be560:

    # Kira_10 "О да, звучит очень заманчиво, Макс! Но у меня так сильно начала кружиться голова! Похоже, я сегодня немного перебрала с алкоголем... Мне лучше прилечь."
    Kira_10 "Oh, yes, that sounds so tempting! But I'm way too dizzy from drinking... I'd better just lie down. "

# game/events/kira.rpy:2853
translate english return_from_club_striptease_c25227f5:

    # Max_07 "Конечно, тётя Кира, ложись... Значит, никакого продолжения не будет?"
    Max_07 "Oh sure, lie down... So I won't be getting any?"

# game/events/kira.rpy:2859
translate english return_from_club_striptease_bc0fb313:

    # Kira_14 "Боюсь, это не закончится ничем хорошим! Может быть в следующий раз..."
    Kira_14 "I'm afraid it wouldn't end well! Maybe next time..."

# game/events/kira.rpy:2860
translate english return_from_club_striptease_8e9ce92e:

    # Max_08 "Вот облом! Ладно, отдыхай и в следующий раз так много не пей. Доброй ночи, тётя Кира."
    Max_08 "What a bummer! Well, get some rest and don't drink so much next time. Good night, Aunt Kira."

# game/events/kira.rpy:2861
translate english return_from_club_striptease_b45fd168:

    # Kira_02 "Я постараюсь. Приятных снов, Макс." nointeract
    Kira_02 "I'll do my best. Have a good night's sleep, Max." nointeract

# game/events/kira.rpy:2880
translate english return_from_club_striptease_513073a9:

    # Kira_05 "О да, звучит очень заманчиво, Макс! Но этой ночью я хочу дать тебе выбор... как закончится мой стриптиз... И это будет сложный выбор! Ты же не против, если мы снимем твои шорты?"
    Kira_05 "Oh yes, that sounds very tempting, Max! But tonight I'm going to give you a difficult choice as to how my striptease will end! You don't mind if we take your shorts off, do you?"

# game/events/kira.rpy:2881
translate english return_from_club_striptease_ab1dbcf5:

    # Max_04 "Конечно! Мне уже нравится, к чему всё идёт..."
    Max_04 "Not at all! I like where this is going already..."

# game/events/kira.rpy:2887
translate english return_from_club_striptease_01774195:

    # Kira_07 "Выбирай с умом, Макс, потому что ты получишь лишь что-то одно! Я могу обласкать твой член либо грудью, либо попкой. Как ты хочешь, чтобы я это сделала?" nointeract
    Kira_07 "Choose wisely, Max, because you can't have both! I can rub your cock with either my tits or my ass. How do you want it?" nointeract

# game/events/kira.rpy:2895
translate english return_from_club_striptease_5da9e65f:

    # Kira_02 "Ты уверен, что хочешь именно так? Может, мне стоит немного покрутить этой роскошной попкой, чтобы ты понял, что теряешь сегодня?"
    Kira_02 "Are you sure this is how you want it? Maybe I should give my gorgeous ass a little twist to make you realise what you're missing today."

# game/events/kira.rpy:2896
translate english return_from_club_striptease_6057aa1a:

    # Max_07 "Ухх, чёрт! Тётя Кира, когда ты так красиво это делаешь, выбор и правда становится очень сложным!" nointeract
    Max_07 "Ugh, shit! Aunt Kira, it's much harder to make a choice when you put it that way!" nointeract

# game/events/kira.rpy:2903
translate english return_from_club_striptease_a466dbaf:

    # Kira_05 "Значит, всё-таки хочешь эти сисечки, да, большой мальчик? И я не могу тебя за это винить! Я знаю, какой это был непростой для тебя выбор. Надеюсь, тебе понравится..."
    Kira_05 "So you want these tits after all, huh, big boy? I can't blame you! I know what a tough choice this has been, I hope you like it..."

# game/events/kira.rpy:2904
translate english return_from_club_striptease_fef1de0b:

    # Max_05 "Мне с самого начала это нравится всё больше и больше!"
    Max_05 "I've been enjoying it more and more since the beginning!"

# game/events/kira.rpy:2916
translate english return_from_club_striptease_4320cff4:

    # Kira_05 "Значит, всё-таки хочешь эту попку, да, большой мальчик? И я не могу тебя за это винить! Я знаю, какой это был непростой для тебя выбор. Надеюсь, тебе понравится..."
    Kira_05 "So you want this ass after all, huh, big boy? I can't blame you! I know what a tough choice this has been, I hope you like it..."

# game/events/kira.rpy:2917
translate english return_from_club_striptease_7a52f47d:

    # Max_05 "Мне не переставало это нравится самого начала. И чем дальше, тем больше!"
    Max_05 "It's been so hard to resist from the very start!"

# game/events/kira.rpy:2926
translate english return_from_club_striptease_e77d55be:

    # Kira_02 "Ты уверен, что хочешь именно так? Может, мне стоит немного покрутить этой роскошной грудью перед твоим лицом, чтобы ты понял, что теряешь сегодня?"
    Kira_02 "Are you sure this is how you want it? Maybe I should give put these puppies in front of your face to make you realise what you're missing today."

# game/events/kira.rpy:2927
translate english return_from_club_striptease_6057aa1a_1:

    # Max_07 "Ухх, чёрт! Тётя Кира, когда ты так красиво это делаешь, выбор и правда становится очень сложным!" nointeract
    Max_07 "Ugh, shit! Aunt Kira, it's much harder to make a choice when you put it that way!" nointeract

# game/events/kira.rpy:2935
translate english return_from_club_striptease_4320cff4_1:

    # Kira_05 "Значит, всё-таки хочешь эту попку, да, большой мальчик? И я не могу тебя за это винить! Я знаю, какой это был непростой для тебя выбор. Надеюсь, тебе понравится..."
    Kira_05 "So you want this ass after all, huh, big boy? I can't blame you! I know what a tough choice this has been, I hope you like it..."

# game/events/kira.rpy:2936
translate english return_from_club_striptease_7a52f47d_1:

    # Max_05 "Мне не переставало это нравится самого начала. И чем дальше, тем больше!"
    Max_05 "It's been so hard to resist from the very start!"

# game/events/kira.rpy:2948
translate english return_from_club_striptease_a466dbaf_1:

    # Kira_05 "Значит, всё-таки хочешь эти сисечки, да, большой мальчик? И я не могу тебя за это винить! Я знаю, какой это был непростой для тебя выбор. Надеюсь, тебе понравится..."
    Kira_05 "So you want these tits after all, huh, big boy? I can't blame you! I know what a tough choice this has been, I hope you like it..."

# game/events/kira.rpy:2949
translate english return_from_club_striptease_fef1de0b_1:

    # Max_05 "Мне с самого начала это нравится всё больше и больше!"
    Max_05 "I've been enjoying it more and more since the beginning!"

# game/events/kira.rpy:2953
translate english return_from_club_boobs_45f4e1e3:

    # Kira_06 "Ого! Похоже, наши с тобой размеры идеально совпали! Ну как, тебе нравятся ощущения, Макс? Нравится, как я это делаю? Или хочешь быстрее?"
    Kira_06 "Wow! Looks like you and I are a perfect match! So, do you like the way it feels, Max? Do you like the way I do it? Or do you want it faster?"

# game/events/kira.rpy:2954
translate english return_from_club_boobs_2c5d28e1:

    # Max_06 "Да-а-а, у тебя такая большая и нежная грудь! Обалденные ощущения! Главное, не останавливайся и продолжай..."
    Max_06 "Yes, your tits are so big and soft! It feels great! Just don't stop..."

# game/events/kira.rpy:2960
translate english return_from_club_boobs_afe4ad00:

    # Kira_09 "Ох, я чувствую, что твоему дружку очень приятно находится в такой тесной компании моих девочек! Он пульсирует всё горячее..."
    Kira_09 "Oh, I can feel your friend is really enjoying being in such close company with my girls! He's throbbing all over..."

# game/events/kira.rpy:2961
translate english return_from_club_boobs_404339d6:

    # Max_20 "{m}Я без ума от этих сисек! Такие мягкие и большие... Она двигается всё быстрее и мне хочется начать хорошенько трахать её между них... Но что-то не так...{/m}"
    Max_20 "{m}I'm crazy about her tits! So soft and big... She's moving faster and faster, I want to fuck them good... But something's wrong...{/m}"

# game/events/kira.rpy:2967
translate english return_from_club_boobs_2d69d825:

    # Kira_10 "Ой, Макс, у меня так сильно начала кружиться голова! Похоже, я сегодня немного перебрала с алкоголем... Мне нужно немного передохнуть и мы продолжим, хорошо?"
    Kira_10 "Oh, Max, I'm getting so dizzy! I think I had a little too much alcohol today... I need to take a break, okay?"

# game/events/kira.rpy:2968
translate english return_from_club_boobs_a38e6fb8:

    # Max_08 "Ну ладно. А ты уверена, что сможешь? Эээ... Тётя Кира? Заснула... И похоже, что крепко. Ну какой же это облом! Нужно уложить её на диван, пусть отдыхает."
    Max_08 "All right, are you sure? Erm... Aunt Kira? She's out cold. Well, that's a bummer. I should put her down on the couch and let her rest."

# game/events/kira.rpy:2975
translate english return_from_club_boobs_20ab4298:

    # Max_01 "{i}Надеюсь, мама не будет сильно её осуждать за то, что она легла спать голая... С кем не бывает! А хороша тётя, так и гипнотизирует своей красотой. Но пора и мне спать...{/i}" nointeract
    Max_01 "{i}I hope her Mom doesn't judge her too much for going to bed naked... With a body like hers, who wouldn't? She's hypnotising me with her beauty. But it's time for me to go to bed...{/i}" nointeract

# game/events/kira.rpy:2986
translate english return_from_club_ass_5e3c81e6:

    # Kira_06 "О да! Твой член так плотно там поместился! Ну как, тебе нравятся ощущения, Макс? Нравится, как я это делаю? Или хочешь быстрее?"
    Kira_06 "Oh yes! Your cock fits so tight in there! How does it feel, Max? Do you like the way I do it? Or do you want it faster?"

# game/events/kira.rpy:2987
translate english return_from_club_ass_aeaa8ecb:

    # Max_06 "Да-а-а, у тебя такая шикарная и упругая попка! Непередаваемые ощущения! Главное, не останавливайся и продолжай..."
    Max_06 "Yeah, you have such a nice ass! I can't believe how it feels. Just keep going..."

# game/events/kira.rpy:2993
translate english return_from_club_ass_8b58636c:

    # Kira_09 "Ох, я чувствую, что твоему дружку очень приятно находится в такой тесной компании моих ягодиц! Он пульсирует всё горячее..."
    Kira_09 "Oh, I can feel your friend is really enjoying being between my ass! He's throbbing all over..."

# game/events/kira.rpy:2994
translate english return_from_club_ass_e058747b:

    # Max_20 "{m}Эта попка просто чумовая! Как же приятно она трётся... двигается всё быстрее! Мне хочется взять и хорошенько оттрахать её сзади... Но что-то не так...{/m}"
    Max_20 "{m}My cock sliding in between her ass feels amazing! She knows exactly how to keep the right pace. I want to take her from behind and fuck her hard... But something's wrong...{/m}"

# game/events/kira.rpy:3000
translate english return_from_club_ass_2d69d825:

    # Kira_10 "Ой, Макс, у меня так сильно начала кружиться голова! Похоже, я сегодня немного перебрала с алкоголем... Мне нужно немного передохнуть и мы продолжим, хорошо?"
    Kira_10 "Oh, Max, I'm getting so dizzy! I think I had a little too much alcohol today... I need to take a break, okay?"

# game/events/kira.rpy:3001
translate english return_from_club_ass_a38e6fb8:

    # Max_08 "Ну ладно. А ты уверена, что сможешь? Эээ... Тётя Кира? Заснула... И похоже, что крепко. Ну какой же это облом! Нужно уложить её на диван, пусть отдыхает."
    Max_08 "All right, are you sure? Erm... Aunt Kira? She's out cold. Well, that's a bummer. I should put her down on the couch and let her rest."

# game/events/kira.rpy:3008
translate english return_from_club_ass_ee61265b:

    # Max_01 "Надеюсь, мама не будет сильно её ругать за то, что она легла спать голая... Со всеми может случиться! А хороша тётя, так и гипнотизирует своей красотой. Но пора и мне спать..." nointeract
    Max_01 "I hope Mom doesn't judge her too much for going to bed naked... With a body like hers, who wouldn't? She's hypnotising me with her beauty. But it's time for me to go to bed..." nointeract

# game/events/kira.rpy:3025
translate english kira_bath_with_eric_e51d277a:

    # Max_01 "Только один человек может в это время не спать и плескаться в ванне. И человек этот - Кира!" nointeract
    Max_01 "Only one person can stay awake during this time and splash around in the bathtub. And that person is Kira!" nointeract

# game/events/kira.rpy:3037
translate english kira_bath_with_eric_1593e4c0:

    # Max_08 "Конечно, почему бы среди ночи не сходить в ванную, когда тебя там будет ждать шикарная тётя Кира, чтобы отсосать..."
    Max_08 "Of course, why not go to the bathroom in the middle of the night with gorgeous Aunt Kira waiting there to give you a blow job..."

# game/events/kira.rpy:3039
translate english kira_bath_with_eric_460fc52f:

    # Max_01 "Наверно, тётя Кира хотела прийти с работы и отдохнуть, а тут ещё и Эрику нужно отсосать сперва..."
    Max_01 "Aunt Kira must have wanted to come home from work and rest, but had to suck Eric's dick first..."

# game/events/kira.rpy:3050
translate english kira_bath_with_eric_21dc0f91:

    # Max_10 "Хватит уже трахать её в рот, как какую-то порнозвезду! Она конечно порнозвезда, но не круглые же сутки..."
    Max_10 "Stop fucking her in the mouth like a porn star! I mean she is, but she's not a 24-hour porn star..."

# game/events/kira.rpy:3052
translate english kira_bath_with_eric_bff48399:

    # Max_07 "Неплохо так Эрик её голову на свой член напяливает! Как будто так и надо..."
    Max_07 "It's nice how Eric puts her head on his dick! It's like it's meant to be..."

# game/events/kira.rpy:3060
translate english kira_bath_with_eric_b8b602d6:

    # Max_09 "Вот же сволочь! Спустил всё до последней капли Кире прямо в рот! Ну Эрик, ты у меня сполна получишь рано или поздно..." nointeract
    Max_09 "What a bastard! He shot every last drop right into Kira's mouth! Well, Eric, I'll get you sooner or later..." nointeract

# game/events/kira.rpy:3065
translate english kira_bath_with_eric_6bca3a88:

    # Max_02 "Кира не упустила ни единой капли мимо! И как будто никто ни у кого только что смачно не отсасывал... Чистая работа!" nointeract
    Max_02 "Kira didn't waste a single drop! She didn't leave any evidence of the last few minutes!" nointeract

translate english strings:

    # game/events/kira.rpy:23
    old "{i}войти{/i}"
    new "{i}Enter{/i}"

    # game/events/kira.rpy:85
    old "{i}идти к Кире{/i}"
    new "{i}Go see Kira{/i}"

    # game/events/kira.rpy:196
    old "{i}присоединиться к тёте{/i}"
    new "{i}Join your Aunt{/i}"

    # game/events/kira.rpy:232
    old "А моё напряжение только растёт, тётя Кира! Может поможешь?"
    new "And my tension is only getting worse, Aunt Kira! Can you help?"

    # game/events/kira.rpy:265
    old "{i}подтянуть её к себе и ласкать пальцами{/i}"
    new "{i}Pull her up and caress her with your fingers{/i}"

    # game/events/kira.rpy:265
    old "{i}пытаться не кончить{/i}"
    new "{i}Try not to cum{/i}"

    # game/events/kira.rpy:306
    old "{i}поработать языком{/i}"
    new "{i}Use the tongue{/i}"

    # game/events/kira.rpy:306
    old "{i}проникнуть в неё пальцами{/i}"
    new "{i}Penetrate her with your fingers{/i}"

    # game/events/kira.rpy:314
    old "{i}работать языком быстрее{/i}"
    new "{i}Work the tongue faster{/i}"

    # game/events/kira.rpy:364
    old "Садись на меня, тётя Кира!"
    new "Sit on top of me, Aunt Kira!"

    # game/events/kira.rpy:364
    old "Давай-ка сюда свою шикарную попку!"
    new "Give me that fat ass of yours!"

    # game/events/kira.rpy:383
    old "{i}приблизиться членом к её лицу{/i}"
    new "{i}Put your cock in her face{/i}"

    # game/events/kira.rpy:402
    old "Ох, тётя Кира... Ты слишком ловко меня дразнишь! Я могу не выдержать..."
    new "Oh, Aunt Kira... You're teasing me too well! I can't stand it..."

    # game/events/kira.rpy:420
    old "Да, давай ещё тётя Кира! Какие же горячие и ненасытные у тебя губки... Да..."
    new "Yes, come on, Aunt Kira! What hot and insatiable lips you have... Yes..."

    # game/events/kira.rpy:431
    old "Тётя Кира, я больше не могу... сейчас кончу! Да-а-а..."
    new "Aunt Kira, I can't take it anymore... I'm going to cum! ugh!..."

    # game/events/kira.rpy:544
    old "{i}кончить в неё{/i}"
    new "{i}Finish in her{/i}"

    # game/events/kira.rpy:544
    old "{i}кончить ей на ноги{/i}"
    new "{i}Finish on her feet{/i}"

    # game/events/kira.rpy:544
    old "{i}кончить ей в рот{/i}"
    new "{i}Cum in her mouth{/i}"

    # game/events/kira.rpy:589
    old "{i}трахать её{/i}"
    new "{i}Fuck her{/i}"

    # game/events/kira.rpy:606
    old "{i}кончить ей на попку{/i}"
    new "{i}I'm going to come on her ass.{/i}"

    # game/events/kira.rpy:738
    old "Да, что-то не спится... К тебе можно?"
    new "Yes, I can't sleep... Can I join you?"

    # game/events/kira.rpy:738
    old "Не могу уснуть, слишком напряжён! {i}(снять шорты){/i}"
    new "I can't sleep, I'm too tense! {i}(Take off my shorts){/i}"

    # game/events/kira.rpy:738
    old "Я просто прогуливался перед сном. Пойду, уже поздно..."
    new "I was just taking a walk before bed. I'll go, it's late..."

    # game/events/kira.rpy:771
    old "{i}кончить ей в рот!{/i}"
    new "{i}Cum in her mouth!{/i}"

    # game/events/kira.rpy:799
    old "{i}спуститься к Кире{/i}"
    new "{i}Go down to Kira{/i}"

    # game/events/kira.rpy:858
    old "{i}получать удовольствие{/i}"
    new "{i}Have fun{/i}"

    # game/events/kira.rpy:928
    old "{i}подойти поближе{/i}"
    new "{i}Come closer{/i}"

    # game/events/kira.rpy:955
    old "Тёте Кире очень идёт быть голой! Может быть, это потому что я привык видеть её голой? В такие округлости просто нельзя не влюбиться..."
    new "Aunt Kira looks so good naked! You just can't help but fall in love with curves everytime you see them..."

    # game/events/kira.rpy:957
    old "Ага! Тётя Кира ещё спит. Хорошо, что её ночнушка слегка просвечивает... Ухх, как она хороша..."
    new "Aha! Aunt Kira is still asleep. Good thing her nightie's a little see-through... Oooh, she looks good..."

    # game/events/kira.rpy:1002
    old "{i}посмотреть, что будет дальше{/i}"
    new "{i}See what happens next{/i}"

    # game/events/kira.rpy:1002
    old "{i}уже поздно, пора спать...{/i}"
    new "{i}It's late. Time to go to bed...{/i}"

    # game/events/kira.rpy:1009
    old "Мне что-то не спится. Можно тоже телек с тобой посмотреть?"
    new "I can't sleep. Can I watch TV with you, too?"

    # game/events/kira.rpy:1009
    old "Я насчёт уроков поцелуев, если момент подходящий..."
    new "I'm talking about kissing lessons, if the moment is right..."

    # game/events/kira.rpy:1009
    old "Я просто хотел пожелать спокойной ночи!"
    new "I just wanted to say good night!"

    # game/events/kira.rpy:1012
    old "Может быть, эротику?"
    new "Erotica, perhaps?"

    # game/events/kira.rpy:1012
    old "Давай смотреть порно?!"
    new "Let's watch porn?"

    # game/events/kira.rpy:1038
    old "Ни капли!"
    new "Not in the slightest!"

    # game/events/kira.rpy:1038
    old "Да ты продолжай, я посижу, посмотрю..."
    new "Yes, you go ahead, I'll sit and watch..."

    # game/events/kira.rpy:1081
    old "Конечно, да!"
    new "Of course, Yes!"

    # game/events/kira.rpy:1118
    old "Вообще-то, с лучшей тётей на свете!"
    new "Actually, with the best Aunt in the world!"

    # game/events/kira.rpy:1168
    old "{i}начать дрочить{/i}"
    new "{i}Start jerking off{/i}"

    # game/events/kira.rpy:1243
    old "{i}начать ласкать её киску языком{/i}"
    new "{i}Enter her pussy with your tongue{/i}"

    # game/events/kira.rpy:1248
    old "{i}раздеть её и повернуть{/i}"
    new "{i}Undress her and move{/i}"

    # game/events/kira.rpy:1268
    old "{i}продолжить языком{/i}"
    new "{i}Continue{/i}"

    # game/events/kira.rpy:1294
    old "С чего начнём?"
    new "Where do we start?"

    # game/events/kira.rpy:1294
    old "Как в прошлый раз?"
    new "Like last time?"

    # game/events/kira.rpy:1319
    old "{i}поцеловать её{/i}"
    new "{i}Kiss her{/i}"

    # game/events/kira.rpy:1325
    old "{i}продолжить целоваться{/i}"
    new "{i}Continue kissing{/i}"

    # game/events/kira.rpy:1387
    old "{i}прикоснуться к её попке{/i}"
    new "{i}Touch her ass{/i}"

    # game/events/kira.rpy:1387
    old "{i}подтянуть её ближе к себе{/i}"
    new "{i}Pull her closer{/i}"

    # game/events/kira.rpy:1464
    old "Давай ещё немного, тётя Кира... У меня ведь неплохо получается?"
    new "Just a little more, Aunt Kira... I'm pretty good at it, right?"

    # game/events/kira.rpy:1477
    old "{i}развязать её ночнушку{/i}"
    new "{i}Untie her nightgown{/i}"

    # game/events/kira.rpy:1543
    old "{i}ласкать её грудь и киску{/i}"
    new "{i}Fondle her breasts and pussy{/i}"

    # game/events/kira.rpy:1565
    old "{i}продолжить пальцами и языком{/i}"
    new "{i}Continue with your fingers and tongue{/i}"

    # game/events/kira.rpy:1590
    old "Как бы мне не кончить уже сейчас! Какой у тебя игривый язычок... Ммм..."
    new "I don't want to cum right now! What a playful tongue you have... Mmm..."

    # game/events/kira.rpy:1611
    old "Да, вот так тётя Кира, твои губки творят чудеса! Охх..."
    new "Yes, that's it, Aunt Kira, your tongue is going to make me explode! Ohh..."

    # game/events/kira.rpy:1626
    old "Тётя Кира, ещё быстрее... Да... Я сейчас кончу... А-а-а..."
    new "Aunt Kira, even faster... Yes... I'm going to cum... Ah..."

    # game/events/kira.rpy:1697
    old "{i}раздеть её{/i}"
    new "{i}Undress her{/i}"

    # game/events/kira.rpy:1697
    old "{i}раздеться вместе{/i}"
    new "{i}Undress together{/i}"

    # game/events/kira.rpy:1907
    old "{i}трахнуть тётю Киру{/i} (миссионерская поза)"
    new "{i}Fuck Aunt Kira{/i} (Missionary)."

    # game/events/kira.rpy:1907
    old "{i}снять шорты и трахнуть тётю Киру{/i} (миссионерская поза)"
    new "{i}Take off your shorts and fuck Aunt Kira{/i} (Missionary)"

    # game/events/kira.rpy:1907
    old "{i}трахнуть тётю Киру{/i} (поза на боку)"
    new "{i}Fuck Aunt Kira{/i} (Spooning)"

    # game/events/kira.rpy:1922
    old "{i}трахнуть тётю Киру (догги-стайл){/i}"
    new "{i}Fuck Aunt Kira (Doggie){/i}"

    # game/events/kira.rpy:1953
    old "Порадуешь меня минетом, тётя Кира?"
    new "Will you give me a blow job, Aunt Kira?"

    # game/events/kira.rpy:2030
    old "{i}кончить ей на живот{/i}"
    new "{i}Cum on her stomach{/i}"

    # game/events/kira.rpy:2275
    old "К тебе можно, тётя Кира?"
    new "Can I join you?"

    # game/events/kira.rpy:2351
    old "{i}раздеться и присоединиться к Кире{/i}"
    new "{i}Undress and join her{/i}"

    # game/events/kira.rpy:2357
    old "{i}ласкать её грудь{/i}"
    new "{i}Fondle her breasts{/i}"

    # game/events/kira.rpy:2721
    old "Давай лучше я уложу тётю Киру!"
    new "Let me put Aunt Kira to bed!"

    # game/events/kira.rpy:2721
    old "Ну, ладно... Пока!"
    new "Oh, well... Bye!"

    # game/events/kira.rpy:2728
    old "{i}отвести тётю в гостиную{/i}"
    new "{i}Take your aunt to the living room{/i}"

    # game/events/kira.rpy:2768
    old "{i}Дать Алисе конфету и отвести тётю в гостиную{/i}"
    new "{i}Give Alice candy and take Kira to the living room{/i}"

    # game/events/kira.rpy:2768
    old "{i}Отвести тётю в гостиную{/i}"
    new "{i}Take Kira to the living room{/i}"

    # game/events/kira.rpy:2790
    old "Такого поворота событий я не ожидал! Можно и задержаться..."
    new "This was a turn of events I hadn't expected! You can stay longer..."

    # game/events/kira.rpy:2790
    old "Я бы задержался, но нужно проверить, как там Алиса..."
    new "I would have stayed longer, but I need to check on Alice..."

    # game/events/kira.rpy:2887
    old "Хочу грудью!"
    new "I want those tits!"

    # game/events/kira.rpy:2887
    old "Хочу попкой!"
    new "I want that ass!"

    # game/events/kira.rpy:2896
    old "Но я всё равно хочу твою грудь!"
    new "But I still want your tits!"

    # game/events/kira.rpy:2896
    old "Я не могу устоять перед твоей попкой. Хочу её!"
    new "I can't resist your ass. I want it!"

    # game/events/kira.rpy:2927
    old "Но я всё равно хочу твою попку!"
    new "But I still want your ass!"

    # game/events/kira.rpy:2927
    old "Я не могу устоять перед твоей грудью. Хочу её!"
    new "I can't resist your tits. I want them!"

# TODO: Translation updated at 2021-10-08 00:56

# game/events/kira.rpy:1940
translate english kira_night_tv_tv_sex2_9ee6709e:

    # Kira_07 "Фух, удивляюсь, как ты ещё не кончил, Макс! Ты становишься всё выносливее... Скоро я перестану за тобой поспевать!"
    Kira_07 "Phew, I'm surprised you haven't cum yet, Max! You're getting tougher and tougher... Soon I won't be able to keep up with you!"

# game/events/kira.rpy:1941
translate english kira_night_tv_tv_sex2_ec39df3a:

    # Max_04 "Брось, тётя Кира! Секс с тобой - сплошное наслаждение..."
    Max_04 "Come on, Aunt Kira! Sex with you is a pleasure..."

# game/events/kira.rpy:1944
translate english kira_night_tv_tv_sex2_5cd76013:

    # Kira_02 "Устраивайся поудобнее... А я позабочусь о том, чтобы ты сладко-сладко кончил прямо в меня! Ничего, что тебе придётся снова любоваться моей попкой?"
    Kira_02 "Make yourself comfortable... And I'll make sure you sweetly cum right into me! Is it okay if you have to admire my ass again?"

# game/events/kira.rpy:1945
translate english kira_night_tv_tv_sex2_4e28d11a:

    # Max_03 "Вот чёрт, это заставит меня кончить куда быстрее!"
    Max_03 "Oh shit, this is going to make me cum a lot faster!"

# game/events/kira.rpy:1948
translate english kira_night_tv_tv_sex2_dcce85b2:

    # Kira_11 "Охх... Да... Тогда я буду опускаться на твой член очень медленно! Чтобы ты прочувствовал всю нежность моей мокрой киски... Ухх, как это обалденно! Ммм... Хочешь быстрее?"
    Kira_11 "Ohhh... Yes... Then I'll go down on your dick very slowly! So you can feel the tenderness of my wet pussy... Oh, that's so awesome! Mmm... Do you want it faster?"

# game/events/kira.rpy:1949
translate english kira_night_tv_tv_sex2_7c90ec0a:

    # Max_20 "Д-а-а... У тебя невероятная попка, тётя Кира! Давай ещё быстрее... Кажется, я уже близко..."
    Max_20 "Yeah... You have an incredible ass, Aunt Kira! Come on, hurry up. I think I'm getting close..."

# game/events/kira.rpy:1952
translate english kira_night_tv_tv_sex2_9385ec1b:

    # Kira_12 "О да! Макс, не сдерживай себя. Можешь кончать прямо в меня! Ох, как хорошо! Ахх..." nointeract
    Kira_12 "Oh yes! Max, don't hold back. You can cum inside me! Oh, that feels good! Ahhh..." nointeract

# game/events/kira.rpy:1957
translate english kira_night_tv_tv_sex2_88fc2651:

    # Kira_07 "Вот так... Нравится кончать в свою тётю, а Макс? Мне вот понравилось!"
    Kira_07 "There you go... You like cumming in your aunt, huh, Max? I love it!"

# game/events/kira.rpy:1958
translate english kira_night_tv_tv_sex2_25fcd1cd:

    # Max_05 "И мне тоже! Значит, до следующего раза?!"
    Max_05 "Me too! So, until next time?!"

# game/events/kira.rpy:2035
translate english kira_night_tv_missionary_9ee6709e:

    # Kira_07 "Фух, удивляюсь, как ты ещё не кончил, Макс! Ты становишься всё выносливее... Скоро я перестану за тобой поспевать!"
    Kira_07 "Phew, I'm surprised you haven't cum yet, Max! You're getting tougher and tougher... Soon I won't be able to keep up with you!"

# game/events/kira.rpy:2036
translate english kira_night_tv_missionary_ec39df3a:

    # Max_04 "Брось, тётя Кира! Секс с тобой - сплошное наслаждение..."
    Max_04 "Come on, Aunt Kira! Sex with you is a pleasure..."

# game/events/kira.rpy:2039
translate english kira_night_tv_missionary_5cf6f999:

    # Kira_02 "Устраивайся поудобнее... А я позабочусь о том, чтобы ты сладко-сладко кончил прямо в меня! Ты же не против, если я буду сверху?"
    Kira_02 "Make yourself comfortable... And I'll make sure you sweet-sweetly cum inside me! You don't mind if I'm on top, do you?"

# game/events/kira.rpy:2040
translate english kira_night_tv_missionary_d4453f2e:

    # Max_03 "О да, прямо то, что надо!"
    Max_03 "Oh yes, that's just the thing!"

# game/events/kira.rpy:2043
translate english kira_night_tv_missionary_0c9eb40f:

    # Kira_11 "Охх... Да... Гарантирую - долго ты не продержишься! Ведь я оттрахаю тебя без всяких нежностей... Жёстко и страстно! Ммм... Хочешь ещё сильнее?"
    Kira_11 "Ohhh... Yes... I guarantee you won't last long. Because I will fuck you without any tenderness... Hard and passionate. Mmm... You want more?"

# game/events/kira.rpy:2044
translate english kira_night_tv_missionary_e712cf57:

    # Max_20 "Ты обалденная, тётя Кира! Давай ещё быстрее... Кажется, я уже близко..."
    Max_20 "You're amazing, Aunt Kira! Come on, hurry up... I think I'm getting close..."

# game/events/kira.rpy:2047
translate english kira_night_tv_missionary_9385ec1b:

    # Kira_12 "О да! Макс, не сдерживай себя. Можешь кончать прямо в меня! Ох, как хорошо! Ахх..." nointeract
    Kira_12 "Oh yes! Max, don't hold back. You can cum inside me! Oh, that feels good! Ahhh..." nointeract

# game/events/kira.rpy:2052
translate english kira_night_tv_missionary_88fc2651:

    # Kira_07 "Вот так... Нравится кончать в свою тётю, а Макс? Мне вот понравилось!"
    Kira_07 "There you go... You like cumming in your aunt, huh, Max? I love it!"

# game/events/kira.rpy:2053
translate english kira_night_tv_missionary_25fcd1cd:

    # Max_05 "И мне тоже! Значит, до следующего раза?!"
    Max_05 "Me too! So, until next time?!"

translate english strings:

    # game/events/kira.rpy:1905
    old "Давай сменим позу, тётя Кира..."
    new "Let's change position, Aunt Kira..."

    # game/events/kira.rpy:1952
    old "{i}кончить в Киру{/i}"
    new "{i}Cum in Kira{/i}"
