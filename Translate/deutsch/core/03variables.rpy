# TODO: Translation updated at 2021-03-02 01:41

translate deutsch strings:

    # game/core/03variables.rpy:22
    old "МОЯ\nКОМНАТА"
    new "MEIN\nZIMMER"

    # game/core/03variables.rpy:22
    old "МОЯ КОМНАТА"
    new "MEIN ZIMMER"

    # game/core/03variables.rpy:22
    old "КОМНАТА\nАЛИСЫ"
    new "ALICE'S\nZIMMER"

    # game/core/03variables.rpy:22
    old "КОМНАТА АЛИСЫ"
    new "ALICE'S ZIMMER"

    # game/core/03variables.rpy:22
    old "КОМНАТА\nАННЫ"
    new "ANN'S\nZIMMER"

    # game/core/03variables.rpy:22
    old "КОМНАТА АННЫ"
    new "ANN'S ZIMMER"

    # game/core/03variables.rpy:22
    old "ВАННАЯ\nКОМНАТА"
    new "BADEZIMMER"

    # game/core/03variables.rpy:22
    old "ВАННАЯ КОМНАТА"
    new "BADEZIMMER"

    # game/core/03variables.rpy:22
    old "ГОСТИНАЯ"
    new "WOHNZIMMER"

    # game/core/03variables.rpy:22
    old "ВЕРАНДА"
    new "TERRASSE"

    # game/core/03variables.rpy:22
    old "ДВОР"
    new "GARTEN"

    # game/core/03variables.rpy:42
    old "Лиза"
    new "Lisa"

    # game/core/03variables.rpy:42
    old "Лизы"
    new "Lisa"

    # game/core/03variables.rpy:42
    old "Лизе"
    new "Lisa"

    # game/core/03variables.rpy:42
    old "Лизу"
    new "Lisa"

    # game/core/03variables.rpy:42
    old "Лизой"
    new "Lisa"

    # game/core/03variables.rpy:42
    old "Лиза, младшая сестрёнка. Милая и весёлая. Она ещё учится в школе. С Лизой мы общаемся на одной волне, хотя изредка ссоримся. Но если что-то случается, защиты ищет именно у меня."
    new "Lisa, kleine Schwester. Sie ist süß und lustig. Sie ist noch in der Schule. Lisa und ich sind auf der gleichen Wellenlänge, obwohl wir uns von Zeit zu Zeit streiten. Aber wenn etwas passiert, bin ich derjenige, der sie beschützt."

    # game/core/03variables.rpy:42
    old "Алиса"
    new "Alice"

    # game/core/03variables.rpy:42
    old "Алисы"
    new "Alice"

    # game/core/03variables.rpy:42
    old "Алисе"
    new "Alice"

    # game/core/03variables.rpy:42
    old "Алису"
    new "Alice"

    # game/core/03variables.rpy:42
    old "Алисой"
    new "Alice"

    # game/core/03variables.rpy:42
    old "Алиса, моя старшая сестра.  В любой непонятной ситуации бьёт по лицу (в лучшем случае). Недавно закончила школу и, так же, как и я, ищет свой путь. Целыми днями сидит в ноутбуке и занимается каким-то своим блогом. Как это часто бывает с братьями и старшими сёстрами, мы не очень ладим..."
    new "Alice, meine ältere Schwester.  In jeder unmöglichen Situation gibt sie mir (bestenfalls) eine Ohrfeige. Sie hat vor kurzem ihren Schulabschluss gemacht und sucht, genau wie ich, ihren eigenen Weg. Den ganzen Tag über sitzt sie an ihrem Laptop und beschäftigt sich mit einer Art Blog von ihr. Wie es bei Brüdern und älteren Schwestern oft der Fall ist, verstehen wir uns nicht besonders gut..."

    # game/core/03variables.rpy:42
    old "Анна"
    new "Ann"

    # game/core/03variables.rpy:42
    old "Анны"
    new "Ann"

    # game/core/03variables.rpy:42
    old "Анне"
    new "Ann"

    # game/core/03variables.rpy:42
    old "Анну"
    new "Ann"

    # game/core/03variables.rpy:42
    old "Анной"
    new "Ann"

    # game/core/03variables.rpy:42
    old "Анна, моя мама. Сама воспитывает нас с двумя сёстрами уже несколько лет. Работает в офисе какой-то компании. Хотя, зарплата у неё вполне приличная, но почти всё уходит на оплату жилья, еду и одежду."
    new "Ann, meine Mutter. Sie zieht meine Schwestern und mich seit mehreren Jahren alleine auf. Sie arbeitet im Büro einer großen Firma. Sie verdient gerade genug, zu überleben, aber Miete, Essen, Kleidung und andere verschiedene Ausgaben verschlingen den größten Teil ihres Gehalts."

    # game/core/03variables.rpy:53
    old "Макс"
    new "Max"

    # game/core/03variables.rpy:53
    old "Макса"
    new "Max"

    # game/core/03variables.rpy:53
    old "Максу"
    new "Max"

    # game/core/03variables.rpy:53
    old "Максом"
    new "Max"

    # game/core/03variables.rpy:53
    old "Максе"
    new "Max"

    # game/core/03variables.rpy:53
    old "Всегда в поисках приключений на свою пятую точку."
    new "Immer auf der Suche nach neuen Nervenkitzeln und Abenteuern."

    # game/core/03variables.rpy:60
    old "спит в нашей комнате (ночь)"
    new "in unserem Zimmer schlafen (Nacht)"

    # game/core/03variables.rpy:60
    old "спит в нашей комнате (утро)"
    new "in unserem Zimmer schlafen ( Morgens)"

    # game/core/03variables.rpy:60
    old "принимает душ"
    new "unter der Dusche"

    # game/core/03variables.rpy:60
    old "читает в нашей комнате"
    new "lesen in unserem Zimmer"

    # game/core/03variables.rpy:60
    old "семейный завтрак"
    new "Familienfrühstück"

    # game/core/03variables.rpy:60
    old "одевается в школу"
    new "Anziehen für die Schule"

    # game/core/03variables.rpy:60
    old "одевается в магазин"
    new "Kleidergeschäft"

    # game/core/03variables.rpy:60
    old "одевается к репетитору"
    new "Anziehen für Privatlehrer"

    # game/core/03variables.rpy:60
    old "в школе"
    new "in der Schule"

    # game/core/03variables.rpy:60
    old "в магазине"
    new "im Geschäft"

    # game/core/03variables.rpy:60
    old "у репетитора"
    new "beim Privatlehrer"

    # game/core/03variables.rpy:60
    old "в бассейне"
    new "im Pool"

    # game/core/03variables.rpy:60
    old "загорает"
    new "sonnen"

    # game/core/03variables.rpy:60
    old "семейный ужин"
    new "Familienessen"

    # game/core/03variables.rpy:60
    old "моет посуду"
    new "das Geschirr spülen"

    # game/core/03variables.rpy:60
    old "лежит с телефоном в нашей комнате"
    new "an ihrem Telefon in unserem Zimmer"

    # game/core/03variables.rpy:60
    old "принимает ванну"
    new "ein Bad nehmen"

    # game/core/03variables.rpy:60
    old "учит уроки"
    new "Hausaufgaben erledigen"

    # game/core/03variables.rpy:86
    old "спит"
    new "schlafen"

    # game/core/03variables.rpy:86
    old "занимается йогой"
    new "Yoga machen"

    # game/core/03variables.rpy:86
    old "готовит завтрак"
    new "Frühstück machen"

    # game/core/03variables.rpy:86
    old "одевается на работу"
    new "sich für die Arbeit anziehen"

    # game/core/03variables.rpy:86
    old "в своей комнате"
    new "in seinem Zimmer"

    # game/core/03variables.rpy:86
    old "на работе"
    new "auf Arbeit"

    # game/core/03variables.rpy:86
    old "читает на веранде"
    new "Lesen auf der Terrasse"

    # game/core/03variables.rpy:86
    old "загорает с Алисой"
    new "Sonnenbad mit Alice"

    # game/core/03variables.rpy:86
    old "в бассейне с Алисой"
    new "im Pool mit Alice"

    # game/core/03variables.rpy:86
    old "готовит ужин"
    new "Abendessen vorbereiten"

    # game/core/03variables.rpy:86
    old "смотрит ТВ"
    new "fernsehen"

    # game/core/03variables.rpy:110
    old "спит (ночь)"
    new "Schlafen (Nacht)"

    # game/core/03variables.rpy:110
    old "спит (утро)"
    new "Schlafen (Morgen)"

    # game/core/03variables.rpy:110
    old "одевается к подруге"
    new "Kleidung für einen Freund"

    # game/core/03variables.rpy:110
    old "загорает с Анной"
    new "Sonnenbad mit Ann"

    # game/core/03variables.rpy:110
    old "в бассейне с Анной"
    new "im Pool mit Ann"

    # game/core/03variables.rpy:147
    old "Повседневная"
    new "täglich"

    # game/core/03variables.rpy:150
    old "Для сна"
    new "Schlafzeug"

    # game/core/03variables.rpy:154
    old "КУПАЛЬНИК"
    new "BADEBIKINI"

    # game/core/03variables.rpy:163
    old "За уроками"
    new "Im Unterricht"

    # game/core/03variables.rpy:183
    old "Для приготовления завтрака"
    new "Zubereitung des Frühstücks"

    # game/core/03variables.rpy:188
    old "Для приготовления ужина"
    new "Zubereitung des Abendessens"

    # game/core/03variables.rpy:191
    old "Для утреннего отдыха"
    new "die morgendliche Ruhe"

    # game/core/03variables.rpy:195
    old "Для вечернего отдыха"
    new "Abendgarderobe"

    # game/core/03variables.rpy:214
    old "ПОСМОТРЕТЬ\nФИЛЬМ"
    new "FILM\nANSEHEN"

    # game/core/03variables.rpy:214
    old "ВЫЙТИ\nИЗ ДОМА"
    new "HAUS\nVERLASSEN"

    # game/core/03variables.rpy:214
    old "ПОИГРАТЬ\nС ЛИЗОЙ"
    new "MIT LISA\nSPIELEN"

    # game/core/03variables.rpy:214
    old "УСТАНОВИТЬ\nКЕЙЛОГГЕР"
    new "KEYLOGGER\nINSTALLIEREN"

    # game/core/03variables.rpy:214
    old "УСТАНОВИТЬ\nКАМЕРУ"
    new "KAMERA\nINSTALLIEREN"

    # game/core/03variables.rpy:214
    old "СПРЯТАТЬ\nПАУКА"
    new "SPINNE\nVERSTECKEN"

    # game/core/03variables.rpy:214
    old "БРОСИТЬ\nПАУКА"
    new "SPINNE\nWERFEN"

    # game/core/03variables.rpy:214
    old "ИСКАТЬ\nПАУКОВ"
    new "SPINNE\nSUCHEN"

    # game/core/03variables.rpy:214
    old "ИСКАТЬ\nСИГАРЕТЫ"
    new "ZIGARETTEN\nSUCHEN"

    # game/core/03variables.rpy:214
    old "ИСКАТЬ\nКНИГУ"
    new "BUCH\nSUCHEN"

    # game/core/03variables.rpy:214
    old "ЧИСТИТЬ\nБАССЕЙН"
    new "POOL\nREINIGEN"

    # game/core/03variables.rpy:214
    old "ЧИТАТЬ"
    new "LESEN"

    # game/core/03variables.rpy:214
    old "ИСКАТЬ\nКАМЕРУ"
    new "KAMERA\nSUCHEN"

    # game/core/03variables.rpy:214
    old "РАЗОБРАТЬ\nКОРОБКИ"
    new "KISTE\nAUSPACKEN"

    # game/core/03variables.rpy:214
    old "ВКЛЮЧИТЬ\nНОУТБУК"
    new "LAPTOP\nEINSCHALTEN"

    # game/core/03variables.rpy:214
    old "СПАТЬ"
    new "SCHLAFEN"

    # game/core/03variables.rpy:214
    old "УСТАНОВИТЬ\nБУДИЛЬНИК"
    new "WECKER\nSTELLEN"

    # game/core/03variables.rpy:214
    old "ВЗДРЕМНУТЬ"
    new "NICKERCHEN"

    # game/core/03variables.rpy:214
    old "ПРИНЯТЬ\nДУШ"
    new "DUSCHE\nNEHMEN"

    # game/core/03variables.rpy:214
    old "ПРИНЯТЬ\nВАННУ"
    new "BAD\nNEHMEN"

    # game/core/03variables.rpy:214
    old "ПОГОВОРИТЬ"
    new "SPRECHEN"

    # game/core/03variables.rpy:214
    old "МЫТЬ\nПОСУДУ"
    new "GESCHIRR\nABWASCHEN"

    # game/core/03variables.rpy:272
    old "ПАУК"
    new "SPINNE"

    # game/core/03variables.rpy:272
    old "Самое страшное существо на свете. С точки зрения Алисы, конечно. Нужно этим воспользоваться в подходящий момент!"
    new "Die furchterregendste Kreatur aller Zeiten. Jedenfalls aus der Sicht von Alice. Ich sollte das zum richtigen Zeitpunkt einsetzen!"

    # game/core/03variables.rpy:272
    old "СКРЫТАЯ КАМЕРА"
    new "VERSTECKTE KAMMERA"

    # game/core/03variables.rpy:272
    old "Высокотехнологичная микро-камера, предназначенная для скрытного наблюдения. Имеет радиомодуль для беспроводной передачи зашифрованного цифрового видеосигнала."
    new "Fortschrittliche Mikrokamera für verdeckte Überwachung. Sie verfügt über ein Funkmodul, so dass sie verschlüsselte digitale Videoströme übertragen kann."

    # game/core/03variables.rpy:272
    old "ФИЛЬМ \"ШКОЛЬНИЦЫ\""
    new "PORNO-FILM \"SCHULMÄDCHEN\""

    # game/core/03variables.rpy:272
    old "Строгая учительница пытается наказать непослушных школьниц..."
    new "Strenge Lehrerin versucht, unartige Schulmädchen zu bestrafen..."

    # game/core/03variables.rpy:272
    old "ШЕЛКОВЫЙ ХАЛАТ"
    new "SEIDEN-BADEMANTEL"

    # game/core/03variables.rpy:272
    old "Короткий, лёгкий, почти шёлковый халат высшего качества. Made in China."
    new "Kurzer, leichter, seidener Bademantel. Tolle Qualität! Hergestellt in China."

    # game/core/03variables.rpy:272
    old "КУПАЛЬНИК КРАСНЫЙ"
    new "ROTER BIKINI"

    # game/core/03variables.rpy:272
    old "Купальник для тех, кто не стесняется своего тела. Скрывает только самые интимные участки. Всё остальное открыто для солнца и глаз окружающих."
    new "Ein sexy Biki für diejenigen, die keine Scheu vor ihrem Körper haben. Verbirgt nur die intimsten Stellen. Alles andere ist der Sonne und den Augen ausgesetzt."

    # game/core/03variables.rpy:272
    old "СИГАРЕТЫ"
    new "ZIGARETTEN"

    # game/core/03variables.rpy:272
    old "Пачка сигарет Lucky Strike. Для настоящих мужчин!"
    new "Eine Packung Zigaretten, Lucky Strike. Für echte Männer!"

    # game/core/03variables.rpy:272
    old "МАЛЕНЬКОЕ ЧЕРНОЕ ПЛАТЬЕ"
    new "KLEINES SCHWARZES KLEIDCHEN"

    # game/core/03variables.rpy:272
    old "Отличный подарок для девушки, желающей произвести фурор на вечеринке или дискотеке."
    new "Tolles Geschenk für ein Mädchen, das auf der Party Eindruck machen möchte."

    # game/core/03variables.rpy:272
    old "ЛЮБЯЩАЯ РУБИ"
    new "LIEBENDE RUBINE"

    # game/core/03variables.rpy:272
    old "Роман о запретной любви между секретаршей и её начальником, полный любви, страсти, эмоций и... мистики."
    new "Ich habe die Gerüchte gehört: Er hat seine Frau umgebracht. Er steht unter Hausarrest. Er ist ein Vampir. Ja, ich weiß, ich bin verrückt."

    # game/core/03variables.rpy:272
    old "ПРЕМЬЕР-МИНИСТР"
    new "MINISTERPRÄSIDENT"

    # game/core/03variables.rpy:272
    old "Новый эротический роман, входящий в Топ-10 лучших романов и новелл США!"
    new "Ein perverser Ministerpräsident, eine einfache Praktikantin und ein Skandal, der sich nicht so richtig anfühlen und nicht so viel kosten sollte."

    # game/core/03variables.rpy:272
    old "БЫТЬ КУКЛОЙ"
    new "SEI EIN DIENSTMÄDCHEN"

    # game/core/03variables.rpy:272
    old "В поместье Картера красивые девушки обучаются, чтобы стать идеальными жёнами для самых влиятельных людей этого мира."
    new "In Carter Manor werden Frauen darauf vorbereitet, die perfekte nächste Traumfrau für die einflussreichsten und mächtigsten Männer der Welt zu werden."

    # game/core/03variables.rpy:272
    old "КНИГА ОРГАЗМОВ"
    new "DAS GROßE BUCH DER ORGASMEN"

    # game/core/03variables.rpy:272
    old "Что получится, если собрать самые горячие истории об оргазмах от 69 различных авторов?"
    new "Was passiert, wenn Sie 69 Autoren zusammenbringen, die ihre heißesten Orgasmusgeschichten erzählen?"

    # game/core/03variables.rpy:272
    old "ИСТОРИЯ О"
    new "GESCHICHTE DER O"

    # game/core/03variables.rpy:272
    old "Это история о доминировании и подчинении. История об одной прекрасной девушке по имени О."
    new "Die Geschichte der O ist eine Geschichte über weibliche Unterwerfung, an der eine schöne Pariser Modefotografin namens O beteiligt ist."

    # game/core/03variables.rpy:272
    old "СТРЕМЯНКА"
    new "Trittleiter"

    # game/core/03variables.rpy:272
    old "Небольшая стремянка, позволяющая достать то, для чего не хватает роста"
    new "Eine kleine Trittleiter, die es ermöglicht, das Unerreichte doch noch zu erreichen."

    # game/core/03variables.rpy:272
    old "WEB STANDARDS"
    new "WEB-STANDARDS"

    # game/core/03variables.rpy:272
    old "Книга рассказывает о способах создавать свои сайты, работающие на любых устройствах."
    new "Das Buch beschreibt, wie man seine eigenen Website erstellen kann, die auf jedem Gerät funktioniert."

    # game/core/03variables.rpy:272
    old "ЛЁГКАЯ ПИЖАМА"
    new "BLAUE PYJAMAS"

    # game/core/03variables.rpy:272
    old "Удобнейшие маечка и шортики. В них не жарко душными летними ночами, и так же уютно в течение всего года."
    new "Die bequemsten Hemden und Shorts. Sie sind in stickigen Sommernächten nicht so warm und das ganze Jahr über genauso gemütlich."

    # game/core/03variables.rpy:272
    old "НОЧНУШКА"
    new "NEGLIGEE"

    # game/core/03variables.rpy:272
    old "Полупрозрачные сорочка и трусики. Облегающий фасон подчёркивает все достоинства и изгибы фигуры, а лёгкое кружево придаст ещё больше сексуальности."
    new "Durchsichtiges Oberteil und Höschen. Passformen betonen alle Vorzüge und Rundungen der Figur, und eine leichte Spitze sorgt für noch mehr Sinnlichkeit."

    # game/core/03variables.rpy:272
    old "Набор косметики"
    new "Schminkset"

    # game/core/03variables.rpy:272
    old "Небольшой набор косметики для повседневного использования. Для женщин - только лучшее..."
    new "Ein kleines Kosmetikset für den täglichen Gebrauch. Für Frauen - nur das Beste..."

    # game/core/03variables.rpy:272
    old "Шоколад \"Ritter Sport\" mini (9 штук)"
    new "Schokolade \"Ritter Sport\" Mini (9 Stück)"

    # game/core/03variables.rpy:272
    old "Шоколадное наслаждение для каждого случая... Множество лакомых сортов с лучшими ингредиентами со всего мира."
    new "Ein Schokoladengenuss für jede Gelegenheit... Viele köstliche Sorten mit den besten Zutaten aus der ganzen Welt."

    # game/core/03variables.rpy:272
    old "Шоколад \"Ritter Sport\" (4 штуки)"
    new "Schokolade \"Ritter Sport\" (4 Stück)"

    # game/core/03variables.rpy:272
    old "Конфеты \"Raffaello\" (16 штук)"
    new "Schokolade \"Raffaello\" (16 Stück)"

    # game/core/03variables.rpy:272
    old "Хрустящие кокосовые конфеты с цельным миндальным орехом. Вместо тысячи слов..."
    new "Knusprige Kokosnüsse mit ganzen Mandeln. Sagt mehr als tausend Worte..."

    # game/core/03variables.rpy:272
    old "Конфеты \"Raffaello\" (24 штуки)"
    new "Schokolade \"Raffaello\" (24 Stück)"

    # game/core/03variables.rpy:272
    old "Конфеты \"Ferrero Rocher\" (16 штук)"
    new "Konfekt \"Ferrero Rocher\" (16 Stück)"

    # game/core/03variables.rpy:272
    old "Сочетание цельного фундука и восхитительного сливочно-орехового крема в хрустящей вафельной оболочке подарит вам неповторимые вкусовые ощущения."
    new "Die Kombination aus ganzen Haselnüssen und einer köstlichen cremigem Nougat in einer knusprigen Waffelschale bieten ein einzigartiges Geschmackserlebnis."

    # game/core/03variables.rpy:272
    old "Конфеты \"Ferrero Rocher\" (24 штуки)"
    new "Konfekt \"Ferrero Rocher\" (24 Stück)"

    # game/core/03variables.rpy:272
    old "КРЕМ ДЛЯ ЗАГАРА"
    new "SONNENCREME"

    # game/core/03variables.rpy:272
    old "Легкий, хорошо впитывающийся препарат для ускорения загара обладает увлажняющими и защитными свойствами. Рекомендуется для применения на пляже и в солярии."
    new "Ein gut einziehendes Präparat zur Beschleunigung der Bräunung mit feuchtigkeitsspendenden und schützenden Eigenschaften. Empfohlen für den Einsatz am Strand und im Solarium."

    # game/core/03variables.rpy:272
    old "МУЖСКИЕ МАЙКА И ШОРТЫ"
    new "T-SHIRT UND KURZE HOSE"

    # game/core/03variables.rpy:272
    old "Свободный и лёгкий летний комплект одежды на каждый день."
    new "Eine lässige und leichte Kombination Sommerkleidung für jeden Tag."

    # game/core/03variables.rpy:272
    old "КОНФЕТЫ С ЛИКЁРОМ"
    new "LIQUEUR-BONBONS"

    # game/core/03variables.rpy:272
    old "Шоколадные конфеты с ликёром. Уникальные ароматизаторы скрывают вкус алкоголя. Отлично поднимают настроение. Очень крепкие."
    new "Schokoladenbonbons mit Likör. Einzigartige Aromen verbergen den Geschmack von Alkohol. Sie sind sehr stark."

    # game/core/03variables.rpy:272
    old "КОМПЛЕКТ ТЁМНОГО НИЖНЕГО БЕЛЬЯ"
    new "SCHWARZE UNTERWÄSCHE SET"

    # game/core/03variables.rpy:272
    old "Отличное нижнее бельё тёмного цвета. Отличный подарок для любимой девушки."
    new "Exotische Unterwäsche in dunkler Farbe. Ein tolles Geschenk für Ihr Lieblingsmädchen."

    # game/core/03variables.rpy:272
    old "СЕКС-ОБРАЗОВАНИЕ"
    new "SEXUALKUNDE"

    # game/core/03variables.rpy:272
    old "Книга обо всём, связанным с сексом. Отлично подходит для обучения подрастающего поколения."
    new "Ein umfassender Leitfaden zu Geschlecht und Körperfunktionen. Großartig für das sichere Unterrichten von Personen mit begrenzter Erfahrung."

    # game/core/03variables.rpy:272
    old "ФОТОАППАРАТ"
    new "KAMERA"

    # game/core/03variables.rpy:272
    old "Профессиональный фотоаппарат с объективом. Не новый, но в отличном состоянии. Подойдёт как для новичков, так и для профессионалов."
    new "Professionelle Kamera mit einem Objektiv. Nicht neu, aber in ausgezeichnetem Zustand. Sowohl für Anfänger als auch für Profis geeignet."

    # game/core/03variables.rpy:272
    old "КОРОТКАЯ ПИКАНТНАЯ СОРОЧКА"
    new "KURZE CHEMIE"

    # game/core/03variables.rpy:272
    old "Соблазнительная чёрная сорочка выполнена из эластичного тюля. В комплект так же входят трусики-стринги."
    new "Das verführerische schwarzes Oberteil ist aus elastischem Tüll gefertigt. Zum Set gehört auch ein String-Höschen."

    # game/core/03variables.rpy:272
    old "ЧЁРНОЕ СЕКСУАЛЬНОЕ БОДИ"
    new "SEXY SCHWARZEN BODY"

    # game/core/03variables.rpy:272
    old "Прозрачное сетчатое боди с открытой спиной чёрного цвета."
    new "Blickdichter Netz-Body mit offenem Rücken in Schwarz."

    # game/core/03variables.rpy:272
    old "ЧЁРНОЕ КРУЖЕВНОЕ БОДИ"
    new "SCHWARZER SPITZENBODY"

    # game/core/03variables.rpy:272
    old "Женское сексуальное нижнее бельё с кружевным узором по краям."
    new "Sexy Damenunterwäsche mit Spitzenmuster an den Rändern."

    # game/core/03variables.rpy:316
    old "Скрытые камеры"
    new "Versteckte Kamera"

    # game/core/03variables.rpy:316
    old "Я нашёл пустую коробку из под скрытой камеры. Видимо, она установлена где-то в доме. Нужно поискать как следует - вдруг, кто-то за нами наблюдал всё это время?"
    new "Ich fand eine leere Schachtel, die eine versteckte Kamera enthielt. Sie ist wahrscheinlich irgendwo im Haus installiert. Ich sollte versuchen, sie zu finden, was, wenn uns jemand die ganze Zeit beobachtet hat?"

    # game/core/03variables.rpy:316
    old "Итак, я нашёл скрытую камеру в гостиной. Она вмонтирована в стену, видимо, во время ремонта. Поэтому, её нельзя вытащить, не повредив. Кроме того, оказалось, что камера сейчас ни к чему не подключена. Может быть, стоит целенаправленно изучить способы подключения камер, чтобы извлечь из неё какую-то пользу..."
    new "Ich habe die versteckte Kamera im Wohnzimmer gefunden. Sie ist in der Wand montiert, ich kann sie nicht herausholen, ohne sie zu beschädigen. Sie ist mit nichts verbunden, vielleicht sollte ich nach Möglichkeiten suchen, das zu beheben, es könnte nützlich sein..."

    # game/core/03variables.rpy:316
    old "Ко мне пришла отличная идея - заработать на трансляции с веб-камеры в интернет, но я об этом знаю очень мало... Может быть, поможет какая-то литература?"
    new "Hier ist eine Idee: Ich könnte etwas Geld verdienen, indem ich den Live-Feed von der versteckten Kamera streame. Zu schade, dass ich nicht wirklich weiß, wie ich alles einrichten kann. Vielleicht sollte ich im Internet nachsehen?"

    # game/core/03variables.rpy:316
    old "Итак, я внимательно прочитал книгу Web Standards и разобрался как сделать свой сайт. Пожалуй, стоит начать именно с этого. Мне потребуются деньги на домен, хостинг и оплату готового шаблона с дизайном. Всего нужно порядка $100."
    new "Nun, ich habe die Web-Standards gelesen und herausgefunden, wie ich meine Website einrichte. Ich denke, das sollte mein Ausgangspunkt sein. Ich werde etwas Geld brauchen, um eine Domain, ein Hosting und eine Designvorlage zu erwerben. $100 sollten reichen."

    # game/core/03variables.rpy:316
    old "Наконец-то, у меня есть свой собственный сайт, где транслируется изображение с камеры из гостиной! Теперь нужно увеличить аудиторию, ведь чем больше людей, тем больше доход от рекламных баннеров. Сделать это можно с помощью вложений в рекламу своего сайта. Кроме того, если в кадре происходит что-то интересное, аудитория также растёт."
    new "Endlich habe ich meine eigene Streaming-Website! Jetzt muss ich die Zahl meiner Zuschauer erhöhen, um mehr Geld zu verdienen. Und dafür muss ich irgendwo für meine Website werben. Mein Publikum soll wachsen, wenn etwas Aufregendes im Streaming passiert."

    # game/core/03variables.rpy:316
    old "{i}{b}Внимание:{/b} Пока это всё, что можно сделать для данной \"возможности\" в текущей версии игры.{/i}"
    new "{i}{b}Achtung:{/b} In der aktuellen Version des Spiels haben Sie das Ende dieser Möglichkeit erreicht.{/i}"

    # game/core/03variables.rpy:324
    old "Наставник"
    new "LEHRER"

    # game/core/03variables.rpy:324
    old "Кажется, Лиза совсем ничего не знает о мальчиках. Возможно, она даже порно ни разу не видела, раз так удивилась обычному утреннему стояку. Может быть, стоит заняться просветительской деятельностью среди своей младшей сестрёнки? Но с чего начать? Поговорить?"
    new "Sieht aus, als wüsste Lisa nichts über die männliche Anatomie. Sie war von meiner Morgenlatte so überrascht, dass sie wahrscheinlich noch nie Pornos gesehen hat. Vielleicht sollte ich ihr bei der Erziehung helfen. Ich fange damit an, es im Gespräch zu erwähnen."

    # game/core/03variables.rpy:324
    old "Я поговорил с Лизой и стало ясно, что для того, чтобы чему-то учить, нужно сначала завоевать авторитет."
    new "Ich habe mit Lisa gesprochen, und sie scheint mich nicht genug zu respektieren, um mir zuzuhören, das muss ich in Ordnung bringen, bevor ich mit meinem Unterricht weitermache."

    # game/core/03variables.rpy:324
    old "Лиза снова увидела мой утренний стояк. На этот раз маму она не позвала, что уже хорошо. Хотя, смотрела она на мой член не только с подозрением, но ещё и с интересом. По крайней мере, мне так показалось. Думаю, нужно с ней об этом поговорить..."
    new "Lisa hat wieder meine Morgenlatte gesehen. Diesmal hat sie Mom nicht gerufen, was gut ist. Ich habe das Gefühl, dass sie meinen Schwanz diesmal mit etwas anderem als Misstrauen angestarrt hat... Ich denke, wir sollten mit ihr darüber reden..."

    # game/core/03variables.rpy:324
    old "Это, конечно, не точно, но Лиза хотела потрогать мой член, думая, что я спал. И судя по её реакции на то, что я это увидел, даже она сама с себя удивилась. Посмотрим, что она потом об этом скажет..."
    new "Die Neugierde überwältigte sie. Ich erwischte Lisa dabei, wie sie versuchte, mich zu berühren, weil sie dachte, ich würde schlafen. Ihrer Reaktion nach zu urteilen, war sie genauso überrascht wie ich. Mal sehen, was sie später dazu sagt..."

    # game/core/03variables.rpy:324
    old "Похоже, интерес Лизы к противоположному полу и всему, что связано со взрослой жизнью, растёт не по дням, а по часам. Не знаю уж, мой стоящий по утрам член так её раззадорил или ещё что-то, но она согласилась меня слушаться по вопросам о мальчиках и всем этим взрослым штучкам... Вот только прежде, чем учить, может быть стоит самому чему-то научиться? А самое главное - при этом ничего не испортить, а то она снова откажется..."
    new "Es scheint, dass Lisas Interesse für das andere Geschlecht und alles, was mit dem Erwachsenenleben zu tun hat, sprunghaft wächst. Ich bin mir nicht sicher, wie sehr meine Morgenlatte diese ganze Aufregung erregt hat, aber sie hat sich bereit erklärt, mir bei Fragen über Jungen und den ganzen Erwachsenen-Kram zuzuhören... Aber bevor Ich Sie unterrichten kann, sollten Sie vielleicht selbst etwas lernen? Das Wichtigste ist, nicht zu weit zu gehen..."

    # game/core/03variables.rpy:324
    old "Итак, Лиза поинтересовалась, чему же я собираюсь её учить. И сразу же поставила меня в глупую ситуацию. Я вроде бы должен её учить, но сам не умею даже целоваться! Нужно срочно найти кого-то, кто бы мне помог в этом... Да, легко сказать..."
    new "Also fragte mich Lisa, was ich ihr beibringen würde, und sie brachte mich sofort in eine heikle Situation. Ich soll ihr beibringen, wie man küsst, aber was weiß ich schon! Ich muss sofort jemanden finden, der mir dabei hilft... vielleicht leichter gesagt als getan..."

    # game/core/03variables.rpy:324
    old "Кажется, я нашёл ту, которая научит меня целоваться. И это... моя тётя! Это так странно... Ну она точно в этом вопросе понимает достаточно. Да я был бы рад любому учителю, но такой - идеален! Да, тётя Кира сказала, что лучше об этом поговорить в более интимной обстановке. Например, когда она ночью смотрит телек."
    new "Ich glaube, ich habe jemanden gefunden, der mir beibringen kann, wie man küsst. Meine Tante! Das ist so seltsam... Nun, sie weiß sicherlich genug über das Thema. Ich wäre mit jedem Lehrer glücklich, aber das ist besser als das, was ich erwartet habe! Tante Kira meinte, es wäre besser, in einem intimeren Rahmen darüber zu sprechen. Zum Beispiel, wenn sie abends fernsieht."

    # game/core/03variables.rpy:324
    old "Тётя Кира превосходный учитель поцелуев! Она согласилась меня иногда учить. Кто знает, может быть она научит и чему-то ещё?\n\nНу всё. Теперь нужно практиковаться в поцелуях с тётей Кирой и передавать полученные знания Лизе... Только вот её ещё нужно убедить..."
    new "Tante Kira ist eine ausgezeichnete Kusslehrerin! Sie hat sich bereit erklärt, mich zu unterrichten. Wer weiß, vielleicht führt das zu etwas anderem?\n\nIch muss nur die Praxis aufrechterhalten, und mein Wissen an Lisa weitergeben... Aber sie muss vielleicht noch etwas überzeugt werden..."

    # game/core/03variables.rpy:324
    old "Я поговорил с Лизой насчёт её обучения. Она выдвинула ряд условий, среди которых запрет мне к ней прикасаться, если она против. Ну уже что-то."
    new "Ich sprach mit Lisa über ihre Ausbildung. Sie stellte eine Reihe von Bedingungen, darunter, dass sie ohne Erlaubnis nicht berührt werden dürfe. Nun, zumindest ist das ein Anfang."

    # game/core/03variables.rpy:324
    old "Похоже обучаться Лиза согласна только если у неё хорошее настроение..."
    new "Es scheint, dass Lisa nur dann bereit ist zu lernen, wenn sie gut gelaunt ist..."

    # game/core/03variables.rpy:324
    old "Мне удалось впечатлить Лизу своим искусством целоваться! Хотя, пока на искусство это не тянет, но ей понравилось! Всё-таки, тётя Кира отличный учитель. Но нужно продолжать практиковаться."
    new "Ich habe es geschafft, Lisa mit meinen Kussfähigkeiten zu beeindrucken! Ich glaube, ich habe noch einen Weg vor mir, aber es hat ihr trotzdem gefallen! Tante Kira ist wirklich eine großartige Lehrerin. Ich sollte weiter üben."

    # game/core/03variables.rpy:324
    old "Лиза намекнула, что не против двигаться дальше и я решил увлечь её поцелуями настолько, чтобы ей нравились при этом и мои прикосновения. А для этого нужно больше целоваться с тётей Кирой по ночам..."
    new "Lisa scheint sehr viel empfänglicher für Berührungen zu werden. Ich glaube, ich bin da an etwas dran. Ich übe besser noch etwas mit Kira."

    # game/core/03variables.rpy:324
    old "Лизе очень нравится как я целуюсь. Она даже намекнула, что пора бы научить её ещё чему-то полезному. На этот раз, как мне кажется, стоит подтянуть теорию. Может быть, купить ей какую-то книжку по анатомии, чтобы она разобралась в физиологии и поняла, какие естественные процессы могут происходить между мужчинами и женщинами?"
    new "Lisa gefällt es sehr, wie ich sie küsse. Sie hat sogar angedeutet, dass sie vielleicht daran interessiert ist, etwas Fortgeschritteneres zu lernen. Es ist an der Zeit, meine Theorie zu verfeinern. Vielleicht sollte ich ihr ein Anatomiebuch kaufen, damit sie mehr über die natürlichen Vorgänge, die zwischen Männern und Frauen ablaufen, lernen kann?"

    # game/core/03variables.rpy:324
    old "Я решил, что и мне самому будет полезно прочитать купленную для Лизы книгу. Вдруг, тем самым, я что-то для себя интересное открою..."
    new "Ich entschied, dass es für mich nützlich wäre, das Buch zu lesen, das ich für Lisa gekauft hatte. Vielleicht entdecke ich dabei etwas Interessantes für mich..."

    # game/core/03variables.rpy:324
    old "Книга прочитана! Не то, чтобы много всего нового я для себя открыл, но есть то, что может помочь нам с Лизой ещё больше сблизиться. А пока можно ей книжку подарить..."
    new "Ich habe das Buch zu Ende gelesen! Ich habe eigentlich nichts Neues gelernt, aber ich habe etwas bemerkt, das Lisa und mir helfen könnte, uns näher zu kommen. In der Zwischenzeit sollte ich ihr das Buch geben."

    # game/core/03variables.rpy:324
    old "Я подарил Лизе книжку, которая содержит много ответов на её вопросы. Нужно будет поинтересоваться через какое-то время, что она об этой книге думает..."
    new "Ich habe Lisa das Buch gegeben, das hoffentlich einige ihrer eher perversen Fragen beantworten wird. Ich werde sie fragen müssen, was sie denkt, nachdem ich ihr etwas Zeit zum Lesen gegeben habe."

    # game/core/03variables.rpy:324
    old "Я подарил Лизе книжку, которая содержит много ответов на её вопросы. Да мне и самому было бы полезно её почитать, но лень.\n\n\nНужно будет поинтересоваться через какое-то время, что она об этой книге думает..."
    new "Ich habe Lisa das Buch gegeben, das einige ihrer Fragen beantworten sollte. Ja, es wäre nützlich für mich, es selbst zu lesen, aber ich bin zu faul.\n\n\nIch werde sie nach einer Weile fragen müssen, was sie über dieses Buch denkt..."

    # game/core/03variables.rpy:324
    old "Только я подарил Лизе книгу по сексуальному образованию, как тут же нарисовался Эрик. Он, видите ли, вместе с моей мамой, собирается взяться за её сексуальное воспитание! \n\nВ обмен на возможность подглядывать за этим процессом и кое-каким бонусом, с которым мне нужно будет определиться со временем, я согласился его поддержать. Мы же с ним как-никак \"друзья\"."
    new "Kaum hatte ich Lisa ein Buch über Sexualerziehung gegeben, tauchte Eric auf. Er und Mom werden ihre Sexualerziehung übernehmen! \n\nAls Gegenleistung für die Möglichkeit, diesen Prozess auszuspionieren, und einen unbestimmten Bonus habe ich zugestimmt, zu helfen. Wir sind ja schließlich \"Freunde\"."

    # game/core/03variables.rpy:324
    old "Только я подарил Лизе книгу по сексуальному образованию, как тут же нарисовался Эрик. Он, видите ли, вместе с моей мамой, собирается взяться за её сексуальное воспитание! \n\nВ обмен на возможность подглядывать за этим процессом, я согласился его поддержать. Незачем мне враждовать с Эриком."
    new "Kaum hatte ich Lisa ein Buch über Sexualerziehung gegeben, tauchte Eric auf. Er und Mom werden ihre Sexualerziehung übernehmen! \n\nIm Austausch für die Möglichkeit, diesen Prozess auszuspionieren, habe ich zugestimmt, zu helfen. Ich muss Eric nicht verärgern."

    # game/core/03variables.rpy:324
    old "Только я подарил Лизе книгу по сексуальному образованию, как тут же нарисовался Эрик. Он, видите ли, вместе с моей мамой, собирается взяться за её сексуальное воспитание! \n\nНе смотря на то, что мы с ним \"дружим\", я постарался убедить его, что Лизе ещё рановато это познавать... Это не особо помогло, но Эрик дал мне пару недель на \"чтение книг по теме сексуального образования\" с Лизой. По крайней мере, я выиграл немного времени, которое стоит использовать с умом..."
    new "Kaum hatte ich Lisa ein Buch über Sexualerziehung gegeben, tauchte Eric auf. Er und Mom werden ihre Sexualerziehung übernehmen! \n\nObwohl er und ich \"Freunde\" sind, habe ich versucht, ihn davon zu überzeugen, dass es für Lisa noch ein bisschen früh ist, um zu lernen... Es hat nicht viel geholfen, aber Eric gab mir ein paar Wochen Zeit, um mit Lisa \"Bücher über Sexualerziehung\" zu lesen. Wenigstens habe ich mir etwas Zeit verschafft, die ich weise nutzen kann..."

    # game/core/03variables.rpy:324
    old "Только я подарил Лизе книгу по сексуальному образованию, как тут же нарисовался Эрик. Он, видите ли, вместе с моей мамой, собирается взяться за её сексуальное воспитание! \n\nЯ, естественно, отказался ему в этом содействовать. Враждовать, так враждовать! Вряд ли теперь у меня есть много времени, чтобы избавиться от него, так что нужно скорее искать такой способ..."
    new "Kaum hatte ich Lisa ein Buch über Sexualerziehung gegeben, tauchte Eric auf. Er und Mom werden ihre Sexualerziehung übernehmen! \n\nNatürlich habe ich mich geweigert, ihm dabei zu helfen. Auf die Feindschaft! Ich glaube nicht, dass ich jetzt viel Zeit habe, ihn loszuwerden, also sollte ich lieber nach einem solchen Weg suchen..."

    # game/core/03variables.rpy:324
    old "Как выяснилось, Эрик проплатил репетитора для Лизы по воскресеньям, чтобы улучшить её оценки, а в действительности, чтобы втереться к ней в доверие. Теперь он и моя мама будут по понедельникам проводить для Лизы уроки сексуального воспитания..."
    new "Wie sich herausstellte, hat Eric für Lisas Sonntagsnachhilfe bezahlt, um ihre Noten zu verbessern, in Wirklichkeit versucht er, ihr näher zu kommen. Jetzt werden er und Mom Lisa Montags Aufklärungsunterricht geben..."

    # game/core/03variables.rpy:324
    old "Как выяснилось, Эрик проплатил дополнительные курсы для Лизы в школе, чтобы улучшить её оценки, а в действительности, чтобы втереться к ней в доверие. А заодно и от меня отстранить, ведь теперь моя помощь с уроками ей особо и не нужна. И ещё он и моя мама будут по понедельникам проводить для Лизы уроки сексуального воспитания..."
    new "Wie sich herausstellte, hat Eric für Lisas Sonntagsnachhilfe bezahlt, um ihre Noten zu verbessern, in Wirklichkeit versucht er, ihr näher zu kommen. Ganz zu schweigen davon, sie weniger abhängig von mir zu machen, so dass sie meine Hilfe nicht braucht. Jetzt werden er und Mom Lisa Montags Aufklärungsunterricht geben..."

    # game/core/03variables.rpy:350
    old "Особые книги"
    new "Besondere Bücher"

    # game/core/03variables.rpy:350
    old "Алиса читает какие-то книги, но не хочет говорить о них. На порно журналы не похоже... Что же ещё там может быть? Нужно попытаться выяснить это как можно скорее... Любопытно же!"
    new "Alice liest gerade einige geheimnisvolle Bücher, über die sie nicht sprechen möchte. Sie sehen nicht wie Pornos aus, was könnte das wohl sein? Ich sollte dem nachgehen."

    # game/core/03variables.rpy:350
    old "Я нашёл какую-то книгу, но из названия ничего не понятно, а читать саму книгу ни времени, ни желания нет. Может быть, поискать о ней информацию в интернете?"
    new "Ich habe ein Buch gefunden, aber der Name ist seltsam, und ich möchte meine Zeit nicht damit verschwenden, es zu lesen. Ich sollte es im Internet suchen."

    # game/core/03variables.rpy:350
    old "Вот это да! Я был не так далёк от истины. Конечно, это не порно, но уж точно эротика, да ещё какая! Видимо, Алисе нравятся любовные романы с эротическими оттенками, так сказать. Может быть, стоит ей подарить подобную книгу и посмотреть на её реакцию?"
    new "Wow, ich hatte fast Recht! Es ist kein Porno, aber es ist definitiv Erotik, und es ist auch nicht die sanfte Art! Ich schätze, Alice mag Liebesgeschichten mit einem erotischen Touch. Vielleicht sollte ich ihr welche schenken und ihre Reaktion sehen."

    # game/core/03variables.rpy:350
    old "Судя по реакции Алисы, подарок ей понравился. Видимо, стоит периодически заходить в книжный интернет-магазин и смотреть новинки. Возможно, таким образом удастся чуть-чуть улучшить отношения с Алисой."
    new "Ihrer Reaktion nach zu urteilen, gefiel ihr mein Geschenk. Ich sollte ab und zu im E-Book-Store nach Neuerscheinungen Ausschau halten."

    # game/core/03variables.rpy:357
    old "Одноклассник"
    new "Schulfreund"

    # game/core/03variables.rpy:357
    old "Очевидно, Лиза по уши влюблена в какого-то своего одноклассника по имени Алекс. Как она рассказала, у него есть подруга Оливия, самая красивая девочка в школе. Лиза ждёт от меня помощи в виде совета - как ей быть дальше.\n\nВ голову приходит сразу три варианта: предложить ей забыть об этом парне, предложить ей сражаться с Оливией за Алекса до конца или же помочь переключить внимание Оливии на кого-то другого..."
    new "Lisa ist offensichtlich in ihren Schulfreund Alex verliebt. Sie erzählte mir, dass er eine Freundin hat, Olivia, das schönste Mädchen der Schule. Ich kann Lisa helfen, indem ich ihr den nächsten Schritt vorschlage.\n\nIch sehe drei Möglichkeiten: ihr sagen, sie soll ihn ganz vergessen, mit Olivia um ihn kämpfen, oder vielleicht könnten wir Olivias Aufmerksamkeit auf jemand anderen lenken..."

    # game/core/03variables.rpy:357
    old "Да, и похоже о своём мальчике Лиза разговаривает только, когда у неё хорошее настроение... \n\n{i}{b}Внимание:{/b} Пока это всё, что можно сделать для данной \"возможности\" в текущей версии игры.{/i}"
    new "Oh, und wie es aussieht, spricht Lisa nur über ihn, wenn sie gut gelaunt ist...\n\n{i}{b}Achtung:{/b} In der aktuellen Version des Spiels haben Sie das Ende dieser \"Möglichkeit\" erreicht.{/i}"

    # game/core/03variables.rpy:357
    old "Я предложил Лизе забыть о её парне и, похоже, она прислушалась к моему совету. Конечно, она расстроилась, но может быть это и к лучшему? Возможно, она сосредоточит своё внимание на ком-то, кто всегда рядом?"
    new "Ich sagte Lisa, sie solle Alex vergessen, und sie folgte meinem Rat. Sie ist sehr verärgert, aber vielleicht ist es besser so? Vielleicht richtet sie ihre Aufmerksamkeit jetzt auf jemanden, den sie etwas besser kennt?"

    # game/core/03variables.rpy:357
    old "Я убедил Лизу, что нужно бороться за своё счастье. Осталось только разработать план и достичь поставленной цели!"
    new "Ich habe Lisa überzeugt, dass sie für ihr Glück kämpfen sollte. Jetzt muss ich mir nur noch einen Plan ausdenken und dieses Ziel erreichen!"

    # game/core/03variables.rpy:357
    old "Я разработал хитрый план, который сделает всех счастливыми: Лиза должна подужиться с Оливией, стать её лучшей подругой, познакомить её со мной, а я её соблазню... Ну, попытаюсь. Но если получится, то у меня будет девушка, а у Лизы её Алекс. Отличный план! Вот только как соблазняют самых красивых девочек школы?"
    new "Mir ist eine Idee gekommen, die alle glücklich machen würde. Lisa muss sich mit Olivia anfreunden, mich ihr vorstellen und dann werde ich Olivia verführen... Nun, ich werde es versuchen. Aber wenn ich es schaffe, bekomme ich eine Freundin und Lisa bekommt Alex. Toller Plan! Nun, wie würde man vorgehen, um das hübscheste Mädchen der Schule zu verführen?"

    # game/core/03variables.rpy:364
    old "Блог"
    new "Blog"

    # game/core/03variables.rpy:364
    old "Алиса рассказала о своём блоге и о своей проблеме, связанной с потерей вещей во время переезда. Теперь ей нечего показывать, не в чем вести блог и, вообще, она в печали. Кажется, она согласна на любую помощь и идеи, даже от меня! Может быть, посидеть в интернете, поискать какое-то решение?"
    new "Alice hat mir von ihrem Blog und dem \"Mangel an Kleidung\"-Problem erzählt. Jetzt hat sie nichts vorzuweisen, nichts, was sie im Blog anziehen kann, und sie hat ein gebrochenes Herz. Sie bittet verzweifelt um jede Art von Hilfe, sogar von mir! Vielleicht sollte ich im Internet nach Lösungen suchen?"

    # game/core/03variables.rpy:364
    old "В результате поисков информации в интернете о том, чем могла бы заняться Алиса, удалось кое-что выяснить. Самыми популярными оказались блоги, где ведущие - девушки. Причём, чем более откровенные наряды и чем больше грудь, тем более популярны шоу. С грудью, конечно, ничего не поделать, а вот наряды... Может быть, поговорить с ней об этом?"
    new "Nachdem ich umfangreiche Recherchen im Internet durchgeführt hatte, machte ich eine schockierende Entdeckung. Alle der beliebtesten Blogger sind weiblich! Darüber hinaus scheint ihre Popularität eng mit der Knappheit ihres Outfits und der Größe ihrer Brüste zusammenzuhängen. Nun, gegen Alices Brüste kann ich nicht viel tun, aber einige erotischere Outfits... sollte ich mit ihr darüber sprechen?"

    # game/core/03variables.rpy:364
    old "Я пообщался с Алисой насчёт своих выводов о популярности блога и намекнул, что можно рекламировать не только крема и лаки, но и нижнее бельё, например. Но крутить своей попкой перед камерой она не собирается, так что покрутит сама того не зная, когда я поставлю скрытую камеру в её комнату. Как знать, сколько всего интересного можно будет увидеть..."
    new "Ich diskutierte meine Schlussfolgerungen mit Alice und schlug vor, dass sie nicht nur für Cremes und Kosmetika werben sollte, sondern zum Beispiel auch für Dessous. Aber sie wird noch keine Show abziehen. Zumindest nicht wissentlich. Ich frage mich, was eine gut platzierte, versteckte Kamera in ihrem Zimmer enthüllen könnte?"

    # game/core/03variables.rpy:364
    old "Я пообщался с Алисой насчёт своих выводов о популярности блога и намекнул, что можно рекламировать не только крема и лаки, но и нижнее бельё, например. Но крутить своей попкой перед камерой она не собирается... А зря, ведь она и так уже это делает на скрытую камеру, которую я поставил в её комнате. Как знать, может мне удастся увидеть что-то, чего ещё не было..."
    new "Ich diskutierte meine Schlussfolgerungen mit Alice und schlug vor, dass sie nicht nur für Cremes und Kosmetika werben sollte, sondern zum Beispiel auch für Dessous. Auch wenn sie vielleicht nicht bereit ist, eine Show zu veranstalten, hat sie mir dank der versteckten Kamera in ihrem Zimmer trotzdem eine gegeben. Ich sollte es jetzt, nachdem wir dieses Gespräch geführt haben, noch einmal überprüfen..."

    # game/core/03variables.rpy:364
    old "Я решил снова предложить Алисе идею по развитию её блога - рекламировать нижнее бельё. Удивительно, но она согласилась! Правда, теперь мне нужно найти нижнее бельё для того, чтобы она заинтересовала свою аудиторию и привлекла внимание рекламодателей..."
    new "Ich erinnerte Alice an die Idee, in ihrem Blog für Dessous zu werben, und überraschenderweise stimmte sie zu! Ich sollte etwas finden, das sie sexy anzieht. Etwas, das wirklich die Aufmerksamkeit der Leute erregen kann..."

    # game/core/03variables.rpy:364
    old "Я подарил Алисе симпатичный комплект нижнего белья. Ей очень понравилось. Правда, увидеть мне ничего не удалось, только Алису уже в этом самом белье, но даже это было волнующе... Что самое любопытное, она намекнула, что можно поискать и что-то более... сексуальное!"
    new "Ich habe Alice ein süßes Unterwäscheset geschenkt. Sie mochte sie wirklich. Ich habe jedoch nichts zu sehen bekommen. Sie trug sie, bevor ich zusehen durfte, aber selbst das war aufregend... Aber das Interessante war... sie deutete an, dass ich versuchen könnte, nach etwas mehr erotischerem zu suchen!"

    # game/core/03variables.rpy:364
    old "Я подарил Алисе симпатичный комплект нижнего белья. Ей очень понравилось. Она даже при мне его примерила! Увидел я не так чтобы много всего, но было волнующе... Что самое любопытное, она намекнула, что можно поискать и что-то более... сексуальное!"
    new "Ich habe Alice ein süßes Unterwäscheset geschenkt. Sie mochte sie wirklich. Ich durfte ihr sogar beim Anprobieren zusehen! Das war aufregend... Aber das Interessante war... sie deutete an, dass ich versuchen könnte, nach etwas mehr erotischerem zu suchen!"

    # game/core/03variables.rpy:364
    old "Я подобрал кое-что более сексуальное для Алисы, а именно - полупрозрачное боди. В меру откровенное, потому что иначе я рискую не увидеть его на ней. А при дарении, пока она была под впечатлением, я ещё и умудрился уговорить её попозировать мне... нужны же ей качественные фотоснимки для развития блога! \n\nМного я не наснимал, всё-таки мы с Алисой ещё не настолько близки, но уже лучше находим общий язык. Посмотрим, что будет дальше..."
    new "Ich habe etwas Aufreizenderes für Alice ausgesucht, einen halbtransparenten Body. Er ist immer noch bescheiden, weil ich sonst riskiere, sie darin nicht zu sehen. Während ich ihn ihr präsentierte, gelang es mir auch, sie zu überreden, für mich zu posieren... Sie brauchte hochwertige Fotos für ihren Blog! \n\nEs ist nicht so, dass sie zu anzüglich waren. Auch wenn Alice und ich uns noch nicht so nahe stehen, habe ich das Gefühl, dass wir anfangen, eine gemeinsame Basis zu finden. Wir werden sehen, was als nächstes passiert..."

    # game/core/03variables.rpy:364
    old "Я подобрал кое-что более сексуальное для Алисы, а именно - полупрозрачное боди. В меру откровенное, потому что иначе я рискую не увидеть его на ней. А при дарении, пока она была под впечатлением, я ещё и умудрился уговорить её попозировать мне... нужны же ей качественные фотоснимки для развития блога! \n\nФотосессия вышла очень интересной, спасибо за это конфетам с алкоголем. Мне удалось сделать несколько весьма горячих снимков с Алисой и они прекрасно подойдут для моей коллекции... Хоть мы с Алисой всё ещё не очень близки, но уже лучше находим общий язык. Посмотрим, что будет дальше..."
    new "Ich habe etwas Aufreizenderes für Alice ausgesucht, einen halbtransparenten Body. Er ist immer noch bescheiden, weil ich sonst riskiere, sie darin nicht zu sehen. Während ich ihn ihr präsentierte, gelang es mir auch, sie zu überreden, für mich zu posieren... Sie brauchte hochwertige Fotos für ihren Blog! \n\nDas Foto-Shooting war dank der Pralinen und des Alkohols interessant. Ich habe es geschafft, einige sehr heiße Bilder von Alice für meine Sammlung zu machen... Obwohl Alice und ich uns noch nicht so nahe stehen, habe ich das Gefühl, dass wir anfangen, eine gemeinsame Basis zu finden. Wir werden sehen, was als nächstes passiert..."

    # game/core/03variables.rpy:364
    old "Только мои отношения с Алисой начали понемногу налаживаться, как ко мне подвалил Эрик с расспросами о том, чем там моя старшая сестрёнка занимается за компьютером... \n\nЯ рассказал ему всё, что знал о её блоге. Ну а как иначе, мы же друзья. Теперь он собирается помочь ей с развитием блога и мне лучше этому не мешать... а ещё лучше - помогать ему с этим."
    new "Gerade als meine Beziehung zu Alice anfing, besser zu werden, kam Eric zu mir rüber und fragte, was meine große Schwester am Computer macht... \n\nIch habe ihm alles erzählt, was ich über ihren Blog wusste. Natürlich habe ich das, wir sind Freunde. Jetzt wird er ihr dabei helfen, und ich sollte ihr besser nicht in die Quere kommen... Oder besser noch, ich schaue, ob ich helfen kann."

    # game/core/03variables.rpy:364
    old "Только мои отношения с Алисой начали понемногу налаживаться, как ко мне подвалил Эрик с расспросами о том, чем там моя старшая сестрёнка занимается за компьютером... \n\nЯ решил не облегчать ему жизнь и сказал, что мало об этом знаю. Эрик же, чтобы не тратить своё время, попросил меня за несколько недель узнать как можно больше о занятиях Алисы..."
    new "Gerade als meine Beziehung zu Alice anfing, besser zu werden, kam Eric zu mir rüber und fragte, was meine große Schwester am Computer macht... \n\nIch beschloss, es ihm nicht leicht zu machen und sagte, dass ich nichts wüsste. Jetzt will Eric, dass ich nachforsche und ihm in ein paar Wochen berichte, was ich weiß..."

    # game/core/03variables.rpy:364
    old "Только мои отношения с Алисой начали понемногу налаживаться, как ко мне подвалил Эрик с расспросами о том, чем там моя старшая сестрёнка занимается за компьютером... \n\nЯ решил прекратить вражду с Эриком и рассказал ему всё, что знал о её блоге. Теперь он собирается помочь ей с развитием блога и мне лучше этому не мешать... а ещё лучше - помогать ему с этим."
    new "Gerade als meine Beziehung zu Alice anfing, besser zu werden, kam Eric zu mir rüber und fragte, was meine große Schwester am Computer macht... \n\nIch beschloss, meinen Krieg mit ihm zu beenden und erzählte ihm alles, was ich wusste. Jetzt wird er Alice mit ihrem Blog helfen, und ich sollte ihm besser nicht in die Quere kommen... Oder besser noch, ich schaue, ob ich helfen kann."

    # game/core/03variables.rpy:364
    old "Только мои отношения с Алисой начали понемногу налаживаться, как ко мне подвалил Эрик с расспросами о том, чем там моя старшая сестрёнка занимается за компьютером... \n\nУ меня совершенно нет желания помогать ему подкатывать к моей сестрёнке, что ему и сказал. У нас ведь вражда! Главное, чтобы мне это боком не вышло..."
    new "Gerade als meine Beziehung zu Alice anfing, besser zu werden, kam Eric zu mir rüber und fragte, was meine große Schwester am Computer macht... \n\nIch habe absolut keine Lust, ihm dabei zu helfen, meine Schwester anzumachen, was ich ihm auch gesagt habe. Wir haben eine Fehde. Ich will nicht verletzt werden..."

    # game/core/03variables.rpy:364
    old "Теперь, каждую среду, пока мама принимает ванну, Эрик тусуется у Алисы в комнате, якобы помогая ей с блогом. Понятно, что на самом деле он хочет поглазеть на Алису в нижнем белье... а в будущем и в трусики к ней залезть."
    new "Während Mom mittwochs im Bad ist, hängt Eric in Alices Zimmer ab. Angeblich versucht er, ihr mit ihrem Blog zu helfen, aber es ist klar, dass er sie nur anstarren will, während sie Dessous trägt... und versuchen, in ihr Höschen zu kommen, wenn er die Chance dazu hat."

    # game/core/03variables.rpy:364
    old "Теперь, каждую среду, пока мама принимает ванну, Эрик тусуется у Алисы в комнате, якобы помогая ей с блогом. Понятно, что на самом деле он хочет поглазеть на Алису в нижнем белье... а в будущем и в трусики к ней залезть.\n\nА ещё мне повезло узнать, что Эрик собирается купить ей новое кружевное боди! Обидно, конечно, что она попросила об этом не меня. Интересно, как Эрик отреагирует, если я его опережу с покупкой..."
    new "Während Mom mittwochs im Bad ist, hängt Eric in Alices Zimmer ab. Angeblich versucht er, ihr mit ihrem Blog zu helfen, aber es ist klar, dass er sie nur anstarren will, während sie Dessous trägt... und versuchen, in ihr Höschen zu kommen, wenn er die Chance dazu hat.\n\nIch hatte auch das Glück, herauszufinden, dass Eric ihr einen neuen Spitzenbodysuit kaufen wollte! Schade nur, dass sie nicht von mir danach gefragt hat. Ich frage mich, wie Eric reagieren würde, wenn ich ihm zuvorkommen würde..."

    # game/core/03variables.rpy:364
    old "Если я хочу это сделать, то нужно торопиться, чтобы успеть подарить ей боди до субботы... И дарить надо, когда Алиса занимается блогом, тогда она может быть переоденется при мне!"
    new "Wenn ich das machen will, muss ich mich beeilen, um ihr vor Samstag einen Body zu besorgen... Und ich muss ihn ihr geben, wenn Alice in einem Blog beschäftigt ist, dann kann sie sich vor mir umziehen!"

    # game/core/03variables.rpy:364
    old "Что бы я там не хотел, теперь уже поздно! Мне остаётся только наблюдать..."
    new "Was ich auch will, jetzt ist es zu spät! Ich kann nur noch zusehen..."

    # game/core/03variables.rpy:364
    old "Эрик подарил Алисе кружевное боди, которое она просила. Похоже, это их сблизило, чего он и добивался, а вот я не успел его обойти. Нужно скорее избавляться от Эрика, иначе он заберёт у меня всё..."
    new "Eric gab Alice den Spitzenbody, um den sie gebeten hatte. Es schien sie einander näher zu bringen, was er ja auch wollte, aber ich kam nicht dazu. Ich muss Eric bald loswerden, sonst nimmt er mir alles weg..."

    # game/core/03variables.rpy:364
    old "Эрик подарил Алисе кружевное боди, которое она просила. Похоже, это их сблизило, чего он и добивался. И будет добиваться дальше..."
    new "Eric gab Alice den Body, um den sie gebeten hatte. Es schien sie einander näher zu bringen, ganz nach seinem Plan..."

    # game/core/03variables.rpy:364
    old "Мне удалось опередить Эрика с покупкой кружевного боди и подарить его Алисе первым! Она даже переоделась при мне и не слишком при этом прикрывалась. Правда в конце у неё ногу свело, но это значит, что у меня теперь есть весомый повод забраться в массаже её прелестных ножек несколько дальше, чем раньше... И мне стоит попробовать это сделать!"
    new "Ich habe Eric geschlagen und Alice den Body zuerst gegeben! Sie zog ihn sogar vor mir an. Beim Anziehen verkrampfte ihr Bein, aber das gab mir einen guten Grund, ihre schönen Beine noch ein bisschen weiter zu massieren als vorher... Ich sollte versuchen, ihr danach eine Massage zu geben!"

    # game/core/03variables.rpy:364
    old "Эрик оказался, мягко говоря, не в восторге от того, что я опередил его с дарением кружевного боди для Алисы.\n\nОн предупредил, что если я ещё раз испорчу его планы, друзьями нам дальше не быть... А в качестве наказания, лишил меня всех возможных \"премиальных\", что мне теперь нужно как-то исправить..."
    new "Eric war, gelinde gesagt, nicht begeistert, dass ich ihm zuvorkam, indem ich Alice einen Spitzenbody schenkte. Er warnte mich, dass wir keine Freunde mehr sein würden, wenn ich seine Pläne noch einmal durchkreuzte... Und zur Strafe hat er mir meinen ganzen \"Bonus\" weggenommen, den ich nun ausgleichen muss..."

    # game/core/03variables.rpy:364
    old "Эрик оказался, мягко говоря, не в восторге от того, что я опередил его с дарением кружевного боди для Алисы.\n\nОн пригрозил, что у меня теперь будут большие проблемы! Как будто меня этим можно испугать..."
    new "Eric war nicht glücklich darüber, dass ich ihm zuvorkam, als er Alice den Body gab. \n\nJetzt hat er mir gedroht, als ob ich in Schwierigkeiten wäre! Als ob mir das Angst einjagen würde..."

    # game/core/03variables.rpy:393
    old "Купальник"
    new "Badesachen"

    # game/core/03variables.rpy:393
    old "Я заметил, что Лиза всё время в одном и том же закрытом купальнике. Так же невозможно толком загореть? Она тоже так считает, но другого нет. Остальные пропали с другими вещами во время переезда. Может быть, удастся как-то ей помочь?"
    new "Mir ist aufgefallen, dass Lisa immer den gleichen einteiligen Badeanzug trägt. Wie soll sie sich darin bräunen? Sie ist einverstanden, aber sie hat nichts anderes zum Anziehen, alles ging verloren, als wir umzogen. Vielleicht kann ich ihr irgendwie helfen?"

    # game/core/03variables.rpy:393
    old "Во время завтрака Лиза намекнула маме, что нужно бы купить во время шоппинга купальник, который не скрывает половину тела и в котором можно загорать..."
    new "Beim Frühstück deutete Lisa Mom an, dass sie einen neuen Bikini braucht, damit sie braun werden kann."

    # game/core/03variables.rpy:393
    old "Выяснилось, что во время последнего шоппинга забыли купить Лизе купальник, о котором она просила. Теперь придётся ждать до следующих выходных, когда Эрик снова повезёт всех за покупками. Но в этот раз сам Эрик пообещал ей подарить этот купальник. Может быть, его опередить?"
    new "Mon hat vergessen, einen neuen Bikini für Lisa zu kaufen. Jetzt muss sie bis zum nächsten Wochenende warten, wenn Eric mit allen einkaufen gehen wird. Dieses Mal hat Eric versprochen, selbst einen zu kaufen. Ich sollte ihm zuvorkommen!"

    # game/core/03variables.rpy:393
    old "Мне удалось накопить немного денег и купить тот самый купальник, который так хотела Лиза. Кажется, она меня любит! А Эрик теперь обломается со своими подарками, ведь я уже подарил то, что так хочет младшая сестрёнка!"
    new "Ich hatte genug Geld, um Lisa den Bikini zu kaufen, den sie wollte. Ich glaube, sie weiß ihn wirklich zu schätzen. Und Eric kann mich mal."

    # game/core/03variables.rpy:393
    old "Мне не удалось купить и подарить Лизе купальник до того, как это сделал для неё Эрик. Кажется, их отношения улучшились. Вот если бы я подарил тот чёртов купальник, то Лиза была бы мне очень благодарна, а не ему..."
    new "Ich konnte Lisa den Bikini, den sie haben wollte, nicht vor Eric kaufen. Ich glaube, ihre Beziehung zu ihm hat sich verbessert. Schade, dass ich ihr diesen dummen Bikini nicht geben konnte, damit Lisa mir dankbar wäre."

    # game/core/03variables.rpy:402
    old "Тусовщица"
    new "Partygirl"

    # game/core/03variables.rpy:402
    old "Алиса настойчиво просит маму купить ей вечернее платье для посещения ночных клубов. Та ей регулярно отказывала, пока Эрик не предложил подарить ей платье во время следующего шоппинга. Нужно поспешить, если я хочу сделать это первым!"
    new "Alice bat Mom um ein neues Abendkleid. Normalerweise würde Mom nein sagen, aber dieses Mal sagte Eric, dass er ihr beim nächsten Einkaufsbummel eines kaufen würde. Ich sollte mich beeilen, wenn ich ihm zuvorkommen und ihr das Geschenk selbst überreichen will."

    # game/core/03variables.rpy:402
    old "Мне удалось скопить приличную сумму и купить платье для Алисы. Теперь, нужно успеть его подарить ей, пока это не сделал кто-то другой..."
    new "Es gelang mir, etwas Geld zu verdienen und ein Kleid für Alice zu kaufen. Jetzt muss ich es ihr schenken, bevor Eric ihr ein Kleid schenkt."

    # game/core/03variables.rpy:402
    old "Вот что значит не везёт. Я долго копил на красивое короткое платье для Алисы и даже купил его, но меня опередил Эрик... И что самое обидное, он купил точно такое же, но подарил его первым. Теперь у меня есть бесполезное платье..."
    new "Das bringt Unglück. Ich habe so viel Geld für ein schönes kurzes Kleid für Alice ausgegeben, aber Eric ist mir zuvorgekommen. Und natürlich hat er ihr die gleiche Art von Kleid besorgt, also kann ich ihr nicht einfach meins geben."

    # game/core/03variables.rpy:402
    old "Вот что значит не везёт. Я долго копил на красивое короткое платье для Алисы, но меня опередил Эрик... Не удивительно. У него же куча денег. Он может себе позволить такие подарки... Кажется, теперь Алиса к нему относится гораздо лучше..."
    new "Das bringt Unglück. Ich habe so sehr versucht, dieses Kleid zu kaufen, aber am Ende spielte es keine Rolle mehr. Kein Wunder, Eric ist stinkreich, er kann es sich sicher leisten. Jetzt hat sich sein Verhältnis zu Alice verbessert."

    # game/core/03variables.rpy:402
    old "Да, я сделал это! Мне удалось не только накопить на платье, но и подарить его до того, как нечто подобное сделал Эрик. А он собирался. С его то деньгами это не сложно... Теперь Алиса ко мне относится намного лучше. Ещё бы! Теперь она может ходить по ночным клубам..."
    new "Ja, ich hab's geschafft! Es ist mir nicht nur gelungen, nicht nur das Kleid für sie zu kaufen, sondern ich habe es ihr auch geschenkt, bevor Eric es konte. Und das hatte er auch vor. Es ist nicht schwer mit seinem Geld... Alice behandelt mich jetzt viel besser. Absolut! Jetzt kann sie in Nachtclubs gehen..."

    # game/core/03variables.rpy:402
    old "Лиза рассказала мне по секрету кое-что интересное. Оказывается, Алиса не умеет пить. А точнее, капля алкоголя её меняет!"
    new "Lisa hat mir ein saftiges Geheimnis verraten. Es stellte sich heraus, dass Alice nicht besonders trinkfest ist. Oder, genauer gesagt, der Alkohol verändert ihr Verhalten drastisch."

    # game/core/03variables.rpy:402
    old "Именно поэтому она и не соглашается ни на какие алкогольные напитки. Но, может быть, можно придумать что-то другое?"
    new "Deshalb trinkt sie auch nie. Vielleicht kann ich mir etwas einfallen lassen?"

    # game/core/03variables.rpy:402
    old "Итак, у меня есть коробка конфет с ликёром. Судя по надписи на коробке, аромат утончённый. Это значит, есть шанс, что Алиса не заметит алкоголь. Интересно, этого хватит, чтобы её как-то подтолкнуть к этим... безбашенным поступкам?"
    new "Nun, ich habe mir eine Schachtel Schoko-Likör-Pralinen besorgt. Sie sollten so dezent sein, dass die Schwester keinen Alkohol bemerkt. Ich frage mich, ob das reichen würde, damit sie sich... verändern kann."

    # game/core/03variables.rpy:402
    old "Всё получилось. И прошло как-то... слишком неожиданно. О таком даже не принято никому рассказывать. Видимо, лишняя капля алкоголя сорвала Алисе крышу и она напилась в ночном клубе. Ну что же, будем надеяться, что она сама ничего не помнит об этом, а иначе мне не жить..."
    new "Alles hat gut geklappt. Und ganz... unerwartet. Ich kann niemandem davon erzählen. Alice hat wohl ihre Hemmungen verloren und hat sich im Club betrunken. Hoffen wir, dass sie sich an nichts mehr erinnert, sonst bin ich erledigt..."

    # game/core/03variables.rpy:413
    old "Дым сигарет"
    new "Zigarettenrauch"

    # game/core/03variables.rpy:413
    old "Оказалось, что Алиса курит. Причём, она старается это делать, когда дома никого нет. Это можно использовать... Рассказать маме самому или подставить Алису, чтобы мама сама догадалась... А может быть, всё это позволит наладить с Алисой отношения? Столько вариантов. Какой же выбрать?"
    new "Wie sich herausstellt, ist Alice Raucherin und versucht, eine zu rauchen, wenn niemand in der Nähe ist. Ich könnte das benutzen... Ich könnte es Mom erzählen oder sogar Alice beschuldigen? Oder vielleicht könnte es unsere Beziehung verbessern, indem es ein Geheimnis bleibt? So viele Möglichkeiten, was soll ich wählen?"

    # game/core/03variables.rpy:413
    old "Мама всё узнала про сигареты и Алиса была наказана. Причём, серьёзно так, у всех на глазах, да по голой заднице. Что самое печальное - Алиса знает, что это я её сдал. Так что, она меня теперь ненавидит, наверное... Неизвестно, получится ли теперь завоевать её доверие..."
    new "Mom bemerkte den Rauchen und Alice bekam eine harte Strafe. Mom versohlt ihren nackten Hintern vor allen Leuten. Leider weiß Alice, dass ich sie verpfiffen habe, so dass sie mich wahrscheinlich hasst. Ich bin mir nicht sicher, ob ich ihr Vertrauen je wieder gewinnen kann."

    # game/core/03variables.rpy:413
    old "Мама почувствовала сигаретный дым, но я не сдал Алису. Кажется, сестрёнка это оценила. Вот только что мешает её подставить, чтобы мама узнала об этом не от меня, а как бы сама что-то нашла? И я не при делах!\n\nА может быть, эта вредная привычка Алисы поможет мне улучшить с ней отношения?"
    new "Mom bemerkte Rauch, aber ich habe Alice nicht verpfiffen. Ich könnte Alice immer noch auf eine Art und Weise reinlegen, die mich nicht belastet, oder vielleicht könnte ich meine Beziehung zu Alice verbessern."

    # game/core/03variables.rpy:420
    old "Школьница"
    new "Schulmädchen"

    # game/core/03variables.rpy:420
    old "Итак, в новой школе Лиза оказалась не такой отличницей, как раньше. Видимо, в новой школе уровень знаний требуется намного более высокий. Ну что же, это хорошая возможность проявить свои школьные знания и помочь младшей сестрёнке разобраться в материале... Или же наоборот, помешать получить хорошую оценку и понаблюдать, как мама её накажет? А может быть, попросить у неё что-то за помощь? Но сначала надо поговорить с Лизой и спросить, что она обо всём этом думает?"
    new "Lisas Noten haben einen Sprung nach unten gemacht. Es sieht so aus, als ob die neue Schule ein höheres Bildungsniveau erfordert. Das ist eine gute Gelegenheit, mein Wissen zu zeigen und meiner kleinen Schwester zu helfen... Oder ihre Noten zu sabotieren und zu sehen, wie Mom sie bestrafen wird. Oder vielleicht etwas als Gegenleistung für meine Hilfe zu verlangen? Aber zuerst muss ich mit Lisa sprechen und sehen, was sie über die Situation denkt."

    # game/core/03variables.rpy:420
    old "Я поговорил с Лизой насчёт уроков и намекнул, что моя помощь будет стоить... некоторых услуг. Конечно, она отказалась. Но это пока. До первого наказания. Как только её отшлёпают у меня на глазах, всё изменится! Но для этого надо сделать так, чтобы она снова получила двойку..."
    new "Ich habe mit Lisa gesprochen und angedeutet, dass meine Hilfe sie einige... Dienstleistungen kosten wird. Sie lehnte mich jedoch ab. Das heißt, bis zu ihrer ersten Bestrafung. Sie wird ihre Meinung ändern, nachdem Mom ihr vor mir den Hintern versohlt hat. Jetzt muss ich nur noch bei ihren Hausaufgaben versagen."

    # game/core/03variables.rpy:420
    old "Я поговорил с Лизой и пообещал ей, что помогу с уроками. Причём, абсолютно безвозмездно. То есть - даром. Конечно, это не очень свойственно для меня, но зато есть все шансы наладить отношения с сестрёнкой и завоевать её доверие..."
    new "Ich habe mit Lisa gesprochen und ihr zugesagt, ihr beim Lernen zu helfen. Umsonst. Aus gutem Grund. Natürlich ist das nicht sehr typisch für mich, aber ich kann die Beziehung zu meiner Schwester verbessern und ihr Vertrauen gewinnen."

    # game/core/03variables.rpy:420
    old "Кажется, я убедил Лизу меня слушаться под страхом наказания. Теперь она будет спать только в футболке и трусах. Ну ничего, я только начал.\n\nТеперь я могу под видом помощи с уроками, подправлять вычисления так, чтобы ей поставили двойку и можно было бы снова наблюдать за её наказанием... Кто знает, может быть, удастся развести её на нечто большее?"
    new "Ich glaube, ich habe Lisa überzeugt, mir aus Angst vor Bestrafung zuzuhören. Jetzt schläft sie nur noch in T-Shirt und Höschen. Und das ist erst der Anfang! Jetzt kann ich unter dem Vorwand, ihr zu helfen, ihre Hausaufgaben schlecht machen, damit ich zusehen kann, wie sie bestraft wird... Wer weiss, vielleicht kann ich sogar noch weiter gehen?"

    # game/core/03variables.rpy:420
    old "Хоть я и пообещал помогать Лизе с уроками, но делать этого я не стал. И без того было много дел. После очередной двойки и наказания от мамы, она подошла ко мне и упрашивала о помощи. Я согласился, но с условием, что она будет спать только в футболке и трусиках. И думаю, что мне удастся её ещё на что-нибудь раскрутить..."
    new "Obwohl ich versprochen hatte, Lisa bei ihrem Unterricht zu helfen, habe ich es nicht getan. Und es gab schon viel zu tun. Nach einem weiteren F und der Bestrafung von Mom kam sie zu mir und bat um Hilfe. Ich willigte ein, aber mit der Bedingung, dass sie nur in T-Shirt und Höschen schlafen würde. Und ich glaube, ich kann sie dazu bringen, noch etwas mehr zu tun..."

    # game/core/03variables.rpy:420
    old "Правда, Лиза слегка на меня обиделась, но это я переживу. Подарю ей что-нибудь вкусненькое и она оттает... \n\n{i}{b}Внимание:{/b} Пока это всё, что можно сделать для данной \"возможности\" в текущей версии игры.{/i}"
    new "Es stimmt, Lisa ist ein bisschen beleidigt von mir, aber ich komme darüber hinweg. Ich gebe ihr etwas Leckeres und sie wird auftauen... \n\n{i}{b}Achtung:{/b} In der aktuellen Version des Spiels haben Sie das Ende dieser \"Möglichkeit\" erreicht.{/i}"

    # game/core/03variables.rpy:420
    old "Я помогал Лизе с уроками какое-то время, причём безвозмездно, но это стало довольно скучным делом и я перестал уделять ей внимание с этим. И вот, после очередной двойки и наказания от мамы, она обратилась ко мне за помощью. Я согласился, но с условием, что она будет спать только в футболке и трусиках. И думаю, что мне удастся её ещё на что-нибудь раскрутить..."
    new "Ich habe Lisa eine Zeit lang kostenlos bei ihren Hausaufgaben geholfen, aber es wurde ziemlich langweilig, und ich hörte auf, mich darum zu kümmern. Und so kam sie, nach einem weiteren F und einer Bestrafung von Mom, zu mir und bat um Hilfe. Ich willigte ein, aber unter der Bedingung, dass sie nur in T-Shirt und Höschen schlafen würde. Und ich glaube, ich kann sie dazu bringen, noch etwas mehr zu tun..."

    # game/core/03variables.rpy:420
    old "Пообещав помогать Лизе с уроками по принципу \"ты мне - я тебе\", я через некоторое время совсем про неё забыл. И вот, после очередной двойки и наказания от мамы, Лиза напомнила о том, что я ей обещал. Пожалуй, мне стоит уделять её урокам больше внимания..."
    new "Nachdem ich versprochen hatte, Lisa bei ihrem Lernen zu helfen, vergaß ich unsere Gegenleistung. Und so erinnerte mich Lisa nach einer weiteren Runde von Bestrafungen durch Mom an das, was ich ihr versprochen hatte. Vielleicht sollte ich ihrem Unterricht mehr Aufmerksamkeit schenken..."

    # game/core/03variables.rpy:420
    old "Под видом помощи с уроками Лизы, я намеренно делал ошибки так, чтобы ей ставили двойки и наказывали... В чём теперь она меня и подозревает. Не удивительно, что Лиза сильно обиделась на меня, но вместе с тем, от моей помощи не отказалась..."
    new "Unter dem Deckmantel, Lisa beim Lernen zu helfen, habe ich absichtlich Fehler gemacht, damit sie ein D bekommt und bestraft wird... Und genau dessen verdächtigt sie mich jetzt. Kein Wunder, dass Lisa sehr wütend auf mich war, aber gleichzeitig wurde meine Hilfe nicht abgelehnt..."

    # game/core/03variables.rpy:420
    old "Сомневаюсь, что теперь получится её подставить, уж слишком большое недоверие она ко мне испытывает. \n\n{i}{b}Внимание:{/b} Пока это всё, что можно сделать для данной \"возможности\" в текущей версии игры.{/i}"
    new "Ich bezweifle, dass es möglich sein wird, jetzt absichtlich Fehler zu machen, sie ist mir zu misstrauisch. \n\n{i}{b}Achtung:{/b} In der aktuellen Version des Spiels haben Sie das Ende dieser \"Möglichkeit\" erreicht.{/i}"

    # game/core/03variables.rpy:420
    old "Я помогал Лизе с уроками какое-то время, причём безвозмездно, но это стало довольно скучным делом и я намеренно стал делать ошибки так, чтобы ей ставили двойки и наказывали... И вот, после очередной двойки и наказания от мамы, она обратилась ко мне за помощью. Я согласился, но с условием, что она будет спать только в футболке и трусиках. И думаю, что мне удастся её ещё на что-нибудь раскрутить..."
    new "Ich habe Lisa eine Zeit lang kostenlos bei ihren Hausaufgaben geholfen, aber es wurde ziemlich langweilig und ich habe absichtlich Fehler gemacht, damit sie eine Vier bekommt und bestraft wird... Und so kam sie, nach einem weiteren F und einer Bestrafung von Mom, zu mir und bat um Hilfe. Ich stimmte zu, aber unter der Bedingung, dass sie nur in T-Shirt und Höschen schläft. Und ich glaube, ich kann sie dazu bringen, nocht etwas mehr zu tun..."

    # game/core/03variables.rpy:436
    old "Арахнофобия"
    new "Angst vor Spinnen"

    # game/core/03variables.rpy:436
    old "Любопытно. Похоже, Алиса до смерти боится пауков. А надо заметить, что климат тут очень подходит для такого рода насекомых... Нужно будет поймать одного и подкинуть Алисе, вот забавно будет! Вот только как и где их искать?"
    new "Sieht so aus, als hätte Alice schreckliche Angst vor Spinnen. Und das Klima hier ist sehr gut für diese Art von Insekten... Eines zu fangen und bei Alice abzuladen, sollte urkomisch sein! Wie würde ich jetzt eins bekommen?"

    # game/core/03variables.rpy:436
    old "Я порылся в интернете и выяснил, что в нашей местности очень легко встретить один вид пауков. К счастью, он совершенно безвредный, но размеры бывают весьма солидные. Ночью их не найти, а вот когда солнце только начинает прогревать землю, они выползают на свет и проверяют свою добычу."
    new "Ich habe im Internet nachgeschaut, und es gibt eine lokale Art von Spinnen, die perfekt für mein Ziel wäre! Sie ist völlig harmlos und ziemlich groß. Sieht aus, als könnte ich versuchen, eine am Morgen zu fangen, wenn Spinnen ihre Netze auf Beute kontrollieren."

    # game/core/03variables.rpy:436
    old "Итак, я поймал огромного паука. Это не так сложно, если знать в какое время и где их ловить. Теперь нужно испытать этого монстра на сестрёнке. Вот только бы выбрать подходящий момент..."
    new "Ich habe mir eine riesige Spinne geholt, wie sich herausstellt, ist es ganz einfach, wenn man weiß, was man tut. Jetzt muss ich dieses Monster nur noch an meiner Schwester testen. Wo wäre der beste Ort für mein Experiment..."

    # game/core/03variables.rpy:436
    old "Алиса определённо боится пауков! Визг слышали все соседи со всей округи, это точно! Вот только если Алиса узнает, что я это всё специально подстроил, меня ждёт смерть. Надеюсь, быстрая..."
    new "Ja, Alice leidet unter einem riesigen Arachnophobie. Ihre Schreie wurden im ganzen Haus gehört. Wenn Alice jemals herausfindet, dass ich der Schuldige bin, wird sie mich auf jeden Fall umbringen. Ich hoffe, ich sterbe schnell."

    # game/core/03variables.rpy:436
    old "Алиса так боится пауков, что согласилась раздеться ради того, чтобы я убил одного, который залез в её постель. Ага, сам он залез, как же... Тем не менее, этой её фобией можно пользоваться в своих интересах. Посмотрим, к чему это всё приведёт..."
    new "Alice hat solche Angst vor Spinnen, dass sie zugestimmt hat, sich nackt auszuziehen, nur damit ich eine in ihrem Bett töten kann. Ich bin so glücklich, dass Spinnen immer wieder grundlos in Alices Schlafzimmer klettern... Wie auch immer, ich frage mich, was ich noch von Alice bekommen kann, wenn ich eine Spinne töte..."

    # game/core/03variables.rpy:436
    old "После очередного избавления от паука я намекнул Алисе, что было бы не плохо, если бы я спал рядом и охранял её. Она отказалась. Видимо, слишком мало пауков попадает к ней в постель..."
    new "Ich habe eine weitere Spinne für Alice getötet und sie gebeten, in ihrem Zimmer zu bleiben, um sie zu bewachen. Sie hat mir einen Korb gegeben. Ich schätze, sie braucht mehr Spinnen in ihrem Bett, um ihre Meinung zu ändern..."

    # game/core/03variables.rpy:436
    old "Похоже, я способствую развитию арахнофобии у Алисы. Во всяком случае, прогресс очевиден. Теперь Алиса задумывается о том, что было бы неплохо иногда впускать меня в постель для защиты от пауков. Но пока только задумывается... Значит, стоит продолжать..."
    new "Ich scheine die Arachnophobie in Alice zu fördern. Wie auch immer, der Fortschritt ist offensichtlich. Jetzt denkt Alice, es wäre vielleicht nett, mich manchmal ins Bett zu lassen, um sie vor Spinnen zu schützen. Aber während sie darüber nachdenkt... Also sollte ich so weitermachen..."

    # game/core/03variables.rpy:436
    old "Да! Алиса разрешила мне спать рядом. Конечно, потребовалось столько паучьих жертв, но оно того стоило! Первый шаг сделан, разрешение получено, а дальше уже всё пойдёт как по маслу!"
    new "Alice hat mich endlich in ihrem Bett schlafen lassen! Schweigeminute für all die Spinnen, die ihr Leben für diese großartige Leistung geopfert haben. Schritt eins ist geschafft, jetzt muss ich es nur noch weiter versuchen..."

    # game/core/03variables.rpy:436
    old "Наши забавы с Алисой в постели, как и ожидалось, дошли до самого интересного... Да она сама на меня прыгает! Слава паукам!"
    new "Unser Spaß im Bett mit Alice ist, wie erwartet, interessant geworden... Sie springt mich an! Ein Hoch auf die Spinnen!"

    # game/core/03variables.rpy:448
    old "Альфа"
    new "Alpha"

    # game/core/03variables.rpy:448
    old "Итак, я познакомился с этим самым Эриком, бойфрендом мамы. Не могу сказать о нём ничего хорошего или плохого, но он мне не очень понравился. Практически сразу он предложил мне выбрать дружить с ним или воевать. Вот только зачем мне с ним дружить? Что это даст? Да и воевать... Что он может мне сделать? Так или иначе, но он дал на размышления одну неделю. Ну, подумаю как следует и решу что попало..."
    new "Ich habe diesen Eric, Mom's Freund, getroffen. Ich kann nichts Gutes oder Schlechtes über ihn sagen, aber ich mag ihn nicht. Er bat mich, zwischen Freund und Krieg zu wählen. Warum sollte ich nun mit ihm befreundet sein wollen, was würde das bringen? Dasselbe gilt für den Krieg... Was könnte er mir antun? Jedenfalls gab er mir eine Woche Bedenkzeit. Ich schätze, ich muss das durchdenken und mich entscheiden."

    # game/core/03variables.rpy:448
    old "С появлением Эрика мои дела стали неожиданно плохи... Во-первых, он предложил отправить меня в какой-то военный лагерь. Вроде бы для того, чтобы сделать из меня мужика... Да я уже Мужик! Сдались мне эти лагеря...\n\nВо-вторых, он убедил маму, что наказывать нас надо как-то иначе. Нет бы что хорошее подсказал, но это... В общем, со следующей недели нас будут наказывать на глазах друг у друга... голыми! Это такой позор! Это же каким надо быть извращенцем, чтобы такое выдумать.\n\nПонятно, что будет прикольно лишний раз увидеть сиськи сестёр, но и меня могут выпороть на глазах у всех. Это же как подорвёт авторитет!\n\nСначала эта дурацкая идея с военным лагерем, теперь ещё и это... Нужно что-то срочно делать!"
    new "Eric ist definitiv eine Veränderung zum Schlechteren. Zuerst schlug er vor, mich in irgendein Militärlager zu schicken. \"Um einen Karakter aufzubauen\". Ich habe einen riesigen Karakter!\n\n\n Dann überzeugte er Mom, dass wir ein neues Bestrafungssystem haben sollten. Warum konnte er nicht etwas Gutes vorschlagen... Jedenfalls werden wir ab nächster Woche voreinander den Hintern versohlt bekommen! Und zwar nackt! Das ist demütigend. Was für ein Perverser würde auf diese Idee kommen? Das wird meine Autorität ruinieren! Ich muss sofort etwas unternehmen!"

    # game/core/03variables.rpy:448
    old "Ну что же, у меня такое чувство, что я заключил сделку с дьяволом. Эрик пообещал мне платить какие-то деньги и разрешил подглядывать за ними с мамой. В свою очередь, я не должен мешать ему совращаться свою семью... Кто знает, может быть это позволит и мне поучаствовать в чём-то таком... Если он разрешит..."
    new "Ich habe das Gefühl, einen Pakt mit dem Teufel geschlossen zu haben. Eric versprach, mir etwas Geld zu zahlen und gab mir die Erlaubnis, ihm und meiner Mutter beim Sex hinterherzuspionieren. Ich meinerseits muss zulassen, dass er meine ganze Familie korrumpiert. Wer weiß, vielleicht lässt er mich an seiner Aktion teilnehmen..."

    # game/core/03variables.rpy:448
    old "Да пошёл он, этот Эрик! Я отказался от его сомнительного предложения и не собираюсь иметь с ним ничего общего. Пусть попробует только полезть, я ему покажу! Глаза мои его бы не видели... Конечно, я отказался от всего что он там мог предложить, но зато моя совесть чиста и моя семья в моих руках!"
    new "Zum Teufel mit ihm! Ich habe sein zweifelhaftes Angebot abgelehnt und werde nicht mit ihm zusammenarbeiten. Ich werde nichts von seinem Angebot bekommen, aber mein Gewissen ist rein und meine Familie ist in meinen Händen!"

    # game/core/03variables.rpy:454
    old "Кнут или пряник?"
    new "Zuckerbrot oder Peitsche?"

    # game/core/03variables.rpy:454
    old "Лиза заметила, что я подглядывал, как она принимает душ. Я попробовал извиниться и пообещал купить ей какую-нибудь сладость. В обмен на это Лиза не станет рассказывать обо всём маме, но времени у меня немного..."
    new "Lisa bemerkte, dass ich sie beim Duschen beobachtete. Ich versuchte, mich zu entschuldigen und versprach, ihr Süßigkeiten zu kaufen. Im Gegenzug wird Lisa Mom nichts sagen, aber ich habe nicht viel Zeit..."

    # game/core/03variables.rpy:454
    old "К сожалению, я не успел подарить Лизе сладость в срок и поэтому она рассказала маме, как я за ней подглядывал. Возможно, в следующий раз, если Лиза меня заметит, я успею..."
    new "Ich hatte keine Gelegenheit, Lisa ihre Süßigkeiten rechtzeitig zu besorgen, also erzählte sie Mom von meiner Spionage. Wenn Lisa mich wieder erwischt, habe ich vielleicht noch eine Chance..."

    # game/core/03variables.rpy:454
    old "Я купил для Лизы шоколад \"Ritter Sport\" и она была очень довольна! Ещё бы, ведь оказалось, что это её любимая сладость! Может быть, в следующий раз, стоит подарить ей большые шоколадки?"
    new "Ich kaufte eine Schokolade \"Ritter Sport\" für Lisa und sie war sehr glücklich! Natürlich stellte sich heraus, dass es ihre Lieblingssüßigkeit war! Vielleicht sollte ich ihr das nächste Mal eine große Schokoladentafel schenken."

    # game/core/03variables.rpy:454
    old "Я купил для Лизы конфеты \"Raffaello\" и она приняла их, но без особого восторга! Возможно, в следующий раз, следует подарить ей что-то другое?"
    new "Ich kaufte Lisa \"Raffaello\" Süßigkeiten und sie nahm sie an, aber nicht sehr begeistert. Vielleicht sollten Sie ihr nächstes Mal etwas anderes geben?"

    # game/core/03variables.rpy:454
    old "Я купил для Лизы конфеты \"Ferrero Rocher\" и как оказалось, они ей не нравятся! В следующий раз, лучше подарить что-то другое..."
    new "Ich habe Lisa \"Ferrero Rocher\" Bonbons gekauft, und wie sich herausstellte, mag sie sie nicht! Nächstes Mal wäre es besser, ihr etwas anderes zu geben..."

    # game/core/03variables.rpy:454
    old "Как оказалось, за нравящиеся Лизе сладости можно получить крепкие объятия! И это не смотря на то, что я за ней подглядывал. Посмотрим, что ещё мне удастся получить от сестрёнки..."
    new "Sieht so aus, als bekäme man eine große Umarmung, wenn man Süßigkeiten besorgt, die Lisa mag! Und das trotz der Tatsache, dass ich ihr nachspioniert habe. Mal sehen, was ich noch von meiner Schwester bekommen kann..."

    # game/core/03variables.rpy:454
    old "Обнимашки - не единственное, что я получил от Лизы за сладости. Она была в таком восторге, что даже чмокнула меня в щёку! Вот уж не думал, что подглядывания за сестрёнкой могут обернуться такими приятными неожиданностями! Интересно, что будет дальше..."
    new "Umarmungen sind nicht das Einzige, was ich als Gegenleistung dafür bekommen kann, dass ich Lisa Süßigkeiten gebe. Sie war so glücklich, dass sie mich sogar auf die Wange geküsst hat! Ich hätte nicht gedacht, dass sich das Bespitzeln meiner Schwester als so angenehme Überraschung erweisen könnte! Ich frage mich, wie es weitergeht..."

    # game/core/03variables.rpy:454
    old "Похоже, сладостей Лизе уже мало и она хочет халатик, как у Алисы. Это уже серьёзная покупка! На этот раз времени у меня больше, но всё равно лучше поторопиться с подарком. Интересно, что меня будет за это ждать..."
    new "Es scheint, dass Süßigkeiten für Lisa nicht genug sind. Jetzt will sie einen Bademantel wie den von Alice. Das ist ein ernsthafter Kauf! Ich habe mehr Zeit, um es zu besorgen, aber es ist trotzdem besser, sich mit dem Geschenk zu beeilen. Ich frage mich, was ich dafür bekomme..."

    # game/core/03variables.rpy:454
    old "От моих подарков Лиза уже не просто целует меня в щёку, а запрыгивает на меня с крепкими объятиями! Может быть и мне уже пора в такие моменты проявлять какую-то активность? Так что подглядывать полезно, если правильно извиниться."
    new "Lisa gibt mir nicht mehr nur Küsse, sie springt mich an, um mich zu umarmen! Vielleicht sollte ich die Initiative ergreifen? Es könnte sich jetzt lohnen, zu gucken, solange ich die richtige Entschuldigung dafür finde."

    # game/core/03variables.rpy:466
    old "Кто не рискует..."
    new "Wer kein Risiko eingeht..."

    # game/core/03variables.rpy:466
    old "Я подглядывал, как Алиса принимает душ и она меня заметила. Пытаясь извиниться я пообещал купить ей какую-нибудь сладость, в обмен на то, чтобы она не рассказывала обо всём маме. На удивление, я ещё жив, но времени у меня немного..."
    new "Ich beobachtete Alice beim Duschen und sie bemerkte mich. Als ich versuchte, mich zu entschuldigen, versprach ich ihr, ihr Süßigkeiten zu kaufen, im Gegenzug dafür, dass sie Mom nicht alles erzählt. Überraschenderweise bin ich noch am Leben, aber ich habe nicht mehr viel Zeit..."

    # game/core/03variables.rpy:466
    old "Увы, я не успел подарить Алисе сладость в срок и поэтому она рассказала маме, как я за ней подглядывал. Возможно, в следующий раз, если Алиса меня заметит, я успею..."
    new "Leider habe ich Alice nicht rechtzeitig was Süßes geschenkt, und so erzählte sie Mom, wie ich ihr nachspioniert habe. Vielleicht schaffe ich es nächstes Mal, wenn Alice mich entdeckt..."

    # game/core/03variables.rpy:466
    old "Я купил для Алисы конфеты \"Ferrero Rocher\" и она была очень довольна! Ещё бы, ведь оказалось, что это её любимая сладость! Может быть, в следующий раз, стоит подарить ей большую коробку с этими конфетами?"
    new "Ich habe Alice Süßigkeiten \"Ferrero Rocher\" gekauft und sie war sehr glücklich! Natürlich stellte sich heraus, dass es ihr Lieblingskonfekt war! Vielleicht sollten ich ihr das nächste Mal eine grosse Schachtel dieser Süssigkeiten schenken."

    # game/core/03variables.rpy:466
    old "Я купил для Алисы шоколад \"Ritter Sport\" и она приняла его, но без особого восторга! Возможно, в следующий раз, следует подарить ей что-то другое?"
    new "Ich kaufte eine Schokolade \"Ritter Sport\" für Alice und sie nahm sie an, aber ohne viel Enthusiasmus! Vielleicht sollte ich ihr beim nächsten Mal etwas anderes geben?"

    # game/core/03variables.rpy:466
    old "Я купил для Алисы конфеты \"Raffaello\" и как оказалось, они ей не нравятся! В следующий раз, лучше подарить что-то другое..."
    new "Ich habe als Süßigkeiten \"Raffaello\" für Alice gekauft, und wie sich herausstellte, mag sie sie nicht! Nächstes Mal ist es besser, etwas anderes zu geben..."

    # game/core/03variables.rpy:466
    old "Эх... Если бы я знал, что Алиса станет выкручивать мне уши, то подумал бы, стоит ли ей дарить сладости! Хотя, тогда я рискую быть наказанным при всех, а это хуже... Так что лучше и в следующий раз рискнуть, вдруг повезёт и Алиса ничего мне не сделает."
    new "Äh... Wenn ich gewusst hätte, dass Alice mir nur die Ohren verdrehen würde, hätte ich daran gedacht, ihr Süßigkeiten zu geben! Obwohl ich dann riskiere, vor allen Leuten bestraft zu werden, was noch schlimmer ist... Es ist also besser, beim nächsten Mal ein Risiko einzugehen, für den Fall, dass ich Glück habe und Alice mir nichts antut."

    # game/core/03variables.rpy:466
    old "Как оказалось, за нравящиеся Алисе сладости можно не только сохранить уши в целости, но и ощутить её скромные объятия! И это не смотря на то, что я за ней подглядывал. Посмотрим, что ещё мне удастся получить от сестрёнки..."
    new "Wie sich herausstellte, kann man für die Süßigkeiten, die Alice mag, nicht nur die Ohren intakt lassen, sondern auch ihre bescheidene Umarmung spüren! Und das, obwohl ich ihr nachspioniert habe. Mal sehen, was ich noch von meiner großen Schwester bekomme..."

    # game/core/03variables.rpy:466
    old "Похоже, сладостей Алисе уже мало и теперь ОНА будет ставить условия! Она хочет, чтобы я купил ей пижаму и это уже покупка посерьёзнее! На этот раз времени у меня больше, но всё равно лучше поторопиться с подарком. Интересно, что меня будет за это ждать..."
    new "Es scheint, dass Alices Süßigkeiten nicht genug sind, und jetzt wird sie Bedingungen stellen! Sie will, dass ich ihr einen Pyjama kaufe, und das ist eine größere Anschaffung! Ich habe diesmal mehr Zeit, aber es ist immer noch besser, sich mit dem Geschenk zu beeilen. Ich frage mich, was ich dafür bekomme..."

    # game/core/03variables.rpy:466
    old "Алиса согласилась пустить меня к себе в комнату, пока примеряла пижаму! Конечно, она просила, чтобы я не подглядывал, хотя прекрасно знала, что я буду... И угрозы её стали какими-то шуточными. Интересно, к чему это приведёт? Так что подглядывать полезно, если правильно извиниться."
    new "Alice willigte ein, mich in ihr Zimmer zu lassen, während sie ihren Pyjama anprobierte! Natürlich bat sie mich, nicht zu gucken, obwohl sie wusste, dass ich... Und ihre Drohungen wurden zu einer Art Witz. Ich frage mich, wozu das führen wird? Gucken ist also erlaubt, wenn man sich richtig entschuldigt."

    # game/core/03variables.rpy:478
    old "Кристина"
    new "Christina"

    # game/core/03variables.rpy:478
    old "Моими подозрительными заказами всякой женской одежды заинтересовалась девчонка из доставки. Как выяснилось, её зовут Кристина. \nЯ не решился подкатить к ней... Она бы меня наверняка отшила. Хотя, попробовать всё равно можно было..."
    new "Das Liefermädchen wurde misstrauisch wegen meiner ständigen Bestellungen von Frauenkleidern. Ihr Name ist Christina. \nIch habe mich nicht getraut, sie anzumachen, sie würde mich definitiv abweisen. Obwohl, es ist ja nicht so, als hätte ich etwas zu verlieren, wenn ich es versuche..."

    # game/core/03variables.rpy:478
    old "Моими подозрительными заказами всякой женской одежды заинтересовалась девчонка из доставки. Как выяснилось, её зовут Кристина. \nНе самым умным решением было к ней сразу же подкатывать, но что сделано, то сделано. Понятно, что она бы в любом случае меня отшила, но попробовать стоило..."
    new "Das Liefermädchen interessierte sich für meine verdächtigen Bestellungen von Frauenkleidern. Ihr Name ist Christina. \nDie klügste Entscheidung für mich war, sie sofort anzubaggern, aber was geschehen ist, ist geschehen. Es ist klar, dass sie mich sowieso abgewiesen hätte, aber einen Versuch war es wert..."

    # game/core/03variables.rpy:484
    old "Контроль"
    new "Control"

    # game/core/03variables.rpy:484
    old "Что это было? Мама поймала меня на подглядывании за ними, пока они трахались, собиралась наказать, но Эрик сказал нет и мама согласилась?! Моя мама, которая запрещала мне смотреть даже эротику, согласилась, чтобы я присутствовал при таком? Это невероятно. Либо Эрик какой-то колдун, либо дело в другом и нужно в этом разобраться, как следует."
    new "Was ist gerade passiert? Mom hat mich dabei erwischt, wie ich sie und Eric beim Ficken beobachtet habe, sie wollte mich bestrafen, aber Eric ist eingeschritten und hat mich zusehen lassen? Und Mom hat es mir erlaubt?! Meine Mom, die mich nicht mal Erotikfilme sehen lässt, lässt mich so etwas sehen? Das ist unglaublich. Entweder hat Eric Gedankenkontrollkräfte, oder hier ist etwas anderes im Spiel."

    # game/core/03variables.rpy:488
    old "Компромат на Эрика"
    new "Erpressungsmaterial über Eric"

    # game/core/03variables.rpy:488
    old "Мне случайно удалось заметить, как Эрик посреди ночи стоит около окна в комнату Алисы и дрочит на неё! Не ожидал я такое увидеть... Мне казалось, что Эрик из тех, кто скорее проститутку снимет, чем будет просто дрочить, но как оказалось... я ошибался.\n\nНаверняка он делает это уже не первый раз! Надо понаблюдать за ним по ночам, чтобы заполучить снимок с его грязными делишками... Лучше иметь против него козырь, на случай чего..."
    new "Ich habe zufällig bemerkt, wie Eric mitten in der Nacht am Fenster zu Alices Zimmer stand und sich einen runterholte! Ich hatte nicht erwartet, das zu sehen... Ich dachte, Eric wäre der Typ, der lieber eine Nutte aufreißt, als sich einen runterzuholen, aber wie sich herausstellte... lag ich falsch. \n\nIch wette, das ist nicht das erste Mal, dass er so was macht. Wir sollten ihn nachts beobachten, um uns ein Bild von seinen schmutzigen Taten zu machen... Es ist besser, ein Druckmittel gegen ihn zu haben, falls etwas passiert..."

    # game/core/03variables.rpy:488
    old "Получилось! Я сфотографировал, как Эрик дрочит на Алису! Конечно, на самом деле на снимке не понятно, на кого или на что он дрочит, так что не слишком-то эта фотография мне поможет в случае какой-нибудь заварушки с Эриком.\n\nБыло бы хорошо, если бы Эрик обнаглел настолько, что стал дрочить на Алису прямо посреди её комнаты... Вот это уже бы тянуло на компромат! В кадр правда попадёт Алиса, но основное внимание на такой фотографии будет приковано к Эрику. Дело за малым - придумать, как заманить его в комнату Алисы..."
    new "Ich hab's getan! Ich habe ein Foto von Eric gemacht, wie er sich auf Alice einen runterholt. Natürlich ist nicht wirklich klar, auf wen oder was er sich einen runterholt, also ist es keine große Hilfe für mich, falls es irgendwelchen Ärger gibt. \n\nNein, es wäre schön, wenn Erik so dreist wäre, sich auf Alice mitten in ihrem Zimmer einen runterzuholen... Das wäre mal eine Art von Dreck! Alice wäre auf dem Bild, aber der Fokus des Fotos läge auf Erik. Man muss sich nur überlegen, wie man ihn in Alices Zimmer locken kann..."

    # game/core/03variables.rpy:488
    old "Я так и знал, что Эрик не устоит перед голой и спящей Алисой! Правда, можно ли его за это винить? Я бы на его месте тоже не устоял... Теперь, остаётся лишь успеть сделать снимок, пока он в её комнате..."
    new "Ich wusste, dass Eric einer nackten, schlafenden Alice nicht widerstehen konnte! Wirklich, kannst du ihm das verübeln? Ich würde auch nicht widerstehen können... Jetzt müssen wir nur noch das Foto machen, während er in ihrem Zimmer ist..."

    # game/core/03variables.rpy:488
    old "Вот всё и получилось! У меня есть два снимка, на которых Эрик, как грязный извращенец, дрочит пока все спокойно спят... А на одном из снимков даже видно на кого он дрочит...\n\nМне стоит быть осторожнее с этими снимками, Эрик столько всего делает для моих сестёр, что они запросто могут рассказать о том, что у меня есть. А так же хорошенько подумать над тем, как я буду использовать эти снимки.\n\nЯ могу придержать этот компромат, пойти с ним к Эрику в ближайшее время или же вовсе как-то подставить его..."
    new "Das war's also! Ich habe zwei Bilder von Eric, wie er sich wie ein Perverser einen runterholt, während alle friedlich schlafen... Und auf einem der Bilder kann man sogar sehen, zu wem er sich einen runterholt...\n\n Ich sollte mit diesen Bildern vorsichtig sein, Eric tut so viel für meine Schwestern, dass sie leicht vererkennen könnten, was ich habe. Ich sollte auch sorgfältig darüber nachdenken, wie ich sie verwende.\n\nIch kann die hier behalten, sie mitnehmen, zu Eric bringen, oder ich kann ihn irgendwie reinlegen..."

    # game/core/03variables.rpy:620
    old "Общение"
    new "Kommunikation"

    # game/core/03variables.rpy:620
    old "Навыки общения: Чувства человека"
    new "Kommunikationsfähigkeit: Die Sinne des Menschen"

    # game/core/03variables.rpy:620
    old "Это базовый курс обучения навыкам общения и содержит в себе основную информацию об органах чувств, о реакции человека на ваши слова, а также основы языка тела."
    new "Dieser Kurs vermittelt Ihnen grundlegende Kommunikationsfähigkeiten und enthält Informationen über Sinne, menschliche Reaktionen auf Ihre Worte und grundlegende Körpersprache."

    # game/core/03variables.rpy:620
    old "Навыки общения: Основы коммуникации"
    new "Kommunikationsfähigkeit: Grundlagen der Kommunikation"

    # game/core/03variables.rpy:620
    old "Это второй курс обучения навыкам общения. Из него вы узнаете как повлиять на решение человека без слов, как подобрать правильный слова в нужной ситуации для достижения своей цели."
    new "Dies ist der zweite Kurs, der Ihnen Kommunikationsfähigkeiten vermittelt. Sie erfahren, wie Sie ohne Worte die Entscheidungen von Menschen beeinflussen können und wie Sie in wichtigen Situationen Fragen und Aussagen richtig formulieren, um Ihr Ziel zu erreichen."

    # game/core/03variables.rpy:620
    old "Навыки общения: Без границ"
    new "Kommunikationsfähigkeit: keine Grenzen"

    # game/core/03variables.rpy:620
    old "Это продвинутый длительный курс, предназначенный для тех, кто хочет постоянно обучаться и совершенствоваться. На данный момент это последний доступный курс."
    new "Dies ist ein fortgeschrittener Langzeitkurs für diejenigen, die lernen und sich selbst verbessern wollen. Derzeit ist es der letzte verfügbare Kurs."

    # game/core/03variables.rpy:736
    old "Эрик"
    new "Eric"

    # game/core/03variables.rpy:736
    old "Эрика"
    new "Eric"

    # game/core/03variables.rpy:736
    old "Эрику"
    new "Eric"

    # game/core/03variables.rpy:736
    old "Эриком"
    new "Eric"

    # game/core/03variables.rpy:736
    old "Эрике"
    new "Eric"

    # game/core/03variables.rpy:736
    old "Ещё до того, как мама познакомила меня со своим внезапным ухажёром Эриком, я начал чувствовать, что ничего хорошего он в мою жизнь не принесёт. Слишком уж он подозрительный..."
    new "Schon bevor Mom mir ihren neuen Freund Eric vorgestellt hat, hatte ich das Gefühl dass er ein schlechter Einfluß sein würde. Irgendetwas fühlt sich bei ihm einfach nicht richtig an..."

    # game/core/03variables.rpy:777
    old "Кира"
    new "Kira"

    # game/core/03variables.rpy:777
    old "Киры"
    new "Kira"

    # game/core/03variables.rpy:777
    old "Кире"
    new "Kira"

    # game/core/03variables.rpy:777
    old "Киру"
    new "Kira"

    # game/core/03variables.rpy:777
    old "Кирой"
    new "Kira"

    # game/core/03variables.rpy:777
    old "Тётя Кира, мамина младшая сестра. Конечно, у неё и раньше не были замечены какие-либо комплексы, но сейчас она стала такой... такой..."
    new "Tante Kira, die jüngere Schwester vom Mom. Früher war sie nicht gerade schüchtern, aber jetzt ist sie so... so..."

    # game/core/03variables.rpy:888
    old "Любимая тётя"
    new "Lieblingstante"

    # game/core/03variables.rpy:888
    old "Итак, к нам приехала тётя Кира, мамина младшая сестра. Конечно, и раньше не были замечены у неё какие-либо комплексы, но сейчас она стала такой... такой... А ещё она увидела мой член, в первый же день! Так неловко. Но и себя тётя Кира показала во всей красе, в таком купальнике, если его можно назвать купальником... Да ещё такие намёки на моего папу. Неужели, они были настолько... знакомы? Нужно выпытать у неё всё, что только возможно."
    new "Also kam Tante Kira, Moms kleine Schwester, zu uns. Natürlich hatte sie vorher keine Komplikationen gesehen, aber jetzt ist sie so... Also... Und sie sah auch meinen Schwanz am ersten Tag! So peinlich. Aber Tante Kira zeigte sich auch in all ihrer Pracht, in einem Bikini wie diesem, wenn man das überhaupt als Bikini bezeichnen kann... Und solche Hinweise auf meinen Vater. War es wirklich so... ...kannten sie sich? Ich muss alles aus ihr herausholen, was ich kann."

    # game/core/03variables.rpy:888
    old "Я рассказал тёте Кире всё про Эрика, вот как есть, так и сказал. Кажется, она сомневается в моих словах, но пообещала аккуратно всё выяснить и разузнать. Может быть, даже с самим Эриком пообщается..."
    new "Ich habe Tante Kira alles über Eric erzählt. Sie scheint an meinen Worten zu zweifeln, versprach aber, nachzuforschen und vielleicht sogar selbst mit ihm zu sprechen..."

    # game/core/03variables.rpy:888
    old "Вот это да! Моя тётя снимается в порно! Неужели, я живу рядом с порнозвездой? Теперь понятно, почему меня к ней так и тянет. Её просто окружает аура секса! Может быть, ещё больше сблизиться с ней будет не так сложно, как я думал..."
    new "Meine Tante macht Pornos! Lebe ich ernsthaft mit einem Pornostar zusammen? Kein Wunder, dass ich mich so zu ihr hingezogen fühle. Sie ist vollkommen sexsüchtig! Vielleicht wird es nicht so schwer, ihr näher zu kommen, wie ich dachte..."

    # game/core/03variables.rpy:888
    old "Кира предложила мне немного заработать. Нужно лишь её пофотографировать. Вот только для этих целей нужен фотоаппарат. Конечно, вряд-ли она заплатит мне столько, сколько стоит фотоаппарат, зато есть шанс, что мне что-нибудь обломится другое за эту фотосессию..."
    new "Kira hat sich eine Möglichkeit ausgedacht, wie ich mir ein kleines zusätzliches Taschengeld verdienen könnte, indem ich ein paar Fotos für sie mache. Ich brauche nur eine Kamera. Natürlich wird sie mir nicht helfen, eine zu kaufen, aber es besteht die Möglichkeit, dass ich auf andere Weise entschädigt werde..."

    # game/core/03variables.rpy:888
    old "Фотосессия вышла классная, хоть ничего нового я для себя и не открыл. Ну почти, были интересные моменты, а это намного лучше, чем вообще ничего! Теперь нужно немного подождать, чтобы стало понятно, насколько удачными получились снимки. Может я даже что-то и получу за эту фотосессию..."
    new "Das Fotoshooting verlief gut, aber ich habe nichts Neues gelernt. Zum Glück gab es ein paar interessante Momente, die es lohnend machten! Jetzt muss ich abwarten, wie die Bilder geworden sind. Hoffentlich kann ich dabei etwas Gutes herausholen..."

    # game/core/03variables.rpy:888
    old "Итак, снимки вышли удачными и мы с Кирой договорились на новую фотосессию, пока никого не будет дома. Но ещё не ясно, когда мы её проведём... Интересно, что такое Кира хочет достать для съёмок!? Остаётся только ждать..."
    new "Die Bilder kamen gut an, und Kira will ein weiteres Fotoshooting ansetzen. Diesmal ist zwar niemand zu Hause, aber ich bin mir nicht sicher, wann wir es machen werden. Ich frage mich, was sie vorhat? Alles, was ich tun kann, ist warten..."

    # game/core/03variables.rpy:888
    old "Вот и состоялась вторая фотосессия! И она была отпадной! Не только потому что Кира решила пофотографироваться на тему БДСМ, но и потому что у нас был секс... Мой первый, настоящий секс... Теперь я официально больше не девственник! Вернее, неофициально... я ведь трахался с тётей. Надеюсь, не в последний раз! Хотя, с этим Эриком, который во всё лезет, лучше быть очень осторожным..."
    new "Und das war das zweite Foto-Shooting! Erstaunlich! Nicht nur, weil Kira ihre wilde Seite mit etwas BDSM zum Vorschein brachte, sondern weil wir tatsächlich Sex hatten! Mein erstes... Endlich... bin ich offiziell keine Jungfrau mehr! Ich meine... Inoffiziell... Ich kann niemandem wirklich erzählen, dass ich meine Tante gefickt habe. Davon abgesehen, sollte ich in Erics Nähe vorsichtig sein. Er scheint immer irgendwie neugierig zu sein..."

# TODO: Translation updated at 2021-04-11 22:25

translate deutsch strings:

    # game/core/03variables.rpy:399
    old "Забота о попках"
    new "spezielle Arsch-Behandlung"

    # game/core/03variables.rpy:399
    old "Интересно получилось! Я ради интереса сказал Алисе, что больше не хочу за неё заступаться, когда её наказывают, но готов это делать и дальше, если она согласится, чтобы её шлёпал я.\nОна сперва приняла такой уговор в штыки, но после нескольких наказаний от мамы всё же согласилась, чтобы её шлёпал я. По крайней мере, если получилось спасти Алису от маминой руки. Надо так же не забыть обсудить с ней, когда можно её отшлёпать.\n\nПравда есть небольшой нюанс, благодаря которому Алиса и согласилась на это... Я пообещал, что отшлёпаю её нежно. Да уж, будет не просто устоять и не влепить по её попке за то, как стервозно она себя вела..."
    new "Das ist interessant! Ich habe Alice gesagt, dass ich nicht mehr für sie einstehen will, wenn sie bestraft wird, aber dass ich es weiterhin tun würde, wenn sie zustimmt, von mir bestraft zu werden.\nZumindest dann, wenn Alice vor Mom gerettet habe. Ich sollte auch daran denken, mit ihr darüber zu sprechen, wann es okay ist, ihr den Arsch zu versohlen. Ich habe ihr versprochen, dass ich ihren Arsch  sanft versohlen werde.\n\nJa, es wird schwer sein, ihr nicht richtig den Arsch zu versohlen, weil sie so eine Schlampe ist..."

    # game/core/03variables.rpy:399
    old "Зрелище действительно завораживающее! Умудриться раздеть и отшлёпать свою старшую сестрёнку не многим, наверно, доводилось...\nХоть она капризничает и сыпет угрозами при этом, но похоже моя настойчивость взяла верх. Любоваться её голой и упругой попкой одно удовольствие, как и шлёпать по ней.\n\nИ теперь мне понятно, как не перегибать палку, чтобы наслаждаться этим приватным наказанием как можно дольше..."
    new "Es ist wirklich ein faszinierender Anblick! Ich glaube nicht, dass viele Leute es jemals geschafft haben, ihre große Schwester auszuziehen und zu bestrafen...\nObwohl sie launisch und gefährlich ist, scheint meine Hartnäckigkeit das Beste aus ihr herauszuholen. Es macht genauso viel Spaß, ihren nackten und knackigen Arsch zu bewundern, wie sie zu bestrafen.\n\Und jetzt weiß ich auch, das ich es nicht übertreiben darf, um diese private Bestrafung so lange wie möglich zu genießen..."

# TODO: Translation updated at 2021-05-02 21:54

translate deutsch strings:

    # game/core/03variables.rpy:295
    old "Оливии"
    new "Olivia"

    # game/core/03variables.rpy:295
    old "Оливию"
    new "Olivia"

    # game/core/03variables.rpy:295
    old "Оливией"
    new "Olivia"

# TODO: Translation updated at 2021-05-15 16:54

translate deutsch strings:

    # game/core/03variables.rpy:295
    old "Оливия, одноклассница моей младшей сестрёнки Лизы. Довольно милая девчонка. А главное с изюминкой... Ходит по школе без трусиков, а у себя по дому вообще голая, как и её родители, они ведь натуристы. Это классно, что у Лизы появилась такая интересная подружка!"
    new "Olivia, die Klassenkameradin meiner kleinen Schwester Lisa. Ziemlich süßes Mädchen. Und was noch dazu kommt, sie hat ein Geheimnis... Sie läuft in der Schule ohne Höschen herum, und zu Hause ist sie komplett nackt, genau wie ihre Eltern, denn die sind Naturisten. Es ist toll, dass Lisa so eine interessante Freundin hat!"

