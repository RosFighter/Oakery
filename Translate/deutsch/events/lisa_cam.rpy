# TODO: Translation updated at 2021-03-02 01:41

# game/events/lisa_cam.rpy:8
translate deutsch cam0_lisa_sleep_night_cab47b53:

    # Max_01 "Лиза сладко спит..."
    Max_01 "Lisa schläft tief und fest..."

# game/events/lisa_cam.rpy:17
translate deutsch cam0_lisa_sleep_morning_36c03e6c:

    # Max_01 "Лиза ещё спит..."
    Max_01 "Lisa schläft noch..."

# game/events/lisa_cam.rpy:26
translate deutsch cam0_lisa_shower_cf65001d:

    # Max_09 "Лизы не видно через эту камеру... Может посмотреть через другую?"
    Max_09 "Ich kann Lisa durch diese Kamera nicht sehen. Soll ich eine andere ausprobieren?"

# game/events/lisa_cam.rpy:28
translate deutsch cam0_lisa_shower_99a12737:

    # Max_09 "Лизы не видно через эту камеру..."
    Max_09 "Ich kann Lisa durch diese Kamera nicht sehen..."

# game/events/lisa_cam.rpy:36
translate deutsch cam0_lisa_shower_30b79938:

    # Max_04 "Младшая сестрёнка принимает душ... Прекрасная Лиза - прекрасное утро!"
    Max_04 "Lisa duscht gerade... Ich liebe es, ihr zuzusehen, wie sie ihren straffen jungen Körper säubert!"

# game/events/lisa_cam.rpy:59
translate deutsch cam1_lisa_shower_d17010c3:

    # Max_03 "Лиза, прежде чем принять душ, любуется собой перед зеркалом. И мы этим со зрителями тоже полюбуемся..."
    Max_03 "Sie betrachtet sich selbst im Spiegel. Jetzt kann das Publikum sie auch anschauen."

# game/events/lisa_cam.rpy:65
translate deutsch cam1_lisa_shower_cf65001d:

    # Max_09 "Лизы не видно через эту камеру... Может посмотреть через другую?"
    Max_09 "Ich kann Lisa durch diese Kamera nicht sehen. Soll ich eine andere ausprobieren?"

# game/events/lisa_cam.rpy:73
translate deutsch cam0_lisa_read_b78fc025:

    # Max_07 "Люблю смотреть, как Лиза читает. Вернее, люблю позы, в которых она читает..."
    Max_07 "Ich liebe es, Lisa beim Lesen zuzuschauen. Ich glaube, sie merkt nicht, wie suggestiv ihre Körperhaltung ist..."

# game/events/lisa_cam.rpy:104
translate deutsch cam0_lisa_dressed_school_f7db1942:

    # Max_09 "Ничего интересного я здесь уже не увижу, Лиза полностью оделась."
    Max_09 "Hier gibt es nichts Interessantes. Lisa ist vollständig angezogen."

# game/events/lisa_cam.rpy:115
translate deutsch cam0_lisa_dressed_school_e79b2255:

    # Max_07 "Отлично! Лиза наряжается, чтобы отправиться в школу..." nointeract
    Max_07 "Perfekt! Lisa macht sich fertig, um zur Schule zu gehen..." nointeract

# game/events/lisa_cam.rpy:126
translate deutsch cam0_lisa_dressed_school_e61ca9e9:

    # Max_02 "Ухх! Сейчас она такая голенькая и милая..." nointeract
    Max_02 "Ja! Ihr süßer Körper ist eine tolle Show..." nointeract

# game/events/lisa_cam.rpy:140
translate deutsch cam0_lisa_dressed_school_e16161a7:

    # Max_04 "Как классно, что моя сестрёнка - такая соблазнительная школьница. Уверен, зрителям это нравится!"
    Max_04 "Meine kleine Schwester ist ein so ungezogenes Mädchen unter dieser Uniform. Ich hoffe, es gefällt dem Publikum!"

# game/events/lisa_cam.rpy:151
translate deutsch cam0_lisa_dressed_shop_f7db1942:

    # Max_09 "Ничего интересного я здесь уже не увижу, Лиза полностью оделась."
    Max_09 "Nichts Interessantes. Lisa ist vollständig angezogen."

# game/events/lisa_cam.rpy:162
translate deutsch cam0_lisa_dressed_shop_2978876d:

    # Max_07 "Отлично! Лиза наряжается, чтобы отправиться на шопинг..." nointeract
    Max_07 "Lisa macht sich zum Einkaufen fertig..." nointeract

# game/events/lisa_cam.rpy:173
translate deutsch cam0_lisa_dressed_shop_e61ca9e9:

    # Max_02 "Ухх! Сейчас она такая голенькая и милая..." nointeract
    Max_02 "Ja! Ihr süßer Körper ist eine tolle Show..." nointeract

# game/events/lisa_cam.rpy:187
translate deutsch cam0_lisa_dressed_shop_639be09f:

    # Max_04 "Повезло мне с сестрёнкой! Обворожительна в любой одежде и ещё больше - без неё..."
    Max_04 "Sie ist so verlockend, egal, was sie trägt! Keiner der Jungs könnte sich zurückhalten, wenn er wüsste, was sie zu verbergen hat..."

# game/events/lisa_cam.rpy:195
translate deutsch cam0_lisa_sun_fafb408a:

    # Max_01 "Лиза загорает и радует этим моих зрителей! И меня, конечно же..."
    Max_01 "Lisa beim Sonnenbaden ist eine tolle Show für das Publikum! Und für mich, natürlich..."

# game/events/lisa_cam.rpy:202
translate deutsch cam1_lisa_sun_ce238a73:

    # Max_09 "Через эту камеру ничего не видно... Может посмотреть через другую?"
    Max_09 "Ich kann durch diese Kamera nichts sehen. Soll ich eine andere ausprobieren?"

# game/events/lisa_cam.rpy:210
translate deutsch cam0_lisa_swim_39c0bef3:

    # Max_09 "Ничего толком не видно... Стоит взглянуть через другую камеру..."
    Max_09 "Ich kann nicht wirklich etwas sehen. Es könnte sich lohnen, eine andere Kamera auszuprobieren..."

# game/events/lisa_cam.rpy:212
translate deutsch cam0_lisa_swim_06c19036:

    # Max_09 "Ничего не разглядеть... Нужно установить камеру, которая охватила бы весь бассейн..."
    Max_09 "Ich kann nichts sehen. Ich sollte eine Kamera installieren, die den gesamten Pool abdeckt."

# game/events/lisa_cam.rpy:220
translate deutsch cam1_lisa_swim_0cc13309:

    # Max_01 "Приятно наблюдать за младшей сестрёнкой у водички..."
    Max_01 "Lisa scheint es zu genießen, den Pool zu benutzen..."

# game/events/lisa_cam.rpy:229
translate deutsch cam0_lisa_dishes_0b71076a:

    # Max_01 "Лиза моет посуду. А ведь я мог бы ей помочь..."
    Max_01 "Lisa macht den Abwasch. Sie scheint wirklich dankbar zu sein, wenn ich ihr helfe..."

# game/events/lisa_cam.rpy:239
translate deutsch cam0_lisa_phone_14f355ab:

    # Max_01 "Сестрёнка бездельничает и залипла в свой телефон. Но лежит красиво..."
    Max_01 "Lisa spielt nur an ihrem Telefon, aber sie liegt wunderschön da..."

# game/events/lisa_cam.rpy:249
translate deutsch cam0_lisa_bath_b32c78c2:

    # Max_01 "Лиза почти набрала воду, хотя я смотрю на кое-что другое..."
    Max_01 "Die Wanne ist fast voll... aber meine Aufmerksamkeit ist woanders..."

# game/events/lisa_cam.rpy:256
translate deutsch cam0_lisa_bath_4e37cfa9:

    # Max_04 "Эх, Лиза... Не вытирайся! Ты мокренькая тоже обалденная..."
    Max_04 "Oh, Lisa... Trockne dich nicht ab! Du bist so schön nass..."

# game/events/lisa_cam.rpy:262
translate deutsch cam0_lisa_bath_8714e3c3:

    # Max_05 "Давай, сестрёнка, не стесняйся показать как можно больше всего интересного..."
    Max_05 "Komm, Schwesterchen, zeig so viele interessante Dinge, wie du kannst..."

# game/events/lisa_cam.rpy:269
translate deutsch cam1_lisa_bath_cf65001d:

    # Max_09 "Лизы не видно через эту камеру... Может посмотреть через другую?"
    Max_09 "Ich kann Lisa durch diese Kamera nicht sehen... Soll ich eine andere ausprobieren?"

# game/events/lisa_cam.rpy:277
translate deutsch cam0_lisa_homework_8bf186e7:

    # Max_01 "Лиза учит уроки. Может, стоило ей помочь?!"
    Max_01 "Lisa macht ihre Hausaufgaben. Vielleicht hätte ich ihr helfen sollen?!"

