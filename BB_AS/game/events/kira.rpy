...
label kira_sleep_morning:
    ...
    if kira.sleepnaked:
        $ txt = _("{m}Тёте Кире очень идёт быть голой! Может быть, это потому что я привык видеть её голой? В такие округлости просто нельзя не влюбиться...{/m}")
    else:
        $ txt = _("{m}Ага! Тётя Кира ещё спит. Хорошо, что её ночнушка слегка просвечивает... Ухх, как она хороша...{/m}")
    menu:
        Max_04 "[txt!t]"
        "{i}подойти поближе{/i}":
...
label kira_bath_with_eric:
    ...
    menu:
        ...
        "{i}воспользоваться стремянкой{/i}" if flags.ladder > 2:
            ...
            if kira.dcv.battle.stage > 2:
                Max_08 "{m}Конечно, почему бы среди ночи не сходить в ванную, когда тебя там будет ждать шикарная тётя Кира, чтобы отсосать...{/m}"   #если Макс не сдавал Киру
            else:
                Max_01 "{m}Наверно, тётя Кира хотела прийти с работы и отдохнуть, а тут ещё и Эрику нужно отсосать сперва...{/m}"   #если Макс сдал Киру
            ...
            if kira.dcv.battle.stage > 2:
                Max_10 "{m}Хватит уже трахать её в рот, как какую-то порнозвезду! Она конечно порнозвезда, но не круглые же сутки...{/m}"   #если Макс не сдавал Киру
            else:
                Max_07 "{m}Неплохо так Эрик её голову на свой член напяливает! Как будто так и надо...{/m}"   #если Макс сдал Киру
            # присоединиться (если взят бонус с Кирой от Эрика)
            # уйти
            $ spent_time += 10
            ...
            if kira.dcv.battle.stage > 2:
                menu:
                    Max_09 "{m}Вот же сволочь! Спустил всё до последней капли Кире прямо в рот! Ну Эрик, ты у меня сполна получишь рано или поздно...{/m}"   #если Макс не сдавал Киру
                    "{i}уйти{/i}":
                        pass
            else:
                menu:
                    Max_02 "{m}Кира не упустила ни единой капли мимо! И как будто никто ни у кого только что смачно не отсасывал... Чистая работа!{/m}"   #если Макс сдал Киру
                    "{i}уйти{/i}":
                        pass
