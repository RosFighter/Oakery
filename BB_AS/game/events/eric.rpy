            ...
            if flags.eric_jerk and '02:00'<=tm<'02:30':
                ...
                if not prenoted and not flags.eric_noticed:
                    ...
                    if all([flags.eric_wallet == 2, flags.eric_photo2, not len(house[1].cams)]):
                        ...
                        menu:
                            Max_07 "{m}Ага! Мама одна... Значит, Эрик почти наверняка сейчас дрочит на Алису. Можно подождать хотя бы полчаса, чтобы он вернулся к моей маме, и затем уже спокойно его подставить...{/m}"
                    ...
                    if all([flags.eric_wallet == 2, flags.eric_photo2, len(house[1].cams)]):
                        ...
                        menu:
                            Max_07 "{m}Ага! Мама одна... Значит, Эрик почти наверняка сейчас дрочит на Алису. С помощью камер я смогу проследить, когда он пойдёт спать и затем уже можно будет спокойно его подставить...{/m}"
...
label jerk_balkon:
    ...
    if not alice.sleepnaked:
        ...
        if all([flags.eric_wallet == 2, flags.eric_photo2, not len(house[1].cams)]):
            ...
            menu:
                Max_07 "{m}Отлично! Эрик именно там, где мне и нужно... Можно подождать хотя бы полчаса, чтобы он вернулся к моей маме, и затем уже спокойно его подставить...{/m}"
                "{i}затаиться на полчаса{/i}":
                    jump frame_eric
        ...
        if all([flags.eric_wallet == 2, flags.eric_photo2, len(house[1].cams)]):
            ...
            menu:
                Max_07 "{m}Отлично! Эрик именно там, где мне и нужно... С помощью камер я смогу проследить, когда он пойдёт спать и затем уже можно будет спокойно его подставить...{/m}"
                "{i}следить за Эриком через скрытые камеры{/i}":
                    jump follow_cameras_eric
        menu:
