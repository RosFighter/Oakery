...
label eric_ann_tv:
    ...
    if eric.stat.handjob:
        $ txt = _("{m}Кажется, мама с Эриком смотрят какой-то фильм. Наверняка, снова порно...{/m}")
        ...
        $ txt = _("{m}Кажется, мама с Эриком смотрят какой-то фильм. Интересно, какой...{/m}")
    ...
    if tv_scene == 'bj':
        ...
        if eric.stat.blowjob:
            $ txt = _("{m}Ого! На экране стало интереснее! А мама, видимо, снова потянулась отсасывать Эрику...{/m}")
            $ txt2 = _("{m}На экране уже во всю идёт самое интересное и мама скинула полотенце! Вот бы посмотреть на эту голую попку с другого ракурса... Может быть, подойти ближе?{/m}")
        else:
            $ txt = _("{m}Ого! На экране стало интереснее! А куда это мама так наклонилась?{/m}")
            $ txt2 = _("{m}Ничего себе! Эрик стянул с мамы полотенце и я вижу её голую попку! Может быть, подойти ближе?{/m}")
        menu:
            Max_08 "[txt!t]"
        ...
        ### Дальний план, дрочка
        if eric.stat.blowjob:
            $ txt = _("{m}Ого! На экране стало интереснее! А мама, вроде, снова начала дрочить Эрику...{/m}")
            $ txt2 = _("{m}На экране уже во всю идёт самое интересное и мама скинула полотенце! Жаль, что её грудь не очень хорошо видно... Может быть, подойти ближе?{/m}")
        else:
            $ txt = _("{m}Ого! На экране стало интереснее! А что это мама там делает?{/m}")
            $ txt2 = _("{m}Ничего себе! Эрик стянул с мамы полотенце и я вижу её голую грудь! Может быть, подойти ближе?{/m}")
        menu:
            Max_08 "[txt!t]"
            "{i}продолжать смотреть{/i}":
    ...
    if eric.stat.handjob:
        menu:
            Max_10 "{m}Если меня заметят, пока я смотрю порно на большом экране и подглядываю за тем, что происходит здесь же, наяву, меня точно накажут{/m}"
            "{i}продолжать смотреть{/i}":
        ...
        menu:
            Max_10 "{m}Ого! Если меня заметят, пока я подглядываю за ТАКИМ, меня точно накажут. Нужно срочно уходить!{/m}"
            "{i}подойти ближе{/i}":
                jump .closer1
            "{i}взглянуть со стороны{/i}" if eric.stat.handjob != 0 and mgg.stealth >= 11.0:
                $ _alt = True
        ...
        "{i}заглянуть в окно{/i}":
            ...
            if flags.eric_jerk and '02:00'<=tm<'02:30':
                ...
                if not prenoted and not flags.eric_noticed:
                    # Эрика не видели
                    if all([flags.eric_wallet == 2, flags.eric_photo1 == 1, not len(house[1].cams)]):
                        # Макс шёл подглядывать за комнатой Анны, но у него не установленны скрытые камеры
                        menu:
                            Max_07 "{m}Ага! Мама одна... Значит, Эрик почти наверняка сейчас дрочит на Алису. Можно подождать хотя бы полчаса, чтобы он вернулся к моей маме, и затем уже спойкойно его подставить...{/m}"
                            "{i}затаиться на полчаса{/i}":
                                scene BG char Alice bed-night-01
                                $ renpy.show('Alice sleep-night '+pose3_2)
                                show Eric jerk off 02
                                with Fade(0.4, 0, 0.3)
                                jump frame_eric
                    if all([flags.eric_wallet == 2, flags.eric_photo1 == 1, len(house[1].cams)]):
                        # Макс шёл подглядывать за комнатой Анны
                        menu:
                            Max_07 "{m}Ага! Мама одна... Значит, Эрик почти наверняка сейчас дрочит на Алису. С помощью камер я смогу проследить, когда он пойдёт спать и затем уже можно будет спойкойно его подставить...{/m}"
                            "{i}следить за Эриком через скрытые камеры{/i}":
                                jump follow_cameras_eric
                    menu:
                        Max_09 "{m}О! Мама спит одна... Как она прекрасна, особенно голая... А Эрик где? Уж не у Алисы ли в комнате?!{/m}"
                        "{i}проверить{/i}":
                            jump jerk_balkon
...
label eric_ann_shower:
    ...
    $ spent_time += 10
    menu:
        Max_00 "{m}Похоже, мама вместе с Эриком принимают душ... Или что они там ещё могут делать?{/m}"
    ...
    label .start_peeping:
        ...
        play music spying
        menu:
            Max_07 "{m}Вот это да... Похоже намечается что-то большее, чем просто принять душ! Боюсь даже представить, что будет, если меня поймают, пока я подглядываю... за этим...{/m}"
...
label jerk_balkon:
    # Макс решил подглядеть за Анной или Алисой через окно (Эрик около окна Алисы)
    # eric-voyeur-alice-01
    ...
    if not alice.sleepnaked:
        ...
        show Eric jerk off 02
        with Fade(0.4, 0, 0.3)
        if all([flags.eric_wallet == 2, not len(house[1].cams)]):
            # Макс шёл подглядывать за комнатой Алисы, но у него не установленны скрытые камеры
            menu:
                Max_07 "{m}Отлично! Эрик именно там, где мне и нужно... Можно подождать хотя бы полчаса, чтобы он вернулся к моей маме, и затем уже спойкойно его подставить...{/m}"
                "{i}затаиться на полчаса{/i}":
                    jump frame_eric

        if all([flags.eric_wallet == 2, len(house[1].cams)]):
            # Макс шёл подглядывать за комнатой Алисы
            menu:
                Max_07 "{m}Отлично! Эрик именно там, где мне и нужно... С помощью камер я смогу проследить, когда он пойдёт спать и затем уже можно будет спойкойно его подставить...{/m}"
                "{i}следить за Эриком через скрытые камеры{/i}":
                    jump follow_cameras_eric
        menu:
            Max_03 "{m}Ага, Эрик здесь! Не устоял перед голой Алисой и дрочит прямо посреди её комнаты... Вот же грязное животное!{/m}"
...
label jerk_yard:
    ...
    $ eric.stat.mast += 1
    menu:
        Max_07 "{m}Эрик всё дрочит на Алису! И не лень ему вставать среди ночи для этого?!{/m}"
        "{i}сбегать за фотоаппаратом и пойти на балкон{/i}" ('lucky', 50) if flags.eric_photo1 < 1:  #если снимка у окна нет
            jump jerk_photohant1
...
label jerk_photohant1:
    ...
    if rand_result:
        ...
        hide FG
        menu:
            Max_09 "{m}А если попробовать загнать Эрика в комнату Алисы?! Тогда бы у меня появился шанс сделать уже более определённый снимок, где видно на кого дрочит Эрик. Только вот как...{/m}"
            "{i}уйти{/i}":
...
label start_eric_wallet:
    # у Макса есть 2 снимка с Эриком / Макс не может выплатить компенсацию Эрику за Лизу / Макс прервал практику Лизы и Эрика
    # сразу после ужина (четверг)

    # terrace-punish-evening-00 + after-dinner-eric-(01/01a) + after-dinner-ann-(01/01a)
    ...
    # отношения, настроение и влияние Макса с Алисой, Лизой и Анной слетают

    # сохраняем отношение/настроение/влияние для последующего восстановления
    $ rmi = {
        'm_r' : ann.relmax,
        'm_m' : ann.mood,
        'm_i' : infl[ann].m[0],
    ...
    $ lisa.relmax   = 0
    $ lisa.mood     = 0
    $ infl[ann].sub_m(infl[ann].m[0])
    $ infl[alice].sub_m(infl[alice].m[0])
    $ infl[lisa].sub_m(infl[lisa].m[0])
    ...
    $ notify_list.append(__("{r}Отношения Макса с семьёй значительно ухудшились. Они расстроены и не доверяют ему.{/r}"))
