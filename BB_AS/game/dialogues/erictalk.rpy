...
label eric_voy_wtf:
    ...
    $ flags.voy_stage = 2   # теперь можно подглядывать безнаказано (вроде)
    $ poss['control'].open(1)
    $ flags.eric_fee -= 20
    return
...
label lessons_from_Eric:
    if flags.voy_stage==4:
        $ poss['control'].open(4)
        $ flags.voy_stage = 5   # первый "урок"
        play music eric
        jump .first_bj
    elif flags.voy_stage==5:
        $ poss['control'].open(5)
        $ flags.voy_stage = 6   # второй "урок"
        play music eric
        jump .second_bj
    elif flags.voy_stage==6:
        $ poss['control'].open(6)
        $ flags.voy_stage = 7   # третий "урок"
        play music eric
        jump .third_bj
    else:
        if get_rel_eric()[0] == 2:
            # Фальшивая дружба
            $ poss['control'].open(7)
            $ poss['control'].open(10)
        else:
            # Настоящая дружба
            if flags.lisa_sexed > 10:
                # Эрик уже уступил Лизу
                $ poss['control'].open(8)
            else:
                $ poss['control'].open(7)

        $ flags.voy_stage = 8   # теория от Эрика завершается
        jump .fourth_visit
    ...
    label .fourth_visit:    # четвёртый заход, шанс на то, что Анна будет в чулках 50%
        ...
        if all([get_rel_eric()[0] > 1, not lisa.dcv.battle.stage]):
            ...
            if get_rel_eric()[0] == 3:
                ...
                if ann.dcv.feature.stage < 4:
                    # Макс ещё не узнал секрет об Анне от Киры
                    ...
                    # Макс уже узнал о секрете Анны от Киры
        ...
        elif all([get_rel_eric()[0] > 1]):
            # выбор пути дружбы уже установлен через Лизу
            if get_rel_eric()[0] == 3:
                # Настоящая дружба
            ...
            # выбор пути дружбы ещё не установлен
...
label Eric_talk_about_Lisa_0:
    ...
    menu:
        ...
        "Нет, можешь обучать... (Настоящая дружба)" if get_rel_eric()[0] in [1, 3]: # GetRelMax('eric')[0]>0:
            # дружба или нейтралитет
        ...
        "А ей не рановато? (Фальшивая дружба)" if get_rel_eric()[0] in [1, 2]:      # GetRelMax('eric')[0]>3:
            ...
            if flags.voy_stage == 8:    # был 4й заход в спальню АиЭ
                $ poss['control'].open(10)

        "Лизу тебе не отдам! (Откровенная вражда)" if get_rel_eric()[0] < 0:        # not GetRelMax('eric')[0]>0:
            # вражда
...
label Eric_talk_about_Alice_0:
    ...
    menu:
        ...
        #если с Эриком дружба
        "Значит, тебя её блог интересует? (Настоящая дружба)" if get_rel_eric()[0] in [1, 3]:   # GetRelMax('eric')[0]>0:
            $ renpy.show('Max talk-terrace 03'+mgg.dress)
        ...
        "Я мало что об этом знаю... (Фальшивая дружба)" if get_rel_eric()[0] in [1, 2]:         # GetRelMax('eric')[0]>0:
            $ renpy.show('Max talk-terrace 02'+mgg.dress)
            ...
            $ alice.dcv.battle.set_lost(7)
            $ poss['blog'].open(10)
            if flags.voy_stage == 8:    # был 4й заход в спальню АиЭ
                $ poss['control'].open(10)

        "Вот у неё и спрашивай! (Откровенная вражда)" if get_rel_eric()[0] < 0:                 # not GetRelMax('eric')[0]>0:
            $ renpy.show('Max talk-terrace 04'+mgg.dress)
            $ renpy.show('Eric talk-terrace 02'+eric.dress)
...
label Eric_talk_about_Alice_1:
    # через неделю после первого диалога, в случае отсрочки
    scene BG talk-terrace-00
    $ renpy.show('Eric talk-terrace 01'+eric.dress)
    $ renpy.show('Max talk-terrace 01'+mgg.dress)
    with fade4
...
label Eric_talk_about_lace_lingerie:
    ...
    if get_rel_eric()[0] > 1:
        ...
        if flags.can_ask:
            $ flags.can_ask = -1    # чулки заблокированы
        $ poss['blog'].open(19)
    else:
        # с Эриком вражда
        Max_09 "Что надо?"
...
#     # возможно только при дружбе с Эриком (с Лизой начились уроки у Анны и Эрика + у Алисы есть 4 нижних белья для блога)
#     # стартовая фраза "Хочу рассказать тебе кое-что о Кире..."
...
label Eric_talk_about_Kira_1:
    ...
    # стартовая фраза "Я хотел поговорить о Кире..."
    ...
    if GetRelMax('eric')[0]<0:
        ...
        # дружба, но Макс не рассказал о Кире
        ...
        if alice.dcv.intrusion.stage in [8, 9]:
            # за Максом нет косяка по боди для Алисы
            ...
            # за Максом есть косяк по боди для Алисы
        ...
        if flags.voy_stage == 11:   # бонус по Анне был, но остановлен
            # шоу возобновляется
            $ flags.voy_stage = 14  # возобновление шоу
            if alice.dcv.intrusion.stage in [8, 9]:
                # за Максом нет косяка по боди для Алисы
                ...
                # теперь порно-шоу с Анной можно снова поймать в течении недели
            else:
                # за Максом есть косяк по боди для Алисы
                ...
                # теперь порно-шоу с Анной можно снова поймать в течении недели, но без установки на чулки
                # а также лишается бонуса на деньги

        else:
            # шоу только начинается
            $ flags.voy_stage = 12  # старт порно-шоу
            if alice.dcv.intrusion.stage in [8, 9]:
                # за Максом нет косяка по боди для Алисы
                ...
                Eric_01 "Хорошо, Макс. Как скажешь."
                Max_05 "Круто!" nointeract
                # теперь порно-шоу с Анной можно поймать в течении недели
            else:
                # за Максом есть косяк по боди для Алисы
                ...
                if flags.bonus_from_eric.count('money'):
                    ...
                    $ flags.bonus_from_eric.append('nomoney')
                Max_11 "Ладно, давай так." nointeract
                # теперь порно-шоу с Анной можно поймать в течении недели, но без установки на чулки
                # а также лишается бонуса на деньги
    ...
    $ wcv.catch_Kira.stage = 2
    $ spent_time += 20
    return


label Eric_tribute:
    # "Вот деньги, чтобы ты не лез к Лизе...":   #если у Макса на счету есть сумма, которую нужно заплатить
    Eric_03 "Ну вот, хоть какую-то пользу от тебя можно заиметь."
    Max_15 "Но смотри... Если всё равно будешь лезть к Лизе..."

    if flags.eric_photo1 and eric_obligation.volume < 1000:
        # Макс добыл 1 снимок Эрика
        ...
        menu:
            Eric_06 "Нет, Макс. Уверяю, это моё последнее требование, которое касается Лизы. Жду через неделю с моими денежками."
            "{i}уйти{/i}":
                $ eric_obligation.pay()             # оплачиваем текущую неделю
                $ eric_obligation.volume = 1000     # увеличиваем размер компенсации
    else:
        menu:
...
label Eric_tribute_no_money:
    # "У меня нет денег, чтобы ты не лез к Лизе."
    ...
    menu:
        Eric_13 "Скоро узнаешь. А теперь проваливай!"
        "{i}уйти{/i}":
...
label Eric_with_dictaphone:
    ...
    menu:
        ...
        "Может, договоримся? {i}(добыть компромат){/i}":
            pass
    ...
    menu:
        Eric_07 "И с какой стати мне тебя оправдывать?"
        "С той, что я могу заплатить тебе больше, чем якобы должен." if not flags.eric_photo1:
        ...
        "С той, что у меня есть кое-что на тебя." if flags.eric_photo1:
    ...
    menu:
        "Ты мутный тип, а моя мама заслуживает лучшего!":
        ...
        "Сестрёнок моих ты не получишь!":
            ...
            menu:
                #успех
                Max_03 "[good_mission!t]{m}Отлично! Самое главное я записал, а именно его планы относительно нашей маленькой Лизы. Этого должно хватить, чтобы на него начали не просто подозрительно смотреть, а выгнали ко всем чертям...{/m}"
                "{i}вернуться в свою комнату{/i}":
                    $ flags.asked_phone = 3
                    jump lisa_about_stolen_phone


label Eric_talk_about_Lisa_3:
    # Разговор с Эриком после ужина, если была 2-ая практика (Настоящая дружба)
    ...
    menu:
        "{i}уйти мыть посуду{/i}":
            $ flags.lisa_sexed = 11         # Эрик уступил Лизу Максу (Д+)
            $ lisa.dcv.seduce.stage = 5     # можно поговорить с Лизой о книжке
            $ poss['seduction'].open(32)    # Макс поговорил с Эриком о своих опасениях за практику с Лизой (Настоящая дружба)
    # Макс моет посуду
...
label Eric_stopLisa_bonus:
    # После отдачи Лизы Максу закрывается шоу с Анной (Настоящая дружба)
    # кат-сцена после следующего ужина (если Эрик отдал Лизу Максу)
    # max&eric-terrace-00 + max + eric
    scene BG talk-terrace-00
    $ renpy.show('Eric talk-terrace 01'+eric.dress)
    $ renpy.show('Max talk-terrace 01'+mgg.dress)
    ...
    menu:
        "{i}уйти{/i}":
            pass
    $ flags.voy_stage = 11      # бонус с потрахушками приостановлен
    $ flags.can_ask = 0         # чулки деактивированы
...
label Eric_ask_stockings:
    # Дополнительный разговор по потрахушкам (Настоящая дружба)
    # "Чтобы на маме были не только очки для сна, но и чулки!"

    if alice.dcv.intrusion.stage == 7:
        # за Максом есть косяк по боди для Алисы
        Eric_13 "А вот хрен тебе, Макс, а не Аня в чулках! Не надо было дарить Алисе нижнее бельё первее меня."
        $ flags.can_ask = -1    # больше спрашивать про чулки не получится
        Max_14 "Вот блин!" nointeract

    else:
        # за Максом нет косяка по боди для Алисы
        Eric_03 "О-о-о, пацан, это я с радостью устрою. Твоя мама в чулках - это бомба!"
        $ flags.can_ask = 3     # Анна будет в чулках
        Max_02 "Спасибо! Рад, что мы на одной волне." nointeract

    menu:
        "{i}уйти{/i}":
            $ eric.daily.blog_we = 1    # используем как индикатор состоявшегося разговора о чулках

    if flags.voy_stage in [9, 12]:
        $ flags.voy_stage += 1
    $ spent_time = 10
    return


label Eric_ask_no_stockings:
    # "Чтобы на маме были только очки для сна!"

    Eric_02 "Что, Макс, чулки надоели? Не вопрос, твоя мама и без чулков жутко сексуальна."
    $ flags.can_ask = 2     # Анна без чулок
