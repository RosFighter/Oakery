...
label smoke_toples:
    menu:
        ...
        "Я передумал. Можешь одеться.":
            menu:
                Alice_15 "Спасибо, ваше величество! А чего это ты так расщедрился? Я больше не в твоём вкусе?"
                "Ну, я перегнул палку...":
                    menu:
                ...
                "Это мой шаг к дружбе":
                    menu:
                        Alice_04 "Даже так?! Ну, дружбу я не обещаю, но могу пытаться с тобой как-то уживаться. А если серьёзно, то спасибо, Макс. Я это оценила..."
                        "{i}уйти{/i}":
                            $ AddRelMood('alice', 0, 120)
                "Да надоела!":
                    menu:
                        Alice_08 "Что?! А ну-ка вали отсюда, пока цел! И оденусь теперь, будь уверен!"
                        "{i}свалить{/i}":
                            pass
                "Ты знаешь, пусть всё остаётся как есть...":
                    menu:
                        Alice_08 "Ах вот как? Опять передумал? Всё, ты мне надоел, свали отсюда!"
                        "{i}свалить{/i}":
                            $ AddRelMood('alice', 0, -50)
                            $ spent_time += 10
                            jump Waiting
...
label smoke_not_toples:
    Alice_00 "Ты чего-то хотел, Макс?"
    Max_00 "Алиса, мы же вроде договаривались, что ты будешь без верха курить?"
    Alice_00 "Мне надоело, Макс, сколько можно? Хватит уже быть таким извращенцем!"
    Max_00 "Ну хорошо, уговорила, можешь курить одетой. Но, раз уж ты нарушила нашу договоренность, ты должна показать мне свою грудь."

    $ renpy.show("Alice smoke "+renpy.random.choice(["01", "02", "03"])+"c")

    Alice_00 "Ладно, один разок, но не вздумай маме расказывать. Ни про это, ни про сигареты."
    Max_00 "Конечно!"
    menu:
        Alice_00 "Ну всё, полюбовался и хватит. Вали отсюда, дай спокойно покурить..."
        "Ага...":
            pass
        ...
        "А ты чего в трусах?":
            menu:
                ...
                "Мы же договорились - без трусов!":
                    ...
                    Max_00 "Ухожу, ухожу..."
                "Да шучу я...":
                    Alice_01 "Ты наверное думаешь, что у тебя забавные шутки, да? Так вот нет. Абсолютно несмешные, дебильные шутки, как и ты сам. Всё, свалил отсюда. Я занята..."
                    Max_00 "Как скажешь..."
        "Я передумал. Можешь носить трусы...":
            menu:
                Alice_04 "А чего передумал? А, хотя не важно. Я рада, а то мне всё натирает в джинсах... Хотя, тебе такие подробности знать не нужно. Спасибо за разрешение, ваше величество. Теперь дай покурю..."
                "{i}уйти{/i}":
                    $ flags['smoke'] = None
                    $ flags['smoke.request'] = None
                    $ flags['noted'] = False
...
label smoke_not_nopants:
    Alice_00 "Ты чего-то хотел, Макс?"
    Max_00 "Алиса, что за дела?!"
    Alice_00 "Это ты сейчас о чем, Макс?"
    Max_00 "Мы, кажется, договаривались, что ты днем трусы одевать не будешь. А я видел на тебе трусы."
    Alice_00 "Когда это ты их мог увидеть?"
    Max_00 "Неважно, главное, что они на тебе были."
    ...
    Max_00 "Ну хорошо, я тебя освобождаю от уговора. Но для компенсации ты должна показать мне сиськи!"

    $ renpy.show("Alice smoke "+renpy.random.choice(["01", "02", "03"])+"c")

    Alice_00 "Ну на, любуйся, раз уж дня прожить не можешь без извращений. Проще один раз показать, чем все натирать будет..."
    Max_00 "Классные сиськи!"
    Alice_00 "Спасибо. А теперь иди, погуляй где нибудь. Дай докурить спокойно."
    Max_00 "Хорошо."
...
label smoke_sleep:
    menu:
        ...
        "Ничего, просто любуюсь...":
            Alice_13 "Налюбовался? А вот теперь постой в сторонке, пока я покурю! Давай, вали уже..."
            Max_00 "Хорошо, хорошо..."
        "Я передумал. Можешь спать в лифчике, если хочешь.":
            menu:
                Alice_04 "А чего передумал? А, хотя не важно. Я рада, а то мне неудобно ночью, если выйти куда-то нужно, да и мама заметить может. Спасибо за разрешение, ваше величество. Теперь дай покурю..."
                "{i}уйти{/i}":
                    $ flags['smoke'] = None
                    $ flags['smoke.request'] = None
                    $ flags['noted'] = False
