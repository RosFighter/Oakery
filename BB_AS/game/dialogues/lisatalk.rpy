...
label Lisa_HomeWork:
    ...
    menu:
        ...
        "Ага... {i}(сделать ошибки){/i}" if possibility["sg"].stn > 2:
            jump .make_bag
        "Давай всё сделаю сам! {i}(без ошибок){/i}" if possibility["sg"].stn > 2:
            jump .self
    ...
    label .show_breast:
        ...
        if RandomChance(_chance):  # убеждение успешно
            ...
            if GetMood("lisa")[0] < 3:
                ...
                Lisa_09 "Ну всё, а теперь сделай работу на отлично!"
            else:
                ...
                Lisa_05 "Ну все, полюбовался и хватит, а теперь сделай работу на отлично!"
            Max_00 "Вот теперь убедила!"
            $ renpy.show("Lisa lessons-help "+pose3_1+chars["lisa"].dress)
            $ renpy.show("Max lessons-help "+pose3_1+mgg.dress)
    ...
    label .make_bag:
        ...
        $ ch_vis = str(int(_chance/10)) + "%"
        menu:
            Lisa_12 "Макс, мне кажется, что здесь должен быть другой ответ. Это же не так решается..."
            "Да, Лиза, ты права...":
    ...
    label .self:
        ...
        $ talk_var["truehelp"] += 1
        Max_00 "Так, что тут у нас... Ох, вроде недавно школу закончил... ну, учился недавно ещё, а уже ничего не помню... Хотя... Вот. Да, всё верно!"
        Max_00 "Ну всё, Лиза, я закончил!"
        $ renpy.show("Lisa lessons-help "+pose3_1+chars["lisa"].dress)
        $ renpy.show("Max lessons-help "+pose3_1+mgg.dress)
        $ punlisa[0][0] = 3
        if talk_var["truehelp"] == 6:
            ...
            Lisa_02 "О, супер! Макс! Ты такой молодец! Всё время меня выручаешь... Я бы сама долго возилась и, наверняка, ошибок бы наделала... Спасибо тебе!"
        else:
