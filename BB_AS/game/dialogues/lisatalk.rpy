...
label lisa_ment_kiss1:
    ...
    Lisa_02 "Готова к чему?"
    Max_00 "Как к чему? К уроку поцелуев!"
    Lisa_10 "Опять ты за своё... Что, где-то набрался знаний? Погуглил или на ютубе подсмотрел?"
    Max_00 "Гораздо лучше! Личный опыт!"
    ...
    Max_00 "Это не важно. Ты готова учиться или нет?"
    menu:
        ...
        "Спектакль? Это не спектакль! Но я тебя слушаю...":
            pass
    Lisa_10 "Во-первых, если уж ты и собрался меня учить, то я хочу, чтобы наши отношения были как учитель и ученик, а не как озабоченный парень и девушка"
    Max_00 "И что это значит?"
    Lisa_00 "Это значит, Макс, что руки не распускай. Ты говоришь чему учишь и учишь именно этому. Никаких приставаний или лишних движений. Договорились?"
    Max_00 "Буду вести себя как истинный джентльмен!"
    menu:
        ...
        "Зато сколько интересного может произойти в комнате...":
            ...
            Max_00 "Извини, Лиза. Я глупость сморозил..."
            Lisa_02 "Рада, что ты это осознаёшь..."
            Max_00 "Да, извини... А что там во-вторых?"
    Lisa_10 "Во-вторых, когда я скажу нет, ты остановишься и не будешь продолжать или, тем более, меня лапать. Понял?"
    Max_00 "Понял. Буду делать только то, о чём договоримся!"
    Lisa_09 "Я конечно сомневаюсь, что ты с первого раза всё поймёшь, но учти, если что, второго шанса у тебя не будет. Я просто откажусь с тобой иметь дело и учи кого-нибудь ещё. Понял?"
    Max_00 "Да всё я понял!"
    Lisa_02 "Ну ладно. Теперь я спокойна. Так чему ты сегодня решил меня учить?"
    Max_00 "Как чему? Поцелуям. Я кое-что умею и хочу тебя научить..."
    menu:
        Lisa_10 "Может, в другой раз? Что-то я не в настроении сейчас..."
...
label lisa_ment_kiss:
    ...
    Lisa_00 "Не поняла... К чему я должна быть готова?"
    Max_00 "К поцелуям, конечно!"
    if talk_var['kiss_lessons'] < 3:
        menu:
            Lisa_10 "Опять ты со своими дурацкими идеями... Тебе заняться нечем?"
            "Ну кто ещё тебя научит? А сейчас я готов тебе помочь! {color=[_ch2.col]}(Убеждение. Шанс: [_ch2.vis]){/color}":
                if RandomChance(_ch2.ch):
                    $ Skill('social', 0.2)
                    Lisa_01 "[succes!t]Только не приставать. Только поцелуи. Договорились?"
...
label lisa_kiss_lesson:
    ...
    menu:
        Max_19 "{i}Это же надо, я целуюсь со своей младшей сестрёнкой! Кажется, мне бы любой сейчас позавидовал... К тому же, такая девочка!{/i}"
        "{i}Продемонстрировать своё мастерство{/i} {color=[_kiss.col]}(Убеждение. Шанс: [_kiss.vis]){/color}":
            if RandomChance(_kiss.ch):
                $ Skill('kissing', 0.2)
                Lisa_03 "[lisa_good_kiss]Ну всё, Макс, хорошего помаленьку. Было приятно, даже очень... Будем считать, что я чему-то даже научилась..."
                Max_00 "Рад, что помог... И мне тоже было приятно..."
                $ AddRelMood('lisa', 0, 100)
                $ talk_var['kiss_lessons'] += 1
                if talk_var['kiss_lessons'] == 8:
                    Lisa_03 "Слушай, Макс, а это всё, чему ты решил меня научить? Может быть, в твоём учебном плане есть ещё что-то? Ну там, скажем, немного теории или что-то в этом роде?"
                    Max_00 "Конечно!"
                    Max_00 "{i}Хм... Любопытный намёк, вот только на что? Теории хочет? Может быть, какую-то книжку купить ей про секс и всё такое?{/i}"

            else:
                $ Skill('kissing', 0.1)
                Lisa_09 "[lisa_bad_kiss]Что-то не очень, Макс... Тебе бы ещё потренироваться. Конечно, я не очень разбираюсь в том как надо, но сейчас мне не нравится..."
                Max_00 "Ладно, в другой раз получится лучше..."
