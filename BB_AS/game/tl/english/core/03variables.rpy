# TODO: Translation updated at 2019-12-06 14:46

translate english strings:

    # game/core/03variables.rpy:6
    old "МОЯ\nКОМНАТА"
    new "MY ROOM"

    # game/core/03variables.rpy:6
    old "КОМНАТА\nАЛИСЫ"
    new "ALICE'S\nROOM"

    # game/core/03variables.rpy:6
    old "КОМНАТА\nАННЫ"
    new "ANN'S\nROOM"

    # game/core/03variables.rpy:6
    old "ВАННАЯ\nКОМНАТА"
    new "BATHROOM"

    # game/core/03variables.rpy:6
    old "ГОСТИНАЯ"
    new "LOUNGE"

    # game/core/03variables.rpy:6
    old "ВЕРАНДА"
    new "TERRACE"

    # game/core/03variables.rpy:6
    old "ДВОР"
    new "YARD"

    # game/core/03variables.rpy:22
    old "Лиза"
    new "Lisa"

    # game/core/03variables.rpy:22
    old "Лизы"
    new "Lisa"

    # game/core/03variables.rpy:22
    old "Лизе"
    new "Lisa"

    # game/core/03variables.rpy:22
    old "Лизу"
    new "Lisa"

    # game/core/03variables.rpy:22
    old "Лизой"
    new "Lisa"

    # game/core/03variables.rpy:22
    old "Лиза, младшая сестрёнка. Милая и весёлая. Она ещё учится в школе. С Лизой мы общаемся на одной волне, хотя изредка ссоримся. Но если что-то случается, защиты ищет именно у меня."
    new "This is Lisa, my younger sister. She's very cute, and has a playful personality.  She's still going to school. Even though we fight sometimes we're actually pretty close, and she always looks to me for for help whenever there's trouble."

    # game/core/03variables.rpy:22
    old "Алиса"
    new "Alice"

    # game/core/03variables.rpy:22
    old "Алисы"
    new "Alice"

    # game/core/03variables.rpy:22
    old "Алисе"
    new "Alice"

    # game/core/03variables.rpy:22
    old "Алису"
    new "Alice"

    # game/core/03variables.rpy:22
    old "Алисой"
    new "Alice"

    # game/core/03variables.rpy:22
    old "Алиса, моя старшая сестра.  В любой непонятной ситуации бьёт по лицу (в лучшем случае). Недавно закончила школу и, так же, как и я, ищет свой путь. Целыми днями сидит в ноутбуке и занимается каким-то своим блогом. Как это часто бывает с братьями и старшими сёстрами, мы не очень ладим..."
    new "Alice-my older sister. She has a short fuse, if she's angry at me I can expect a slap to the face at best. She recently graduated from school and is supposed to be looking for work, but all I see her do all day blog on her laptop. She thinks I'm an idiot and we don't really get along. Why do older sisters have to be so bitchy?"

    # game/core/03variables.rpy:22
    old "Анна"
    new "Ann"

    # game/core/03variables.rpy:22
    old "Анны"
    new "Ann"

    # game/core/03variables.rpy:22
    old "Анне"
    new "Ann"

    # game/core/03variables.rpy:22
    old "Анну"
    new "Ann"

    # game/core/03variables.rpy:22
    old "Анной"
    new "Ann"

    # game/core/03variables.rpy:22
    old "Анна, моя мама. Сама воспитывает нас с двумя сёстрами уже несколько лет. Работает в офисе какой-то компании. Хотя, зарплата у неё вполне приличная, но почти всё уходит на оплату жилья, еду и одежду."
    new "Ann, my Mom. She's been raising my sisters and I by herself for several years. She works in the office at some big company. She earns enough to survive, but rent, food, clothing, and other various expenses eat up most of her salary."

    # game/core/03variables.rpy:33
    old "Макс"
    new "Max"

    # game/core/03variables.rpy:33
    old "Макса"
    new "Max"

    # game/core/03variables.rpy:33
    old "Максу"
    new "Max"

    # game/core/03variables.rpy:33
    old "Максом"
    new "Max"

    # game/core/03variables.rpy:33
    old "Максе"
    new "Max"

    # game/core/03variables.rpy:33
    old "Всегда в поисках приключений на свою пятую точку"
    new "Always looking for new thrills and adventures."

    # game/core/03variables.rpy:46
    old "спит в нашей комнате (ночь)"
    new "sleeping in our room (night)"

    # game/core/03variables.rpy:46
    old "принимает душ"
    new "in the shower"

    # game/core/03variables.rpy:46
    old "читает в нашей комнате"
    new "reading in our room"

    # game/core/03variables.rpy:46
    old "одевается в школу"
    new "getting dressed for school"

    # game/core/03variables.rpy:46
    old "одевается в магазин"
    new "clothes shop "

    # game/core/03variables.rpy:46
    old "одевается за закрытыми дверями"
    new "getting dressed behind closed doors"

    # game/core/03variables.rpy:46
    old "в школе"
    new "at school"

    # game/core/03variables.rpy:46
    old "в магазине"
    new "at the store"

    # game/core/03variables.rpy:46
    old "в бассейне"
    new "in the pool"

    # game/core/03variables.rpy:46
    old "загорает"
    new "sunbathing"

    # game/core/03variables.rpy:46
    old "моет посуду"
    new "washing dishes"

    # game/core/03variables.rpy:46
    old "лежит с телефоном в нашей комнате"
    new "on her phone in our room"

    # game/core/03variables.rpy:46
    old "принимает ванну"
    new "in the bath"

    # game/core/03variables.rpy:46
    old "учит уроки"
    new "teaching"

    # game/core/03variables.rpy:69
    old "спит"
    new "sleeps"

    # game/core/03variables.rpy:69
    old "занимается йогой"
    new "doing yoga"

    # game/core/03variables.rpy:69
    old "готовит завтрак"
    new "making breakfast"

    # game/core/03variables.rpy:69
    old "одевается на работу"
    new "getting dressed for work"

    # game/core/03variables.rpy:69
    old "в своей комнате"
    new "in his room"

    # game/core/03variables.rpy:69
    old "на работе"
    new "at work"

    # game/core/03variables.rpy:69
    old "читает на веранде"
    new "reading on the terrace"

    # game/core/03variables.rpy:69
    old "готовит ужин"
    new "preparing dinner"

    # game/core/03variables.rpy:69
    old "смотрит ТВ"
    new "watching TV"

    # game/core/03variables.rpy:91
    old "куда-то одевается"
    new "getting dressed somewhere"

    # game/core/03variables.rpy:122
    old "ПОСМОТРЕТЬ\nФИЛЬМ"
    new "WATCH\nTHE MOVIE"

    # game/core/03variables.rpy:122
    old "ВЫЙТИ\nИЗ ДОМА"
    new "LEAVE\nTHE HOUSE"

    # game/core/03variables.rpy:122
    old "ПОИГРАТЬ\nС ЛИЗОЙ"
    new "PLAY WITH\nLISA"

    # game/core/03variables.rpy:122
    old "УСТАНОВИТЬ\nКЕЙЛОГГЕР"
    new "INSTALL\nKEYLOGGER"

    # game/core/03variables.rpy:122
    old "УСТАНОВИТЬ\nКАМЕРУ"
    new "INSTALL\nCAMERA"

    # game/core/03variables.rpy:122
    old "СПРЯТАТЬ\nПАУКА"
    new "HIDE\nSPIDER"

    # game/core/03variables.rpy:122
    old "БРОСИТЬ\nПАУКА>"
    new "THROW\nSPIDER"

    # game/core/03variables.rpy:122
    old "ИСКАТЬ\nПАУКОВ>"
    new "LOOK FOR\nSPIDERS"

    # game/core/03variables.rpy:122
    old "ИСКАТЬ\nСИГАРЕТЫ"
    new "LOOK FOR\nCIGARETTES"

    # game/core/03variables.rpy:122
    old "ИСКАТЬ\nКНИГУ"
    new "LOOK FOR\nBOOK"

    # game/core/03variables.rpy:122
    old "ЧИСТИТЬ\nБАССЕЙН"
    new "CLEAN\nPOOL"

    # game/core/03variables.rpy:122
    old "ЧИТАТЬ"
    new "READ"

    # game/core/03variables.rpy:122
    old "ИСКАТЬ\nКАМЕРУ"
    new "LOOK FOR\nCAM"

    # game/core/03variables.rpy:122
    old "РАЗОБРАТЬ\nКОРОБКИ"
    new "UNPACK\nBOXES"

    # game/core/03variables.rpy:122
    old "ВКЛЮЧИТЬ\nНОУТБУК"
    new "TURN ON\nLAPTOP"

    # game/core/03variables.rpy:122
    old "СПАТЬ"
    new "SLEEP"

    # game/core/03variables.rpy:122
    old "УСТАНОВИТЬ\nБУДИЛЬНИК"
    new "SET THE\nALARM"

    # game/core/03variables.rpy:122
    old "ВЗДРЕМНУТЬ"
    new "NAP"

    # game/core/03variables.rpy:122
    old "ПРИНЯТЬ\nДУШ"
    new "TAKE A\nSHOWER"

    # game/core/03variables.rpy:122
    old "ПРИНЯТЬ\nВАННУ"
    new "TAKE A\nBATH"

    # game/core/03variables.rpy:122
    old "ПОГОВОРИТЬ"
    new "TALK"

    # game/core/03variables.rpy:122
    old "МЫТЬ\nПОСУДУ"
    new "WASH\nDISHES"

    # game/core/03variables.rpy:177
    old "ПАУК"
    new "SPIDER"

    # game/core/03variables.rpy:177
    old "Самое страшное существо на свете. С точки зрения Алисы, конечно. Нужно этим воспользоваться в подходящий момент!"
    new "The scariest creature ever. From Alice's point of view anyways. I should find a way to use it at the right moment!"

    # game/core/03variables.rpy:177
    old "СКРЫТАЯ КАМЕРА"
    new "HIDDEN CAM"

    # game/core/03variables.rpy:177
    old "Высокотехнологичная микро-камера, предназначенная для скрытного наблюдения. Имеет радиомодуль для беспроводной передачи зашифрованного цифрового видеосигнала."
    new "Advanced micro camera for  concealed surveillance. Includes a radio-module for encrypted digital video streaming."

    # game/core/03variables.rpy:177
    old "Электроника"
    new "Electronics"

    # game/core/03variables.rpy:177
    old "ФИЛЬМ \"ШКОЛЬНИЦЫ\""
    new "PORN MOVIE \"SCHOOLGIRL\""

    # game/core/03variables.rpy:177
    old "Строгая учительница пытается наказать непослушных школьниц..."
    new "Strict teacher punishing naughty schoolgirls..."

    # game/core/03variables.rpy:177
    old "ШЕЛКОВЫЙ ХАЛАТ"
    new "SILK BATHROBE"

    # game/core/03variables.rpy:177
    old "Короткий, лёгкий, почти шёлковый халат высшего качества. Made in China."
    new "Short, light, silk bathrobe. Great quality! Made in China."

    # game/core/03variables.rpy:177
    old "Одежда"
    new "Clothes"

    # game/core/03variables.rpy:177
    old "КУПАЛЬНИК КРАСНЫЙ"
    new "RED SWIMSUIT"

    # game/core/03variables.rpy:177
    old "Купальник для тех, кто не стесняется своего тела. Скрывает только самые интимные участки. Всё остальное открыто для солнца и глаз окружающих."
    new "A sexy swimsuit for those who aren't shy about their bodies. Hides only the most intimate areas. Everything else is exposed to the sun and the eyes...

    # game/core/03variables.rpy:177
    old "СИГАРЕТЫ"
    new "CIGARETTES"

    # game/core/03variables.rpy:177
    old "Пачка сигарет Lucky Strike. Для настоящих мужчин!"
    new "Pack of smokes, Lucky Strike. For real men!"

    # game/core/03variables.rpy:177
    old "Другое"
    new "Misc."

    # game/core/03variables.rpy:177
    old "МАЛЕНЬКОЕ ЧЕРНОЕ ПЛАТЬЕ"
    new "LITTLE BLACK DRESS"

    # game/core/03variables.rpy:177
    old "Отличный подарок для девушки, желающей произвести фурор на вечеринке или дискотеке."
    new "Great gift for a girl who wants to make an impression."

    # game/core/03variables.rpy:177
    old "ЛЮБЯЩАЯ РУБИ"
    new "LOVING RUBY"

    # game/core/03variables.rpy:177
    old "Роман о запретной любви между секретаршей и её начальником, полный любви, страсти, эмоций и... мистики."
    new "I’ve heard all the rumors: He killed his wife. He’s under house arrest. He’s a vampire. Yes, I know I’m crazy."

    # game/core/03variables.rpy:177
    old "Книги"
    new "Books"

    # game/core/03variables.rpy:177
    old "ПРЕМЬЕР-МИНИСТР"
    new "PRIME MINISTER"

    # game/core/03variables.rpy:177
    old "Новый эротический роман, входящий в Топ-10 лучших романов и новелл США!"
    new "A kinky Prime Minister, a vanilla intern, and a scandal that shouldn't feel so good or cost so much."

    # game/core/03variables.rpy:177
    old "БЫТЬ КУКЛОЙ"
    new "BE A DOLL"

    # game/core/03variables.rpy:177
    old "В поместье Картера красивые девушки обучаются, чтобы стать идеальными жёнами для самых влиятельных людей этого мира."
    new "At Carter Manor, women are groomed to become the perfect trophy wives for the most influential and powerful men in the world."

    # game/core/03variables.rpy:177
    old "КНИГА ОРГАЗМОВ"
    new "THE BIG BOOK OF ORGASMS"

    # game/core/03variables.rpy:177
    old "Что получится, если собрать самые горячие истории об оргазмах от 69 различных авторов?"
    new "What happens when you bring together 69 authors sharing their hottest orgasm stories?"

    # game/core/03variables.rpy:177
    old "ИСТОРИЯ О"
    new "STORY OF O"

    # game/core/03variables.rpy:177
    old "Это история о доминировании и подчинении. История об одной прекрасной девушке по имени О."
    new "Story of O is a tale of female submission involving a beautiful Parisian fashion photographer named O. "

    # game/core/03variables.rpy:177
    old "Стремянка"
    new "Stepladder"

    # game/core/03variables.rpy:177
    old "Небольшая стремянка, позволяющая достать то, для чего не хватает роста"
    new "a small stepladder, allowing you to reach greater heights."

    # game/core/03variables.rpy:177
    old "WEB STANDARDS"
    new "WEB STANDARDS"

    # game/core/03variables.rpy:177
    old "Книга рассказывает о способах создавать свои сайты, работающие на любых устройствах."
    new "The book teaches you how to create custom websites that work with any device."

    # game/core/03variables.rpy:334
    old "Скрытые камеры"
    new "Hidden cams"

    # game/core/03variables.rpy:334
    old "Я нашёл пустую коробку из под скрытой камеры. Видимо, она установлена где-то в доме. Нужно поискать как следует - вдруг, кто-то за нами наблюдал всё это время?"
    new "Looks like the empty packaging for a hidden camera!. It's probably installed somewhere in the house. I should try to find it, what if someone is using it to spy on us?"

    # game/core/03variables.rpy:334
    old "Итак, я нашёл скрытую камеру в гостиной. Она вмонтирована в стену, видимо, во время ремонта. Поэтому, её нельзя вытащить, не повредив. Кроме того, оказалось, что камера сейчас ни к чему не подключена. Может быть, стоит целенаправленно изучить способы подключения камер, чтобы извлечь из неё какую-то пользу..."
    new "I've found the hidden cam in the living room. It's mounted in the wall and it looks like I can't pull it out without damaging it. It's not connected to anything right now. Maybe I should find a way to make use of it..."

    # game/core/03variables.rpy:334
    old "Ко мне пришла отличная идея - заработать на трансляции с веб-камеры в интернет, но я об этом знаю очень мало... Может быть, поможет какая-то литература?"
    new "Here's an idea: I could earn some money by live streaming with the hidden camera! Too bad i'm not sure how to set it up exactly...Maybe there are some resources on the web?"

    # game/core/03variables.rpy:334
    old "так, я внимательно прочитал книгу Web Standards и разобрался как сделать свой сайт. Пожалуй, стоит начать именно с этого. Мне потребуются деньги на домен, хостинг и оплату готового шаблона с дизайном. Всего нужно порядка $100."
    new "Well, I've read WebStandards and figured out how to set up my website. I guess that should be my starting point. I'll need some money to buy a domain, hosting and design template. 100$ should do."

    # game/core/03variables.rpy:334
    old "Наконец-то, у меня есть свой собственный сайт, где транслируется изображение с камеры из гостиной! Теперь нужно увеличить аудиторию, ведь чем больше людей, тем больше доход от рекламных баннеров. Сделать это можно с помощью вложений в рекламу своего сайта. Кроме того, если в кадре происходит что-то интересное, аудитория также растёт."
    new "Finally, I have my own streaming website! Now I need to grow my audience to earn more money. And for that I need to advertise my website somewhere, which will also cost money. I'm sure I can get more people watching if my cameras capture something interesting..."

    # game/core/03variables.rpy:334
    old "{i}{b}Внимание:{/b} Пока это всё, что можно сделать для данной «возможности» в текущей версии игры.{/i}"
    new "{i}{b}Attention:{/b} You've reached the end of this opportunity in the current version of the game.{/i}"

    # game/core/03variables.rpy:334
    old "Наставник"
    new "Mentor"

    # game/core/03variables.rpy:334
    old "Кажется, Лиза совсем ничего не знает о мальчиках. Возможно, она даже порно ни разу не видела, раз так удивилась обычному утреннему стояку. Может быть, стоит заняться просветительской деятельностью среди своей младшей сестрёнки? Но с чего начать? Поговорить?"
    new "Looks like Lisa doesn't know anything about male anatomy. She was so surprised by my morning wood, she probably hasn't even seen porn. Maybe I should help educate her. I'll start by talking to her."

    # game/core/03variables.rpy:334
    old "Я поговорил с Лизой и стало ясно, что для того, чтобы чему-то учить, нужно сначала завоевать авторитет."
    new "I talked with Lisa and she doesn't respect me enough to listen, I need to fix that before continuing with my lessons."

    # game/core/03variables.rpy:334
    old "Особые книги"
    new "Special books"

    # game/core/03variables.rpy:334
    old "Алиса читает какие-то книги, но не хочет говорить о них. На порно журналы не похоже... Что же ещё там может быть? Нужно попытаться выяснить это как можно скорее... Любопытно же!"
    new "Alice is secretly reading books she doesn't want to talk about. They don't look like porn, what could it possibly be? I should investigate."

    # game/core/03variables.rpy:334
    old "Я нашёл какую-то книгу, но из названия ничего не понятно, а читать саму книгу ни времени, ни желания нет. Может быть, поискать о ней информацию в интернете?"
    new "I've found a book, but the name is strange and I don't want to waste my time on reading it. I should look it up on the web."

    # game/core/03variables.rpy:334
    old "Судя по реакции Алисы, подарок ей понравился. Видимо, стоит периодически заходить в книжный интернет-магазин и смотреть новинки. Возможно, таким образом удастся чуть-чуть улучшить отношения с Алисой."
    new "Judging by her reaction she liked my present. I should frequent the e-book store every once in a while to look for new releases. "

    # game/core/03variables.rpy:334
    old "Одноклассник"
    new "Schoolmate"

    # game/core/03variables.rpy:334
    old "Очевидно, Лиза по уши влюблена в какого-то своего одноклассника по имени Алекс. Как она рассказала, у него есть подруга Оливия, самая красивая девочка в школе. Лиза ждёт от меня помощи в виде совета - как ей быть дальше.\n\nВ голову приходит сразу три варианта: предложить ей забыть об этом парне, предложить ей сражаться с Оливией за Алекса до конца или же помочь переключить внимание Оливии на кого-то другого..."
    new "Lisa is obviously in love with her schoolmate Alex. She told me he has a girlfriend Olivia, the most beautiful girl in the school. I can help Lisa by suggesting her next step.\n\nI can see three options: tell her to forget all about him, suggest she fights with Olivia for him or maybe we could channel Olivia's attention onto someone else..."

    # game/core/03variables.rpy:334
    old "Да, и похоже о своём мальчике Лиза разговаривает только, когда у неё хорошее настроение..."
    new "Oh, and it looks like Lisa only talks about him when she's in a good mood."

    # game/core/03variables.rpy:334
    old "Я предложил Лизе забыть о её парне и, похоже, она прислушалась к моему совету. Конечно, она расстроилась, но может быть это и к лучшему? Возможно, она сосредоточит своё внимание на ком-то, кто всегда рядом?"
    new "I told Lisa to forget about Alex and she followed my advice. She's very upset, but maybe it's for the best? Maybe now she'll focus her attention on someone closer?"

    # game/core/03variables.rpy:334
    old "Я убедил Лизу, что нужно бороться за своё счастье. Осталось только разработать план и достичь поставленной цели!"
    new "I convinced Lisa she should fight for her happiness. Now I just have to come up with a plan and reach this goal!"

    # game/core/03variables.rpy:334
    old "Я разработал хитрый план, который сделает всех счастливыми: Лиза должна подужиться с Оливией, стать её лучшей подругой, познакомить её со мной, а я её соблазню... Ну, попытаюсь. Но если получится, то у меня будет девушка, а у Лизы её Алекс. Отличный план! Вот только как соблазняют самых красивых девочек школы?"
    new "I've come up with an idea that would make everyone happy. Lisa will have to befriend Olivia, introduce me and then I'll seduce her... Well, I'll try. But if I succeed I'll get a girlfriend and Lisa will get Alex. Great plan! Now, how would one go about seducing the prettiest girl in school?"

    # game/core/03variables.rpy:334
    old "Блог"
    new "Blog"

    # game/core/03variables.rpy:334
    old "Алиса рассказала о своём блоге и о своей проблеме, связанной с потерей вещей во время переезда. Теперь ей нечего показывать, не в чем вести блог и, вообще, она в печали. Кажется, она согласна на любую помощь и идеи, даже от меня! Может быть, посидеть в интернете, поискать какое-то решение?"
    new "Alice has told me about her blog and her \"lack of clothes\" problem. Now she has nothing to show, nothing to wear in the blog and she's heartbroken. She's desperate for any kind of help, even from me! Maybe I should look on the web for solutions?"

    # game/core/03variables.rpy:334
    old "В результате поисков информации в интернете о том, чем могла бы заняться Алиса, удалось кое-что выяснить. Самыми популярными оказались блоги, где где ведущие - девушки. Причём, чем более откровенные наряды и чем больше грудь, тем более популярны шоу. С грудью, конечно, ничего не поделать, а вот наряды... Может быть, поговорить с ней об этом?"
    new "After searching the web I learned that the most popular blogs have female hosts, and their popularity directly correlates with the size of their breasts and how revealing their outfits are. I can't really do anything about Alice's cup size, but her outfits... Maybe I should talk to her?"

    # game/core/03variables.rpy:334
    old "Я пообщался с Алисой насчёт своих выводов о популярности блога и намекнул, что можно рекламировать не только крема и лаки, но и нижнее бельё, например. Удивительно, но Алиса согласилась. Правда, теперь мне нужно найти нижнее бельё для того, чтобы она заинтересовала свою аудиторию и привлекла внимание рекламодателей..."
    new "I talked with Alice about her blog and tried to tell her she could advertise lingerie. Amazingly, she agreed. Now I just have to buy her some lingerie, so she'll be able to build a new audience and attract some advertisers..."

    # game/core/03variables.rpy:334
    old "Я подарил Алисе симпатичный комплект нижнего белья. Ей очень понравилось. Она даже при мне его примерила! Правда, я почти ничего не увидел, но было волнующе... Что самое любопытное, она намекнула, что можно поискать и что-то более... сексуальное!"
    new "I gave Alice a cute lingerie set. She loved it. She even tried it on in front of me! I didn't get to see much, but the experience was very exciting anyway... And the best part is, she hinted that she's willing to try on something even sexier if I buy it!"

    # game/core/03variables.rpy:334
    old "Мой очередной подарок Алисе произвёл эффект, но не совсем тот, на который я расчитывал. Чёрное маленькое боди без верха не подойдёт для её блога... Однако, подойдёт для кого-то, но для кого она не сказала... Столько секретов..."
    new "My new gift didn't quite have the effect I was hoping for. She said she's not going to wear a topless piece for a blog but instead she'll wear it for someone special,  but refused to give me any details.  So secretive..."

    # game/core/03variables.rpy:334
    old "Ах да! Алиса сообщила, что с нею связался какой-то рекламодатель, который будет высылать ей нижнее бельё и потом платить за это! Теперь я ей больше не нужен..."
    new "Oh, yeah, she also told me she was approached by a sponsor.  They'll send her lingerie and then they'll pay her for wearing it!  She doesn't need me anymore, though..."

    # game/core/03variables.rpy:334
    old "Купальник"
    new "Swimsuit"

    # game/core/03variables.rpy:334
    old "Я заметил, что Лиза всё время в одном и том же закрытом купальнике. Так же невозможно толком загореть? Она тоже так считает, но другого нет. Остальные пропали с другими вещами во время переезда. Может быть, удастся как-то ей помочь?"
    new "I noticed Lisa always wears the same one-piece swimsuit. How is she supposed to tan in that? She knows I'm right, but she doesn't have anything else to wear, everything got lost while we moved here.  Maybe I can help her out somehow?"

    # game/core/03variables.rpy:334
    old "Во время завтрака Лиза намекнула маме, что нужно бы купить во время шоппинга купальник, который не скрывает половину тела и в котором можно загорать..."
    new "At breakfast Lisa suggested that she needs a new swim suit so she can tan properly"

    # game/core/03variables.rpy:334
    old "Выяснилось, что во время последнего шоппинга забыли купить Лизе купальник, о котором она просила. Теперь придётся ждать до следующих выходных, когда Эрик снова повезёт всех за покупками. Но в этот раз сам Эрик пообещал ей подарить этот купальник. Может быть, его опередить?"
    new "Mom forgot to buy a new swimsuit for Lisa. Now she has to wait for next weekend when Eric will take everyone shopping. This time Eric promised to buy one for her. Maybe I should beat him to it?"

    # game/core/03variables.rpy:334
    old "Мне удалось накопить немного денег и купить тот самый купальник, который так хотела Лиза. Кажется, она меня любит! А Эрик теперь обломается со своими подарками, ведь я уже подарил то, что так хочет младшая сестрёнка!"
    new "I have enough money to buy Lisa the swimsuit she wanted. I think she really appreciates it. And Eric can shove it."

    # game/core/03variables.rpy:334
    old "{i}{b}Внимание:{/b} Пока это всё, что можно сделать для данной «возможности» в текущей версии игры. Продолжение следует...{/i}"
    new "{i}{b}Attention:{/b} You've reached the end of this opportunity in the current version of the game. {/i}"

    # game/core/03variables.rpy:334
    old "Мне не удалось купить и подарить Лизе купальник до того, как это сделал для неё Эрик. Кажется, их отношения улучшились. Вот если бы я подарил тот чёртов купальник, то Лиза была бы мне очень благодарна, а не ему..."
    new "I couldn't buy Lisa the swimsuit that she wanted before Eric did. I think their relationship improved. Too bad I couldn't give her that stupid swimsuit so Lisa would be grateful of me."

    # game/core/03variables.rpy:381
    old "Значит, у тебя есть блог?"
    new "So, you have a blog?"

    # game/core/03variables.rpy:381
    old "Насчёт блога..."
    new "About the blog..."

    # game/core/03variables.rpy:381
    old "О школе..."
    new "About school..."

    # game/core/03variables.rpy:381
    old "А ты чего так загораешь?"
    new "Why are you sunbathing like this?"

    # game/core/03variables.rpy:381
    old "Насчёт твоего парня..."
    new "About your boyfriend..."

    # game/core/03variables.rpy:381
    old "Насчёт посуды..."
    new "About dishes..."

