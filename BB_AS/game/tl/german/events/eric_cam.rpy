# TODO: Translation updated at 2021-09-01 17:03

# game/events/eric_cam.rpy:13
translate german cam0_eric_ann_sleep_b363be42:

    # Max_07 "{m}Как же повезло, что у меня такая горячая мама... Стойте-ка, а Эрик где?! Может, пошёл в ванную комнату? Или ещё куда...{/m}"
    Max_07 "{m}Was für ein Glück, dass ich so eine heiße Mutter habe... Warte mal, wo ist Eric?! Vielleicht ist er auf die Toilette gegangen oder so...{/m}"

# game/events/eric_cam.rpy:28
translate german cam0_eric_ann_sleep_73c88870:

    # Max_07 "{m}Как же повезло, что у меня такая горячая мама... А Эрик опять где-то полуночничает...{/m}"
    Max_07 "{m}Wie glücklich ich bin, so eine heiße Mutter zu haben... Und Eric ist um Mitternacht wieder draußen...{/m}"

# game/events/eric_cam.rpy:33
translate german cam0_eric_ann_sleep_97963875:

    # Max_01 "{m}Как же повезло, что у меня такая горячая мама... Выглядит потрясающе, аж глаза отрывать не хочется!{/m}"
    Max_01 "{m}Was für ein Glück, dass ich so eine heiße Mutter habe... Sie sieht umwerfend aus, man kann die Augen nicht von ihr lassen!{/m}"

# game/events/eric_cam.rpy:37
translate german cam0_eric_ann_sleep_84da07d8:

    # Max_01 "{m}Мама и Эрик спят совсем голые. Ни стыда, ни совести...{/m}"
    Max_01 "{m}Mom und Eric schlafen völlig nackt. Keine Scham, kein Gewissen...{/m}"

# game/events/eric_cam.rpy:47
translate german cam0_eric_ann_shower_f2cae77e:

    # Max_09 "{m}Мамы и Эрика не видно через эту камеру... Может посмотреть через другую?{/m}"
    Max_09 "{m}Mom und Eric kann man durch diese Kamera nicht sehen... Kann ich durch die andere Kamera sehen?{/m}"

# game/events/eric_cam.rpy:49
translate german cam0_eric_ann_shower_25391a36:

    # Max_09 "{m}Мамы и Эрика не видно через эту камеру...{/m}"
    Max_09 "{m}Mom und Eric kann man durch diese Kamera nicht sehen...{/m}"

# game/events/eric_cam.rpy:75
translate german cam0_eric_ann_shower_5424f711:

    # Max_07 "{m}Вот это да... Похоже намечается что-то большее, чем просто принять душ!{/m}"
    Max_07 "{m}Wow... Sieht aus, als gäbe es mehr zu tun, als nur zu duschen!{/m}"

# game/events/eric_cam.rpy:80
translate german cam0_eric_ann_shower_03b39878:

    # Max_08 "{m}Да уж, устроился Эрик хорошо... Мама отсасывает ему с таким наслаждением, аж оторваться не может!{/m}"
    Max_08 "{m}Sieht aus, als hätte sich Eric gut eingelebt... Mom genießt es wirklich, ihm einen zu blasen!{/m}"

# game/events/eric_cam.rpy:83
translate german cam0_eric_ann_shower_aa6dfd62:

    # Max_04 "{m}Охх... Вот же Эрику повезло... Ведь у мамы такие нежные и ласковые руки!{/m}"
    Max_04 "{m}Eric hat Glück... Mom ist so zärtlich mit ihren Händen!{/m}"

# game/events/eric_cam.rpy:86
translate german cam0_eric_ann_shower_709adb56:

    # Max_06 "{m}Охренеть! Вот это страсть! Они так увлечены друг другом... И похоже, маме это очень нравится!{/m}"
    Max_06 "{m}Wow! Sie sehen aus, als ob sie wirklich begeistert sind! Wenn ich Mom ins Gesicht sehe, genießt sie es...{/m}"

# game/events/eric_cam.rpy:89
translate german cam0_eric_ann_shower_4746ba6b:

    # Max_05 "{m}Ого! Эрик трахает маму сзади, да так активно... И... кажется, ей это очень нравится, она даже двигается ему навстречу... и изнывает от страсти!{/m}"
    Max_05 "{m}Wow! Eric nimmt Mom mit so viel Kraft von hinten... sie drückt sogar ihren Arsch am Ende jedes Stoßes gegen ihn!{/m}"

# game/events/eric_cam.rpy:94
translate german cam0_eric_ann_shower_4d366672:

    # Max_09 "{m}Вот чёрт! Эрик кончает маме прямо на лицо, как в каком-то порно! Причём, ей это настолько нравится, что она улыбается и ловит его сперму своим ртом!{/m}"
    Max_09 "{m}Eric hat Mom gerade einen Cumshot gelandet, wie in einem Porno! Sie hat sich sogar etwas von seinem Sperma in den Mund geschoben!{/m}"

# game/events/eric_cam.rpy:97
translate german cam0_eric_ann_shower_df0cc161:

    # Max_01 "{m}Ну да! Кто бы сомневался, что Эрик не продержится слишком долго. Мама своё дело знает!{/m}"
    Max_01 "{m}Nun, wer hätte bezweifelt, dass Eric nicht lange durchhalten würde. Mom weiß, was sie tut!{/m}"

# game/events/eric_cam.rpy:100
translate german cam0_eric_ann_shower_c373b66f:

    # Max_07 "{m}Ох, чёрт... Эрик уже кончил... Хорошо, что не в маму... Счастливый сукин сын... И она ещё улыбается?!{/m}"
    Max_07 "{m}Oh, Eric ist schon fertig. Der Glückspilz, und sie lächelt immer noch?{/m}"

# game/events/eric_cam.rpy:103
translate german cam0_eric_ann_shower_09186916:

    # Max_08 "{m}Чёрт возьми... он уже кончил... Счастливый ублюдок... забрызгал маме всю спину с попкой своей спермой!{/m}"
    Max_08 "{m}Verdammt, er ist schon fertig, der Glückspilz. Ist über Moms Rücken und ihren Arsch gekommen!{/m}"

# game/events/eric_cam.rpy:120
translate german cam1_eric_ann_shower_cea35296:

    # Max_07 "{m}Охх... Боже мой, какие нежности. Похоже, сейчас что-то начнётся...{/m}"
    Max_07 "{m}Ohh... das sieht intim aus. Es sieht so aus, als würde etwas Interessantes passieren.{/m}"

# game/events/eric_cam.rpy:123
translate german cam1_eric_ann_shower_7b4dc000:

    # Max_10 "{m}Моя мама снова отсасывает этому... Эрику! Да с такой страстью! Ей что, действительно так нравится это делать или она его настолько любит?{/m}"
    Max_10 "{m}Mom bläst ihm wieder einen... Ich frage mich, ob dies ihre Art ist, ihre sexuelle Frustration abzubauen, oder ob sie ihn tatsächlich liebt?{/m}"

# game/events/eric_cam.rpy:126
translate german cam1_eric_ann_shower_8267cfe3:

    # Max_08 "{m}О Боже! Как бы я мечтал оказаться на месте это счастливого ублюдка! Когда её мокрая попка так красиво скачет на члене, голова начинает идти кругом!{/m}"
    Max_08 "{m}Verdammt! Ich würde töten, um an seiner Stelle zu sein! Ihr geiler Arsch rutscht so hypnotisch an seinem Schwanz auf und ab, dass mir schwindlig wird!{/m}"

# game/events/eric_cam.rpy:129
translate german cam1_eric_ann_shower_fb18430e:

    # Max_10 "{m}Ничего себе! Вот это они вытворяют! Эрик трахает маму, разложив её у зеркала как какую-то шлюшку...{/m}"
    Max_10 "{m}Das ist es, was sie vorhaben! Eric nimmt Mom vor den Spiegel und spreizt sie, als wäre sie eine Schlampe...{/m}"

# game/events/eric_cam.rpy:132
translate german cam1_eric_ann_shower_0a7c41ee:

    # Max_08 "{m}Чёрт возьми! Она приняла всю его сперму себе в рот и на лицо, и теперь с такой жадностью и удовольствием слизывает её с его члена...{/m}"
    Max_08 "{m}Sie hat sein ganzes Sperma in ihren Mund und über ihr Gesicht bekommen, jetzt leckt sie gierig seinen Schwanz sauber...{/m}"

# game/events/eric_cam.rpy:135
translate german cam1_eric_ann_shower_ae2539a9:

    # Max_10 "{m}А вот и финал не заставил себя ждать! И похоже всем всё понравилось, как и всегда...{/m}"
    Max_10 "{m}Nun, es hat nicht lange gedauert, auf das Finale zu warten! Sieht so aus, als hätten alle Spaß gehabt, wie immer...{/m}"

# game/events/eric_cam.rpy:140
translate german cam1_eric_ann_shower_f2cae77e:

    # Max_09 "{m}Мамы и Эрика не видно через эту камеру... Может посмотреть через другую?{/m}"
    Max_09 "{m}Mom und Eric kann man durch diese Kamera nicht sehen... Kann ich durch die andere Kamera sehen?{/m}"

# game/events/eric_cam.rpy:151
translate german cam0_eric_resting_9b8dcbac:

    # Max_01 "{m}О, \"В мире животных\" показывают! То ли шимпанзе на стероидах, то ли горилла...{/m}"
    Max_01 "{m}Oh, ist das der Natur-Kanal? Ich sehe wohl gerade eine Guerilla oder einen Schimpansen auf Steroiden...{/m}"

# game/events/eric_cam.rpy:171
translate german cam0_eric_ann_tv_adbf2bd8:

    # Max_01 "{m}Мама с Эриком смотрят какой-то фильм. Наверняка, опять порнушку...{/m}"
    Max_01 "{m}Mom und Eric sehen sich einen Film an. Wahrscheinlich noch mehr Pornos...{/m}"

# game/events/eric_cam.rpy:173
translate german cam0_eric_ann_tv_cf0927e9:

    # Max_01 "{m}Мама с Эриком смотрят какой-то фильм. Интересно, какой?{/m}"
    Max_01 "{m}Mom und Eric sehen sich einen Film an. Welchen wohl?{/m}"

# game/events/eric_cam.rpy:179
translate german cam0_eric_ann_tv_0268c438:

    # Max_08 "{m}Как же повезло Эрику! Я бы тоже с огромной радостью так посмотрел что угодно... если бы мамины нежные и умелые ручки ласкали мой член...{/m}"
    Max_08 "{m}Wie oft muss ich noch sagen, dass Eric Glück hat? Ich würde töten, um Moms geschickte Hände auf meinen Schwanz zu haben...{/m}"

# game/events/eric_cam.rpy:181
translate german cam0_eric_ann_tv_6ba07bce:

    # Max_08 "{m}Блин, мама, тебе что, настолько неинтересно то, что происходит на экране или ты просто любишь отсасывать Эрику?!{/m}"
    Max_08 "{m}Verdammt, Mom, dich interessiert nicht einmal, was auf dem Fernseher zu sehen ist. Du konzentrierst dich nur darauf, Eric einen zu blasen!{/m}"

# game/events/eric_cam.rpy:187
translate german cam0_eric_ann_tv_d86ae6d5:

    # Max_09 "{m}Ну конечно, ведь просто так смотреть, что происходит на экране они не будут, обязательно надо вот это вот делать...{/m}"
    Max_09 "{m}Natürlich reicht es nicht aus, nur fernzusehen. Sie müssen auch das tun...{/m}"

# game/events/eric_cam.rpy:191
translate german cam0_eric_ann_tv_8045496a:

    # Max_10 "{m}А, ну да, без полотенца однозначно лучше... Но вы же в гостиной! А если зайдёт кто-то, а вы тут развлекаетесь...{/m}"
    Max_10 "{m}Oh, nun, es ist definitiv besser ohne Handtuch... Aber du bist doch im Wohnzimmer! Was ist, wenn jemand reinkommt...{/m}"

# game/events/eric_cam.rpy:201
translate german cam0_eric_ann_fucking_5719e975:

    # Max_09 "{m}Утомлённые мама с Эриком лежат на кровати. Благо, они не только трахаются всё время, но ещё и разговаривают...{/m}"
    Max_09 "{m}Mom und Eric liegen im Bett, wahrscheinlich erschöpft. Sieht aus, als würden sie tatsächlich miteinander reden, wenn sie nicht gerade ficken.{/m}"

# game/events/eric_cam.rpy:213
translate german cam0_eric_ann_fucking_e1e7d2c7:

    # Max_07 "{m}Через окно подсмотреть мне не дали, значит заценим через камеру... А они тут развлекаются, только в путь!{/m}"
    Max_07 "{m}Ich durfte nicht durchs Fenster schauen, also werden wir durch die Kamera sehen... Und natürlich haben sie Spaß!{/m}"

# game/events/eric_cam.rpy:216
translate german cam0_eric_ann_fucking_e24d430b:

    # Max_07 "{m}Подсмотреть через окно я не рискнул, могут заметить, а вот через камеру могу наслаждаться безнаказанно, сколько захочу... И посмотреть есть на что!{/m}"
    Max_07 "{m}Ich wage es besser nicht, durch das Fenster zu gucken, sie könnten es bemerken. Aber ich kann so viel durch die Kamera schauen, wie ich will, und es sieht so aus, als hätte ich einen guten Zeitpunkt gewählt!{/m}"

# game/events/eric_cam.rpy:219
translate german cam0_eric_ann_fucking_30904857:

    # Max_07 "{m}Ого! Вот это я удачно заглянул! Всё равно, что порно посмотреть...{/m}"
    Max_07 "{m}Wow! Das ist fast so gut wie Pornos gucken...{/m}"

# game/events/eric_cam.rpy:228
translate german cam0_eric_ann_fucking_42470dcb:

    # Max_08 "{m}Ну вот они и кончили. По крайней мере, Эрик так точно, а вот мама...{/m}"
    Max_08 "{m}Nun, sie sind fertig. Zumindest Eric, aber Mom ist...{/m}"

# game/events/eric_cam.rpy:233
translate german cam0_sexed_lisa_451f5f08:

    # Max_08 "{m}Жаль камеры не передают звук. Нужно будет обязательно спросить Лизу, о чём они разговаривали!{/m}"
    Max_08 "{m}Es ist schade, dass die Kameras keinen Ton übertragen. Ich werde Lisa fragen müssen, worüber sie gesprochen haben!{/m}"

# game/events/eric_cam.rpy:243
translate german cam0_sexed_lisa_71c1a2f8:

    # Max_07 "{m}Лиза пришла на урок к маме и Эрику. Посмотрим, что будет...{/m}"
    Max_07 "{m}Lisa hat es zu ihrer Stunde geschafft. Mal sehen, was passiert...{/m}"

# game/events/eric_cam.rpy:251
translate german cam0_sexed_lisa_c0ac64d9:

    # Max_09 "{m}Вот же Эрик! Член он свой вывесил... Интересно, что они ей там рассказывают?{/m}"
    Max_09 "{m}Da ist Eric! Er hat seinen Schwanz rausgeholt... Ich frage mich, was sie Lisa erzählen.{/m}"

# game/events/eric_cam.rpy:259
translate german cam0_sexed_lisa_91d200d1:

    # Max_08 "{m}Ага, дрочка пошла... И уговорил же Эрик маму на всё это...{/m}"
    Max_08 "{m}Ja, da sitzen die beiden... Und Eric hat Mom zu all dem überredet...{/m}"

# game/events/eric_cam.rpy:269
translate german cam0_sexed_lisa_71c1a2f8_1:

    # Max_07 "{m}Лиза пришла на урок к маме и Эрику. Посмотрим, что будет...{/m}"
    Max_07 "{m}Lisa hat es zu ihrer Stunde geschafft. Mal sehen, was passiert...{/m}"

# game/events/eric_cam.rpy:277
translate german cam0_sexed_lisa_525949f3:

    # Max_09 "{m}Мама при Лизе уже надрачивает Эрику как будто так и надо! Интересно, что они ей там рассказывают при этом?{/m}"
    Max_09 "{m}Mom holt Eric schon wieder vor Lisa einen runter! Ich frage mich, worüber sie reden?{/m}"

# game/events/eric_cam.rpy:285
translate german cam0_sexed_lisa_0dd5dbf3:

    # Max_08 "{m}Ага, мама повышает градус показом своей шикарной груди... И уговорил же Эрик маму на всё это!{/m}"
    Max_08 "{m}Ja, Mom heizt mit der Zurschaustellung ihrer prächtigen Titten ordentlich ein... Und Eric hat sie zu all dem überredet!{/m}"

# game/events/eric_cam.rpy:295
translate german cam0_sexed_lisa_71c1a2f8_2:

    # Max_07 "{m}Лиза пришла на урок к маме и Эрику. Посмотрим, что будет...{/m}"
    Max_07 "{m}Lisa hat es zu ihrer Stunde geschafft. Mal sehen, was passiert...{/m}"

# game/events/eric_cam.rpy:303
translate german cam0_sexed_lisa_43160aa4:

    # Max_09 "{m}Подрочить Эрику на глазах Лизы в образовательных целях? Конечно, обычное дело, не обращайте внимания... Интересно, что они ей там рассказывают при этом?{/m}"
    Max_09 "{m}Eric vor Lisa zu Erziehungszwecken einen runterzuholen? Natürlich, das übliche Zeug... Ich frage mich, was sie ihr erzählen?{/m}"

# game/events/eric_cam.rpy:311
translate german cam0_sexed_lisa_36fcd85f:

    # Max_08 "{m}О, от такого я бы не отказался! Только от одних мыслей о маминых руках, нежно массирующих мои шары, хочется кончить...{/m}"
    Max_08 "{m}Oh, dazu würde ich nicht nein sagen! Allein der Gedanke an Moms Hände, die sanft meine Eier massieren, bringt mich dazu, abspritzen zu wollen...{/m}"

# game/events/eric_cam.rpy:321
translate german cam0_sexed_lisa_71c1a2f8_3:

    # Max_07 "{m}Лиза пришла на урок к маме и Эрику. Посмотрим, что будет...{/m}"
    Max_07 "{m}Lisa hat es zu ihrer Stunde geschafft. Mal sehen, was passiert...{/m}"

# game/events/eric_cam.rpy:329
translate german cam0_sexed_lisa_4bfb346c:

    # Max_09 "{m}Всё дрочат и дрочат... Видимо, закрепляют материал. Ну сколько можно?!{/m}"
    Max_09 "{m}Wichser... Ich schätze, sie sammeln sich. Wie oft werden sie das noch machen?!{/m}"

# game/events/eric_cam.rpy:337
translate german cam0_sexed_lisa_6918e27b:

    # Max_08 "{m}О, Эрик не выдержал и кончил... Хотя, вряд ли. Скорее всего, они решили показать Лизе для чего это всё было нужно.{/m}"
    Max_08 "{m}Oh, Eric konnte es nicht mehr aushalten und kam... Wahrscheinlich wollten sie Lisa zeigen, worum es geht.{/m}"

# TODO: Translation updated at 2021-09-14 23:25

# game/events/eric_cam.rpy:136
translate german cam0_eric_resting_b4d3f901:

    # Max_01 "{m}Кажется, подмешанное Эрику в еду средство испортило ему всё настроение и он не рискует идти к моей сестрёнке... Именно на это я и рассчитывал!{/m}"
    Max_01 "{m}Es scheint, dass Erics Abendmahl ihn in Schwirigkeiten versetzt hat und er es nicht riskieren will, zu meiner Schwester zu gehen... Das ist genau das, womit ich gerechnet habe!{/m}"

# game/events/eric_cam.rpy:363
translate german cam0_eric_shat_4bc4874c:

    # Max_09 "{m}Ничего толком не видно... Стоит взглянуть через другую камеру...{/m}"
    Max_09 "{m}Man kann nicht wirklich etwas sehen... Es lohnt sich, einen Blick in die andere Kamera zu werfen...{/m}"

# game/events/eric_cam.rpy:365
translate german cam0_eric_shat_d2f3bb58:

    # Max_09 "{m}Ничего толком не видно...{/m}"
    Max_09 "{m}Man kann nicht wirklich etwas sehen...{/m}"

# game/events/eric_cam.rpy:379
translate german cam1_eric_shat_58fccd25:

    # Max_02 "{m}Эрик во всю оценивает эффективность слабительного, которое я ему подмешал... Надеюсь, эта обезьяна знает, как пользоваться туалетной бумагой, потому что банановых листьев у нас тут нет!{/m}"
    Max_02 "{m}Erik versucht immer noch, die Wirksamkeit des Abführmittels zu testen, das ich ihm gegeben habe... Ich hoffe, dieser Affe weiß, wie man Toilettenpapier benutzt, denn wir haben hier keine Bananenblätter!{/m}"

# game/events/eric_cam.rpy:381
translate german cam1_eric_shat_a8ebe479:

    # Max_02 "{m}Ты что, Эрик, съел что-то не то, что ли?! Бедняга. Ну, держись там...{/m}"
    Max_02 "{m}Hast du etwas Falsches gegessen, Eric?! Armer Kerl. Nun, halte durch...{/m}"

# game/events/eric_cam.rpy:377
translate german cam0_eric_ann_resting_cb75645d:

    # Max_02 "{m}Похоже, потрахушек сегодня не намечается... Интересно, с чего бы это?!{/m}"
    Max_02 "{m}Sieht so aus, als gäbe es heute Abend kein Ficken... Ich frage mich, warum das so ist.{/m}"

# game/events/eric_cam.rpy:387
translate german cam0_eric_sleep_bce0a0b5:

    # Max_07 "{m}Остаётся надеяться, что я подмешал ему не слишком много успокоительного... Главное, что он сейчас ни к кому не пристаёт.{/m}"
    Max_07 "{m}Ich hoffe nur, ich habe ihn nicht zu sehr betäubt... Das Wichtigste ist, dass er im Moment niemanden stört.{/m}"

# game/events/eric_cam.rpy:397
translate german cam0_eric_ann_try_fucking_5da5d134:

    # Max_01 "{m}Не стоит? Ну ничего, бывает... Но всё равно приятно осознавать, что это происходит из-за моих действий.{/m}"
    Max_01 "{m}Lohnt sich das nicht? Das ist okay, das kommt vor... Aber es ist trotzdem schön zu wissen, dass es an meinem Handeln liegt.{/m}"

# TODO: Translation updated at 2021-11-11 14:52

# game/events/eric_cam.rpy:16
translate german cam0_eric_ann_sleep_c5aa1a5f:

    # Max_07 "{m}Ага! Мама одна... Значит, Эрик почти наверняка сейчас дрочит на Алису. Нужно проверить...{/m}"
    Max_07 "{m}Aha! Mom ist allein... Eric holt sich also mit ziemlicher Sicherheit gerade auf Alice einen runter. Das sollten man sich ansehen...{/m}"

# game/events/eric_cam.rpy:421
translate german cam_before_frame_eric_cb1c7bf7:

    # Max_07 "{m}Отлично! Эрик именно там, где мне и нужно... Нужно дождаться, когда он вернётся к моей маме, и затем уже спокойно его подставлять...{/m}" nointeract
    Max_07 "{m}Das ist großartig! Eric ist genau da, wo ich ihn brauche... Ich muss warten, bis er zu meiner Mutter zurückgeht, und dann kann ich ihn in Ruhe bloßstellen...{/m}" nointeract

# game/events/eric_cam.rpy:437
translate german cam_before_frame_eric_2da5afe7:

    # Max_01 "{m}Отлично! Пора...{/m}" nointeract
    Max_01 "{m}Großartig! Es ist soweit...{/m}" nointeract

translate german strings:

    # game/events/eric_cam.rpy:436
    old "{i}идти к комнате Алисы{/i}"
    new "{i}in Alices Zimmer gehen{/i}"
# TODO: Translation updated at 2021-12-27 01:36

# game/events/eric_cam.rpy:447
translate german cam0_eric_kira_night_swim_4bc4874c:

    # Max_09 "{m}Ничего толком не видно... Стоит взглянуть через другую камеру...{/m}"
    Max_09 "{m}Man kann nicht wirklich etwas sehen... Es lohnt sich, einen Blick in die andere Kamera zu werfen...{/m}"

# game/events/eric_cam.rpy:449
translate german cam0_eric_kira_night_swim_14337802:

    # Max_09 "{m}Ничего не разглядеть... Нужно установить камеру, которая охватила бы весь бассейн...{/m}"
    Max_09 "{m}Man kann nichts sehen... Man muss eine Kamera installieren, die den gesamten Pool erfasst...{/m}"

# game/events/eric_cam.rpy:459
translate german cam1_eric_kira_night_swim_45123771:

    # Max_10 "{m}Вот чёрт! Похоже, Кира ублажает Эрика, чтобы он нас не заложил маме...{/m}"
    Max_10 "{m}Oh, Scheiße! Kira scheint Eric zu befriedigen, damit der uns nicht an Mom verpfeift...{/m}"

