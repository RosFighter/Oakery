...
define talks = {
    ...
    'alice_gift' : TalkTheme('alice', _("У меня для тебя обещанная вкусняшка!"), 'alice_gift_sweets', "all([alice.sorry.owe, alice.sorry.there_in_stock(), alice.plan_name in ['sun', 'read', 'resting', 'blog'], (alice.daily.oiled!=2 or alice.flags.touched)])"),
    ...
    'l.nextkiss' : TalkTheme('lisa', _("Ну что, готова?"), 'lisa_ment_kiss', "all([lisa.plan_name=='read', lisa.dcv.seduce.done, poss['seduction'].st()>7, flags.stopkiss<1])"),
...
define gifts = {
    ...
    'alice' : [
        ...
        Gift(['ferrero-m', 'ferrero-b'], _("Прикупил для тебя немного сладенького!"), 'alice_gift_sweets', -1, "all(['pajamas' in alice.gifts, alice.plan_name in ['sun', 'read', 'resting', 'blog'], alice.dcv.sweets.done, (alice.daily.oiled!=2 or alice.flags.touched), not alice.sorry.owe])"),
        ],
    'ann'   : [
