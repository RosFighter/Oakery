init python:
    ...
    def set_gift_swimsuit():
        ...
        tl.plan_name = random_choice(['sun', 'read'])
        tl.flags.promise = True
        mg = MaxProfile('mgg', "Макс", "Макса", "Максу", "Макса", "Максом", "Максе")
        mg.dress = random_choice(['a', 'b']) if 'max-a' in persistent.mems_var else 'a'
        my_scope = {
                'lisa' : tl,
                'mgg'         : mg,
            }
    ...
    def set_gift_pajamas():
        ...
        al.plan_name = random_choice(['sun', 'resting'])
        if al.plan_name != 'sun':
            al.dress = random_choice(['a', 'd']) if 'kira' in persistent.mems_var else 'a'
            _tm = random_choice(['10:20', '21:00'])
        else:
            al.dress = 'a'
            _tm = '15:30'
        al.sorry.owe = True
    ...
    def set_talk_tv():
        ...
        if 'pajamas' in persistent.mems_var:
            dr_var.append(3)
        if 'smoke_nojeans' in persistent.mems_var:  # and random_choice([False, True]):
            dr_var.append(4)
        if 'kira' in persistent.mems_var:
            dr_var.append(5)
        var = random_choice(dr_var)
        ...
        al.stat.footjob = 3
        al.flags.m_foot = 7
        al.dcv.seduce.done = random_choice([False, True])
    ...
    def set_kira_bathfj():
        ...
        tk.stat.handjob = random_choice([1, 2])
    ...
    # Я спасу их от солнца
    def set_olivia_repeat_sunscreen():
        tl = Profile('lisa', "Лиза", "Лизы", "Лизе", "Лизу", "Лизой", "Лизе")
    ...
    def set_spider_massage():
        ...
        al = Profile('alice', "Алиса", "Алисы", "Алисе", "Алису", "Алисой", "Алисе")
        # al.daily.oiled = random_choice([1, 2])
        al.daily.oiled = 2
    ...
    def set_alice_body_photoset1():
        ...
        my_scope = {
            ...
            'alice'         : al,
            'mgg'           : mg,
            'items'         : {'sexbody1':Item("ЧЁРНОЕ СЕКСУАЛЬНОЕ БОДИ", have=True), 'sexbody2':Item("ЧЁРНОЕ КРУЖЕВНОЕ БОДИ"), 'photocamera':Item("ФОТОАППАРАТ", have=True)},
            'expected_photo': [],
    ...
    def set_kira_strip_01():
        ...
        al = Profile('alice', "Алиса", "Алисы", "Алисе", "Алису", "Алисой", "Алисе")
        al.daily.drink = random_choice([0, 1])
    ...
    # Ещё больше массажа у ТВ
    def set_ann_tv_continuation_massage():
        ta = Profile('ann', "Анна", "Анны", "Анне", "Анну", "Анной", "Анне")
        mg = MaxProfile('mgg', "Макс", "Макса", "Максу", "Макса", "Максом", "Максе")
        mg.dress = get_max_dress(ex='a')
    ...
    def set_lessons_Eric_01():
        ...
        my_scope = {
            'mgg'       : mg,
            'stockings' : random_outcome(50),
            }
        return my_scope

    # Секс-шоу с первых рядов
    def set_eric_ann_fuck_glasses_sleeping():
        mg = MaxProfile('mgg', "Макс", "Макса", "Максу", "Макса", "Максом", "Максе")
        mg.dress = get_max_dress(ex='a')
        my_scope = {
            'mgg'       : mg,
            'stockings' : random_outcome(50) if 'stockings' in persistent.mems_var else 0,
            }
        return my_scope
